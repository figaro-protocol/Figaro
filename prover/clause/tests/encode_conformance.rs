...
#[test]
fn ghg_standard_and_scope() {
    // Consolidated figaro-ghg: free-form `standard` string + `scope` uint,
    // in declaration order. Vector from sdk encodeContentFromSpec.
    assert_encode(
        "figaro-ghg",
        json!({ "standard": "ISO 14064", "scope": 1 }),
        "",
    );
}

#[test]
fn ghg_scope_absent_encodes_zero() {
    // scope is optional — absent encodes as the uint256 zero-value (the spec
    // default is applied by the validator, not the encoder).
    assert_encode(
        "figaro-ghg",
        json!({ "standard": "ISO 14064" }),
        "",
    );
}
...
#[test]
fn geo_basic() {
    // figaro-geo v1: origin/destination geohashes + mass/volume. classOfService
    // split out into its own clause. Vector from sdk encodeContentFromSpec.
    assert_encode(
        "figaro-geo",
        json!({
            "originGeohash": "dr5ru",
        ...
        "",
    );
}
...
fn unknown_clause_has_no_embedded_spec() {
    assert!(
        embedded_spec_json_by_key("figaro-bogus").is_none(),
