...
//!                        (for the FigaroBatchVerifier deploy), then stop.
//!                        No real-proof step.
//!   `SP1_REAL_PROOF=1` — after the mock stage, generate a real Core
//!                        proof on the local machine. Slow (~minutes);
//!                        proves cryptographic correctness of the
//!                        mock-verified execution. Requires the k256 SP1
//!                        precompile patch (pinned in the workspace
//!                        Cargo.toml) to fit consumer-machine memory.
...
async fn main() {
    ...
    println!("Spec bindings hash:  {:?}", pv.spec_bindings_hash);
    ...
    // (SP1_PROGRAM_VKEY for the on-chain FigaroBatchVerifier deploy),
    // then stop. setup() is the cheap part of proving (~12s, no OOM);
    // this path never reaches the memory-heavy prove step.
    ...
    //
    // Gate with SP1_REAL_PROOF=1 because real proving is slow. Mock
    // execution above already validates the program's correctness; the
    // real proof is a cryptographic attestation of that execution.
