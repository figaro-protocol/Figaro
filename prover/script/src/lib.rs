...
pub fn build_canonical_batch_input(with_content_proof: bool) -> BatchInput {
    ...
    // Seller-attestation schema + content (computed up front for use in
    // both the agreement leaf and the AttestationContentProof).
    // figaro-ghg-protocol-v1 is the simplest cross-checking schema (one
    // optional uint8 field). Family `keccak256("emissions")` mirrors
    // script/Deploy.s.sol's family assignment for the GHG schemas.
    ...
    let buyer_attest_struct =
        attest_buyer_struct_hash(&process_id, &order_hash, &schema_id, 0, &B256::ZERO);
