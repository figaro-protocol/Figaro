...
- **Mechanism modules:** `DutchAuction.sol`, `OperatorRegistry.sol`, `AssemblyRegistry.sol` (permissionless assembly anchoring — the assembly artifact family's registry, parallel to `SchemaRegistry`/`OperatorRegistry`), `ProcessOffsetReceipt.sol` (Path A carbon-offset receipts anchor — separate primitive per separation-of-concerns; receipts are not attestations, no agreement clause required), `SwapAndCommitCoordinator.sol` (off-protocol executor — lets a buyer and/or seller post the bond in a non-bond-currency token via a Permit2 pull + a fixed swap router, then calls `commit`; kernel untouched, commitment stays bilaterally signed, permissionless alternative coordinators allowed).
...
- **Mocks:** `mocks/MockERC20.sol`, `MockERC20FeeOnTransfer.sol`, `MockPermitToken.sol`, `MockOffsetAggregator.sol`, `MockKlerosArbitrableProxy.sol`, `MockKlerosArbitrator.sol`, `MockPermit2.sol`, `MockUniversalRouter.sol`; `echidna/EchidnaFuzzer.sol`, `EchidnaToken.sol`, `EchidnaFigToken.sol`, `EchidnaRpgfMinter.sol`.
