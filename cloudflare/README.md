...
### 5.5 Arbitration forums (~1 minute, optional)

The dispute layer is DECLARATIVE — Figaro does not arbitrate and deploys no
forum contracts. `/dispute` builds the process-scoped evidence bundle (pinned
to IPFS) and the parties take it to whatever recourse forum their agreement
names; the forum runs in its own interface. There is nothing to deploy here.

One optional knob: if a deployment wants a forum to iframe `/evidence-display`
(some forums render submitted evidence in-page), set the server env
`EVIDENCE_DISPLAY_FRAME_ANCESTORS` to the space-separated list of that
deployment's recognized forum origins (CSP source syntax), e.g.
`'self' https://resolve.kleros.io https://*.kleros.io`. Unset, the route
admits no third-party ancestor. A forum is deployment config, never a code
default — the protocol is provider-agnostic.
