...
/// @dev V5 actors are EOAs (private keys for EIP-712 signing). The harness
///      sets allowances via approveFor() at deploy time.
contract EchidnaTokenV5 is EchidnaToken {
...
/// @title EchidnaFuzzerV5 — Property-based fuzzing for FigaroCore V5
/// @notice Fuzz-tests the V5 protocol kernel via HEVM-signed commitments.
///
///         V5 key differences from V4:
///         - Unified commit() with single Commitment struct (processId field)
///         - resolveProcess takes Commitment[] (not bytes32[])
///         - Content-addressed orderHash = keccak256(processId ++ structHash)
///         - EIP-712 domain version "3"
///         - orderStatus mapping (uint8: 0=unknown, 1=committed, 2=resolved)
///         - No separate commitOrder/commitSubOrder
///
///         Invariants tested (7):
...
contract EchidnaFuzzerV5 {
    ...
    EchidnaTokenV5 public immutable token;
    ...
    constructor() {
        core = new FigaroCore();
        token = new EchidnaTokenV5(address(this));
