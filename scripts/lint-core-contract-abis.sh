...
# + the FIG token. frontend/lib/core/ — and its contract config, contracts.ts —
...
# not belong in lib/core/, and no ABI may be DEFINED here (parseAbi). Non-core
...
CORE_DIR="$ROOT/frontend/lib/core"

# The ONLY ABIs allowed anywhere in lib/core/: the five core Figaro contracts
...
# ── Only the allowed ABI symbols may appear in lib/core/ ──
while IFS=: read -r file line tok; do
  [[ -z "${tok:-}" ]] && continue
  if ! in_list "$tok" $ALLOWED; then
    echo "✖ ${file#"$ROOT"/}:$line — non-core ABI '$tok' in lib/core/."
...
# ── No ABI may be DEFINED in lib/core/ (parseAbi) — definitions live in the SDK ──
while IFS= read -r hit; do
  [[ -z "$hit" ]] && continue
  echo "✖ ${hit#"$ROOT"/}"
  echo "    Local contract-ABI definition in lib/core/. Define it in sdk/src/abis.ts and import it."
  fail=1
...
echo "✓ core-contract ABIs OK (lib/core/ holds only the five core Figaro ABIs + ERC20_ABI; no local defs)."
