...
The list above is indicative, not exhaustive — `.claude/agents/` is the authority (`ls .claude/agents/`). These are the **operator's** repo-building subagents — private tools for developing Figaro itself. They rely on the canonical `figaro-kernel-discipline` skill at `.claude/skills/` and the `kernel-warn.sh` hook at `.claude/hooks/`. The skill is the single source of truth for kernel rules; the subagents are tool-constrained executors.
