...
export function OperatorRegistrationModule({ context }: ModuleProps) {
    ...
    const capability = context.capabilities.find(
        (candidate) => candidate.action.executionType === "transaction"
            && (candidate.action.kind === "register-operator" || candidate.action.kind === "update-operator-profile")
    );
    ...
    const handleSubmit = useCallback(async () => {
        ...
        context.onExecuteCapability(
            capability,
            capability.action.kind === "update-operator-profile"
            ...
            {!isRegister && (
                <div className="border-t border-gray-100 pt-3 mt-3">
                    ...
                    <p className="text-xs text-gray-500 mb-2">
                        Deactivate first, then withdraw your deposit after the lock period.
                    </p>
                    <button
                        type="button"
                        onClick={() => {
                            if (!window.confirm("Withdraw your registration deposit? This will clear your operator registration.")) return;
