...
export function HandoffDetailsModule({ moduleId, context }: ModuleProps) {
    ...
    const handleSubmit = () => {
        if (!address.trim() || Number(fulfillerMaxPrice) <= 0) return;
        ...
        window.dispatchEvent(
            new CustomEvent("figaro:handoff-manifest", {
                detail: { manifest, fulfillerMaxPrice },
            }),
        );
        setSubmitted(true);
    };
    ...
    return (
        ...
        >
            ...
            <button
                ...
                disabled={!address.trim() || Number(fulfillerMaxPrice) <= 0 || !verified}
                ...
                style={address.trim() && Number(fulfillerMaxPrice) > 0 && verified && accentTone
