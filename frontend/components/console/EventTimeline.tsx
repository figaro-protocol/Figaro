...
/**
 * EventTimeline — Chronological event stream from the synced ProcessGraph.
 * Shows OrderCommitted, OrderResolved, ProcessResolved events.
 */

import { useFigaro } from "@/lib/console/provider";
import { OrderState } from "@figaro/core";
import type { Process, Order } from "@figaro/core";
...
function buildTimeline(processes: Process[]): TimelineEntry[] {
    ...
    for (const process of processes) {
        for (const order of process.orders.values()) {
            ...
            if (order.state === OrderState.Resolved && order.sellerPayout !== undefined) {
                entries.push({
                    type: "resolved",
                    blockNumber: order.blockNumber + 1, // approximate; resolved after committed
                    orderHash: order.orderHash,
                    processId: order.processId,
...
export function EventTimeline() {
    const { myProcesses, syncVersion } = useFigaro();
    const entries = buildTimeline(myProcesses);
        ...
        <div className="flex flex-col gap-0 p-2">
            ...
            <div className="relative ml-4 border-l border-zinc-700 pl-4">
                {entries.slice(0, 50).map((entry, i) => {
                    ...
                    return (
                        <div key={`${entry.orderHash}-${entry.type}-${i}`} className="relative mb-3 pb-1">
