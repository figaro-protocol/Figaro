...
export function RegisteredAssemblyWorkspace({ artifact, shellLabel, runtimeContext }: Props) {
    ...
    const selectedViews = assembly.views.filter((view) =>
        view.viewId === "assembly-overview" || view.viewId === selectedRole?.defaultLandingView
    );
    ...
    const assemblyRuntimeCapabilities = deriveAssemblyCapabilities(
        assembly.identity.id,
        selectedRole?.roleKind,
        roleScopedMechanisms.visibleMechanisms,
        operatorProfile,
    );
    const allRuntimeCapabilities = [...assemblyRuntimeCapabilities, ...runtimeCapabilities];
        ...
        <>
            ...
            >
                <AssemblyShell
                    ...
                    subtitle={shellPresentation.subtitle || "Prototype assembly shell rendered from assembly metadata, derived role contexts, and reusable mechanism-level UI modules."}
                    skin={skinBundle}
                    sidebar={(
                        <>
                    ...
                    main={(
                        <>
                            <div className="rounded-lg border border-neutral-200 bg-white p-4 text-sm text-black">
                                <p className="font-semibold mb-2">Derived Assembly Snapshot</p>
                                ...
                                <p>Network: <span className="font-mono">{assemblyModel.network}</span></p>
                                <p>Available networks: <span className="font-mono">{assemblyModel.availableNetworks.join(", ")}</span></p>
                                <p>Declared roles: <span className="font-mono">{assemblyModel.roles.length}</span></p>
