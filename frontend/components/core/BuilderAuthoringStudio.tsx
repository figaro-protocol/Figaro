...
import {
    publishInstitutionAssemblyAction,
    unregisterInstitutionAssemblyAction,
} from "@/app/builders/authoring/actions";
import { Button } from "@/components/ui/Button";
import { Card } from "@/components/ui/Card";
import { FormField } from "@/components/ui/FormField";
import { Input } from "@/components/ui/Input";
import { InstitutionArtifactWorkspace } from "@/components/core/InstitutionArtifactWorkspace";
...
import {
    ...
    cloneInstitutionAssemblyDraft,
    INSTITUTION_ASSEMBLY_SECTION_KEYS,
    InstitutionAssemblySectionKey,
    InstitutionAssemblySectionText,
...
import { AssemblyValidationIssue } from "@/lib/shared/institutionAssemblyValidation";

interface Props {
    artifacts: RegisteredInstitutionArtifact[];
}

type SectionErrorMap = Partial<Record<InstitutionAssemblySectionKey, string>>;
...
function issueSectionKey(path: string): InstitutionAssemblySectionKey | undefined {
    const [section] = path.split(".");
    return INSTITUTION_ASSEMBLY_SECTION_KEYS.find((entry) => entry === section);
}

function groupIssuesBySection(issues: AssemblyValidationIssue[]) {
    const grouped: Partial<Record<InstitutionAssemblySectionKey, AssemblyValidationIssue[]>> = {};
...
function areSectionTextEqual(
    left: InstitutionAssemblySectionText,
    right: InstitutionAssemblySectionText
): boolean {
    return INSTITUTION_ASSEMBLY_SECTION_KEYS.every((sectionKey) => left[sectionKey] === right[sectionKey]);
}

function updateAssemblySection(
    assembly: InstitutionAssembly,
    sectionKey: InstitutionAssemblySectionKey,
...
function rewriteSlugReferences(assembly: InstitutionAssembly, nextSlug: string): InstitutionAssembly {
...
function JsonSectionEditor(props: {
    ...
    label: string;
    value: string;
    onChange: (sectionKey: InstitutionAssemblySectionKey, value: string) => void;
    syntaxError?: string;
...
export function BuilderAuthoringStudio({ artifacts }: Props) {
    const [registeredArtifacts, setRegisteredArtifacts] = useState<RegisteredInstitutionArtifact[]>(artifacts);
    const initialAssembly = cloneInstitutionAssemblyDraft(registeredArtifacts[0]?.assembly ?? buildBlankInstitutionAssembly());
    const initialSectionText = buildInstitutionAssemblySectionText(initialAssembly);
    const [draftAssembly, setDraftAssembly] = useState<InstitutionAssembly>(initialAssembly);
    const [sectionText, setSectionText] = useState<InstitutionAssemblySectionText>(initialSectionText);
    ...
    const [savedAssemblyJson, setSavedAssemblyJson] = useState(serializeInstitutionAssemblyDocument(initialAssembly));
    const [savedSectionText, setSavedSectionText] = useState<InstitutionAssemblySectionText>(initialSectionText);
    ...
    const draftArtifact = buildDraftInstitutionArtifact(
        deferredAssembly,
    ...
    const hasUnsavedChanges =
        serializeInstitutionAssemblyDocument(draftAssembly) !== savedAssemblyJson
        || !areSectionTextEqual(sectionText, savedSectionText);
    const assemblyOutputPath = `lib/shared/assemblies/${draftAssembly.identity.slug || "institution"}.reference.json`;
    const registryOutputPath = "lib/shared/institutionAssembly.ts";
    const prototypeOutputPath = `/builders/prototype/${draftAssembly.identity.slug || "institution"}`;
    ...
    function applyAssembly(nextAssembly: InstitutionAssembly, options?: { selectedTemplateSlug?: string; importStatus?: string | null }) {
        const nextSectionText = buildInstitutionAssemblySectionText(nextAssembly);
        setDraftAssembly(nextAssembly);
        setSectionText(nextSectionText);
        setSectionErrors({});
        setImportStatus(options?.importStatus ?? null);
        setSavedAssemblyJson(serializeInstitutionAssemblyDocument(nextAssembly));
    ...
    function updateSection(sectionKey: InstitutionAssemblySectionKey, value: string) {
    ...
    function handleBlankDraft() {
        const blank = buildBlankInstitutionAssembly({
            name: "Draft Institution",
            slug: "draft-institution",
            description: "New builder-authored institution assembly.",
    ...
    function handleCloneSelected() {
        ...
        requestDraftReplacement({
            assembly: cloneInstitutionAssemblyDraft(selected.assembly),
    ...
    async function handleImportFile(event: ChangeEvent<HTMLInputElement>) {
        ...
        try {
            ...
            const parsedAssembly = parseInstitutionAssemblyDocument(JSON.parse(text), file.name);
            requestDraftReplacement({
    ...
    async function handleExport() {
        ...
        const blob = new Blob([serializeInstitutionAssemblyDocument(draftAssembly)], { type: "application/json" });
        const url = URL.createObjectURL(blob);
        const anchor = document.createElement("a");
        anchor.href = url;
        anchor.download = `${draftAssembly.identity.slug || "institution"}.reference.json`;
        anchor.click();
    ...
    async function handlePublish() {
        ...
        startTransition(async () => {
            try {
                const result = await publishInstitutionAssemblyAction(serializeInstitutionAssemblyDocument(draftAssembly));
                ...
                setSavedAssemblyJson(serializeInstitutionAssemblyDocument(result.assembly));
                setSavedSectionText(buildInstitutionAssemblySectionText(result.assembly));
                setRegisteredArtifacts((current) => [
                    ...current,
                    buildDraftInstitutionArtifact(result.assembly, current.map((artifact) => artifact.assembly)),
                ]);
    ...
    async function handleUnregister(slug: string, shouldDeleteFile: boolean) {
        ...
        startTransition(async () => {
            try {
                const result = await unregisterInstitutionAssemblyAction(slug, shouldDeleteFile);
            ...
            <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.1fr,0.9fr]">
                <div className="space-y-6">
                    <Card className="p-6 space-y-6">
                        <div>
                            ...
                            <p className="text-lg text-black max-w-3xl">
                                Edit a draft institution assembly, validate it against the live parser and registry rules, preview the derived institution shell, and publish a new registered prototype into the workspace.
                            </p>
                        </div>

                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <FormField label="Institution Name" inputId="authoring-name-input">
                                <Input
                            ...
                            <FormField label="Institution Slug" inputId="authoring-slug-input">
                                <Input
                    ...
                    <div className="grid grid-cols-1 gap-5">
                        {INSTITUTION_ASSEMBLY_SECTION_KEYS.map((sectionKey) => (
                            <JsonSectionEditor
                                key={sectionKey}
                ...
                <div className="space-y-6">
                    ...
                    <Card className="p-6 space-y-4">
                        ...
                        <div className={`rounded border px-3 py-2 text-sm ${publicationReady ? "border-green-200 bg-green-50 text-green-800" : "border-red-200 bg-red-50 text-red-700"}`}>
                            {publicationReady
                                ? "Publish is ready: this will write the authored JSON document and register it in institutionAssembly.ts."
                    ...
                    <Card className="p-6 space-y-6">
                        <div>
                            ...
                            <h2 className="text-2xl font-bold text-black">Derived Institution Shell</h2>
                            <p className="text-sm text-neutral-600 mt-2">
                                The preview below reuses the same derived institution workspace used by published prototype routes.
                            </p>
                        </div>

                        <InstitutionArtifactWorkspace
                            artifact={{
