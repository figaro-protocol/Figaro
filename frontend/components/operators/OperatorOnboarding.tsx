...
export function OperatorOnboarding() {
    ...
    const { updateProfile, isPending: updatePending, isConfirming: updateConfirming, isSuccess: updateSuccess } = useUpdateOperatorProfile();
    const { deactivate, isPending: deactivating, isConfirming: deactivateConfirming, isSuccess: deactivateSuccess } = useDeactivateOperator();
    const { reactivate, isPending: reactivating, isConfirming: reactivateConfirming, isSuccess: reactivateSuccess } = useReactivateOperator();
    ...
    const isActive = profile ? profile[1] : false;
    const currentURI = profile ? profile[2] : undefined;
    const registeredBlock = profile ? profile[3] : null;
    ...
    const busy =
        ...
        regPending || regConfirming || updatePending || updateConfirming;
    ...
    // Load current IPFS profile into form for editing
    useEffect(() => {
    ...
    useEffect(() => {
        if (!registeredBlock || !lockPeriod || !publicClient) { setUnlockTimestamp(null); return; }
    ...
    useEffect(() => {
        if (updateConfirming) setStatus({ type: "pending", message: "Waiting for confirmation..." });
    }, [updateConfirming]);
    useEffect(() => {
        if (deactivateConfirming) setStatus({ type: "pending", message: "Waiting for confirmation..." });
    }, [deactivateConfirming]);
    useEffect(() => {
        if (reactivateConfirming) setStatus({ type: "pending", message: "Waiting for confirmation..." });
    ...
    useEffect(() => {
        if (updateSuccess) { setStatus({ type: "success", message: "Profile updated." }); refetch(); }
    }, [updateSuccess, refetch]);

    useEffect(() => {
        if (deactivateSuccess) { setStatus({ type: "success", message: "Operator deactivated." }); refetch(); }
    }, [deactivateSuccess, refetch]);
    useEffect(() => {
        if (reactivateSuccess) { setStatus({ type: "success", message: "Operator reactivated." }); refetch(); }
    }, [reactivateSuccess, refetch]);
    useEffect(() => {
        if (withdrawSuccess) { setStatus({ type: "success", message: "Deposit withdrawn. Registration removed." }); refetch(); }
            ...
            <div className="space-y-4">
                <div className="border border-gray-200 rounded-lg px-6 py-5">
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-sm font-semibold text-black">Operator inactive</span>
                    </div>
                    <p className="text-sm text-gray-600 mb-5">
                        Deactivated and not visible to assemblies.
                        Reactivate to resume, or withdraw your deposit to remove your registration entirely.
                    </p>
                    <div className="flex gap-3 flex-wrap">
                        <button
                            onClick={handleReactivate}
                            disabled={reactivating || reactivateConfirming}
                            className="px-4 py-2 text-sm bg-black text-white rounded hover:bg-gray-800 disabled:opacity-50"
                        >
                            {reactivating || reactivateConfirming ? "Reactivating..." : "Reactivate"}
                        </button>
                        <button
    ...
    return (
        <div className="space-y-10">
                ...
                {isRegistered && (
                    <div className="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 mb-8">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-semibold text-black">Active operator</span>
                        </div>
                ...
                <div className="border-t border-gray-100 pt-8 space-y-4">
                    {!isRegistered && deposit && (
                        <div className="bg-gray-50 border border-gray-200 rounded-lg px-4 py-4">
                            ...
                            <p className="text-xs text-gray-500">
                                Refundable. Prevents spam registrations. Returned in full when you withdraw.
                            </p>
                        </div>
                    )}
            ...
            {/* Danger zone — active operators only */}
            {isRegistered && isActive && (
                <div className="border border-red-100 rounded-lg px-6 py-5">
                    ...
                    <div className="space-y-6">
                        <div>
                            <p className="text-sm font-semibold text-black mb-1">Deactivate operator</p>
                            <p className="text-xs text-gray-500 mb-3">
                                Marks you inactive. Not visible to assemblies. You can reactivate at any time.
                            </p>
                            <button
                                onClick={handleDeactivate}
                                disabled={deactivating || deactivateConfirming || busy}
                                className="px-4 py-2 text-sm border border-red-300 text-red-600 rounded hover:bg-red-50 disabled:opacity-50"
                            >
                                {deactivating || deactivateConfirming ? "Deactivating..." : "Deactivate"}
                            </button>
                        </div>
                        <div className="border-t border-red-100 pt-5">
                            <p className="text-sm font-semibold text-black mb-1">Withdraw deposit</p>
                            <p className="text-xs text-gray-500 mb-3">
                                Deactivate first, then withdraw. Returns your deposit and removes your registration.
                                {lockPeriod && lockPeriod > 0n && (
                                    <> Deposits are locked for {Math.ceil(Number(lockPeriod) / 86400)} days after registration.</>
                                )}
                            </p>
                            <button
                                onClick={handleWithdraw}
                                ...
                                className="px-4 py-2 text-sm border border-red-300 text-red-600 rounded hover:bg-red-50 disabled:opacity-50"
                            >
                                {withdrawing ? "Withdrawing..." : "Withdraw deposit"}
                            </button>
                        </div>
                    </div>
                </div>
