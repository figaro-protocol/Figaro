...
/**
 * OperatorEditCatalogue — re-uses the wizard's catalogue form to
 * edit a registered operator's pinned catalogue. Routes from the
 ...
 *      then dispatch `OperatorRegistry.updateProfile(newProfileURI)`.
 *
 ...
 * and a price.") since a registered operator with an empty
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (cancelled) return;
                const profile = tryParseOperatorProfileDocument(profileDoc);
                if (!profile) {
 ...
 * pattern on /operators: muted link expands inline to a
