...
export type RuntimeIdentityManifestValidationSeverity = 'error' | 'warning';

export type RuntimeIdentityManifestValidationCode =
...
export interface RuntimeIdentityManifestValidationIssue {
    severity: RuntimeIdentityManifestValidationSeverity;
    code: RuntimeIdentityManifestValidationCode;
    message: string;
    subjectAddress?: `0x${string}`;
    bindingId?: string;
}

export interface RuntimeIdentityManifestDocument {
    version: string;
    subjects: unknown[];
    institutionBindings: unknown[];
    sellerCatalogueMetadata?: unknown[];
    assetDocuments?: unknown[];
}

export interface ParsedRuntimeIdentityManifest {
    version: string;
    subjects: SubjectRecord[];
    institutionBindings: InstitutionBindingRecord[];
    ...
    validationIssues: RuntimeIdentityManifestValidationIssue[];
}
...
function hasMatchingBindingReference(subject: SubjectRecord, binding: InstitutionBindingRecord) {
...
function buildManifestValidationIssues(
    ...
    subjectProvenance: SubjectProvenanceRecord[],
    sourceLabel: string
): RuntimeIdentityManifestValidationIssue[] {
    const issues: RuntimeIdentityManifestValidationIssue[] = [];
    ...
    for (const binding of institutionBindings) {
        ...
        for (const roleBinding of binding.roleBindings) {
            if (roleBinding.scope === 'institution' && (roleBinding.assemblyRoleKinds?.length ?? 0) === 0) {
                issues.push({
                    severity: 'warning',
    ...
    for (const metadata of sellerCatalogueMetadata) {
        ...
        const hasEatsBinding = institutionBindings.some(
        ...
        if (!hasEatsBinding) {
            issues.push({
                ...
                message: `${sourceLabel} seller catalogue metadata for ${metadata.subjectAddress} has no figaro-eats institution binding.`,
                subjectAddress: metadata.subjectAddress,
            });
        }
    }
    ...
    for (const subject of subjects) {
        const subjectBindings = institutionBindings.filter(
...
export function parseRuntimeIdentityManifestDocument(
    value: unknown,
    sourceLabel = 'runtime identity manifest'
): ParsedRuntimeIdentityManifest {
    const record = asRecord(value, sourceLabel);
    const subjects = asArray(record.subjects, `${sourceLabel}.subjects`);
    const institutionBindings = asArray(record.institutionBindings, `${sourceLabel}.institutionBindings`);
    ...
    const parsedInstitutionBindings = institutionBindings.map((entry, index) => parseInstitutionBindingDocument(entry, `${sourceLabel}.institutionBindings[${index}]`));
    ...
    const subjectProvenance = listSubjectProvenanceRecords(parsedSubjects, parsedInstitutionBindings);
    const validationIssues = buildManifestValidationIssues(
        parsedSubjects,
        parsedInstitutionBindings,
    ...
    return {
        ...
        institutionBindings: parsedInstitutionBindings,
...
export function createRuntimeIdentityDataSourceFromManifestDocument(
    value: unknown,
    sourceLabel = 'runtime identity manifest',
    sourceMetadata?: RuntimeIdentitySourceMetadata
): RuntimeIdentityDataSource & ParsedRuntimeIdentityManifest {
    const parsedManifest = parseRuntimeIdentityManifestDocument(value, sourceLabel);

    return {
        ...parsedManifest,
        listSubjectRecords: () => parsedManifest.subjects,
        listInstitutionBindings: () => parsedManifest.institutionBindings,
