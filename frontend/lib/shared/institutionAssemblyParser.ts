...
const SCOPE_TYPES = new Set<ScopeType>(["institution", "process", "order", "role", "mechanism"]);
...
function parseIdentity(value: unknown, path: string): InstitutionIdentity {
    const record = asRecord(value, path);
    return {
...
function parseNarrative(value: unknown, path: string): NarrativeLayer | undefined {
    ...
    const record = asRecord(value, path);
    return {
        institutionSummary: asOptionalString(record.institutionSummary, `${path}.institutionSummary`),
...
export function parseInstitutionAssemblyDocument(value: unknown, sourceLabel = "assembly document"): InstitutionAssembly {
