...
import { parseRuntimeAssetDocument } from '@/lib/shared/runtimeManifest';
import type { SellerBrandingMetadata, SellerCatalogueMetadata } from '@/lib/shared/sellerCatalogueMetadata';
import type { RuntimeIdentityManifestValidationIssue } from '@/lib/shared/runtimeManifest';
...
export interface ResolvedInstitutionShellPresentation {
...
export interface ResolvedInstitutionSkinBundle {
    sourceKind: ResolvedInstitutionShellPresentation['sourceKind'];
    skinId: string;
    subjectAddress?: `0x${string}`;
    bindingId?: string;
...
export interface ResolvedSemanticRuntimeSnapshot {
    assembly: RegisteredInstitutionArtifact['assembly'];
    validation: RegisteredInstitutionArtifact['validation'];
    model: RegisteredInstitutionArtifact['model'];
    riskBoundaries: RegisteredInstitutionArtifact['riskBoundaries'];
    runtime: {
        networkTarget?: string;
        sourceMetadata: RuntimeIdentitySourceMetadata;
        validationIssues: RuntimeIdentityManifestValidationIssue[];
        ...
        visibleModuleIds: string[];
        institutionCapabilities: CapabilityModel[];
        selectedShellPresentation: ResolvedInstitutionShellPresentation;
    };
}
...
export function resolveInstitutionShellPresentation(
    ...
    const subtitle = pickFirstText(
        ...
        assembly.narrative?.institutionSummary,
    ) ?? '';
...
export function resolveInstitutionSkinBundle(
    shellPresentation: ResolvedInstitutionShellPresentation,
): ResolvedInstitutionSkinBundle | undefined {
...
export async function resolveInstitutionSkinBundleFromTransport(
    shellPresentation: ResolvedInstitutionShellPresentation,
    evidenceTransport: Pick<IpfsService, 'resolveFetchUrl'>,
    options: {
        fetcher?: RuntimeAssetDocumentFetcher;
...
): Promise<ResolvedInstitutionSkinBundle | undefined> {
    const baseSkinBundle = resolveInstitutionSkinBundle(shellPresentation);
    ...
    try {
        ...
        return resolveInstitutionSkinBundle({
            ...shellPresentation,
            assetDocument,
        });
    } catch {
...
export function requireAssemblyRuntimeContext(
    ...
    if (!resolved) {
        throw new Error(`Unknown institution assembly: ${slug}`);
    }
...
export function resolveSemanticRuntimeSnapshot(
    ...
    const selectedViewModuleIds = context.artifact.assembly.views
        .filter((view) =>
            view.viewId === 'institution-overview'
    ...
    return {
        ...
        runtime: {
            ...
            boundSubjects: context.boundSubjects.map((subject) => ({
                subject,
                shellPresentation: resolveInstitutionShellPresentation(context.artifact.assembly, subject),
            ...
            institutionCapabilities: deriveInstitutionCapabilitiesFromRuntime(
                context.artifact.assembly.identity.id,
                selectedRole?.roleKind,
                scopedMechanismSelection.visibleMechanisms,
            ),
            selectedShellPresentation: resolveInstitutionShellPresentation(
