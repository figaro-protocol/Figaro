...
import { buildBlankAssembly, cloneAssemblyDraft } from "@/lib/shared/assemblyDraft";
import { validateAssembly, type AssemblyValidationResult } from "@/lib/shared/assemblyValidation";
import { listAssemblies } from "@/lib/shared/assemblyRegistry";
...
export interface RegisterSchemaAction extends BuildAction {
    ...
    schemaKey: string;
    version: number;
    /** keccak256 of the off-chain JSON spec at its canonical URI. Zero when
     *  the spec is not yet pinned. */
    uriHash?: `0x${string}`;
}
...
interface BuildProviderState {
    ...
    updateDraft: (slug: string, assembly: Assembly) => void;
...
export function BuildProvider({
    ...
    const [registeredAssemblies, setRegisteredAssemblies] = useState<Assembly[]>(() => listAssemblies());
    ...
    const createDraft = useCallback((name: string, slug: string) => {
        const assembly = buildBlankAssembly({ name, slug });
        setDrafts((prev) => {
    ...
    const forkAssembly = useCallback((slug: string) => {
        ...
        const assembly = cloneAssemblyDraft(source);
        assembly.identity = {
            ...assembly.identity,
    ...
    const updateDraft = useCallback((slug: string, assembly: Assembly) => {
        setDrafts((prev) => {
            const next = new Map(prev);
    ...
    const validateDraft = useCallback((slug: string): AssemblyValidationResult => {
        ...
        const result = validateAssembly(draft.assembly);
        setDrafts((prev) => {
            const next = new Map(prev);
    ...
    const updateDraftSection = useCallback((slug: string, section: string, jsonText: string): boolean => {
        ...
        try {
            ...
            updateDraft(slug, updated as Assembly);
            return true;
        } catch {
            return false;
        }
