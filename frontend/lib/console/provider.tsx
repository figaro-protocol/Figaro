...
import type { Process, Hex, Address } from "@figaro/core";
import {
    SCHEMA_REGISTRY_ABI,
} from "@figaro/core";
...
import { serializeInstitutionAssemblyDocument } from "@/lib/shared/institutionAssemblyDraft";
...
interface FigaroProviderState {
    ...
    /** Sequencer client (null when no URL configured). */
    sequencer: SequencerClient | null;
            ...
            try {
                ...
                switch (action.type) {
                    ...
                    case "attest-as-seller": {
                        ...
                        // Default schema: lifecycle-v1, stage 1 (fulfillment), zero content ref
                        const { computeSchemaId } = await import("@figaro/core/extensions");
                        const schemaId = computeSchemaId("figaro-lifecycle-v1");
                        const stage = 1;
                ...
                const outcome = await executeBuildAction(buildAction, {
                    registerSchema: async (action) => {
                        ...
                        return schemaWriter.writeContract({
                            ...
                            functionName: "registerSchema",
                            args: [schemaId, BigInt(action.version), schemaId],
                        });
                    },
                    publishAssembly: async (action) => {
                        const { publishInstitutionAssemblyAction } = await import("@/app/builders/authoring/actions");
                        return publishInstitutionAssemblyAction(serializeInstitutionAssemblyDocument(action.assembly));
                    },
                });
