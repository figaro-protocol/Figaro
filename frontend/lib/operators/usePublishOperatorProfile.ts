...
/**
 * usePublishOperatorProfile — atomic publish flow for the operator
...
export interface PublishOperatorInput {
     ...
     *  enforce this but the operator UX expects it (see Step 3 gate). */
    items: CatalogueItemMetadata[];
    /** Operator's preferred unit system; goes onto the catalogue doc. */
...
export interface PublishOperatorOutcome {
    hash: `0x${string}`;
    profileURI: string;
    catalogueURI: string;
}
...
export function usePublishOperatorProfile() {
    const client = usePublicClient();
    const { writeContractAsync, data: hash, isPending, error: writeError } =
        useWriteContract();
    const { isLoading: isConfirming, isSuccess } = useWaitForTransactionReceipt({ hash });

    async function publish(input: PublishOperatorInput): Promise<PublishOperatorOutcome> {
        const registry = getOperatorRegistry();
        if (!registry) {
            throw new Error(
                "OperatorRegistry address not configured (NEXT_PUBLIC_OPERATOR_REGISTRY).",
            );
        }
        if (!client) {
