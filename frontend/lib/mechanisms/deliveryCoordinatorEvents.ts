 ...
 * Proximity proofs are now standard attestations under the figaro-proximity-v1
 * schema, with proof data (band, nonce, deviceSig) in the off-chain contentRef.
 *
 * Used by buildExtendedTimeline() to produce rich dispute evidence that
 * includes:
 *   - En-route / picked-up / delivered attestations
 *   - Proximity proof attestations (separate schema, contentRef holds proof)
 *
...
const PROXIMITY_SCHEMA_KEY = "figaro-proximity-v1";
 ...
 * Attestation event. Proximity proofs are now standard attestations under
 * the figaro-proximity-v1 schema — proof data is in the off-chain contentRef.
 */
