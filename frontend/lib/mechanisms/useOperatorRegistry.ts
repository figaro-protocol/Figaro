 ...
 * Replaces the old deprecated useOperatorProfile stub.
 */
/** [role, active, metadataURI, registeredBlock] */
export type OperatorProfileData = readonly [number, boolean, string, bigint | null];

/**
 * Returns the full on-chain state for an operator, derived from events.
 * Returns undefined if not registered or withdrawn.
 */
export function useOperatorProfile(address: `0x${string}` | undefined) {
    ...
    useEffect(() => {
        ...
        getOperatorState(client, chainId, address).then((state) => {
            if (cancelled) return;
            if (state) {
                setData([state.role, state.active, state.metadataURI, state.registeredBlock] as const);
            } else {
                setData(undefined);
            }
            setIsLoading(false);
...
export function useUpdateOperatorProfile() {
    ...
    async function updateProfile(role: number, metadataURI: string) {
        if (!registry) return;
        return writeContractAsync({
            address: registry,
            abi: OPERATOR_REGISTRY_ABI,
...
export function useDeactivateOperator() {
    ...
    return { deactivate, isPending, isConfirming, isSuccess, error, hash };
}
...
export function useReactivateOperator() {
    ...
    return { reactivate, isPending, isConfirming, isSuccess, error, hash };
}
