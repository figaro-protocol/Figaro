...
function sourceFromAssembly(assemblyId: string, referenceId: string, sourceLabel = "institution assembly") {
    return {
        truthClass: "institution-declared" as const,
        sourceLabel,
...
function deriveRecognizedRoles(mechanism: MechanismAssembly, assembly: InstitutionAssembly): string[] {
...
function deriveMechanismAttachments(mechanism: MechanismAssembly, assembly: InstitutionAssembly): AttachmentModel[] {
    ...
    const contractAttachments: AttachmentModel[] = deriveContractKeys(mechanism, assembly.contracts).map((contractKey) => ({
        ...
        targetId: mechanism.mechanismId,
        label: contractKey,
        description: `${contractKey} is part of the mechanism boundary declared by the institution assembly.`,
    ...
    return [
        {
            ...
            mechanismId: mechanism.mechanismId,
            label,
            description: `${mechanism.displayName} contributes institution-level coordination semantics through the assembly layer.`,
...
export function deriveRiskBoundaryModelsFromAssembly(assembly: InstitutionAssembly): Record<string, RiskBoundaryModel> {
...
function deriveMechanismModel(mechanism: MechanismAssembly, assembly: InstitutionAssembly): MechanismModel {
...
export function deriveInstitutionModelFromAssembly(assembly: InstitutionAssembly): InstitutionModel {
    ...
    return {
        ...
        source: sourceFromAssembly(assembly.identity.id, "institution-model"),
    };
}