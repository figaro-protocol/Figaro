 ...
 *   v4|{pickup_gh}|{dropoff_gh}|{cos}|{mass_g}|{vol_ml}|enc:{addr}|enc:{notes}|items:{b64json}              [LEGACY — items cleartext]
 *   v5|{pickup_gh}|{dropoff_gh}|{cos}|{mass_g}|{vol_ml}|epk:{buyer_pubkey}|enc:{addr}|enc:{notes}|items:{b64json}  [LEGACY — items cleartext]
 *   v6|{pickup_gh}|{dropoff_gh}|{cos}|{mass_g}|{vol_ml}|enc:{addr}|enc:{notes}|enc:items:{b64(iv||ct)}
 *   v7|{pickup_gh}|{dropoff_gh}|{cos}|{mass_g}|{vol_ml}|epk:{buyer_pubkey}|enc:{addr}|enc:{notes}|enc:items:{b64(iv||ct)}
 *
 ...
 * v4/v5 — added line items, but cleartext (legacy; superseded by v6/v7)
 * v6 — v2 + AES-256-GCM sealed line items (same per-order key as address/notes)
 * v7 — v3 + AES-256-GCM sealed line items (same per-order key, ECDH key exchange)
 *
 * Fields:
...
export interface HandoffManifest {
    ...
    /** Buyer's compressed ephemeral secp256k1 public key (v3/v5/v7 only). */
    ephemeralPublicKey?: string;
    /** Itemized order contents. Encrypted in v6/v7 with the same per-order
     *  AES-256-GCM key that protects address + notes. Cleartext only in v4/v5
     *  (legacy). When openManifest is called without a key on a v6/v7 hex,
     *  this field is left undefined to signal the sealed state. */
    lineItems?: LineItem[];
}
...
/** Encode line items as a compact base64url JSON field. Cleartext form
 *  used by legacy v4/v5 manifests; v6/v7 use `encryptLineItems` instead. */
...
/** Decode a `items:{b64json}` field back to LineItem[]. Cleartext form;
 *  see `decryptLineItems` for the v6/v7 sealed counterpart. */
...
export function decodeManifest(hex: string): HandoffManifest | null {
    ...
    try {
        ...
        // Helper: extract line items from trailing fields. Cleartext only
        // (v1/v4/v5 carry `items:`); v6/v7 use `enc:items:` and require a
        // key to decrypt — sync `decodeManifest` returns `lineItems:
        // undefined` for those, signalling the sealed state.
        const extractLineItems = (allParts: string[]): LineItem[] | undefined => {
            const cleartext = allParts.find((p) => p.startsWith("items:"));
            if (cleartext) return decodeLineItems(cleartext);
            const sealed = allParts.find((p) => p.startsWith("enc:items:"));
            if (sealed) return undefined; // sealed without key
            return [];
        };

        // v3/v5/v7: 9+ fields — includes ephemeral public key before encrypted fields
        if ((parts[0] === "v3" || parts[0] === "v5" || parts[0] === "v7") && parts.length >= 9) {
        ...
        // v1/v2/v4/v6: 8+ fields (no ephemeral pubkey)
        if (!["v1", "v2", "v4", "v6"].includes(parts[0]) || parts.length < 8) return null;
        const [version, pickupGeohash, dropoffGeohash, cos, massGrams, volumeMl, addrField, notesField] = parts;
        const isSealed = version === "v2" || version === "v4" || version === "v6";
        return {
            version: "v1" as const,
            pickupGeohash: pickupGeohash || "------",
            dropoffGeohash: dropoffGeohash || "------",
...
// ---------------------------------------------------------------------------
// Encrypted line-item field (v6/v7)
//
// Format: `enc:items:{base64url(12-byte IV || ciphertext)}`
//   ciphertext = AES-256-GCM(JSON.stringify([{n,q}, ...]))
//   The AES key is the same per-order key that seals address + notes,
//   shared with the fulfiller via ECDH alongside address/notes (no extra
//   key exchange is required to add line-item encryption).
// ---------------------------------------------------------------------------

/** Encrypt line items into an `enc:items:` field for v6/v7 manifests. */
export async function encryptLineItems(items: LineItem[], keyB64: string): Promise<string> {
    if (!items.length) return "";
    const compact = items.map((i) => ({ n: i.name, q: i.quantity }));
    const sealed = await aesGcmEncrypt(JSON.stringify(compact), keyB64);
    return "enc:items:" + sealed;
}

/** Decrypt an `enc:items:` field from a v6/v7 manifest. Returns an empty
 *  array on any failure (wrong key, corrupted ciphertext, malformed JSON).
 *  Callers that need to distinguish "absent" from "sealed-without-key"
 *  should check the manifest version first. */
export async function decryptLineItems(field: string, keyB64: string): Promise<LineItem[]> {
    const PREFIX = "enc:items:";
...
export async function sealManifest(
    ...
    // Version selection:
    //   no items, no ECDH       → v2 (sealed addr+notes, no items)
    //   no items, with ECDH     → v3 (sealed addr+notes + epk)
    //   with items, no ECDH     → v6 (sealed addr+notes + sealed items)
    //   with items, with ECDH   → v7 (sealed addr+notes + epk + sealed items)
    // v4 / v5 (cleartext items) are legacy and are not produced by sealManifest;
    // openManifest still decodes them for backwards compatibility.
    ...
    if (hasItems) {
        parts.push(await encryptLineItems(m.lineItems!, k));
    }
...
/**
 * Decode a v2/v3/v4/v5/v6/v7 manifest hex string.
 * - If keyB64 is provided: decrypts address, notes, and (v6/v7) line items.
 * - If keyB64 is null: encrypted fields are returned as sentinel strings or
 *   `lineItems: undefined` so callers can detect the sealed state.
...
export async function openManifest(
    ...
    try {
        ...
        // Helper: extract line items from trailing fields. v4/v5 use
        // cleartext `items:`; v6/v7 use `enc:items:` and require keyB64
        // to decrypt — without a key, returns undefined to signal sealed.
        const extractLineItemsAsync = async (allParts: string[]): Promise<LineItem[] | undefined> => {
            const cleartext = allParts.find((p) => p.startsWith("items:"));
            if (cleartext) return decodeLineItems(cleartext);
            const sealed = allParts.find((p) => p.startsWith("enc:items:"));
            if (sealed) {
                if (!keyB64) return undefined;
                return decryptLineItems(sealed, keyB64);
            }
            return [];
        };

        // v3/v5/v7: 9+ fields — ephemeral public key at index 6
        if ((parts[0] === "v3" || parts[0] === "v5" || parts[0] === "v7") && parts.length >= 9) {
            ...
            const lineItems = await extractLineItemsAsync(parts);
            return {
                version: "v1" as const,
                pickupGeohash: pickupGeohash || "------",
                dropoffGeohash: dropoffGeohash || "------",
        ...
        // v2/v4/v6: 8+ fields
        if (!["v2", "v4", "v6"].includes(parts[0]) || parts.length < 8) {
