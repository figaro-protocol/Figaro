...
export const metadata: Metadata = {
    ...
    description:
        "Between the network opening and the first retroactive reward, the florin has to reach ordinary hands. Match rounds do it permissionlessly: the DAO pays for work it wants done, and a crowd of donors decides who earned it — by a published formula, no committee, no application.",
};
...
export default function MatchRounds() {
    return (
        <section className="container mx-auto px-6 pt-24 pb-16 max-w-2xl">
            ...
            <p className="text-base text-ink-body leading-relaxed mb-5">
                A round is two contracts. The first is a donation rail that holds nothing and gates nothing: a donor sends tokens straight through to a recipient &mdash; any address at all, because there is no recipient list to be on; the set of recipients is simply whoever received a donation. Each donation is a real transfer <em>and</em> a steering signal. The second is a match pool, funded separately in florins, that splits across those same recipients by a published quadratic-funding rule &mdash; a rule that rewards <em>breadth</em> of support. A recipient backed by many independent donors is matched far more than one backed by a single large donor; a recipient with only one donor is matched nothing at all. A small donation floor makes each counted donor real capital rather than a free vote, no single donor can steer more than a set cap toward one recipient, and no recipient can take more than fifteen percent of the whole match. The crowd&apos;s consensus, not its wallets, is what the pool pays.
            </p>
            <p className="text-base text-ink-body leading-relaxed mb-5">
                No committee settles it, and no one applies. The split is computed the same open way the author reward is: anyone recomputes it from the public donation events and posts the result under a bond; a posting that doesn&apos;t match the rule can be challenged and thrown out; a posting no one challenges finalizes, and recipients claim. The formula is a published file whose fingerprint is anchored on the chain, so anyone can rerun it against the record and challenge a mismatch before a single florin moves. There is no panel to lobby and no ballot to win &mdash; only arithmetic anyone can check.
            </p>
            <p className="text-base text-ink-body leading-relaxed">
                It is left open on purpose. At genesis there is no membership list to gate a round on, and drawing one would be exactly the gatekeeping the protocol exists to remove &mdash; so anyone can be a recipient and the crowd does the steering. That is the division of labor: the author reward is <em>retroactive</em>, paying for adoption already recorded; a match round is <em>prospective and open</em>, funding work going forward with a donating crowd choosing who. Both run on a published rule, both settle in the open, and neither has a committee or a vote.
            </p>
            ...
            <ul className="space-y-3 text-base">
                <li>
                    ...
                    <span className="text-ink-body"> &mdash; the live surface: a round, once open, lives at /rounds?pool=&lt;address&gt; &mdash; donate, recompute the split, post it under a bond, finalize, claim.</span>
                </li>
                <li>
