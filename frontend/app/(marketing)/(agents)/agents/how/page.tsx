    ...
    return (
        <>
            <MarketingHero
                ...
                lead={
                    <>
                        A courier&apos;s software takes the delivery leg, posts the bond, and gets paid &mdash; with nobody at the keyboard and nobody&apos;s permission. This page is about agents that <em>participate</em> that way: buyer, seller, courier, or auditor wallets driven by software, signing the same commitments and locking the same doubled bond a human-driven wallet would. A wallet is a wallet; a signature is a signature; a bond is a bond. What the wider market now calls agentic commerce, Figaro already is &mdash; not an agent mode bolted on, but the plain consequence of a kernel that reads a signature, never a species.
                    </>
                }
            >
            ...
            <MarketingSection title="Asset, wallet, operator — three layers.">
                ...
                <p className="text-base text-ink-body leading-relaxed mb-5">
                    A wallet driven by software is one case of a more general pattern. Behind every wallet sits a real-world asset &mdash; a kitchen, a delivery van, a credentialed person&apos;s labor, a public service like a customs clearance. The <em>asset</em> is off-chain and stays on its owner&apos;s books at its carrying value; the protocol never tokenizes it. The <em>wallet</em> is the on-chain representation of that asset&apos;s participation: an address that holds the asset&apos;s token receipts, points to its credentials, and produces the EIP-712 signatures that bind it to a commitment. The <em>operator</em> is whoever controls the wallet&apos;s signing key on the asset&apos;s behalf. Which word applies turns on whose value-add the wallet carries, never on whether a person or a program holds the key: an agent selling its own service is a <em>seller</em> in its own right, exactly as a person selling their own labor is; an agent holding the key for a kitchen or a van is that asset&apos;s <em>operator</em>, exactly as a person running someone else&apos;s kitchen would be.
                </p>
                <p className="text-base text-ink-body leading-relaxed mb-5">
                    The kernel sees only the wallet &mdash; a signature and a bond. Whether the operator behind it is a person at a keyboard or a long-running service is beneath what the kernel sees and irrelevant to resolution. &ldquo;Agent&rdquo; on this page is the operator layer: the agent is the thing operating a wallet that represents a real-world asset.
                </p>
            ...
            <MarketingSection title="Bond-weighted reputation.">
                ...
                <p className="text-base text-ink-body leading-relaxed">
                    Inside the trade, the buyer&apos;s own resolution is what releases every bond at once &mdash; so no evaluator stands between an agent and its payment, and nothing grades the work. A forum &mdash; whether or not the agreement named one &mdash; weighs the data from outside the trade and before that resolution; it rules on the parties, and cannot resolve the process itself.
                </p>
            </MarketingSection>

            <MarketingSection title="No API keys, no rate limits, no data moats.">
                <p className="text-base text-ink-body leading-relaxed mb-5">
                    A courier&apos;s agent finds its next delivery leg the way anyone else does &mdash; by reading the chain. A leg is the unit an agent takes: one bonded link in a chain that can carry many sellers &mdash; each bound by its own signature, all of them resolving together on the buyer&apos;s single resolution &mdash; so an agent can hold one link of a trade far larger than anything it could carry alone. Coordination happens through the public data the chain and its events already carry &mdash; an agent indexes them directly and derives whatever view its question needs: where the work is, where it is going, what resolved, what came before. No list of views exists to be complete; the data layer is the same for every reader. They do not all carry the same warrant, though: a bond the protocol enforced and a service area a seller declared are both public, and only one of them was checked &mdash; which of the four boundaries each layer sits behind is set out on <Link href="/data" className="underline">Data</Link>.
                </p>
                <p className="text-base text-ink-body leading-relaxed">
                    An agent reads every one of those signals straight from the chain and the pinned documents, permissionlessly. Competing agents and collaborating ones see the same signals; advantage comes from better interpretation, not better access. And every process that resolves adds to that public record, so the corpus an agent learns from is one that never stops growing.
                </p>
            </MarketingSection>
            ...
            <MarketingSection title="ERC-8004 interop, by metadata convention.">
                ...
                <p className="text-base text-ink-body leading-relaxed mb-5">
                    Autonomous agents that want cross-protocol discoverability declare ERC-8004-compatible service endpoints in their <code>MembersRegistry.metadataURI</code> JSON. No new smart contract is needed and nothing has to be registered &mdash; the registry already carries arbitrary metadata. An author who wants the endpoint shape published as a term of an agreement rather than as profile metadata can <Link href="/clauses" className="underline">register a clause</Link> for it; none is reserved for the purpose.
                </p>
                <p className="text-base text-ink-body leading-relaxed">
                    An agent&apos;s <code>did:web</code> identifier resolves to a DID Document whose verification methods name the agent&apos;s on-chain address in CAIP-10 form &mdash; a consistency check, not proof of control, since a DID Document is self-published. The round-trip &mdash; resolve a document, check it against the address, build one for your own wallet &mdash; ships in <code>@figaro-protocol/sdk/agent</code>; the calls are named and specified in the <a href="https://github.com/figaro-protocol/Figaro/blob/main/sdk/README.md#figaro-protocolsdkagent--agent-coordination" target="_blank" rel="noopener noreferrer" className="underline">SDK README</a>, never restated here.
                </p>
            </MarketingSection>

            <MarketingSection title="How an operator works — the SDK and a policy." bottomPad="wide">
                <p className="text-base text-ink-body leading-relaxed mb-5">
                    An operator drives <code>@figaro-protocol/sdk/agent</code> for one wallet: sync the chain, see the actions available to that wallet (initiate a process, resolve, attest as buyer or as seller &mdash; proposed from the live registries and the chain&apos;s process state), apply the owner&apos;s policy, and pass each approved action to the policy signer &mdash; a separate process the owner runs, where the key lives &mdash; to sign and submit. The SDK ships the whole toolkit, the signer included; nothing else is installed. An operator can be a person clicking &ldquo;approve,&rdquo; a rule-based script, or an LLM agent &mdash; the protocol does not care which. It does care what signs: the kernel verifies both commitment signatures by ECDSA recovery alone and runs no ERC-1271 check, so any signer producing a standard secp256k1 signature drives a wallet here &mdash; an externally-owned account, a hardware wallet, an MPC or threshold scheme that outputs one signature &mdash; while a contract account such as a Safe cannot hold a buyer or seller role at all, and has to transact through a funded externally-owned account it controls.
                </p>
                <p className="text-base text-ink-body leading-relaxed mb-5">
