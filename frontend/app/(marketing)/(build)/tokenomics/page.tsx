    ...
    return (
        <>
            ...
            <MarketingSection title="Four kinds of token meet in one trade.">
                <p className="text-base text-ink-body leading-relaxed mb-5">
                    A trade is denominated in a single unit, fixed at the first signature and binding on every later link, so the parties have to agree on one. The ordinary answer is a unit recognized widely enough to make agreeing cheap &mdash; a stablecoin, or the network&apos;s own asset. Nothing about the mechanism prefers any of them: the arithmetic compares a bond against a value, and any standard token quotes both.
                </p>
                <p className="text-base text-ink-body leading-relaxed mb-5">
                    A designer who publishes an assembly may pin the unit every trade running it is denominated in &mdash; including a token of their own. Then demand for that token tracks their assembly&apos;s adoption: a moat they build, and whose risk they carry. A buyer who would rather not hold it picks an assembly denominated in something they already do; the exit is one click, which is what keeps the moat honest.
                </p>
                <p className="text-base text-ink-body leading-relaxed mb-5">
                ...
                <p className="text-base text-ink-body leading-relaxed">
                    The florin is the first kind and only the first kind: the pure case of a unit chosen because everyone expects everyone else to accept it. It is not a community&apos;s token and not a designer&apos;s moat. It carries no issuer, no redemption claim, no yield, and no path by which anyone is charged for it, and it is never required and never favored &mdash; no operation of the protocol denominates in it, and nothing anywhere is cheaper, faster, or better secured for choosing it. Strip all of that away and one use is left, which is the one it was built for: two strangers who share no other unit can still agree on this one, and choosing it says so out loud.
                </p>
            </MarketingSection>

            <MarketingSection title="Nobody pays for any of this." bottomPad="wide">
                <p className="text-base text-ink-body leading-relaxed mb-5">
                    Nothing is taken anywhere in the kernel &mdash; no skim on a payment, no charge for a registration, nothing minted when a trade resolves. The reserve was set aside at genesis, and no levy on trade fills it: no volume of trade adds a florin to it or brings one forward.
                </p>
                <p className="text-base text-ink-body leading-relaxed mb-5">
                    What designing and selling do require is a live stake: ETH held against your registration, on the designer&apos;s side and the seller&apos;s side both, and use counts only while both stay live. The stake is returnable &mdash; withdraw it and your entry stops being surfaced and stops scoring at once, and the stake itself follows: a designer&apos;s right away, a seller&apos;s only after a cooldown fixed at deployment &mdash; so a fresh seller identity always costs parked-stake time before the last one&apos;s returns. That is the whole loop, and it runs the right way round: what you carry is exposure to the growth your own work produces, never a rent taken out of someone else&apos;s trade. And the stake is the base network&apos;s own token by design: work that gets used draws trade onto the network, trade is paid for in the network&apos;s compute, and the network&apos;s token is what that compute is bought with &mdash; so every registrant holds their stake in the one asset their own success creates demand for. Three tokens, three jobs, never collapsed: the stake is the network&apos;s token, the bond is the trade&apos;s ERC-20, the reward is the florin.
                </p>
                <p className="text-base text-ink-body leading-relaxed">
