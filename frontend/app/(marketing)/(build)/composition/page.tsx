            ...
            <MarketingSection title="What is already wired in.">
                ...
                <ul className="space-y-4">
                    ...
                    <LabelledListRow label="Multisender" id="multisender" uppercase>
                        <strong>Disperse.</strong> Post-resolution payout routing through the composed public multisender: one payment, many recipients, one transaction &mdash; it goes through whole or not at all. A resolved wallet splits its own receipts the moment they land, to addresses it earmarked itself: a fiscal set-aside, savings, what it owes a supplier, a mutual-aid contribution. The split is the wallet&apos;s own arithmetic over its own tokens, done afterwards, on its own initiative, spending a balance it already holds &mdash; nothing in the trade routed any of it, and no clause computed it. The composed smart contract is the Disperse deployment &mdash; <code>0xD152f549545093347A162Dce210e7293f1452150</code>, the same address across chains &mdash; and the call is its own <code>disperseToken(token, recipients, values)</code>, made by the wallet itself. Its entry, and the record key a deployment wires it under, are on <Link href="/spec#multisender" className="underline">/spec</Link>. What falls out is a self-sovereign fiscal trail nobody had to assemble later: the set-aside is dated, the recipient is named, and it stands on the same public record as the trade that paid for it.
                    </LabelledListRow>
