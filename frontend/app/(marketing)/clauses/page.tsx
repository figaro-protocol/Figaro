...
export default function Clauses() {
  ...
  "fields": [ { "name": "note", "type": "string", "required": true } ]
                ...
                <p className="text-sm text-ink-body leading-relaxed mb-5">
                    <strong><code>block</code> is how the clause shows up in the UI &mdash; add it only if you want that.</strong> Omit it and the clause still validates and attests; it just won&apos;t surface in the designer or runtime. <code>block</code> is organized into three phase sections, each named for its reader: <code>design</code> (the <code>article</code> it <strong>groups</strong> under, what it <code>nestsUnder</code> as a sub-clause, and <code>fills</code> &mdash; the fields the designer authors into the template), <code>checkout</code> (which fields fold from the seller&apos;s catalogue or profile), and <code>runtime</code> (interaction standards, runtime inputs, hand-off stages). Grouping is <code>article</code>&apos;s job; <code>nestsUnder</code> is only for a genuine sub-detail (never a whole clause under a plain number).
                </p>
                <p className="text-sm text-ink-body leading-relaxed">
                    <code>block.design.article</code> is your clause&apos;s <strong>group</strong> &mdash; coordination, logistics, emissions &mdash; the section it groups under in the designer, and the only classification it needs. It is a <em>reader</em> concern: nothing on chain stores it, and the reward mechanism never reads it. What the reward mechanism reads is a separate, declared value &mdash; the top-level <code>rpgfTag</code> in your spec (the one spec attribute that reaches the chain: registration hashes it into <code>registerClause</code>), <code>keccak256(&quot;geo&quot;)</code> for the substrate-broadening boost and <code>bytes32(0)</code> for untagged, which is the default and carries no penalty. Two axes, never merged: <code>article</code> sorts your clause for people, <code>rpgfTag</code> declares what kind of contribution it is. Validate the whole spec with <code>parseClauseSpec</code> from <code>@figaro/sdk/clauses</code> before you register; it rejects a malformed spec with the exact path that failed.
                </p>
