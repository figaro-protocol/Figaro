...
export default function Clauses() {
  ...
  "fields": [ { "name": "note", "type": "string", "required": true } ]
            ...
            <MarketingSection title="Registered clauses, by article.">
                <p className="text-sm text-ink-body leading-relaxed mb-6">
                    The reference set spans assembly topology, commerce primitives, emissions accounting and offsets, lifecycle and proximity, sovereign process logs, and legal anchoring. One &mdash; <code>figaro-topology-v1</code> &mdash; is agreement-only: committed at agreement signing, with no on-chain validator.
                </p>
                <ClauseInventory />
            </MarketingSection>
            ...
            <MarketingSection title="Where to look">
                <ul className="space-y-3 text-sm text-ink-body leading-relaxed">
                    ...
                    <li><strong>Registration path:</strong> <code>ClauseRegistry.registerClause(clauseId, version, contentHash, metadataURI, family)</code> &mdash; permissionless, first-write-wins, immutable. No validator to bind; registration alone makes the clause attestable. Contract catalogue at <Link href="/spec" className="underline">/spec</Link>.</li>
