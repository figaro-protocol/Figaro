...
const JUMP_LINKS: { href: string; label: string }[] = [
    ...
    { href: "#first-order", label: "First order — the books stop being manufactured" },
...
export default function Consequences() {
    return (
        <>
            ...
            <MarketingSection title="First order: the books stop being manufactured." sectionId="first-order">
                <p className="text-base text-ink-body leading-relaxed mb-5">
                    The first stratum to thin is the one whose product is belief in books. Bookkeeping kept so that someone else will accept it, external audit, escrow agents, reconciliation between two organizations&apos; separately maintained books, compliance reporting, customs brokerage: these are not inefficiencies, they are the price of a world in which a ledger is an assertion by an interested party. That price falls when the data is a byproduct of resolution rather than a separate construction. A resolved process closes its own ledger period, and the buyer&apos;s checkout lines, the audit trail, and the financial presentation are three views of one itemized decomposition &mdash; the argument is <Link href="/papers/self-closing-ledger-periods" className="text-ink-heading font-medium hover:underline">Self-Closing Ledger Periods</Link>.
                </p>
                <p className="text-base text-ink-body leading-relaxed mb-5">
                    These professions do not disappear because they are automated. They thin because the question they answer stops being asked. Their successors move upstream, to design time: authoring the clause that routes a fiscal share, verifying that a published spec says what the people binding to it believe it says, designing an assembly whose data satisfies a given regulator by construction rather than by later assembly. That work is paid for the same way the rest of the commons is &mdash; retroactively, from what actually gets used (<Link href="/clauses" className="text-ink-heading font-medium hover:underline">Clauses</Link>, <Link href="/rpgf" className="text-ink-heading font-medium hover:underline">Rewards</Link>).
                </p>
                <p className="text-base text-ink-body leading-relaxed">
            ...
            <MarketingSection title="Third order: the state's modality inverts." sectionId="third-order">
                ...
                <p className="text-base text-ink-body leading-relaxed mb-5">
                    <strong className="text-ink-heading font-medium">Economic membership decouples from territory.</strong> The precondition for participating is a key and a bond, not a recognized civil-legal subjecthood &mdash; which matters most for the people who occupy the negative space of those frames. The claim is bounded and should stay bounded: it grants a capacity to have commerce where the capacity to have rights is denied, which is not the same thing and is not a substitute for it, and it offers nothing at all to those whose displacement has also stripped away any means to post a bond (<Link href="/papers/wallet-without-polity" className="text-ink-heading font-medium hover:underline">The Wallet Without a Polity</Link>). At the scale of trade routes, a permissionless, decentralized resolution layer can be common ground beneath rival corridor projects, without touching who owns the ports and cables (<Link href="/papers/corridors-without-a-hegemon" className="text-ink-heading font-medium hover:underline">Corridors Without a Hegemon</Link>).
                </p>
                <p className="text-base text-ink-body leading-relaxed mb-5">
                    <strong className="text-ink-heading font-medium">Denomination pluralizes.</strong> Each process is denominated in one token the parties chose, and the choice carries meaning the payment itself does not: a stablecoin expresses a legal-regime preference, a community&apos;s own token expresses where the value is meant to land. A displaced community&apos;s token, spent in Los Angeles or Lima, can sustain value at home without a fiat pipeline. The protocol&apos;s own token is deliberately the least interesting object in this picture: a named Schelling point that no one is charged for, with no yield and no claim on resolution &mdash; designed by refusal, as <Link href="/papers/florin-schelling-point-token" className="text-ink-heading font-medium hover:underline">the florin paper</Link> sets out.
                </p>
                <p className="text-base text-ink-body leading-relaxed">
                    The cost here is the least resolved line in this projection, and it should be read as unresolved rather than answered. Compulsion is how civilization pays for the goods voluntarism under-provides: pandemic preparedness, defense, century-scale environmental response, long-horizon research. Free-riding is individually rational and the payoff horizon exceeds any single process&apos;s duration, so voluntary inclusion in templates cannot substitute for compulsory taxation at the scale those goods require. Fiscal compulsion eroding before its substitutes mature is the way this projection does real damage, and nothing in the mechanism prevents it.
                </p>
            </MarketingSection>

            <MarketingSection title="Fourth order: the economy stops being human-paced." sectionId="fourth-order">
                <p className="text-base text-ink-body leading-relaxed mb-5">
                    Nothing in a signature says what produced it. A wallet driven by software commits, bonds, and is paid on exactly the same footing as one driven by a person (<Link href="/agents" className="text-ink-heading font-medium hover:underline">Agents</Link>). Two properties compound with that. Coordination costs a signature and fixed gas rather than a negotiation, and commit and resolve require no synchrony between the parties. Together they expand the set of coordination worth doing to include everything previously too small, too fast, or too far away to be worth a contract at all.
                </p>
                <p className="text-base text-ink-body leading-relaxed mb-5">
                    On a two-hundred-year horizon, most processes are then machine-to-machine, at frequencies no human observes, with human-meaningful surfaces sitting on top of them the way a dashboard sits on top of a network. Assets hold their own wallets: the aircraft, the kiln, the water-treatment plant earning what its own maintenance costs, with the operator a policy running on the asset&apos;s behalf rather than an owner-manager watching a queue. This is not tokenized ownership of the asset and never becomes it &mdash; the asset stays on its owner&apos;s books at carrying value; the wallet is its signing apparatus and its earnings pocket, and it stays in the market only as long as its receipts cover its real-world operating costs, exactly as a node stays online only while what it earns covers its electricity.
                </p>
                <p className="text-base text-ink-body leading-relaxed mb-5">
                    Institutions become transaction-scoped by default: assembled for one process out of the parties actually doing the work, dissolved at resolution, with the standing institution becoming the exception that has to justify why it stands. And the data allocation of the platform century inverts: the aggregate map public, the detail sealed with whoever produced it and sold, when it is sold, on its owner&apos;s terms (<Link href="/data" className="text-ink-heading font-medium hover:underline">Your Data, Your Terms</Link>). What that newly makes possible is a live and verifiable picture of an economy &mdash; enough to reopen, with evidence, the calculation debates that could previously only be argued in the abstract. A second economy runs on trade&apos;s exhaust, and the people who produced the exhaust are the ones holding it.
                </p>
                <p className="text-base text-ink-body leading-relaxed">
            ...
            <MarketingSection title="Where power goes." sectionId="power">
                <p className="text-base text-ink-body leading-relaxed mb-5">
                    Power gets re-priced and relocated, not eliminated, and the honest version of the projection names where it lands. What the intermediary used to meter redistributes to the hands in the chain, because every value-adder is paid its own line directly rather than a residual after the platform&apos;s cut. Four concentrations survive, and one of them gets stronger.
                </p>
                <ul className="space-y-5 text-base text-ink-body mb-5 ml-6">
                    <li>
                        <strong className="text-ink-heading font-medium">Capital.</strong> Bonding capacity gates participation. The doubled bond converts a behavioral question into a solvency one, and solvency is unevenly distributed. A participant with no access to the tokens a bond requires is simply outside. That is a credit problem the mechanism does not solve and should not be described as solving.
                    </li>
                    <li>
                        <strong className="text-ink-heading font-medium">Design.</strong> Whoever designs the clauses and assemblies that everyone reuses shapes the defaults everyone accepts, and defaults are the quietest form of power there is. Permissionless design mitigates it &mdash; anyone may publish a competing clause, and the reward for design is uniform, counting real usage and nothing else &mdash; but mitigation is not elimination, and a widely adopted default is a position worth holding.
                    </li>
                    <li>
                    ...
                    <li>
                        <strong className="text-ink-heading font-medium">Physical force.</strong> Untouched. The state&apos;s monopoly on violence is not contested by a resolution rule, and nothing on this page should be read as claiming otherwise.
                    </li>
                </ul>
                <p className="text-base text-ink-body leading-relaxed">
                    And the registries have no morality. A cartel composes as cheaply as a cooperative; a supply chain that treats people badly resolves as reliably as one that does not. The mechanism makes coordination cheap and legible &mdash; not good. The one thing legibility changes is that unkindness has to happen in the open, on data it cannot revise afterwards, and at its own expense.
                </p>
            </MarketingSection>
