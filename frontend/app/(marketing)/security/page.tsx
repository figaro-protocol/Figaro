    ...
    return (
        <>
            ...
            <MarketingSection title="Can someone hijack a clause or seller slot?" sectionId="builders-registries">
                ...
                <p className="text-base text-ink-body leading-relaxed mb-5">
                    All three artifact families are anchored by the same anti-spam mechanism: a reclaimable ETH deposit &mdash; staked intent, not a fee. No admin can seize it; the registrant reclaims the exact amount by withdrawing, which de-surfaces the artifact (readers hide what no longer carries a live stake), so polluting a registry costs deposit &times; the time it stayed surfaced. None of the three carries a time lock, and the deposit amount is set per deployment &mdash; the protocol fixes the mechanism, not the number. The asymmetry that does exist is in what a withdrawal leaves behind. A clause&apos;s clauseId binding and an assembly&apos;s composition binding are permanent &mdash; never cleared on withdraw &mdash; because agreements already committed against them must keep resolving forever; only the stake and the discovery surfacing move. A seller registration is keyed to a wallet, and withdrawing clears it, freeing that address to register again: a clause or an assembly is a permanent published artifact, a seller is a live, wallet-keyed identity.
                </p>
                <p className="text-base text-ink-body leading-relaxed">
