  ...
  "multisender": "0x…"
                ...
                <ul className="space-y-3 mb-4">
                    ...
                    <LabelledListRow label="usageCounter · rpgfMinter" labelWidth="wide">
                        <code>UsageCounter</code> counts a settled process&apos;s use of a clause or assembly at the moment it happens: <code>recordClauseUsage</code> is permissionless and verifies for itself that the order resolved and that the artifact was merkle-committed in the signed <code>agreementHash</code>, so nothing about the caller is trusted. Accrual buckets into fixed periods and a period&apos;s counts are final once it ends. <code>RpgfMinter</code> pays each tranche pro rata from its closed period. Nothing is posted, bonded, or challenged, so there is no forum to wire.
                    </LabelledListRow>
