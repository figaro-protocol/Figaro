...
export default function Integrate() {
    return (
        <>
            ...
            >
                <p className="text-sm text-gray-500 leading-relaxed max-w-2xl mt-4">
                    Kernel context lives at <Link href="/protocol" className="underline hover:text-black">Protocol</Link>; contract catalogue at <Link href="/spec" className="underline hover:text-black">/spec</Link>; schema architecture at <Link href="/schemas" className="underline hover:text-black">/schemas</Link>; composition tools at <Link href="/builders" className="underline hover:text-black">/builders</Link>.
                </p>
            </MarketingHero>
            ...
            <MarketingSection eyebrow="Subpaths" title="Read, reconstruct, propose.">
                ...
                <ul className="space-y-4">
                    ...
                    <LabelledListRow label="/schemas" labelWidth="narrow" uppercase>
                        <strong>Meta-schema validator + per-schema content encoders.</strong> Canonical spec format that the SP1 prover and Solidity validators mirror. Layer-A of the three-layer validation pattern (see <Link href="/schemas" className="underline hover:text-black">/schemas</Link>).
                    </LabelledListRow>
                </ul>
            </MarketingSection>
            ...
            <MarketingSection eyebrow="Quick start" title="Install, fetch, reconstruct.">
                <pre
                    tabIndex={0}
                    className="font-mono text-xs bg-gray-50 border border-gray-200 rounded px-3 py-2 mb-3 overflow-x-auto whitespace-pre"
                >
                    <code>npm install @figaro/core viem</code>
                </pre>
                <pre
                    tabIndex={0}
                    className="font-mono text-xs bg-gray-50 border border-gray-200 rounded px-3 py-3 overflow-x-auto whitespace-pre"
                >
...
// — the same state the contract itself holds.`}</code>
            ...
            >
                ...
                <h3 className="text-xl font-bold text-black mb-4">Compositional surfaces.</h3>
                <ul className="space-y-4">
                    <LabelledListRow label="Forums" uppercase>
                        <strong>Kleros, SIAC, ICC, courts.</strong> Parties&apos; agreement designates the forum; Figaro exports its evidence bundle there. Kernel does not adjudicate. Kleros wired today; other forums are off-chain referents named in the agreement. See <a href="/papers/figaro-legal.pdf" className="underline hover:text-black">Paper E (legal)</a>.
                    </LabelledListRow>
            ...
            <MarketingSection eyebrow="Where to look" bottomPad="extra">
                ...
                <p className="mt-8 text-sm text-gray-600 leading-relaxed">
                    Related:&nbsp;
                    <Link href="/protocol" className="underline hover:text-black">Protocol</Link>{" "}(kernel invariants);&nbsp;
                    <Link href="/cryptoeconomics" className="underline hover:text-black">Cryptoeconomics</Link>{" "}(academic frame);&nbsp;
                    <Link href="/builders" className="underline hover:text-black">Builders</Link>{" "}(composition tools);&nbsp;
                    <Link href="/local-commerce" className="underline hover:text-black">Local Commerce</Link>{" "}(reference assembly);&nbsp;
                    <Link href="/discover" className="underline hover:text-black">Discover</Link>{" "}(operator catalogue);&nbsp;
                    <Link href="/compliance" className="underline hover:text-black">Compliance</Link>{" "}(audit framework consuming reconstructed state);&nbsp;
                    <Link href="/schemas" className="underline hover:text-black">Schemas</Link>{" "}(attestation content for indexers).
                </p>
            </MarketingSection>
        </>
    );
