    ...
    return (
        ...
        >
            <PaperSection title="1. Introduction">
                <p>
                    The Figaro kernel is a settlement primitive whose frozen invariants produce a Nash equilibrium &mdash; subgame-perfect at the bilateral edge, where the buyer&rsquo;s post-performance preference for resolving is what makes it credible &mdash; under asymmetric bonding rules and buyer dominance with atomic resolution. As written, the kernel satisfies those invariants under machine checking, and what that covers is stated here in the form it takes wherever the claim is made: the machine checks are the authoring project&rsquo;s own and no external audit of them has been performed; four methods bear on the code &mdash; exhaustive model checking of a state-machine specification, property-based fuzzing of the compiled bytecode, symbolic execution over the entry points, and specification checking of declarative rules against the bytecode &mdash; every bound in the model-checked configuration being two (two buyers, two sellers, two concurrent processes, two sub-orders beyond each root), while the equilibrium argument itself is machine-checked apart from the code, in a dependency-free Lean 4 development (core Lean and its arithmetic decision procedure, no Mathlib) that establishes it at arbitrary chain length with the cumulative accumulator derived rather than assumed, over the payoff table the bounded invariants pin to the kernel. The kernel itself stops at its boundary because that is where the formal results live.
                </p>
                <p>
