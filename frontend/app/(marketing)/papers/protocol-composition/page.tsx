...
const CATALOGUE: [string, string][] = [
    ["Commerce", "the denomination, payment amount, and line items — the commercial half of every order"],
    ...
    return (
        ...
        >
            <PaperSection title="1. Introduction">
                ...
                <p>
                    Section 6 then puts both rules to work on three concrete compositions &mdash; an arbitration forum&rsquo;s ruling consumed as resolution input, a wallet-side swap as denomination on-ramp, and post-resolution payout routing &mdash; two of which the rules license by keeping them out of the protocol.
                </p>
            </PaperSection>
                ...
                <PaperSubsection title="5.2 The coordinator pattern, formally">
                    <FormalBlock label="Proposition 5.2 (Invariant preservation under composition).">
                        ...
                        <ul className="space-y-1 list-none pl-0 text-sm">
                            ...
                            <li>(ii) <em>No alternative resolution path.</em> <Math>{"\\mathcal{M}"}</Math> does not provide an operation that produces value flows equivalent to the kernel&rsquo;s atomic resolution but bypasses it or modifies its preconditions, and holds no discretion over a live process&rsquo;s resolution.</li>
                        ...
                        <table className="w-full border-collapse text-xs">
                            ...
                            <tbody>
                                {[
                                    ...
                                    [
                                        "(ii) No alternative resolution path",
            ...
            <PaperSection title="6. Worked Instances: Three Compositions the Rules License">
                ...
                <PaperSubsection title="6.3 Payout routing as the ordinary next act after resolution">
                    <p>
                        Resolution pays each leg to the address that signed for it: one recipient per leg, by the kernel&rsquo;s own arithmetic, with no splitting construct and no payout schedule. A resolved party very often wants the opposite &mdash; to divide what it just received among several earmarked destinations: a fiscal remittance, an obligation to a lender or a cooperative, a savings position, a share to contributors who worked off the bonded chain. It does that by dispersing its own receipts to several addresses in one subsequent transaction, through an ordinary batch-dispersal contract of the kind public networks already carry.
                    </p>
                    <p>
                        This is a default of resolution in the plain sense &mdash; the common thing a resolved wallet does next, and a surface worth offering beside the proceeds &mdash; and in no other sense. Nothing about it is automatic; the kernel neither triggers it nor could observe that it occurred; and it happens strictly downstream of the bonded state, on funds that are already the recipient&rsquo;s. Against Proposition 5.2 the composition does not even arise: the dispersal contract reads no kernel state, writes none, custodies no bond, and holds no relation to any process. Against Proposition 3.2 it fails the first condition for the same reason the swap does, and belongs where it sits, at the wallet edge.
                    </p>
                    <p>
                        Its interest is evidentiary rather than structural. Because the routing happens on the same public ledger as the resolution that funded it, the trail is a byproduct rather than a filing: each disbursed leg is reconcilable against the resolution it came out of, by anyone, without the recipient maintaining a separate account of it. A participant who wants to demonstrate to a tax authority, a lender, or a cooperative&rsquo;s membership where its receipts went can point at the chain instead of at a bookkeeping claim &mdash; and this holds without the protocol having modeled fiscal routing at all, which is the property the decision rule was protecting when it kept the routing out.
                    </p>
                </PaperSubsection>
