    ...
    return (
        ...
        >
            ...
            <PaperSection title="2. The Settlement Primitive">
                <p>The bonded primitive&rsquo;s mechanism-design derivation and its formal verification are out of scope for the present paper. Three of its results are used below and are stated here in substance, as given: the <em>bonding equilibrium</em>, under which cooperation weakly dominates defection for both parties and the cooperative profile is the unique survivor of iterated elimination of weakly dominated strategies; the <em><Math>{"N"}</Math>-party extension</em>, under which each seller bonds against cumulative upstream value so that the same equilibrium holds at every position of a process chain; and the <em>escape-hatch theorem</em>, under which augmenting the bonded game with any exit action returning some or all of a bond either destroys that dominance for at least one party or makes resolution turn on an unbonded third party outside the bond structure. All three are derived in <Link href="/papers/asymmetric-bonding" className="text-ink-heading hover:underline">Asymmetric Bonding and Buyer Dominance</Link>; they are results this paper composes onto, and Section 5 says explicitly which of its own claims rest on them. We summarize the kernel features the present paper relies on.</p>
            ...
            <PaperSection title="6. Worked Instances: Three Compositions the Rules License">
                ...
                <PaperSubsection title="6.2 A wallet-side swap as denomination on-ramp">
                    ...
                    <p>
                        It happens before the bonded state exists. In the simplest form the party swaps at a public venue in one transaction and commits in another; by the time the signed commitment reaches the kernel, the party holds the process denomination and the kernel sees an ordinary commit with nothing composed onto it. The swap-funded composer of Section 5.3 is the same act with tighter packaging &mdash; the conversion and the commitment in one transaction, with the route bound into the signature so the party cannot be routed somewhere it did not agree to &mdash; and it is the harder case, which is why it was treated there under condition (i) rather than here.
                    </p>
                    <p>
