    ...
    return (
        <PaperLayout slug="florin-schelling-point-token"
            ...
            abstract={
                <>
                    ...
                    <p>
                        The paper sits within a lineage, and the disanalogy is as load-bearing as the analogy. Nakamoto (2008) demonstrated that money could be a protocol rather than an institution, with BTC as the focal Schelling point for a peer-to-peer cash system. Buterin (2014) demonstrated that contracts could be composable, with ETH as the focal point for programmable settlement. The present work extends the lineage one step in <em>what becomes composable</em> &mdash; <em>institutions</em> themselves, through the Figaro primitive &mdash; and the florin is the focal point for participants who coordinate around that composition. What it does not extend is the ladder of monetary function: BTC and ETH each carry a mechanism that requires the unit (issuance cost, gas), whereas the kernel takes any fungible token as bond and payment, so no protocol operation denominates in the florin and none requires it. Its tie to its base is focality alone, and the reader who takes the ladder as &ldquo;the next unit&rdquo; has taken the one reading the design forecloses.
                    </p>
                </>
            }
            references={
        ...
        >
            ...
            <PaperSection title="7. Staged Distribution over Counted Adoption">
                <p>
                    The 600M share is realized by a single minter holding one budget per accrual period, all nine fixed at deployment and validated on deployment against the counting layer&rsquo;s own period schedule, so the two cannot drift apart. Nothing is posted, bonded, or challenged. Adoption of the artifacts the substrate-broadening rule scores is counted on chain at the moment it happens, by a permissionless recording call that verifies for itself that the order settled and that the artifact was committed in the agreement the parties signed; accrual buckets into fixed annual periods, and each period&rsquo;s budget pays for that period alone, once it has closed and its numbers can no longer move. A claim is therefore a division over quantities the chain already holds &mdash; a wallet&rsquo;s artifacts&rsquo; score for the period over the period&rsquo;s total score, minted straight to the claimant &mdash; not a payout against an asserted root, and there is no root, no proof of inclusion in a recipient list, and no list. Two earlier constructions were considered and rejected, both of which reconstructed adoption after the fact: a zero-knowledge proof over a supplied event window, which certifies that the rule was applied to those inputs but not that the inputs match chain history; and a posted root under a bonded challenge, which closes that hole but buys it with a challenge period, a bond-settlement forum, and a liveness assumption. Counting the fact when it occurs needs none of the three. There is no privileged path to add a period, extend the schedule, resize a budget, or alter who is eligible: eligibility is a registry fact about who authored an artifact and whether its stake is still posted.
                </p>
            ...
            <PaperSection title="9. Rejected Design: Euler-Oscillation Emission">
                ...
                <PaperRun title="What we kept.">
                    The 600M reserve, the staged structure (now realized via calendar-bounded accrual periods rather than settlement counts), and the design intent of distributing the bootstrap subsidy over time. The underlying intuition &mdash; that distribution should be staged rather than instantaneous &mdash; survived; the Euler mathematics did not.
                </PaperRun>
            </PaperSection>
