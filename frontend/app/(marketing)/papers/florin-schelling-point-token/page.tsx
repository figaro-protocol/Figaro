    ...
    return (
        <PaperLayout slug="florin-schelling-point-token"
            ...
            abstract={
                <>
                    ...
                    <p>
                        The positive design content is modest and targeted: a fixed one-billion-unit cap enforced by two complementary supply-integrity mechanisms, a one-way deployer-renouncement latch that freezes the allocation plan at deployment, and a single staged distribution of 60% of supply, paid out over nine annual periods on a rising budget schedule, to the authors and designers of record of the clauses and assemblies real trade used &mdash; on identical terms. We lay this design out in detail and close with an honest analysis of how the florin can lose its focal-point status and what the design does and does not defend against.
                    </p>
                    <p>
                        The paper sits within a lineage, and the disanalogy matters as much as the analogy. Nakamoto (2008) demonstrated that money could be a protocol rather than an institution, with BTC as the focal Schelling point for a peer-to-peer cash system. Buterin (2014) demonstrated that contracts could be composable, with ETH as the focal point for programmable settlement. The present work extends the lineage one step in <em>what becomes composable</em> &mdash; <em>institutions</em> themselves, through the Figaro primitive &mdash; and the florin is the focal point for participants who coordinate around that composition. What it does not extend is the ladder of monetary function that ties BTC and ETH to a mechanism that requires the unit; the kernel takes any fungible token as bond and payment, so no protocol operation denominates in the florin and none requires it (Section 3).
                    </p>
                </>
            }
            references={
        ...
        >
            <PaperSection title="1. Introduction">
                <p>
                    The florin is the native token that ships alongside the Figaro settlement primitive. The name is borrowed from the coin Florence struck for the trade of its day, and the borrowing is for salience, not for lineage of function: that florin was a circulating coin with a metal content, while this one is required by no operation of the protocol &mdash; no fee is paid in it, no bond need be posted in it, and nothing on chain depends on holding it. What survives the borrowing is the property that made the historical unit useful &mdash; that everyone knew which unit everyone else meant &mdash; which is focality, and focality is the whole of what the design pursues. This paper describes that design and defends a specific framing of what it is. The framing is the substantive contribution; the supply-integrity and distribution mechanics follow from it.
                </p>
                <p>
            ...
            <PaperSection title="3. Why the Figaro Ecosystem Is Well-Suited to a Focal-Point Token">
                ...
                <p>
                    These use cases are modest. We do not claim the florin is indispensable or that the ecosystem fails without it. We claim only that a focal-point token has a coordination function unavailable to general-purpose tokens, and that some participants will find the function useful.
                </p>
            </PaperSection>
            ...
            <PaperSection title="4. Design Rules for a Focal-Point Token">
                ...
                <PaperRun title="Rule 2: Predictable supply.">
                    Focality is fragile under surprise inflation. A holder who believes supply is fixed may exit on learning it is not. The florin&rsquo;s supply schedule is committed at deployment and cannot be modified: 1B hard cap, staged distribution with immutable per-period budgets and immutable calendar boundaries, one-way deployer-renouncement at the end of the deploy transaction. There are no surprise-inflation paths available to any actor after deployment.
                </PaperRun>
                <PaperRun title="Rule 3: Clean distribution.">
                    Focality is fragile under highly concentrated ownership. If a single holder can dump enough supply to destabilize the price, Schelling convergence is less credible. The florin&rsquo;s allocation is 60% to the authors and designers of record &mdash; clause authors and assembly designers &mdash; by the staged distribution, 30% to a treasury with an ecosystem mandate (the DAO), 7% to founders, and 3% to early supporters (friends and family) carved out of the founder cut. The concentration is higher than ideal for focality (the DAO, founder, and supporter share is 40%), which we address honestly in Section 9 and in the distributional-silhouette concession in Section 10. We do not present the supporter carve-out as a mitigation: re-labeling three points of the founder allocation as friends-and-family does not lower the 40% held outside the staged distribution &mdash; those tokens remain in the same insider, non-mandate hands.
                </PaperRun>
                <PaperRun title="Rule 4: Absence of protocol-activity coupling.">
                    Focality requires that participants hold for alignment reasons. If the token&rsquo;s supply is a function of protocol activity, holders acquire the token by doing things that are not alignment (gaming the emission curve, wash-trading for rewards), and the signal is muddied. Activity-coupled emission is ruled out for exactly this reason: it would make the balance in a wallet mean two different things &mdash; &ldquo;holder by alignment choice&rdquo; and &ldquo;holder because a settlement happened to pay me&rdquo; &mdash; and a token that means two things at once is not a Schelling point (Section 8).
                </PaperRun>
            </PaperSection>
            ...
            <PaperSection title="6. Allocation: 7 / 3 / 30 / 60">
                ...
                <ul className="space-y-1 list-disc pl-6 text-sm">
                    ...
                    <li><strong>Clause authors and assembly designers</strong> &mdash; 60% (600M) &mdash; claimed pro rata over nine annual accrual periods, ending at years 1 through 9, with per-period budgets grouped into three rising tranches (15% / 30% / 55%).</li>
                </ul>
                ...
                <PaperRun title="The bootstrap-dump scenario.">
                    A fair objection to the no-vesting choice is the bootstrap-dump scenario: between genesis (year 0) and the first annual claim at year 1, the founder and supporter wallets together hold a 10% liquid position (7% founder, 3% supporters, all unvested on identical terms) and the focal-point convergence has not yet had time to establish the Schelling stickiness that would make later dump events self-correcting. A founder dump in this window could foreclose focality emergence by signaling to potential aligned holders that the alignment is not credibly committed. The Section 9 attack-surface analysis treats general dump attacks but does not specifically address the bootstrap window where the structural defenses are weakest.
                </PaperRun>
                <p>
                    The honest answer is that the design has accepted this risk rather than mitigated it through vesting. Two reasons. First, vesting would import the same focality-dilution problem the non-vesting choice was designed to avoid: a founder who must vest will be a holder for schedule-driven reasons that are not alignment, and the schedule itself becomes a Schelling object that competes with the underlying alignment signal. Second, a public lock-up commitment outside a vesting contract &mdash; e.g., self-custody with stated non-sale intent for a specified window &mdash; is available to a founder who wishes to mitigate the bootstrap risk through credible signal rather than through mechanism. We have not committed to a specific lock-up window here because the signal&rsquo;s content depends on the founder&rsquo;s standing relationship with the participants rather than on a contractual instrument; the appropriate commitment is a matter for the founders to make publicly and is not a property of the protocol itself.
                </p>
                <PaperRun title="DAO allocation (30%).">
                    A treasury share, available immediately at genesis. The DAO is not a kernel-governance body &mdash; there is no kernel governance to be done &mdash; but a coordination treasury for ecosystem activity (grants, public goods, reference implementations). Releasing the DAO allocation at genesis means the treasury is operational from day one. A fair objection: doesn&rsquo;t a liquid 30% treasury with an ecosystem-grants mandate create exactly the reflexivity that activity-coupled emission would (Rule 4), and doesn&rsquo;t it dilute the florin&rsquo;s focal-point purity (Rule 1)? We concede the objection in its general form. The defense is narrower than the objection implies: the coupling activity-coupled emission would introduce is <em>mechanism-level</em> (a deterministic on-chain function from settlement count to mint), while the coupling a treasury introduces is <em>policy-level</em> (a human-discretionary function from treasury governance to disbursement). Mechanism-level coupling is what the kernel&rsquo;s no-escape-hatches design principle rules out; policy-level coupling is attenuated by two layers of friction (off-chain discretion, revocable grants). The distinction is principled but not absolute: a sufficiently mechanical grant policy would reintroduce the coupling, which is why the DAO&rsquo;s operating procedures are themselves a design artifact that warrants its own treatment rather than a silent assumption.
                </PaperRun>
                <p>
                    A reader who has followed the paper&rsquo;s repeated claim that the kernel is ungoverned &mdash; no admin, no owner, no vote can move a bonded commitment or alter any parameter of the settlement mechanism, ever &mdash; may see a tension in a DAO that governs a 300M-token treasury by discretionary decision. There is none, because the two are different objects at different tiers. The kernel&rsquo;s absence of governance is a property of the settlement mechanism itself: nothing the DAO does, votes on, or spends can touch how a bonded commitment resolves, because the DAO holds no key or role in that mechanism at all. What the DAO governs is its own treasury &mdash; which public goods to fund, which programs to stand up, who gets paid &mdash; the ordinary discretion any treasury-holding body exercises over its own assets. A vote of the DAO can redirect the 300M; no vote, by the DAO or anyone else, can redirect a single bonded commitment.
                </p>
                <PaperRun title="Author-and-designer allocation (60%).">
                    The majority share, allocated over a calendar window in nine annual instalments of rising size. The structure is designed to (a) reach the two recipient classes on identical terms &mdash; the clause authors of record and the assembly designers of record, each credited by their own registry and each scored by the same rule &mdash; (b) reward sustained substrate contribution, since a contributor whose work is still being adopted in year seven is paid for that year as fully as for their first, and (c) avoid the &ldquo;dump-on-launch&rdquo; dynamic of a single distribution event by spreading the supply emission over a nine-year horizon. The recipients and amounts within each period are decided by the substrate-broadening formula &mdash; the retroactive rule that rewards authors and designers of record in proportion to how much the clauses and assemblies they registered broadened the range of coordination the substrate can express &mdash; computed over settled on-chain history. Nothing here is granted to a participant for participating: a wallet that trades heavily, and is the author or designer of record of nothing, receives nothing from this share.
                </PaperRun>
                <p>
                    The nine annual budgets group into three rising tranches: 15% of the reserve across years 1&ndash;2, 30% across years 3&ndash;5, and 55% across years 6&ndash;9, split equally within each group, so the instalments are 45M, 45M, 60M, 60M, 60M, and 82.5M four times over. The schedule rises rather than front-loads or declines. The reason is that this distribution pays on measured adoption, and the measurement is thinnest exactly when the network is youngest: an early year is both the least informative reading of what was actually adopted and the most manipulable, since a small honest denominator is a cheap one to dilute. Putting the largest budgets where the evidence is best aligns the supply emission with the quality of the signal it is paid against. The early-years bootstrap need that motivated front-loading is real, and it is met by a different instrument &mdash; the DAO&rsquo;s 30% treasury, which funds work by human judgment before the evidence exists and is designed for exactly the case where no measurement is yet available.
                </p>
                <PaperRun title="A note on the specific percentages.">
                    The 7/3/30/60 split and the internal 15/30/55 grouping of the distribution reflect a design judgment rather than a first-principles derivation. Any rising schedule of comparable shape would satisfy the measurement argument above, and the specific weights are a calibration we do not attempt to justify beyond &ldquo;plausible under the stated principles.&rdquo; What is not a free calibration is the direction: a schedule that pays most where the denominator is thinnest maximizes exposure at the point of least evidence, and no choice of weights repairs that.
                </PaperRun>
            </PaperSection>

            <PaperSection title="7. Staged Distribution over Counted Adoption">
                <p>
                    The 600M share is realized by a single minter holding one budget per accrual period, all nine fixed at deployment, with no privileged path to add a period, extend the schedule, resize a budget, or alter who is eligible. Adoption of the clauses and assemblies the substrate-broadening rule scores is counted on chain the moment it happens, by a permissionless recording call that verifies for itself that the order settled and that the clause or assembly was committed in the signed agreement, so each period&rsquo;s payout is a division over quantities the chain already holds &mdash; minted straight to the claimant, with no root to post, no bond, and no challenge to any of it.
                </p>
                <PaperRun title="Single mechanism, one schedule.">
                    The single-mechanism design is preferred for two reasons. First, surface area: a single mechanism is a single audit target and a single locus of state. Second, atomic-deploy guarantees: the mechanism&rsquo;s <Math>{"6 \\times 10^8"}</Math> cap is registered before the renouncement latch is tripped, and a single mechanism makes that cap binding atomic.
                </PaperRun>
                <PaperRun title="One-shot per (period, address).">
                    Each (period, address) pair is marked claimed on a successful claim and checked on every entry. A given account can claim at most once per period, presenting every clause it authored and every assembly it designed &mdash; checked against their own registries &mdash; in that single call. No top-up, no re-claim, no rebate.
                </PaperRun>
            ...
            <PaperSection title="8. Design Exclusions as Focality Preservers">
                ...
                <ul className="space-y-2 list-disc pl-6 text-sm">
                    ...
                    <li><strong>Not settlement-emitted.</strong> No path runs from kernel settlement activity to a florin mint; no settlement-evidence mechanism is ever granted mint authority, and the renouncement latch ensures none can be added later. This is the most rigorous application of Rule 4.</li>
            ...
            <PaperSection title="9. Bootstrap and Attack Surface">
                <p>
                ...
                <PaperRun title="Acquiescence attacks.">
                    The quietest attack is no attack at all: nobody converges, and the florin fails to become a focal point. This is possible and unavoidable. The defense is limited &mdash; we can support focal emergence but cannot force it. The honest statement is that if no one recognizes the florin as a focal point, the unit has no Schelling-point value and operates as an unvalued distribution artifact. The supply-integrity and allocation properties continue to hold in that case; only the signaling function fails.
                </PaperRun>
                <PaperRun title="The griefing attack.">
                    A <em>griefing</em> attack is worth considering directly: an adversary who writes code to drain the reward pool not for profit but for spite, given that no governance or fee-extraction capture is available. The attack surface is narrow: a claim mints only to the caller, only for clauses and assemblies the registries record that caller as author or designer of, only in proportion to adoption the chain itself verified and has since frozen, and only up to a per-period budget the minter enforces beneath the token&rsquo;s own independent cap on it. There is no pool to drain: once a period has closed, nothing anyone does can raise their own share or lower anyone else&rsquo;s, because both the numerator and the denominator have stopped moving. The sell-at-claim surface remains (a recipient can claim as soon as a period closes and sell), but that is indistinguishable from the normal operation of the distribution; it is not an exploit.
                </PaperRun>
            </PaperSection>

            <PaperSection title="10. Substrate-Neutrality and Distributional Silhouette">
                <p>
            ...
            <PaperSection title="11. Regulatory Framing">
                <p>
                ...
                <p>
                    (i)&nbsp;<em>Investment of money.</em> A participant acquires florins by claiming a closed period&rsquo;s share for clauses they authored or assemblies they designed, or through secondary-market purchase. The claim is not an investment of money in the ordinary sense (no consideration is delivered to the issuer; what the claimant supplied was authorship the network adopted); the secondary purchase is an investment of money in the same sense any token acquisition is. Prong (i) is satisfied for secondary acquisition and not for a period claim. Most analyses treat this asymmetry as not dispositive &mdash; the secondary market is the more material context &mdash; so we proceed assuming prong (i) is met for the relevant population.
                </p>
                <p>
