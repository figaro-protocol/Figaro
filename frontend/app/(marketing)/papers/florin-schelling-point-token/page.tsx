    ...
    return (
        ...
        >
            ...
            <PaperSection title="7. Cohort-Based Distribution via Staged Merkle">
                <p>
                    The 600M airdrop is realized by a single optimistic minter with three immutable tranches. Each tranche carries an immutable earliest-post time; once it passes, anyone may post that tranche&rsquo;s payout merkle root under an economic bond, together with the window of public chain events over which it was computed. No proof gates the posting and no privileged submitter is trusted: because the clause-author substrate-broadening formula is deterministic over public on-chain history, any observer can recompute the canonical root, and a mismatched posting is voided by an equal-bonded challenge and simply re-posted correctly. A proof-gated variant &mdash; a zero-knowledge proof attesting that the root aggregates the formula &mdash; was considered and rejected: a proof can attest that the formula was applied to the supplied event window, but not that the window matches chain history, leaving the input-provenance hole open at full proving cost, whereas open recompute-under-challenge discharges both. There is no privileged path to add a tranche, extend the schedule, or modify a cohort.
                </p>
