...
const HANDOFF_CHAIN = `        shipper-of-record (origin country)
        ...
        destination inland leg
            |
            | delivery-modality (delivery to consignee)
            v
        ...
        <PaperLayout slug="after-tradelens"
            ...
            abstract={
                <>
                    ...
                    <p>
                        The clauses the composition draws on are largely existing infrastructure (commerce, geolocation, proximity-proof, delivery-modality, carrier-process-log, emissions-disclosure); two new clauses (container-seal, Incoterms-2020-as-anchor) extend the protocol&rsquo;s surface additively, without touching the kernel. We also treat the bill-of-lading transferability question through the cancellable-seller / counter-process pattern that expresses transferability at the protocol layer without weakening the kernel&rsquo;s invariants.
                    </p>
                    <p>
        ...
        >
            ...
            <PaperSection title="4. The Assembly">
                ...
                <ul className="space-y-2 list-disc pl-6 text-sm">
                    <li><em>Shipper-of-record</em>: proximity-proof on goods presented for shipment, optional emissions-disclosure for manufacturing emissions.</li>
                    <li><em>Freight forwarder</em>: proximity-proof on cargo receipt at consolidation; container-seal applied at the consolidation point.</li>
                    ...
                    <li><em>Origin and destination inland carriers</em>: proximity-proof at pickup and dropoff; geolocation pickup and dropoff geohashes.</li>
                    ...
                    <li><em>Port-of-loading and port-of-discharge authorities</em>: payment for terminal handling and gate access; proximity-proof at port-side transfers; container-seal inspected-intact attestations at each port handoff.</li>
                    <li><em>Ocean carrier</em>: an Incoterms-2020 clause anchoring the shipment term and named place; proximity-proof on transfer to the vessel and at discharge; container-seal sealing events through the voyage; geolocation vessel-position attestations on the voyage cadence; emissions-disclosure for voyage emissions; for reefer cargo, cold-chain attestations.</li>
            ...
            <PaperSection title="6. Clauses">
                <p>
                    The composition draws mostly on existing clause infrastructure. Two new clauses are required (plus a customs-clearance candidate, treated below): a container-seal attestation and an Incoterms-2020 anchor. The bill-of-lading transferability question is treated separately in Section 7.
                </p>
                ...
                <ul className="space-y-2 list-disc pl-6 text-sm">
                    ...
                    <li>delivery-modality &mdash; declares the delivery modality and the handoff points where physical exchange occurs (final delivery to the consignee).</li>
                    <li>proximity-policy with proximity-proof &mdash; geofence enforcement and the attestation that a given handoff occurred at the agreed place and time, at each container transfer point.</li>
                    <li>carrier-process-log &mdash; the carrier&rsquo;s per-role event log over the multi-stop carrier process.</li>
                ...
                <p>
                    The clause is the seed of a <em>container-integrity</em> family that could later cover seal-equivalents in other transport modes (rail container seals, air-freight tamper-evident packaging, etc.) without reauthoring the family. The clause name, once registered, binds permanently.
                </p>
                <PaperRun title="New clause: Incoterms 2020.">
                ...
                <p>
                    The right clause is therefore not &ldquo;Incoterm as a traditional contract clause&rdquo; but <em>Incoterm as a reference to a Figaro-native delivery-clause specification</em>. The clause anchors the term plus the named place; the term-to-delivery-clause mapping lives in the off-chain ICC publication (the canonical text) and a runtime term-to-clause table that the assembly&rsquo;s participants compose from. The clause anchors the term, the named place, and the place&rsquo;s geohash (for cross-reference with the geolocation clause); it does not encode per-term semantics. The clause specification carries:
                </p>
                <ul className="space-y-1 list-disc pl-6 text-sm">
                    ...
                    <li><code>placeGeohash</code> (string, 8-character precision for cross-reference with geolocation)</li>
                </ul>
                <p>
                    This split lets future Incoterms revisions ship as new clauses (an Incoterms-2030 clause, etc.) without mutating prior anchors. Per-term mapping verification &mdash; which terms map directly, which require composition with parallel processes (insurance, customs), and which do not transfer at all &mdash; is work the clause-author performs against the kernel code before the clause is registered. Likely outcomes: <em>EXW, FCA, DAP, DPU, DDP, FAS, FOB</em> probably map directly, each becoming a delivery-clause specification (a proximity-proof handoff attestation at the named place, with auxiliary clauses for customs or unloading where the term requires them). <em>CPT and CFR</em> map structurally: the &ldquo;seller pays carriage&rdquo; feature becomes a separate sub-process where the seller is the buyer of carriage. <em>CIP and CIF</em> map similarly plus the &ldquo;seller insures for buyer&rsquo;s benefit&rdquo; feature, which likely requires composition with a parallel insurance process. The Incoterms clause is a clean illustration of the principle that the protocol anchors shared references but does not import every contractual assumption that travels with traditional vocabulary.
                </p>
            ...
            <PaperSection title="7. Bill of Lading Transferability">
                ...
                <p>
                    The pattern is a sketch, not a deliverable mechanism. Three elements lie outside the kernel-discipline guarantee and must be specified by the parties at agreement signing rather than relied on as architectural invariants. <em>Sub-seller consent under the wrapper</em>: the wrapper turns on programmatic acknowledgement of cancellation by the affected sub-seller under a fee schedule pre-agreed at commit time; whether that consent at commit time satisfies the kernel-discipline reading of counterparty consent at the moment of cancellation is a design-discipline question the parties resolve in the off-chain agreement. <em>Cargo-handoff operationalization</em>: a fresh transport process for the remaining legs requires the original carrier to release cargo to the new carrier; the cargo handoff is a real-world event the off-chain agreement attaches an attestation surface to (a proximity-proof handoff attestation between the two carriers, with the original buyer counter-signing). <em>Cancellation-fee arithmetic</em>: the fee schedule must be high enough that sub-sellers do not face an early-exit option they will refuse, and low enough that the transfer use case remains operational; specific values are agreement-shaped, not architectural.
                </p>
                <p>
