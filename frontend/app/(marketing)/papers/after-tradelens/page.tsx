    ...
    return (
        <PaperLayout slug="after-tradelens"
            ...
            abstract={
                <>
                    <p>
                        TradeLens (2018&ndash;2023), the IBM&ndash;Maersk container-shipping consortium that aimed to produce a tamper-proof shared record of every container&rsquo;s journey, shut down in 2023 even after several major carriers had joined, because the industry never consolidated under it &mdash; rival carriers split across competing consortia rather than concentrate a shared industry resource under one competitor&rsquo;s platform. The shutdown was not a software failure. It was a governance failure visible from the consortium&rsquo;s structural shape: a competitor-controlled platform that the rest of the industry would not consolidate under, and one that offered participants shared visibility without offering them any resolution guarantee for holding to the arrangement. The failure was structural, predictable from the shape of the architecture, and not specific to TradeLens, which is one species of a broader pattern &mdash; the coordination apparatus of the prior era, a consortium with a membership and a governing body, ported onto a new technical substrate without changing what the substrate is asked to do.
                    </p>
                    <p>
                        The architectural alternative we develop operates at the same perimeter TradeLens attempted: inter-logistics coordination among ocean carriers, ports of loading and discharge, customs authorities, freight forwarders, NVOCCs, customs agents, trade-finance parties / LC issuers, marine insurers, and inspection services. We present a permissionless, decentralized bonded composition built on the Figaro kernel&rsquo;s two mechanisms (asymmetric bonding plus buyer dominance with atomic resolution). The composition has no consortium, no foundation, no upgrade path, and no central party who can be ratified or refused. Each provider is a wallet standing for an off-chain productive asset, operated by a human or by an autonomous agent on the asset&rsquo;s behalf, with public-authority wallets participating on the same footing as commercial ones. The root buyer at this perimeter is the buyer-of-record at the relevant tier; the kernel is agnostic about which party occupies the buyer slot. At each commit the buyer locks twice that leg&rsquo;s payment and the seller twice everything the process has accumulated through its own link; atomic resolution by the buyer-of-record then resolves every leg at once, on receipt of the goods at the agreed delivery point. Carriers who want to participate compose; carriers who do not do not, with no consequence for either decision.
                    </p>
                    <p>
                        The composition is built rather than proposed, and the clause set it draws on is shipped protocol infrastructure: commerce, cargo, geolocation, modalities, acceptance criteria, cold chain, freight class, emissions, the merchant and courier process-event logs, the registered Incoterms-2020 anchor, and &mdash; for the container&rsquo;s integrity &mdash; a chain-of-custody clause that commits the seal regime at signing and carries the seal&rsquo;s applied, inspected-intact, transferred, breached, and removed events as runtime witness attestations. An order that involves a physical exchange carries one handoff clause naming where it happens, with the proximity policy nested under it: the accepted detection bands are committed at signing, and the proof that the handoff occurred within one of them is filed as a witness stage on that same clause. One clause remains a candidate &mdash; a customs-clearance clause &mdash; and the built composition does without it, the clearance determination reaching the record through the customs agent&rsquo;s own attestations against a pinned filing-terms document. We also treat the bill-of-lading transferability question through the treatment of a cargo sale in transit as an event outside the process, squared by the parties before resolution without weakening the kernel&rsquo;s invariants.
                    </p>
                    <p>
        ...
        >
            ...
            <PaperSection title="4. The Assembly">
                ...
                <DesignGraphCollapseFigure
                    ...
                    figureDesc={
                        ...
                        "branch is recorded anywhere in resolution state. The ordering the " +
                ...
                <ul className="space-y-2 list-disc pl-6 text-sm">
                    ...
                    <li><em>Freight forwarder</em>: the process-event log for the coordination role; geolocation for the consolidation point; a handoff with its proximity policy where the forwarder takes physical receipt.</li>
                    ...
                    <li><em>Origin and destination inland carriers</em>: the courier process-event log, carrying pickup and drop-off as events; one handoff on each carrier&rsquo;s own order, with its proximity policy nested under it; geolocation origin and destination geohashes.</li>
                    ...
                    <li><em>Port-of-loading and port-of-discharge authorities</em>: payment for terminal handling and gate access; one handoff on each authority&rsquo;s own order, with its proximity policy nested under it; chain-of-custody inspected-intact events at each port.</li>
                    <li><em>Ocean carrier</em>: the courier process-event log over the voyage, carrying the loading, position and discharge events the carrier reports; one handoff on the carrier&rsquo;s order, with its proximity policy nested under it; chain-of-custody events through the voyage; geolocation for the leg&rsquo;s endpoints; the freight-class clause where the cargo needs a declared class; an emissions disclosure for voyage emissions; for reefer cargo, the cold-chain clause committing the temperature window and recording interval and carrying the observed record as its witness stage.</li>
                ...
                <PaperRun title="Operational sub-procurement as separate processes.">
                    Each value-adder&rsquo;s own procurement (the carrier&rsquo;s fuel + crew + maintenance, the forwarder&rsquo;s IT + staffing, the inspection service&rsquo;s labor + equipment, the port authority&rsquo;s terminal operations, etc.) is a separate process in which that provider wallet is itself the root buyer. The kernel does not link those processes to the importer&rsquo;s, and nothing needs it to: each process is a complete resolution frame in its own right, and anything that must actually coordinate with the shipment enters the shipment&rsquo;s own process as one more bonded link &mdash; the chain is bounded only by gas. Each provider wallet runs its own bonded processes for its operating expenses against its own counterparties.
                </PaperRun>
            </PaperSection>
            ...
            <PaperSection title="6. Clauses">
                ...
                <ul className="space-y-2 list-disc pl-6 text-sm">
                    ...
                    <li><em>Hand-off</em> &mdash; the point at which a physical exchange occurs on a given order: one handoff per order that has one, and never two. A leg whose single seller touches the cargo more than once therefore has a choice, and it is a composition choice rather than a clause one. Either the order names the one handoff that matters to the parties &mdash; the point the proximity policy witnesses &mdash; and the seller&rsquo;s other movements are events on its process log; or the leg is composed as more than one order, each with a handoff of its own and each bonded in the ordinary way. The built composition takes the first course throughout: one handoff on the carrier&rsquo;s order, one on the drayage order, and none on the orders that move no cargo.</li>
                    <li><em>Proximity policy</em> &mdash; nested under the handoff: the accepted detection bands committed at signing, with the proof that a particular transfer occurred within one of them filed as a runtime witness stage on the same clause, at each container transfer point.</li>
            ...
            <PaperSection title="8. Comparison with Existing Approaches">
                <PaperRun title="TradeLens.">
                    TradeLens was a permissioned consortium ledger; the architecture proposed here is a permissionless bonded kernel. The functional comparison is asymmetric: TradeLens supplied information transparency without resolution; the bonded architecture supplies resolution (with information transparency emerging as a byproduct of the bonded record). The governance comparison is also asymmetric: TradeLens was governed by IBM&ndash;Maersk; the bonded architecture has no governance at the kernel layer.
                </PaperRun>
                <PaperRun title="CargoX.">
                    CargoX is a blockchain-based bill-of-lading platform that emphasizes transferable-electronic-record functionality and MLETR compliance (CargoX, 2023). CargoX is a focused product addressing the BoL transferability problem; the bonded architecture is a more general kernel whose relationship to CargoX is composable rather than competitive. A shipper using the bonded architecture for the bilateral transport coordination could compose CargoX (or its successor) at the BoL layer, with the bonded record providing the underlying resolution evidence and the CargoX layer providing the transferable-instrument semantics.
                </PaperRun>
