    ...
    return (
        ...
        >
            ...
            <PaperSection title="4. The Assembly">
                <p>
                    We now describe the bonded composition that operates at the TradeLens perimeter. The composition has two layers that must be kept distinct: the <em>organizational DAG</em> (how the container physically moves from origin to destination through the inter-logistics value-adders, documented via schema attestations) and the <em>kernel commit structure</em> (who commits to whom on-chain, governed by the kernel&rsquo;s <code>buyer = rootBuyer</code> rule). The kernel is DAG-blind; the schema layer is DAG-aware. The two layers co-exist in one process.
                </p>
                ...
                <PaperRun title="Per-commit schema clauses.">
                    Each kernel commit pairs the standard <code>figaro-commerce-v1</code> payment clause with the schema clauses appropriate to the seller&rsquo;s role:
                </PaperRun>
                <ul className="space-y-2 list-disc pl-6 text-sm">
                    ...
                    <li><em>Customs agent</em>: a customs-clearance attestation recording the customs authority&rsquo;s determination (a candidate operator-process schema, not yet in the registered set), plus payment for entry-filing services.</li>
            ...
            <PaperSection title="6. Schemas">
                <p>
                    The composition draws mostly on existing schema infrastructure. Two new schemas are required: a container-seal attestation and an Incoterms-2020 anchor. The bill-of-lading transferability question is treated separately in Section 7.
                </p>
                <PaperRun title="Existing schemas.">
                ...
                <ul className="space-y-2 list-disc pl-6 text-sm">
                    ...
                    <li><code>figaro-ghg-protocol-v1</code> (with sister schemas for ISO 14064, PAS 2050, EN 16258) &mdash; per-leg emissions disclosure on every transport edge.</li>
                </ul>
                <p>
                    These schemas are already on chain and bound to validators; no further protocol-extension work is needed for them. The customs-clearance determination is recorded by the customs-authority wallet as a schema-typed attestation, but no dedicated customs schema exists in the current set &mdash; it is a candidate operator-process extension, authored under the same extension-doctrine as the two new schemas below.
                </p>
                <PaperRun title="New schema: figaro-container-seal-v1.">
                    Container seals are the integrity mechanism for ocean shipping. A uniquely-numbered seal is applied at the origin and broken only at customs or at the destination consignee. Multiple parties (carrier, ports, customs, consignee) need a single canonical attestation of when the seal was applied, when it was inspected intact, and if it was breached. The schema&rsquo;s Layer A spec carries:
                </PaperRun>
                ...
                <p>
                    The schema is the seed of a <em>container-integrity</em> family that could later cover seal-equivalents in other transport modes (rail container seals, air-freight tamper-evident packaging, etc.) without reauthoring the family. The schemaId binds permanently under the <code>SchemaRegistry</code>&rsquo;s first-write-wins discipline.
                </p>
                <PaperRun title="New schema: figaro-incoterms-2020-v1.">
                ...
                <p>
                    The right schema is therefore not &ldquo;Incoterm as a traditional contract clause&rdquo; but <em>Incoterm as a reference to a Figaro-native delivery-clause specification</em>. The schema anchors the term plus the named place; the term-to-delivery-clause mapping lives in the off-chain ICC publication (the canonical text) and a runtime term-to-clause table that the assembly&rsquo;s participants compose from. The validator contract enforces the anchor (term enum, named place string, place geohash for cross-reference with <code>figaro-geo-v2</code>) but does not encode per-term semantics in Solidity. The Layer A spec carries:
                </p>
                <ul className="space-y-1 list-disc pl-6 text-sm">
                ...
                <p>
                    This split lets future Incoterms revisions ship as new schemaIds (<code>figaro-incoterms-2030-v1</code>, etc.) without mutating prior anchors. Per-term mapping verification &mdash; which terms map directly, which require composition with parallel processes (insurance, customs), and which do not transfer at all &mdash; is work the schema-author performs against the kernel code before the schema is registered. Likely outcomes: <em>EXW, FCA, DAP, DPU, DDP, FAS, FOB</em> probably map directly, each becoming a delivery-clause specification (a <code>figaro-proximity-proof-v1</code> handoff attestation at the named place, with auxiliary clauses for customs or unloading where the term requires them). <em>CPT and CFR</em> map structurally: the &ldquo;seller pays carriage&rdquo; feature becomes a separate sub-process where the seller is the buyer of carriage. <em>CIP and CIF</em> map similarly plus the &ldquo;seller insures for buyer&rsquo;s benefit&rdquo; feature, which likely requires composition with a parallel insurance process. The Incoterms schema is a clean illustration of the extension-doctrine principle that the protocol anchors shared references but does not import every contractual assumption that travels with traditional vocabulary.
                </p>
                <PaperRun title="Schemas the assembly does not include.">
                    The assembly does not include a multi-currency-cross-leg schema (would break same-unit comparability and the bonding equilibrium), a force-majeure-escape schema (would re-introduce the kind of unilateral exit the kernel forbids), or a transferability schema internal to <code>figaro-commerce-v1</code>. The transferability question is treated separately in Section 7 as a protocol-layer composition pattern.
                </PaperRun>
            </PaperSection>
