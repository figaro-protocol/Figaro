        ...
        >
            ...
            <PaperSection title="3. The Settlement Primitive">
                ...
                <PaperRun title="What this implies for shipping coordination.">
                    Mechanism&nbsp;1 prices each leg&rsquo;s enforcement in proportional bond posture without requiring any consortium or platform to underwrite it. Mechanism&nbsp;2 enforces the multi-leg coordination at resolution without requiring any party to govern the multi-leg arrangement. The two mechanisms together supply the coordination function that TradeLens attempted to supply through a consortium-governed shared ledger; they supply it without needing the consortium. Permissionlessness alone removes the veto but gives self-interested rivals nothing to coordinate on; bonded settlement supplies the enforcement a bare permissionless ledger cannot. Both mechanisms price settlement discipline under perfect monitoring; an honest dispute over whether a leg was performed as agreed is adjudicated not by the bond posture but by the forums composed above it. A logistics reader will ask where bunker fuel, terminal handling, chassis time, and reefer power sit in that accounting, since they are consumed the moment a leg is performed. They sit outside it. The payoffs the mechanism defines are the transfers it executes &mdash; bonds pulled at commit, payment released at resolution &mdash; so a provider at its own leg compares <Math>{"+P"}</Math> against <Math>{"-2G"}</Math>, and what the provider burns to move the box is expenditure on its own account, revenue and outlay of the operating asset behind the wallet, which the mechanism neither observes nor settles. What it asks of the provider is only that the wallet hold enough to bond the leg it is quoting; a carrier whose balance no longer covers its bonds has left the market, and it has left it by ordinary commercial arithmetic rather than by any act of the kernel. The composition developed here inherits the result and the monitoring assumption alike.
                </PaperRun>
            </PaperSection>
