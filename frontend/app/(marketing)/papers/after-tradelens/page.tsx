    ...
    return (
        <PaperLayout slug="after-tradelens"
            ...
            abstract={
                <>
                    ...
                    <p>
                        The composition is built rather than proposed, and the clause set it draws on is shipped protocol infrastructure: commerce, cargo, geolocation, modalities, acceptance criteria, cold chain, freight class, emissions, the merchant and courier process-event logs, the registered Incoterms-2020 anchor, and &mdash; for the container&rsquo;s integrity &mdash; a chain-of-custody clause that commits the seal regime at signing and carries the seal&rsquo;s applied, inspected-intact, transferred, breached, and removed events as runtime witness attestations. An order that involves a physical exchange carries one hand-off clause naming where it happens, with the proximity policy nested under it: the accepted detection bands are committed at signing, and the proof that the hand-off occurred within one of them is filed as a witness stage on that same clause. One clause remains a candidate &mdash; a customs-clearance clause &mdash; and the built composition does without it, the clearance determination reaching the record through the customs agent&rsquo;s own attestations against a pinned filing-terms document. We also treat the bill-of-lading transferability question through the cancellable-seller / counter-process pattern that expresses transferability at the protocol layer without weakening the kernel&rsquo;s invariants.
                    </p>
                    <p>
        ...
        >
            ...
            <PaperSection title="5. Bond Posture and Cascade Mechanics">
                ...
                <PaperRun title="What this reproduces of joint liability, and what it does not.">
                    The pressure just described is the coordination-pressure component of the peer enforcement classically analysed in the joint-liability microfinance literature, obtained here without that literature&rsquo;s prerequisites: no repeated interaction, no local information among the sellers, no punishment technology outside the arrangement, and no joint-liability contracting &mdash; each bond is posted individually against that seller&rsquo;s own link, and the coupling comes from atomic resolution rather than from any shared obligation. The claim is confined to that component. Peer <em>selection</em> on private types and peer <em>monitoring</em> under a principal&rsquo;s information asymmetry are separate results in that literature, they need structure the bonded primitive does not supply, and nothing here reproduces them; a cohort of carriers, ports and agents assembled for one shipment does not screen its members and does not watch them. The useful reading for a logistics deployment is narrow and still substantial: the cohort has a stake in the shipment closing that a consortium-governed shared ledger tried to manufacture through transparency and could not, and it has that stake among strangers, on the first shipment, without anyone administering it.
                </PaperRun>
            ...
            <PaperSection title="7. Bill of Lading Transferability">
                ...
                <PaperRun title="The cancellable-seller / counter-process pattern.">
                    The architectural direction is that BoL transferability is a <em>protocol-layer composition</em>, not a kernel-layer property. The candidate pattern is a <em>cancellable-seller wrapper plus counter-process</em>: the original buyer, who wants to transfer the right to the cargo mid-shipment, commits a parallel <em>counter-process</em> <Math>{"P_{\\text{cancel}}"}</Math> in which each sub-seller is compensated for the truncation of its leg through a fee owed by the original buyer and bonded under the standard rule, while the new buyer commits a fresh transport process for the remaining legs. The word <em>cancellation</em> should be read at the commercial level only. Nothing cancels the original process: its orders stand, its bonds stand, and its own buyer still resolves it, at which point the two processes net to the terms the parties wrote. Undoing a commitment is itself a commitment, priced and bonded like any other, which is exactly why the pattern lives above the kernel and not in it.
                </PaperRun>
                <p>
                    The pattern is a sketch, not a deliverable mechanism, included because the BoL-transferability question is the load-bearing concern for any container-shipping deployment and the conventional answer &mdash; negotiable bills of lading endorsed mid-transit &mdash; is precisely what the kernel&rsquo;s buyer-fixed invariant rules out. Three elements lie outside the kernel-discipline guarantee and must be specified by the parties at agreement signing rather than relied on as architectural invariants: sub-seller consent under the wrapper (the wrapper turns on programmatic acknowledgement of cancellation under a fee schedule pre-agreed at commit time, and whether that satisfies the kernel-discipline reading of counterparty consent at the moment of cancellation is a design-discipline question the parties resolve off-chain); cargo-handoff operationalization (the original carrier must release cargo to the new carrier, an event the off-chain agreement attaches an attestation surface to &mdash; a hand-off between the two carriers, witnessed under the proximity policy nested beneath it, with the original buyer counter-signing); and cancellation-fee arithmetic (the schedule must be generous enough that a seller whose leg is truncated prefers the counter-process to holding out, and modest enough that transferring remains worth doing at all, with the specific figures agreement-shaped rather than architectural). Deployments requiring BoL transferability can rely on conventional BoL doctrine through the off-chain agreement, or constrain their use cases to the non-transferable region the kernel admits directly.
                </p>
                <PaperRun title="What the pattern does to the record&rsquo;s transferable-record fit.">
                    The transferable-record question that a shipping reader will press is whether the counter-process pattern spoils the fit, and it does not: the transfer is achieved by bilateral signing across two processes rather than by unilateral endorsement of a single record, so the record whose singularity and control the analysis turns on &mdash; the bonded commitment, singular through its content-addressed order identifier and the status nullifier that admits each identifier exactly once, controlled through the buyer&rsquo;s exclusive power to resolve &mdash; is never endorsed, never duplicated, and never moved.
                </PaperRun>
