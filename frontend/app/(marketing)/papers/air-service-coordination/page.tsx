...
export default function AirServiceCoordinationPaper() {
    return (
        <PaperLayout slug="air-service-coordination"
            ...
            references={
                <>
                    <li>Christensen, C. M., &amp; Raynor, M. E. <em>The Innovator&rsquo;s Solution: Creating and Sustaining Successful Growth</em>. Harvard Business School Press, 2003.</li>
        ...
        >
            ...
            <PaperSection title="2. The Settlement Primitive">
                ...
                <PaperRun title="What this implies for air-service coordination.">
                    The bonded primitive&rsquo;s two mechanisms supply two coordinative properties for air service. Mechanism&nbsp;1 supplies a bilateral commitment whose settlement does not require either party&rsquo;s home jurisdiction&rsquo;s contract-law regime to enforce: settlement is the on-chain release of locked bonds, mediated by no court. The off-chain agreement remains binding under whatever law the parties select for the substantive commitment, but the kernel&rsquo;s settlement guarantee is jurisdiction-agnostic. Mechanism&nbsp;2 supplies the atomic resolution that propagates a single commit-or-don&rsquo;t decision through the entire process; this is what makes operational damages flow proportionally to the unit that caused them rather than being absorbed at whichever contract has the loosest cap. Neither mechanism is novel to the air-service domain; both are general-purpose settlement properties whose application here is the present paper&rsquo;s contribution. Both hold under perfect monitoring with costless performance, and what the bonds price is settlement discipline; an honest dispute over whether a service was performed as agreed is adjudicated by the forums composed above the kernel, not by the bond posture. The costless-performance clause deserves emphasis in this domain rather than a footnote: fuel burn, crew duty hours, gate time, and catering stock are consumed the moment a provider performs, and at any positive performance cost <Math>{"c"}</Math> the provider&rsquo;s comparison against a withholding buyer becomes <Math>{"-2G - c"}</Math> against <Math>{"-2G"}</Math>, so the seller-side weak dominance does not survive unchanged &mdash; the <em>Asymmetric Bonding and Buyer Dominance</em> derivation states that exactness caveat and establishes what does survive in the two-party game: for a cost below the payment plus the provider&rsquo;s bond (<Math>{"c < P + 2G"}</Math>) the cooperative profile remains the unique survivor of iterated elimination, though reached in two rounds and not by seller-side dominance, and a provider assigning probability <Math>{"p"}</Math> to the buyer resolving finds cooperation a best response iff <Math>{"p \\geq c/(P + 2G)"}</Math> &mdash; so cooperation survives a modest positive cost within that bound. That two-round repair does not extend to the <Math>{"N"}</Math>-party mesh invoked above: the <Math>{"N"}</Math>-party equilibrium this paper deploys is claimed at <Math>{"c = 0"}</Math>. The air-service application inherits that survival result and the caveat exactly as it inherits the equilibrium.
                </PaperRun>
            </PaperSection>
            ...
            <PaperSection title="3. The Bonded Architecture Applied to Air Service">
                ...
                <PaperRun title="The demand side: the passenger hires the journey, not the airline.">
                    The decomposition has a demand-side reading in the jobs-to-be-done theory of Christensen and Raynor (2003), later restated by Christensen, Hall, Dillon, and Duncan (2016): customers do not buy products so much as hire them to make progress on a job that arises in a particular circumstance. The passenger&rsquo;s job is the journey &mdash; be in the destination city, in a fit state, by a stated hour. No standing firm is intrinsic to that job. Under the conventional regime the passenger hires an airline, a durable institution that has assembled crew, airframes, fuel contracts, slots, catering, maintenance, and ground handling in advance and amortizes that assembly across every journey it sells. Under the bonded architecture the passenger hires the job, and the assembly hires exactly the real-world assets the job requires &mdash; crew-hours, airframe-time, fuel volume, gate slots, catering counts, a share of the maintenance allocation, ground handling &mdash; for exactly the span the job runs. The institutional boundary is then the job&rsquo;s boundary rather than the firm&rsquo;s: the process comes into existence at the first commit, holds the bonded relations the job needs and no others, and dissolves at atomic resolution. Read this way, the resource-market decomposition of Section 1 is not an accounting exercise performed on an airline; it is the demand side&rsquo;s own description of what it was buying, made directly expressible by a settlement primitive that can hold a set of strangers together for the duration of one job and no longer.
                </PaperRun>
