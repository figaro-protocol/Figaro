        ...
        >
            ...
            <PaperSection title="2. The Settlement Primitive">
                ...
                <PaperRun title="What this implies for air-service coordination.">
                    The bonded primitive&rsquo;s two mechanisms supply two coordinative properties for air service. Mechanism&nbsp;1 supplies a bilateral commitment whose settlement does not require either party&rsquo;s home jurisdiction&rsquo;s contract-law regime to enforce: settlement is the on-chain release of locked bonds, mediated by no court. The off-chain agreement remains binding under whatever law the parties select for the substantive commitment, but the kernel&rsquo;s settlement guarantee is jurisdiction-agnostic. Mechanism&nbsp;2 supplies the atomic resolution that propagates a single commit-or-don&rsquo;t decision through the entire process; this is what makes operational damages flow proportionally to the unit that caused them rather than being absorbed at whichever contract has the loosest cap. Neither mechanism is novel to the air-service domain; both are general-purpose settlement properties whose application here is the present paper&rsquo;s contribution. Both hold under perfect monitoring, and what the bonds price is settlement discipline; an honest dispute over whether a service was performed as agreed is adjudicated by the forums composed above the kernel, not by the bond posture. A reader from this domain will object at once that fuel burn, crew duty hours, gate time, and catering stock are consumed the moment a provider performs, and will want to know where they enter the equilibrium. They do not enter it. The payoffs are the transfers the kernel executes &mdash; the bonds it pulls at commit and the payment it releases at resolution &mdash; so the provider&rsquo;s decision node is the comparison of <Math>{"+P"}</Math> against <Math>{"-2G"}</Math>, with no cost term in it. What a provider consumes to perform is the operating outlay of the asset that owns the wallet: the aircraft wallet, the crew wallet, and the fuel-supplier wallet of Section&nbsp;3 each take payments in as revenue and send payments out as outlay, and both sides of that ledger belong to the asset rather than to the mechanism. The mechanism asks one thing of the provider &mdash; that the wallet hold enough to bond the commitment it is signing &mdash; and an asset whose balance no longer covers its bonds stops quoting, which is how an asset leaves a market rather than anything the kernel does to it. The air-service application inherits the equilibrium and the monitoring assumption on exactly those terms.
                </PaperRun>
            </PaperSection>
