    ...
    return (
        ...
        >
            <PaperSection title="1. Introduction">
                ...
                <p>
                    Each provider in the assembly is an RWA-as-wallet. The <em>asset</em> is the off-chain real-world asset whose participation produces value &mdash; the airframe that flies, the labor capacity of the pilot, the screening service the security authority provides. The <em>wallet</em> is the on-chain representation of that participation: an address holding the asset&rsquo;s earnings as token balances, plus NFTs that point to the off-chain credentials the asset&rsquo;s role requires (the pilot&rsquo;s flight-crew certification, the aircraft&rsquo;s airworthiness certificate, the maintenance provider&rsquo;s authority-issued authorization). The <em>operator</em> is the human or autonomous agent who controls the wallet&rsquo;s signing key on the asset&rsquo;s behalf. The wallet is an active counterparty in the assembly iff the asset is alive and its credentials are current; that conditional is enforced operationally by the operator and at agreement signing by whichever credential schemas the assembly designer composes.
                </p>
                <p>
                    Each provider wallet sustains its participation the way a node on a blockchain network sustains its participation. A node earns fees and rewards from the network and uses them to cover its off-chain operating expenses (machines, electricity, HVAC, rent); when the receipts no longer cover the costs, the operator stops running the node and the node drops out of the network. An RWA-as-wallet operates on the same logic in its market: receipts from bonded processes must cover the asset&rsquo;s off-chain operating expenses (fuel, parts, labor, premises, regulatory fees, capital amortization), or the operator stops bonding the wallet into processes and the wallet drops out of the market it serves. This holds for the catering wallet and for the security-screening wallet alike: the kernel does not distinguish between a &ldquo;market-traded fee&rdquo; and a &ldquo;regulated fee.&rdquo; Both are payments to service-providing wallets that need receipts sufficient to sustain operation. The schemas the assembly uses to verify credentials at agreement signing are generic across vehicle types &mdash; the same credential and schedule schemas serve aircraft, ships, trucks, and other vehicles, with per-vehicle personalization in the schema content and validator-contract configuration.
                </p>
                <p>
            ...
            <PaperSection title="3. The Bonded Architecture Applied to Air Service">
                ...
                <p>
                    Above the kernel, the protocol-and-runtime composability stack attaches schemas to specific orders: the <code>scheduled-departure-v1</code> clause binds to whichever order the parties want to subject to schedule-tolerance enforcement; the <code>figaro-fulfilment-v2</code> clause declares the handoff points where physical exchange occurs, and <code>figaro-proximity-proof-v1</code> attestations attach to those handoff orders. Mechanism contracts operate on specific order patterns, assembly specifications declare which schemas attach where and which view definitions render which surfaces, and UIs render the process per the assembly&rsquo;s declarations. An assembly&rsquo;s structure, once specified, is a fixed verifiable artifact bound to its full composability stack. The bonded-architecture argument of this paper applies to whichever resource markets the designer composes under one process, with the kernel-enforced structural rule holding throughout.
                </p>
            ...
            <PaperSection title="4. Disruption Resolution Under Buyer Dominance">
                ...
                <PaperSubsection title="4.1 The primary mechanism: seller-cohort compensation under bond pressure">
                    ...
                    <p>
                        Sellers negotiate among themselves about which seller bears what fraction of the compensation. The unit whose performance materially failed bears more. Operational parties around the failure who could not have prevented it (the catering provider who had no role in a fuel slip; the gate operator whose service was unaffected) contribute less or not at all. The negotiation is real but routine: each seller knows their own bond exposure, their counterfactual payout under cooperative resolution, and their willingness-to-pay against losing the bond entirely.
                    </p>
                    <p>
            ...
            <PaperSection title="5. Schemas">
                ...
                <p>
                    The two schemas share the <code>flightDesignator</code> field so the arrival event can be matched to its departure within the bonded process. We deliberately do not impose a maximum on <code>departureToleranceSeconds</code> or <code>arrivalToleranceSeconds</code>. Scheduled-service operations vary widely in operationally meaningful tolerance windows, and hard-coding a maximum at the validator contract would foreclose legitimate use cases under the first-write-wins binding discipline that makes the schemaId permanent. The contracting parties choose the tolerance their operation supports; if a domain-specific maximum is later wanted, it can be enforced at a tighter sister schema (e.g. <code>figaro-passenger-departure-v1</code>) without mutating the present binding. The pair is the seed of a <em>scheduled-service</em> family; later extensions could cover scheduled ground transport, scheduled deliveries with tight windows, and other scheduled-binding use cases without reauthoring the family.
                </p>
                <PaperRun title="Schemas the architecture deliberately does not include.">
                    Three classes of schema are conspicuously absent. First, there is no force-majeure schema that allows a counterparty to avoid bond loss in weather events. Force-majeure handling is a <em>composition</em> concern, not a kernel concern: a party who wants to insure against weather-driven delay composes a parallel insurance process; the kernel neither knows about weather nor adjudicates whether a given event qualifies. This is the no-escape-hatches discipline of the kernel, applied to the air-service domain. Second, there is no schema that governs which delays count toward bond loss; the <code>scheduled-departure-v1</code> and <code>scheduled-arrival-v1</code> tolerances are the kernel&rsquo;s only such criteria, and any further refinement is for the off-chain forum that adjudicates disputes the parties bring to it. Third, there is no schema that grants any party a unilateral reschedule right; rescheduling is a new commitment with a new schemaId or a new instance, not a mutation of the existing binding. Append-only identity is the discipline.
                </PaperRun>
            </PaperSection>
