        ...
        >
            ...
            <PaperSection title="6. Runtime Composition">
                ...
                <PaperSubsection title="6.2 Assembly registry pattern">
                    ...
                    <p>
                        The pattern is implemented in <code>frontend/lib/mechanisms/useAssemblyRegistry.ts</code>, which consolidates discovery and indexing (event-derived from <code>AssemblyRegistered</code> logs), content building and canonicalization, and IPFS-pinned publication through the <code>AssemblyRegistry</code> contract; fork-and-draft helpers live in <code>frontend/lib/designer/</code>. An assembly&rsquo;s slug is content-derived &mdash; <code>asm-&lt;hash&gt;</code> over its composition (the clauses it carries, their values, and the DAG) &mdash; so identical compositions map to one slug and the registry&rsquo;s first-write-wins binding dedups them. Published assemblies are event-derived on chain rather than bundled: there is no slug-to-label taxonomy and no on-disk content list, and an assembly&rsquo;s human label is derived from the clauses it composes.
                    </p>
                </PaperSubsection>
