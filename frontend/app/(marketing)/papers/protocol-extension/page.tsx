...
const CATALOGUE: [string, string][] = [
    ...
    ["Proximity policy", "committed detection band (cross-checked at signing)"],
    ["Proximity proof", "per-handoff signed witness (runtime)"],
    ["Consent", "cryptographic acceptance of an off-chain document"],
    ["Arbitration forum", "off-chain arbitration-forum selection"],
    ["Applicable law", "governing-law clause"],
    ["Offset policy", "carbon-offset retirement policy"],
    ["GHG disclosure", "free-form accounting methodology and scope (cross-checked at signing)"],
    ["GHG measurement", "runtime grams CO₂e (runtime)"],
];
        ...
        >
            <PaperSection title="1. Introduction">
                ...
                <PaperRun title="Clause design as a computer-science discipline.">
                    A Figaro clause is not merely a JSON document on a filesystem. It is a typed-data definition under append-only identity discipline, registered permissionlessly first-write-wins, content-addressed to its canonical specification, and made attestable by merkle-binding the clause section against the signed agreement. The discipline is a computer-science research object, not just an implementation detail, and we treat it as such. The reference clause catalogue currently shipped in the protocol is the worked-example reference set.
                </PaperRun>
                <PaperRun title="The coordinator pattern formally.">
                    The <em>coordinator pattern</em> is the discipline under which an external mechanism composes with the kernel without breaking the bonding equilibrium: an extension contract satisfies specified conditions on its read/write profile against kernel state. We state the conditions formally with composition semantics and prove that the conditions are sufficient. The attestation coordinator is the worked verification example, with parametric kernel-immutability properties discharging the sufficient conditions.
                </PaperRun>
            ...
            <PaperSection title="3. The Protocol Extension Doctrine">
                ...
                <PaperSubsection title="3.2 The anchored artifact pattern">
                    ...
                    <FormalBlock label="Definition 3.1 (Anchor record).">
                        <p>
                            A protocol-layer anchor for an artifact family is a tuple <Math>{"\\langle \\text{clauseId}, \\text{version}, \\text{uriHash}, \\text{admitted} \\rangle"}</Math> where clauseId is the anchor identity, version is the version within the family, uriHash is an immutable cryptographic hash of the URI pointing at the off-chain content, and admitted is the binding&rsquo;s monotone admission flag (false until first registration, true thereafter; the discipline forbids deactivation). The clause registry stores only the admission flag as queryable state; version and uriHash live on the registration event log rather than as queryable storage, and the conceptual anchor is reconstructed by reading that event log under the append-only-identity discipline of Section 4.2.
                        </p>
                    </FormalBlock>
                    <p>
            ...
            <PaperSection title="4. Clause Design as a CS Discipline">
                ...
                <PaperSubsection title="4.1 The clause specification and its enforcement layers">
                    ...
                    <PaperRun title="Canonical specification.">
                        A canonical specification of the clause, parsed by a meta-validator over a closed subset of structured-data types (string with specified format, integer, arbitrary-precision integer as decimal string, boolean, enum, array, and object types, with strict closed-clause rejection of unknown fields). The specification is the canonical declaration; everything below it is derived from it. Per-stage overrides express the staged-attestation pattern (an <em>applied</em>-stage attestation may carry different fields than an <em>inspected-intact</em>-stage attestation against the same clause). The specification also carries a short list of category tags that runtime surfaces use for discoverability and grouping; the tags are non-normative for validation but form the discoverability surface for a clause&rsquo;s runtime presentation.
                    </PaperRun>
                    <PaperRun title="Off-chain well-formedness validator.">
                        A client-side implementation of the specification&rsquo;s well-formedness check, run at both bilateral signing points (so neither party signs content the specification would reject) and at read time (so a consumer can re-check an attestation against the clause it was filed under). Well-formedness is an off-chain specification-plus-read-time concern: there is no on-chain per-clause content validation. The validator decodes the clause&rsquo;s canonical content encoding and rejects on any violation.
                    </PaperRun>
                    <PaperRun title="Prover mirror (deferred).">
                        A faithful re-implementation of the well-formedness validator running inside a zero-knowledge prover, intended to produce verifiable attestation receipts during batched off-chain execution. The prover guest program would mirror the canonical validator to enforce the clause during batched attestation execution. This layer is currently deferred; the canonical specification and its off-chain validator serve as the conformance specification for the eventual port.
                    </PaperRun>
                    <PaperRun title="On-chain merkle binding.">
                        The attestation coordinator binds each attestation to the order&rsquo;s signed agreement by verifying a merkle-inclusion proof of the clause section against the agreement hash, and content-hashes the supplied evidence. It validates no clause content shape and invokes no per-clause validator: a clause that was not part of the bilateral agreement cannot land an attestation against the order, but the truth-of-content shape is an off-chain concern. A clause registered in the registry is thus immediately attestable, with zero per-clause on-chain code — open-world by construction.
                    </PaperRun>
                    <PaperRun title="Runtime-tier UI binding.">
                        A frontend integration that consumes the canonical specification — shaping clause-typed forms from the spec and validating the assembled agreement&rsquo;s content against it before either party signs — then carries the encoded content to chain. The runtime tier is not strictly part of the verification stack, but it is part of the clause&rsquo;s deployment surface; an unintegrated clause exists on chain and not in any product.
                    </PaperRun>
                    <PaperRun title="Lockstep contract.">
                        A new clause is not done until the canonical specification, its off-chain validator, and its runtime binding ship in lockstep. If the specification says one thing and a derived layer enforces another, attestations silently divide into two interpretations and the clause&rsquo;s reference integrity collapses. The lockstep contract is operational discipline, not a formal soundness theorem: agreement across the derived layers is enforced by deriving every layer from the single canonical specification and by a test suite covering well-formed input plus every rejection path.
                    </PaperRun>
                </PaperSubsection>
                ...
                <PaperSubsection title="4.3 First-write-wins registration">
                    <p>
                        Clause registration is permissionless and first-write-wins. Any address may register a clauseId; the registration binds that clauseId&rsquo;s anchor permanently, and a subsequent attempt to register the same clauseId is rejected. The first-write-wins discipline is deliberate: an admin-mediated registry would re-introduce governance authority over the clause surface; a permissionless registry without first-write-wins would let any address overwrite an existing anchor and break in-flight attestations filed against it. The combination is the only design that satisfies both no-admin and no-overwrite.
                    </p>
                    <PaperRemark title="Why first-write-wins is structurally necessary.">
                        Consider the alternatives. <em>Admin-mediated registration</em>: an admin can rotate clause anchors, but admin authority over the clause surface re-introduces the discretionary actor the no-escape-hatches discipline rules out. <em>Mutable anchors</em>: any address can rewrite an anchor, breaking existing attestations under that clauseId. <em>Time-locked anchors</em>: a delay buffers the overwrite, but any non-zero time creates a window in which the anchor is uncertain. The first-write-wins rule produces an anchor that is both permissionless and stable: anyone can register it once; no one can change it after.
                    </PaperRemark>
                </PaperSubsection>
                <PaperSubsection title="4.4 Immediate attestability">
                    <p>
                        Because the on-chain layer performs no per-clause content validation — the attestation coordinator merkle-binds the clause section against the signed agreement and content-hashes the evidence, invoking no per-clause validator — a clauseId is attestable the moment it is registered. There is no second binding step to race, and therefore no front-running window between registration and the clause becoming usable.
                    </p>
                    <p>
                        This is the open-world property stated operationally: a never-before-seen clause, registered permissionlessly, is immediately attestable against any order whose signed agreement included it, with zero per-clause on-chain code authored or deployed. The only on-chain admission gate is the merkle-inclusion proof that the clause was part of the bilateral agreement; truth-of-content is an off-chain specification-plus-read-time concern.
                    </p>
                </PaperSubsection>
                <PaperSubsection title="4.5 The reference clause catalogue">
                    <p>The protocol currently ships a coordinated set of reference clauses, treated as a reference set rather than as independent extensions. Among them:</p>
                    <ol className="space-y-1 list-decimal pl-6 text-sm">
                ...
                <PaperSubsection title="4.6 What clause design is not">
                    ...
                    <p>
                        <em>Clauses with admin, pause, or upgrade hooks.</em> The no-escape-hatches discipline applies to clauses as it applies to the kernel. A clause&rsquo;s anchor is immutable and permissionlessly registered once; an admin-controlled clause surface re-introduces the discretionary actor.
                    </p>
                </PaperSubsection>
            </PaperSection>
            ...
            <PaperSection title="5. The Coordinator Pattern">
                ...
                <PaperSubsection title="5.1 Composition semantics">
                    <p>
                        Let <Math>{"\\mathcal{K}"}</Math> denote the Figaro kernel as a state machine with state space <Math>{"\\mathcal{S}_{\\mathcal{K}}"}</Math> (the values of the three stored mappings) and transition function <Math>{"\\delta_{\\mathcal{K}}"}</Math> restricted to the two operations, commit and resolution. Let <Math>{"\\mathcal{M}"}</Math> denote an external mechanism contract with state space <Math>{"\\mathcal{S}_{\\mathcal{M}}"}</Math> and transition function <Math>{"\\delta_{\\mathcal{M}}"}</Math>.
                    </p>
                ...
                <PaperSubsection title="5.2 The coordinator pattern, formally">
                    ...
                    <p>
                        Each condition preserves a subset of the kernel&rsquo;s claims: condition (i) preserves the kernel state predicates the verification stack establishes (status monotonicity, transition correctness, cumulative integrity); condition (ii) preserves buyer dominance; condition (iii) preserves the no-escape-hatches discipline; condition (iv) preserves the agreement-binding surface. A mechanism that satisfies all four can be composed onto the kernel with the kernel&rsquo;s equilibrium guarantees still in force.
                    </p>
                    <PaperRun title="Proof sketch.">
                        The kernel&rsquo;s bonding equilibrium rests on the six invariants of Section 2; each condition maps to preservation of a subset under composition. Condition (i) preserves invariants (i)&ndash;(v): if <Math>{"P : \\mathcal{S}_{\\mathcal{K}} \\to \\mathrm{Bool}"}</Math> is invariant under <Math>{"\\delta_{\\mathcal{K}}"}</Math> and <Math>{"\\delta_{\\mathcal{M}}"}</Math> does not change <Math>{"\\mathcal{S}_{\\mathcal{K}}"}</Math>, then <Math>{"P"}</Math> is invariant under <Math>{"\\delta_{\\mathcal{K} \\otimes \\mathcal{M}}"}</Math>, the disjoint union of the two transition functions. Condition (ii) preserves buyer dominance and atomic resolution by ruling out alternative paths that would bypass them &mdash; an alternative settlement path is exactly the escape hatch the escape-hatch-weakness theorem forbids. Condition (iii) preserves no-escape-hatches by ruling out lock-bypass. Condition (iv) preserves the agreement-binding surface: a mechanism accepting content under a clauseId not present in the order&rsquo;s signed agreement creates a content surface downstream consumers cannot trust. The four are jointly sufficient, and necessary in the worst case &mdash; relaxing any one produces a documented anti-pattern (direct kernel-state mutation; an alternative settlement path; a discretionary lock-bypass; agreement-unbound clause-typed content). We do not claim necessity in every model; weaker conditions on a sufficiently restricted mechanism (e.g., a pure view contract) might suffice for narrower preservation claims.
                    </PaperRun>
                </PaperSubsection>
                <PaperSubsection title="5.3 The attestation coordinator as worked example">
                    <p>
                        The attestation coordinator is the canonical worked example of the coordinator pattern. It accepts attestations against bonded orders and emits typed events without ever modifying kernel state. Its admission gate is a merkle-inclusion proof of the attested clause section against the order&rsquo;s signed agreement hash, together with a content hash of the supplied evidence: only clauses committed at contract-signing time can be attested under, so a clause that was not part of the bilateral agreement cannot land an attestation against the order. The coordinator validates no clause content shape and invokes no per-clause validator; its verification surface discharges Proposition 5.2&rsquo;s conditions explicitly.
                    </p>
                    <p>
                        Condition (i) is discharged by parametric kernel-immutability properties: over every public function of the coordinator, the kernel state before and after execution is unchanged — universally quantified over the coordinator&rsquo;s methods and over kernel-state values. Conditions (ii) and (iii) are discharged by construction: the coordinator has no operation that transfers tokens from the kernel&rsquo;s escrow and no operation that invokes any kernel state-changing function. Condition (iv) is discharged by the merkle-inclusion gate: any attestation whose clause section does not prove inclusion against the order&rsquo;s signed agreement is rejected. The four conditions thus reduce to verification requirements a candidate coordinator can be checked against mechanically.
                    </p>
                </PaperSubsection>
                <PaperSubsection title="5.4 What the pattern is not">
                    <p>
                        The pattern is not a generic external-call disclaimer. A mechanism that calls into the kernel&rsquo;s view functions, reads state, and returns derived values is composition-safe; a mechanism that calls into the kernel&rsquo;s state-changing functions on behalf of a third party may or may not be composition-safe, depending on whether the third-party authorization is properly delegated. A role-resolver interface supplies the authorization-delegation pattern for the latter case (a role-based attestation routed through a mechanism that consults the role resolver to authorize the caller).
                    </p>
                    <p>
                        The pattern is also not a substitute for the clause-design discipline. A mechanism that accepts clause-typed content must admit it only when the clause is bound to the order&rsquo;s signed agreement, and must produce content the clause&rsquo;s off-chain well-formedness check would accept. The two disciplines (coordinator pattern and clause design) are independent and both must be satisfied for an extension to be protocol-grade.
                    </p>
                </PaperSubsection>
            </PaperSection>
            ...
            <PaperSection title="6. Runtime Composition">
                ...
                <PaperSubsection title="6.2 Assembly registry pattern">
                    ...
                    <p>
                        The pattern consolidates discovery and indexing (event-derived from the registry&rsquo;s registration logs), content building and canonicalization, and content-addressed publication through the assembly registry; fork-and-draft authoring composes on top of it. An assembly&rsquo;s slug is content-derived &mdash; a hash over its composition (the clauses it carries, their values, and the DAG) &mdash; so identical compositions map to one slug and the registry&rsquo;s first-write-wins binding dedups them. Published assemblies are event-derived on chain rather than bundled: there is no slug-to-label taxonomy and no on-disk content list, and an assembly&rsquo;s human label is derived from the clauses it composes.
                    </p>
                </PaperSubsection>
                <PaperSubsection title="6.3 Module system">
                    <p>
                        Modules are the unit the assembly&rsquo;s view definitions place in the rendered surface. A module owns a UI component, a set of typed actions it can dispatch, and the semantic context it consumes. In the current implementation the dedicated module registry is lightly populated (the seller-branding module), with the broader module-shaped concerns &mdash; catalogue editing, cart composition, order creation, attestation submission, delivery coordination, GHG disclosure, FIG token operations &mdash; realized across the runtime library and the component tree rather than consolidated under a single registry. The module pattern below is the composition discipline those surfaces follow, whether or not they are formally registered as modules.
                    </p>
                    <p>
                ...
                <PaperSubsection title="6.4 The lens system">
                    ...
                    <p>
                        The lens pattern is the runtime&rsquo;s enforcement of the role-segregation that the kernel enforces at the bonding-and-resolution surface. The kernel&rsquo;s role-segregation is mathematical (only the buyer can trigger atomic resolution); the runtime&rsquo;s is operational (only a buyer-context module can dispatch buyer-side actions). The two reinforce each other: a runtime that exposed seller-side state to a buyer-context module would let the buyer attempt to submit attestations as the seller, which the kernel would reject at signature verification but which the runtime should not have attempted in the first place.
                    </p>
                </PaperSubsection>
                <PaperSubsection title="6.5 The designer canvas">
                    <p>
                        A specialized runtime surface deserves separate treatment: the assembly designer, which wraps a process-graph renderer for topology and an agreement drawer for per-node clause binding. The designer is a composition surface where assembly authors fork an existing reference assembly or start from blank, modify the bonded-process graph, bind clauses to nodes through a side drawer, and save drafts.
                    </p>
                    <p>
                        The canvas is not a wizard, not a recommendation engine, not a generative-AI interface; it is a structural editor for assemblies. The designer pulls the runtime&rsquo;s primitives (commerce commitments, proximity-proof handoff attestations, fulfilment clauses, and so on) onto the canvas as draggable elements; the author composes them into the shape the institutional context requires; the resulting assembly is saved as a draft.
                    </p>
                    <p>
            ...
            <PaperSection title="7. What the Discipline Refuses">
                ...
                <PaperRun title="Admin paths.">
                    Any proposed extension that introduces admin authority over clause registration, mechanism activation, or kernel state is refused. The first-write-wins registration pattern is the specific implementation of the no-admin discipline at the clause surface; analogous patterns apply elsewhere (e.g., the seller registry&rsquo;s permissionless registration without admin gating).
                </PaperRun>
