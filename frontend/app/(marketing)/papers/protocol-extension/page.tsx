        ...
        >
            ...
            <PaperSection title="2. The Settlement Primitive">
                ...
                <PaperRun title="Three tiers.">
                    <em>Kernel</em>: the irreducible settlement primitive. <em>Protocol</em>: the kernel together with extensions composed under the discipline of this paper. <em>Runtime</em>: the protocol together with semantic layers, builder surfaces, and rendered institutional shapes that humans and agents interact with.
                </PaperRun>
            </PaperSection>

            <PaperSection title="3. The Protocol Extension Doctrine">
                <p>
                    The kernel is intentionally narrow: its narrowness is what produces the bonding equilibrium, and widening it would weaken the equilibrium. Extensions must therefore add capability without widening the kernel itself; the doctrine answers how.
                </p>
                ...
                <PaperSubsection title="3.4 The decision rule">
                    ...
                    <p>
                        The proposition is operationally checkable: each condition has a yes/no answer for a given proposed feature, and a feature that fails the check is not a protocol-extension candidate. The rule is binding: the discipline is the willingness to keep features out of the protocol when the rule fails them, not just to add them when it passes them.
                    </p>
            ...
            <PaperSection title="7. What the Discipline Refuses">
                ...
                <PaperRun title="Force-majeure escape clauses.">
                    Any proposed extension that lets a participant avoid bond loss in external events (weather, regulatory action, force majeure) is refused. The structural answer is composition with a parallel insurance process, settled independently of the bonded commitment: bonded settlement does not contemplate external-event-conditional release; insurance does; the two compose and neither replaces the other.
                </PaperRun>
                ...
                <PaperRun title="Fee discounts conditioned on protocol-tier signals.">
                    Any proposed extension that conditions kernel-tier or protocol-tier behavior on a fee discount, a green-bond adjustment, or any other rate-modifying signal is refused. The <Math>{"2\\times"}</Math> bonding ratio is the minimum viable equilibrium under the minimum-multiplier result; a fee-discount mechanism that modifies the effective ratio for some class of participant breaks the equilibrium for that class. Whatever incentive a fee discount would supply belongs at the assembly or seller-registry tier as a parameter the parties choose, not at the bonding rule.
                </PaperRun>
            </PaperSection>
