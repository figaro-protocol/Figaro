...
export default function ProtocolExtensionPaper() {
    return (
        <PaperLayout
            ...
            subtitle="Schema Design, the Coordinator Pattern, and the Software Architecture Above the Kernel"
        ...
        >
            ...
            <PaperSection title="3. The Protocol Extension Doctrine">
                ...
                <PaperSubsection title="3.2 The anchored artifact pattern">
                    ...
                    <FormalBlock label="Definition 3.1 (Anchor record).">
                        <p>
                            A protocol-layer anchor for an artifact family is a tuple <Math>{"\\langle \\text{schemaId}, \\text{version}, \\text{uriHash}, \\text{admitted} \\rangle"}</Math> where schemaId is the anchor identity, version is the version within the family, uriHash is an immutable cryptographic hash of the URI pointing at the off-chain content, and admitted is the binding&rsquo;s monotone admission flag (false until first registration, true thereafter; the discipline forbids deactivation). In the deployed implementation (<code>src/SchemaRegistry.sol</code>), the admission flag is stored as <code>mapping(bytes32 =&gt; bool) public registered</code>; version and uriHash live on the <code>SchemaRegistered</code> event log rather than as queryable storage. The conceptual anchor is reconstructed by reading the event log under the append-only-identity discipline of Section 4.2.
                        </p>
                    </FormalBlock>
                    <p>
            ...
            <PaperSection title="4. Schema Design as a CS Discipline">
                <p>
                ...
                <PaperSubsection title="4.1 The four-layer verification stack">
                    ...
                    <PaperRun title="Layer C: on-chain validator.">
                        A Solidity contract implementing <code>ISchemaValidator</code> that ABI-decodes the schema&rsquo;s content (no on-chain JSON parsing; the on-chain layer reads the canonical ABI encoding produced by Layer A&rsquo;s encoder) and reverts with typed custom errors on any violation. The validator is pure (no external state reads, no <code>block.*</code> or <code>tx.*</code>, no external calls) and binds to one schemaId via a compile-time literal (<code>{`bytes32 public constant override schemaId = keccak256("...")`}</code>); using a constructor-set <code>immutable</code> schemaId would forfeit the EVM-enforced determinism guarantee and is forbidden by the validator-contract pattern.
                    </PaperRun>
                    <PaperRun title="Runtime-tier UI binding.">
                        A frontend integration that consumes the Layer A specification via <code>useSchemaValidator(schemaId)</code>, renders schema-typed forms through the existing module and view system, and delivers the ABI-encoded content to the chain through the Layer C validator. The runtime tier is not strictly part of the verification stack, but it is part of the schema&rsquo;s deployment surface; an unintegrated schema exists on chain and not in any product.
                    </PaperRun>
                    <PaperRun title="Lockstep contract.">
                        A new schema is not done until all four layers ship in lockstep. If Layer A says one thing and Layer C says another, attestations silently divide into two interpretations and the schema&rsquo;s reference integrity collapses. The lockstep contract is operational discipline, not a formal soundness theorem: Layer A and Layer C agreement is enforced at the deploy-script level (the registration script registers the schemaId and binds the validator atomically) and at the test-suite level (Foundry tests in <code>test/schemaValidators/</code> cover well-formed input plus every typed-error revert path).
                    </PaperRun>
                </PaperSubsection>
                <PaperSubsection title="4.2 Append-only identity">
                    <p>Schema identity is append-only. The discipline produces three operational rules:</p>
                    <ol className="space-y-1 list-decimal pl-6 text-sm">
                        <li>No in-place rewriting of schema meaning. A registered schema cannot be modified through any contract operation; the schema record at schemaId is immutable from the moment of registration.</li>
                        ...
                        <li>New meaning requires a new version or a new schema identity. A schema family that needs a substantive change registers a new <code>schemaId</code> (e.g., <code>figaro-foo-v1</code> &rarr; <code>figaro-foo-v2</code>); never mutates the v1 contract or its Layer A spec.</li>
                    </ol>
                    <p>The append-only rule preserves historical interpretability: attestations filed under a schemaId remain readable against the exact schema anchor they were filed under, regardless of subsequent schema-family evolution.</p>
                </PaperSubsection>
                <PaperSubsection title="4.3 First-write-wins binding">
                    <p>
                        The validator-contract binding to a schemaId is permissionless and first-write-wins. <code>AttestationCoordinator.setValidator(schemaId, validator)</code> is callable by any address and binds the validator permanently for that schemaId. Subsequent calls revert with <code>ValidatorAlreadySet</code>. The first-write-wins discipline is deliberate: an admin-mediated binding would re-introduce governance authority over the schema-validation surface; a permissionless binding without first-write-wins would let any address overwrite an existing binding and break in-flight attestations. The combination is the only design that satisfies both no-admin and no-overwrite.
                    </p>
                    <PaperRemark title="Why first-write-wins is structurally necessary.">
                        Consider the alternatives. <em>Admin-mediated binding</em>: an admin can rotate validators, but admin authority over the validation surface re-introduces the discretionary actor the no-escape-hatches discipline rules out. <em>Mutable bindings</em>: any address can rewrite a binding, breaking existing attestations under that schemaId. <em>Time-locked bindings</em>: a delay buffers the overwrite, but any non-zero time creates a window in which the binding is uncertain. The first-write-wins rule produces a binding that is both permissionless and stable: anyone can set it once; no one can change it after.
                    </PaperRemark>
                </PaperSubsection>
                <PaperSubsection title="4.4 The atomic-bind pattern">
                    <p>
                        The first-write-wins rule produces a front-running window between <code>SchemaRegistry.registerSchema</code> and <code>AttestationCoordinator.setValidator</code>. An adversary observing a pending registration can race to set a malicious validator under the new schemaId before the legitimate validator binds. The malicious validator self-attests as bound to the schemaId (satisfying the binding-integrity check) and then accepts content the legitimate validator would have rejected.
                    </p>
                    <p>
                        The mitigation is atomic registration: both writes execute in a single transaction. The 17 reference <code>figaro-*</code> validators are bound this way at genesis (the 18th catalogue schema, <code>figaro-topology-v1</code>, is manifest-only and has no on-chain validator), with <code>script/Deploy.s.sol</code> composing the calls in a single deployment broadcast session controlled by one signer (no third-party interleaving against the deployer&rsquo;s nonce stream). For third-party schemas registered post-deploy, the <code>SchemaRegistrationHelper</code> contract supplies genuine single-transaction atomicity as a stateless, no-admin, no-fee public helper:
                    </p>
                    <CodeBlock>{CODE_HELPER}</CodeBlock>
                    <p>
                        The helper composes the two underlying public calls (<code>SchemaRegistry.registerSchema</code> and <code>AttestationCoordinator.setValidator</code>) atomically. The helper has no authority over either underlying contract; it has no admin, no fee, and no privilege. It is a permissionless composer that makes the two-step pattern safe.
                    </p>
                </PaperSubsection>
                <PaperSubsection title="4.5 The 18-schema catalogue">
                    <p>The protocol currently ships eighteen schemas, treated as a coordinated reference set rather than as independent extensions.</p>
                    <ol className="space-y-1 list-decimal pl-6 text-sm">
                    ...
                    <p>
                        The catalogue exhibits several patterns worth flagging. The <em>sister-schema pattern</em> (the five GHG disclosure schemas, the proximity policy/proof pair, the merchant/courier process schemas) supplies multiple specialized variants of a shared coordination concept; content shape is shared, schemaId is per-standard, and per-standard extensions can be added to a single sister without affecting siblings. The <em>committed-vs-runtime split</em> (proximity policy/proof, GHG disclosure/measurement) separates the band the parties commit at agreement signing (Category-2, byte-equality enforced) from the runtime witness data filed during execution (Category-1, fresh per attestation). The <em>manifest-only clause</em> pattern (<code>figaro-topology-v1</code>) represents a shared-vocabulary anchor whose enforcement is purely off-chain; parties commit to the topology at contract-signing time, the indexer reconstructs the DAG from event logs, and no runtime attestation fires. Each pattern is a discipline-level choice the catalogue&rsquo;s authors made deliberately, and each is reusable by future schemas in the family.
                    </p>
                    <PaperRun title="Class A and Class B records.">
                        A useful evidentiary taxonomy distinguishes records the kernel produces by construction from records participants attest under schema discipline. <em>Class A</em> records are kernel byproducts (commitment digest, bond deposits, cumulative-value accumulator state, order status transitions, resolution events): emitted by the kernel, schema-typed at the kernel level, discretion-free, and cryptographically authenticated. <em>Class B</em> records are discretionary schema-validated attestations (delivery confirmations, GHG disclosures, handoff attestations, custody-of-seal events, lifecycle stage transitions): produced by participants through the <code>AttestationCoordinator</code>, schema-typed at the <em>protocol</em> level, validator-gated, and substantively contestable on truth-of-content grounds. The schema-design discipline of this paper produces Class B records.
                    </PaperRun>
                </PaperSubsection>
                <PaperSubsection title="4.6 What schema design is not">
                    <p>Three classes of would-be extension fall outside the discipline.</p>
                    <p>
                        <em>Per-instance payload schemas</em> that try to make every order-specific data shape into a registered schema. Most order-specific data does not need a registered schema; it needs an order-specific encoder and an off-chain decoder. The discipline applies the decision rule (Proposition 3.2) at the point of asking &ldquo;does this need stable shared interpretation across parties or tools?&rdquo; If the answer is no, the data is a payload, not a schema candidate.
                    </p>
                    <p>
                        <em>Schemas with mutable freeform text or large on-chain payloads</em> (typically larger than ~1KB). Mutable freeform text violates append-only identity. Large payloads violate the minimal-anchor surface. Both move the schema in the wrong direction.
                    </p>
                    <p>
                        <em>Schemas with admin, pause, or upgrade hooks.</em> The no-escape-hatches discipline applies to schemas as it applies to the kernel. A schema&rsquo;s validator is pure, immutable, and permissionless once bound; an admin-controlled validator re-introduces the discretionary actor.
                    </p>
                </PaperSubsection>
            </PaperSection>
            ...
            <PaperSection title="5. The Coordinator Pattern">
                ...
                <PaperSubsection title="5.4 What the pattern is not">
                    ...
                    <p>
                        The pattern is also not a substitute for the schema-validation discipline. A mechanism that accepts schema-typed content must route it through the registered validator; a mechanism that produces schema-typed events must produce content the registered validator would have accepted. The two disciplines (coordinator pattern and schema design) are independent and both must be satisfied for an extension to be protocol-grade.
                    </p>
                </PaperSubsection>
            </PaperSection>
            ...
            <PaperSection title="6. Runtime Composition">
                ...
                <PaperSubsection title="6.1 Seven-layer composition pipeline">
                    ...
                    <ol className="space-y-2 list-decimal pl-6 text-sm">
                        ...
                        <li><strong>Branding fields.</strong> Presentation-only personalization: theme tokens, imagery, type and spacing preferences, non-semantic copy. In the current implementation these are lightweight fields on the operator profile referenced at binding time (accent colour, logo, hero image, theme class, via the operator&rsquo;s branding metadata) rather than a heavyweight separate pipeline stage; we list them as a distinct layer because they sit at a distinct trust boundary (presentation-only, never semantic), not because they require their own composition machinery. Branding fields cannot alter capability validity, mechanism authority, risk boundaries, or settlement semantics.</li>
                    </ol>
                    <p>
            ...
            <PaperSection title="7. What the Discipline Refuses">
                ...
                <PaperRun title="Admin paths.">
                    Any proposed extension that introduces admin authority over schema binding, validator selection, mechanism activation, or kernel state is refused. The first-write-wins binding pattern is the specific implementation of the no-admin discipline at the schema-validation surface; analogous patterns apply elsewhere (e.g., the <code>OperatorRegistry</code>&rsquo;s permissionless registration without admin gating).
                </PaperRun>
                ...
                <PaperRun title="Schemas with mutable freeform text.">
                    Any proposed schema with large-text or mutable-text fields is refused. The minimal-anchor surface and append-only identity together rule out anchoring substantial mutable content on chain. Such content belongs off-chain, content-addressed by hash with the hash registered as the schema&rsquo;s reference.
                </PaperRun>
