    ...
    return (
        <PaperLayout slug="wallet-without-polity"
            ...
            abstract={
                <>
                    <p>
                        The bonded settlement primitive &mdash; its mechanism established and its kernel formally verified, though by its own authors and not yet by an external audit &mdash; exhibits a property whose consequences are most consequential not for populations conventionally treated by labor or commercial law, but for those who occupy the negative space of those frames: refugees, stateless persons, undocumented migrants, and populations whose residency is beyond the reach of stable banking and enforcement infrastructure. The primitive&rsquo;s precondition is a cryptographic key, not a recognized civil-legal subjecthood. This paper develops the consequence.
                    </p>
                    <p>
        ...
        >
            ...
            <PaperSection title="2. The Settlement Primitive">
                ...
                <p>
                    The primitive has two calls, and each carries a mechanism design of its own. <em>Commit</em> forms a bilateral edge under <em>asymmetric bonding</em>: both parties sign the commitment from their own wallets, the buyer locks twice the payment, <Math>{"2P"}</Math>, and the seller locks twice the value the process has accumulated through its own link, <Math>{"2G"}</Math> &mdash; where <Math>{"G"}</Math> counts every payment committed in the process up to and including the seller&rsquo;s own, a figure fixed by arithmetic on the signed accumulator rather than reported by anybody. <em>Resolve</em> settles the process under <em>buyer dominance with atomic resolution</em>: only the buyer may call it, it settles every active order in the process together or none of them, and it is terminal. There is no platform between the parties, no arbitrator authorized to resolve a process, and no admin key permitting external override.
                </p>
                <p>
                    The two calls compose in a definite order, and that order is the equilibrium. After performance, resolving is unconditionally strictly better for the buyer &mdash; resolving returns its bond less the payment it agreed to make, where withholding leaves the whole bond locked and the payment out of everyone&rsquo;s reach inside it &mdash; and given that, performance is each seller&rsquo;s strict best response. The comparison is made on what a defector would keep as well as on what the mechanism moves: a party that walks off with the value at its link still holds that value, and the doubled bond is what leaves the walking-off costly <em>after</em> crediting the party with everything it retains. The goods, the passages of value between the parties, and the parties&rsquo; knowledge of both stay off the record; what the record holds is the bonds, the signed accumulator, and the commitments themselves.
                </p>
                <p>
                    The features that matter for the present paper are three. First, the mechanism does not require either party to be a recognized civil subject; it requires each to control a key that can sign and to hold a stake it can bond. Second, it does not require a forum to enforce the agreement: the bonds are the enforcement, each party&rsquo;s being its own staked deterrent. Third, it does not route value through a banking intermediary; settlement is direct, wallet to wallet, atomic on resolution. What the bonds discipline is settlement &mdash; whether a party that can perform will. They do not settle the honest disagreement about whether what arrived met the agreement: the mechanism holds no test of performance and decides nothing there, so the parties work it out between themselves while the process stands open, with an outside forum ruling on the record if the parties have one they can both reach. For the population this paper concerns, that is often precisely what is missing (&sect;5.3), and the architecture does not depend on it: no forum can resolve a process in the buyer&rsquo;s place, so the bonded positions stand, and the buyer&rsquo;s resolution &mdash; terminal acceptance, taken once it is satisfied &mdash; is the only thing that closes them.
                </p>
                <p>
                    For the present paper both mechanisms are load-bearing. Asymmetric bonding removes the trusted enforcer; buyer dominance with atomic resolution removes the trusted clearing house, settling a whole multi-party process at one signature rather than at an intermediary&rsquo;s ledger entry. The displaced or stateless subject lacks access to either, and the primitive&rsquo;s contribution is to make both unnecessary.
                </p>
            </PaperSection>

            <PaperSection title="3. A Party Where No Civil-Legal Party Exists">
                <p>
                    This paper adopts a premise it does not re-derive: the bonded primitive collapses the old partition between the thing that is owned and the party that acts, so that the productive asset is at once property and party. A wallet is a key-pair, held and transferable; it is also a signer, a bonder, and a receiver of settlement. The extension the present setting demands is stronger than the collapse itself. Where one or both parties lack stable <em>persona</em> status in the host jurisdiction at all, the wallet provides a party-to-the-transaction whose existence does not depend on the prior existence of a recognized civil subject: the wallet can be a party where no civil-legal party exists.
                </p>
                <p>
            ...
            <PaperSection title="6. Operational Constraints">
                <p>
                    The primitive moves the commerce-access gap from architectural impossibility to operational engineering question; it does not solve the engineering question. We name the four constraints that remain, and the World Food Programme&rsquo;s <em>Building Blocks</em> pilot (with UNHCR providing biometric registration, deployed in Jordan since 2017 and expanded across several refugee operations) supplies existing evidence for two of them: independent evaluations (Coppi &amp; Fast, 2019; Hernandez, 2017) find the programs deliver real operational improvements in coordination and disbursement, but locate the binding constraint not in on-chain recognition of the participant &mdash; which the architecture supplies straightforwardly &mdash; but in the on-ramp to local purchasing power and in host-state gatekeeping of who may transact with whom, which are the first two constraints below. The displaced beneficiaries in these pilots do not bond; they receive. The bonded primitive extends the architecture to a use case the pilots have not yet exercised at scale: the displaced participant as <em>counterparty</em> in a productive bilateral exchange rather than as recipient of one-sided disbursement.
                </p>
                ...
                <PaperRun title="Adjudication: no forum both parties can reach.">
                    The population this paper concerns is the one for which the reachable-forum assumption fails most often. A party whose standing is contested has no local court willing to hear its commercial dispute (&sect;5.3), and a counterparty in a recognized jurisdiction has no forum whose process reaches the other side; where a forum can be named at all, the naming is a privilege of the better-placed party. What this costs is specific, and it is worth stating exactly, because the primitive neither supplies a forum nor depends on one. The honest disagreement over whether what arrived met the agreement is precisely the case the bonds do not settle, so a party without a reachable forum loses the aid that would otherwise inform its remedy negotiation. It does not lose the mechanism&rsquo;s own discipline: no forum can resolve a process in the buyer&rsquo;s place, so a forum&rsquo;s absence subtracts nothing from the bonded positions themselves, which stand with each party&rsquo;s stake exposed for as long as the process is open and close only on the buyer&rsquo;s acceptance. The record, meanwhile, travels with the parties should a forum ever become reachable &mdash; it shows what was undertaken and what was never closed, though never what was performed. The constraint is real and it is bounded: it removes an aid to settling disagreements, not the deterrent that makes settling them worthwhile.
                </PaperRun>
