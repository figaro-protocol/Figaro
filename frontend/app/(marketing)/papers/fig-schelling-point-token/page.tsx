        ...
        >
            ...
            <PaperSection title="4. Design Rules for a Focal-Point Token">
                ...
                <PaperRun title="Rule 3: Clean distribution.">
                    Focality is fragile under highly concentrated ownership. If a single holder can dump enough supply to destabilize the price, Schelling convergence is less credible. FIG&rsquo;s allocation is 60% to a schema-author cohort (the staged airdrop), 30% to a treasury with an ecosystem mandate (the DAO), and 10% to founders. The concentration is higher than ideal for focality (the DAO + founder share is 40%), which we address honestly in Section 10 and in the distributional-silhouette concession in Section 11.
                </PaperRun>
            ...
            <PaperSection title="5. Supply Integrity">
                <p>
                    FIG has a fixed maximum supply of <Math>{"10^9"}</Math> units (one billion, denominated in 18-decimal units in the contract). The supply ceiling is enforced by two distinct mechanisms operating at different times.
                </p>
                <PaperRun title="Per-mint enforcement (run-time).">
                    The <code>mint</code> function reverts with <code>SupplyCapExceeded</code> if <code>totalSupply() + amount &gt; MAX_SUPPLY</code>, checked atomically inside a non-reentrant call.
                </PaperRun>
                <PaperRun title="Pre-registration enforcement (deploy-time).">
                    The contract maintains an aggregate <code>totalRegisteredCap</code>: the sum of all registered minter caps. <code>registerMinter</code> reverts if <code>totalRegisteredCap + cap &gt; MAX_SUPPLY</code>. This means the entire allocation plan is committed to the <Math>{"10^9"}</Math> ceiling <em>before any token is minted</em>.
                </PaperRun>
                <p>
                ...
                <p>
                    ...
                    FIG has a one-way Boolean <code>deployerMintRenounced</code>, settable exactly once by the original deployer via <code>renounceDeployerMint</code>. After renouncement, <code>registerMinter</code> reverts and any deployer call to <code>mint</code> reverts. The deploy script (<code>script/DeployMainnet.s.sol</code>) registers exactly two minters &mdash; the deployer wallet (cap <Math>{"4 \\times 10^8"}</Math>, used for the genesis mint) and the airdrop contract (cap <Math>{"6 \\times 10^8"}</Math>) &mdash; and then calls <code>renounceDeployerMint</code> as the final action of the deploy transaction. After the transaction completes:
                </p>
                <ul className="space-y-1 list-disc pl-6 text-sm">
                    <li><Math>{"\\texttt{totalRegisteredCap} = 10^9"}</Math> exactly;</li>
                    <li>the deployer has minted exactly <Math>{"4 \\times 10^8"}</Math> to the designated founder and DAO wallets;</li>
                    <li>only the airdrop contract retains residual mint authority, capped at <Math>{"6 \\times 10^8"}</Math>;</li>
            ...
            <PaperSection title="6. Allocation: 10 / 30 / 60">
                ...
                <PaperRun title="Airdrop allocation (60%).">
                    The majority share, allocated over a calendar window with declining per-stage size. The structure is designed to (a) reach schema-author participants, (b) reward sustained substrate contribution (longer-window cohorts at year 5 and year 9), and (c) avoid the &ldquo;dump-on-launch&rdquo; dynamic of single-event airdrops by spreading the supply emission over a 9-year horizon. The recipients and amounts within each stage are decided by the schema-author substrate-broadening formula, computed retroactively over settled on-chain history; the allocation is not a generic user airdrop.
                </PaperRun>
                <p>
            ...
            <PaperSection title="7. Cohort-Based Distribution via Staged Merkle">
                <p>
                    The 600M airdrop is realized by a single contract, <code>RpgfMinter.sol</code>, with three immutable stages (<code>STAGE_COUNT = 3</code>). Each stage is parameterized by an immutable <code>unlockTime</code>; the per-stage merkle <code>root</code> is submitted at tranche time by a sequencer-role address, gated by verification of an SP1 zero-knowledge proof attesting that the root correctly aggregates the schema-author substrate-broadening formula. Per-stage root submission is one-shot &mdash; the root cannot be replaced once committed. There is no admin-controlled stage addition, schedule extension, or cohort modification.
                </p>
                <PaperRemark title="On the move from baked to submitted roots.">
                    An earlier candidate design baked the three merkle roots into the constructor alongside the unlock times. That shape was retired once the schema-author substrate-broadening aggregation matured: a tranche&rsquo;s recipients depend on settlement history that does not exist at deploy time (the year-9 window is empty at year 0), so pre-deploy root computation became inadequate. The roots are now computed and submitted per tranche, behind a proof that the canonical formula was applied to the supplied window.
                </PaperRemark>
                <PaperRun title="Single contract, three stages.">
                    An earlier candidate design used three separate airdrop contracts. The single-contract design is preferred for two reasons. First, surface area: a single contract is a single audit target, a single set of formal-verification rules, and a single storage account. Second, atomic deploy guarantees: the <code>registerMinter</code> call binds the airdrop contract&rsquo;s <Math>{"6 \\times 10^8"}</Math> cap before the renouncement, and a single contract makes the cap binding atomic.
                </PaperRun>
                <PaperRun title="Leaf format.">
                    The merkle leaf is <code>keccak256(abi.encodePacked(account, amount))</code>, the canonical OpenZeppelin format. Choosing a non-canonical leaf format would have required custom tooling per off-chain proof generator and a corresponding audit cost; we accept the canonical choice.
                </PaperRun>
                <PaperRun title="One-shot per (stage, address).">
                    The <code>claimed[stageIndex][msg.sender]</code> double-mapping is set true on successful claim and checked on every entry. A given account can claim at most once per stage. No top-up, no re-claim, no rebate.
                </PaperRun>
            ...
            <PaperSection title="8. Design Exclusions as Focality Preservers">
                ...
                <PaperRun title="FIG is not settlement-emitted.">
                    There is no path from kernel activity (commit, resolve, attest) to FIG mint. <code>FigaroBatchVerifier</code> &mdash; the layer-2 batch verification contract &mdash; is explicitly not a registered minter and will never be. The deploy script does not register it; the renouncement latch ensures it cannot be added later. This refusal is the most rigorous application of Rule 4 and is worth its own section (Section 9).
                </PaperRun>
            ...
            <PaperSection title="12. Verification Surface">
                <p>
                ...
                <PaperRun title="TLA⁺ (formal/FigToken.tla).">
                    8 invariants verified across 160,844 distinct states in approximately 9 seconds (TLC, breadth-first search, parameters <code>MAX_SUPPLY=5</code>, 3 minters, 6 recipients): <code>Inv_MaxSupply</code>, <code>Inv_NonNegative</code>, <code>Inv_DeployerCannotMintAfterRenounce</code>, <code>Inv_NoMintToZero</code>, <code>Inv_MinterCap</code>, <code>Inv_BalancesSumToSupply</code>, <code>Inv_CapBelowMaxSupply</code>, <code>Inv_SupplyEqualsSumMinted</code>.
                </PaperRun>
                <PaperRun title="Halmos (retired).">
                    The original verification round included a 4-property symbolic-execution harness (<code>test/HalmosStagedMerkleAirdrop.t.sol</code>) covering claim flag set on success, one-shot enforcement per (stage, address), correct balance arithmetic, and the merkle leaf format. The harness was retired alongside the predecessor <code>StagedMerkleAirdrop</code> contract; the replacement <code>RpgfMinter</code> is covered by Foundry tests and a Rust&ndash;Solidity conformance harness but does not yet carry a Halmos harness of its own.
                </PaperRun>
                <PaperRun title="Certora.">
                    6 CVL rules in <code>certora/FigToken.spec</code>: <code>totalSupplyWithinMaxSupply</code>, <code>totalRegisteredCapWithinMaxSupply</code>, <code>minterMintedWithinCap</code>, <code>totalRegisteredCapMonotonic</code>, <code>deployerMintRenouncedIsOneWayLatch</code>, <code>minterCapImmutable</code>. A 3-rule spec for the predecessor airdrop contract was part of the original round but was retired when that contract was replaced by <code>RpgfMinter.sol</code>; the replacement does not yet carry a Certora spec.
                </PaperRun>
                <PaperRun title="Foundry regression.">
                    <code>test/fig/FigToken.t.sol</code>, <code>test/fig/RpgfMinter.t.sol</code>, and <code>test/fig/RpgfMinterConformance.t.sol</code> cover the deploy flow, every custom-error revert path, the Rust&ndash;Solidity conformance assertion on the canonical merkle root, and the canonical 10/30/60 allocation realized by <code>script/DeployMainnet.s.sol</code>.
                </PaperRun>
                <PaperRun title="Headline supply guarantee.">
                    The verification surface yields a strong guarantee: from the moment <code>deployerMintRenounced = true</code> is included in a finalized transaction, the <code>totalSupply()</code> trajectory is bounded above by <Math>{"10^9"}</Math>, no new minters can be registered, and the only remaining mint authority is the airdrop contract whose schedule is itself frozen by its three immutable stages. The supply is deterministic conditional on the merkle leaves; the leaves are committed per tranche behind the substrate-broadening proof. Focal-point participants assessing FIG can verify this directly from on-chain state.
                </PaperRun>
            </PaperSection>
            ...
            <PaperSection title="13. Regulatory Framing">
                ...
                <p>
                    (iv)&nbsp;<em>Derived from the efforts of others.</em> This is the prong on which most token cases turn under the SEC framework. The framework weighs whether an active participant (developer, foundation, promoter) is undertaking essential managerial efforts on which the holder&rsquo;s profit expectation depends. FIG&rsquo;s deploy script registers the deployer as a one-shot minter, mints the founder and DAO allocations, and renounces &mdash; permanently, by code-enforced one-way latch (Section 5). After deploy, the founder&rsquo;s effort is not managerial in any sense the framework recognizes: the founder holds liquid FIG as any holder does, has no protocol-level authority, cannot mint, cannot upgrade, cannot freeze, and has no fiduciary relationship to other holders. The DAO has treasury authority over its own allocation but not protocol authority over FIG. The development of the underlying Figaro protocol is a separate question, governed by a separate and ongoing analysis; FIG is not coupled to that development through any protocol-level mechanism. Prong (iv) is, on our reading, partially met (insofar as ecosystem development is happening and holder profit expectations may depend on it) and partially not met (insofar as the relationship between holder profit and developer effort is reflexive, not contractual or fiduciary). The prong is the hardest to assess because the SEC framework&rsquo;s factors were written with team-managed token projects in mind, and FIG is structurally a less-managed instrument than the framework contemplates.
                </p>
                <p>
            ...
            <PaperSection title="14. Conclusion">
                ...
                <PaperRun title="Open questions.">
                    Three questions remain outside this paper&rsquo;s scope but worth flagging. First, the schema-author substrate-broadening formula that decides which authors appear in which tranche&rsquo;s merkle root is developed as its own mechanism, and the policy and verification framework around each tranche&rsquo;s submission should be audited before that tranche&rsquo;s root is committed. Second, the relationship between FIG and the various treasury communities likely to form around it is genuinely emergent and not centrally designable; the present paper does not attempt to predict or control that emergence. Third, the status of the specific percentages in the allocation is as addressed in Section 6: judgment, not proof; the structural choices are the defensible content.
                </PaperRun>
