            ...
            <PaperSection title="7. Mechanism Composition: The Coordinator Pattern">
                ...
                <FormalBlock label="Proposition 7.1 (Coordinator Pattern — Sufficient Conditions).">
                    ...
                    <ul className="space-y-1 list-none pl-0 text-sm">
                        <li>(i) <strong>No unauthorized kernel-state mutation</strong>: <Math>{"M"}</Math> reads kernel state freely &mdash; storage, the kernel&rsquo;s bonded balance, the event log &mdash; and writes none of it on its own account. The two entry points admit composition asymmetrically, and the asymmetry is the kernel&rsquo;s, not a design choice of the pattern. <em>Commitment</em> admits a relay: the call carries both parties&rsquo; signatures, the kernel recovers them and pulls each bond from the named party, so the caller supplies transport and the kernel remains the writer. <em>Resolution</em> admits none: it takes no signature at all and authorizes on the caller&rsquo;s own address &mdash; the caller must <em>be</em> the root buyer &mdash; so no composer can resolve on a buyer&rsquo;s behalf even holding a buyer signature. Nor can <Math>{"M"}</Math> hold a party role: the kernel admits only an address whose own ECDSA key produced the signature, so a contract wallet that authenticates by returning a validity answer for a third party&rsquo;s signature cannot be a party.</li>
                        <li>(ii) <strong>No alternative settlement path</strong>: <Math>{"M"}</Math> offers no operation producing value flows equivalent to the kernel&rsquo;s atomic resolution while bypassing it or altering its preconditions, and holds no discretion over a live process&rsquo;s settlement.</li>
                        <li>(iii) <strong>No discretionary lock-bypass</strong>: <Math>{"M"}</Math> custodies no kernel bonds and cannot release bonded funds under conditions other than those the kernel&rsquo;s resolution enforces.</li>
                        <li>(iv) <strong>Agreement-bound content</strong> (where applicable): if <Math>{"M"}</Math> accepts content typed by a registered clause identifier, it admits that content only against an order whose signed agreement included the clause, verified by a merkle-inclusion proof of the clause section against the agreement hash; content for a clause absent from the bilateral agreement is rejected.</li>
                        <li>(v) <strong>No off-kernel side-payment</strong>: <Math>{"M"}</Math> does not commit, on-chain or off-chain, to award value contingent on the kernel&rsquo;s resolved bond outcome.</li>
                    </ul>
                </FormalBlock>
                <p>
