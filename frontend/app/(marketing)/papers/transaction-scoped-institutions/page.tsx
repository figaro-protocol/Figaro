    ...
    return (
        ...
        >
            ...
            <PaperSection title="3. The Coasean Threshold Shift">
                ...
                <PaperRemark title="The full bonded stake, both sides of the table.">
                    Proposition 3.1&rsquo;s comparison names the buyer&rsquo;s lockup; the sellers&rsquo; bonds belong in the same accounting, and stating them plainly sharpens the claim rather than qualifying it. In a process of depth <Math>{"n"}</Math> the seller at position <Math>{"i"}</Math> posts <Math>{"2G_i"}</Math> against the cumulative upstream value <Math>{"G_i = \\sum_{j \\leq i} P_j"}</Math>; for the illustrative linear chain of equal payments <Math>{"P"}</Math> this is <Math>{"2iP"}</Math>, and the aggregate seller stake is <Math>{"\\sum_{i=1}^{n} 2iP = P\\,n(n+1)"}</Math>, growing with the square of chain depth, alongside the buyer&rsquo;s aggregate <Math>{"2nP"}</Math>, which grows linearly. The total the process posts is dominated by the seller side and rises with depth. This aggregate is not a hidden cost eroding the proposition&rsquo;s margin; it is the coordination instrument&rsquo;s explicit stake &mdash; the visible, on-the-table price of coordinating an <Math>{"n"}</Math>-party process without a firm, a platform, or a standing legal apparatus. The comparison the proposition draws is unchanged in kind, only completed: the whole bonded stake, both sides counted, is set against the coordination apparatus it replaces &mdash; the platform take-rate, the enforcement expense of the legal route, the standing firm&rsquo;s overhead &mdash; the same margin the proposition already prices, now with nothing left off the table. Where that apparatus levies a recurring extraction on transaction value, the bonded stake is temporarily locked and returned at settlement; the depth-squared growth of the seller side is the honest price of coordinating deeper chains, posted in the open rather than buried in an intermediary&rsquo;s rent. Two properties of the stake bear on the comparison. Its duration is the process&rsquo;s own lifetime, not idle time to apologize for: a maritime leg may hold its bonds four to six weeks, and that span <em>is</em> the coordination being secured. Its depth is bounded by the chain&rsquo;s gas limit, not by any economic ceiling &mdash; the process extends as far as the settlement layer will carry it, and the accounting above holds at every depth.
                </PaperRemark>
            ...
            <PaperSection title="5. Token Denomination as Coordination Signal">
                ...
                <PaperRun title="Schelling-point tokens as a distinct category.">
                    The coordination-signal argument above applies to <em>any</em> token used in a bonded commitment. A second coordination role is played by tokens designed specifically to serve as focal points in the sense of Schelling (1960): anchors around which participants converge to signal alignment with a specific ecosystem, without central authority and without relying on any of the token&rsquo;s financial-mechanism features. BTC and ETH are the precedents for the <em>pattern</em> &mdash; a unit that a population converges on without a central issuer &mdash; and the florin is the focal anchor for participants who coordinate around the bonded-commitment primitive. The precedent is in the convergence, not in the monetary function: BTC and ETH each carry a mechanism that requires the unit, while the florin is required by no protocol operation and no protocol operation denominates in it. A process settles in whatever token its parties chose &mdash; the florin is an ordinary fungible token they may choose like any other, with no preference in the primitive and no penalty for declining it &mdash; and a participant may settle every commitment they ever make without holding one. What it carries instead is a signal none of those choices can carry &mdash; alignment with a specific coordination ecosystem, rather than with a legal system, a community, or a value anchor. Such a token is not <em>backed</em> in the way its value might naively be explained. Bitcoin&rsquo;s issuance carries a marginal-cost floor under proof-of-work and Ethereum&rsquo;s a gas-demand floor; the florin has neither, and the substrate routes no mechanical demand through the unit. Saying the florin is &ldquo;underpinned by trade&rdquo; the way Bitcoin is underpinned by proof-of-work overstates the case: the florin&rsquo;s value is its focality itself &mdash; aligned participants converge on it as the coordination anchor, and that convergence <em>is</em> the value rather than a claim resting on any backing. The ecosystem&rsquo;s trade volume is a public signal a holder may reasonably expect to correlate with continued focal alignment, but the correlation is reflexive &mdash; held together by shared participant belief &mdash; not a cost-floor or fee-floor beneath the unit.
                </PaperRun>
            </PaperSection>
