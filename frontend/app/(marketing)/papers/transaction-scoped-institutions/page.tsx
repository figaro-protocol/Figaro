        ...
        >
            ...
            <PaperSection title="2. The Settlement Primitive">
                ...
                <PaperRun title="Equilibrium properties.">
                    In the resulting one-shot bonding game, cooperation weakly dominates defection for all parties, and the cooperative profile is the unique outcome surviving iterated elimination of weakly dominated strategies &mdash; the refinement that repeatedly discards any strategy that never does better and sometimes does worse. The result extends to <Math>{"N"}</Math>-party processes through <em>cumulative upstream bonding</em>, with the seller&rsquo;s risk-to-reward ratio growing with chain depth. The result holds under perfect monitoring with costless performance: what the bond posture prices is settlement discipline, and an honest dispute over whether performance met the agreement is adjudicated not by the mechanism but by the forums composed above it. The costless-performance clause is a limitation rather than an idealization, and it bears directly on a Coasean argument, since production consumes labor, inputs, and time: at a positive performance cost <Math>{"c"}</Math> the seller compares <Math>{"-2G - c"}</Math> with <Math>{"-2G"}</Math> against a withholding buyer, so the seller-side weak dominance does not survive unchanged and no positive-cost version of the result is established (&sect;3.1 of the derivation cited above states the caveat in full). The organizational-form argument below inherits it: what is compared against the firm is the bonded alternative <em>as proved</em>, at zero performance cost.
                </PaperRun>
