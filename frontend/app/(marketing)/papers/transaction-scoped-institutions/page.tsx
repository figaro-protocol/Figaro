        ...
        >
            ...
            <PaperSection title="2. The Settlement Primitive">
                ...
                <PaperRun title="Equilibrium properties.">
                    In the resulting one-shot bonding game, cooperation weakly dominates defection for all parties, and the cooperative profile is the unique outcome surviving iterated elimination of weakly dominated strategies &mdash; the refinement that repeatedly discards any strategy that never does better and sometimes does worse. The result extends to <Math>{"N"}</Math>-party processes through <em>cumulative upstream bonding</em>, with the seller&rsquo;s risk-to-reward ratio growing with chain depth. The result holds under perfect monitoring with costless performance: what the bond posture prices is settlement discipline, and an honest dispute over whether performance met the agreement is adjudicated not by the mechanism but by the forums composed above it. The costless-performance clause is a limitation rather than an idealization, and it bears directly on a Coasean argument, since production consumes labor, inputs, and time: at a positive performance cost <Math>{"c"}</Math> the seller compares <Math>{"-2G - c"}</Math> with <Math>{"-2G"}</Math> against a withholding buyer, so the seller-side weak dominance does not survive unchanged, though the cooperative profile&rsquo;s survival does in the two-party game: for a cost below the payment plus the seller&rsquo;s bond (<Math>{"c < P + 2G"}</Math>) it remains the unique outcome of iterated elimination, established there in two rounds rather than in one (&sect;3.1 of the derivation cited above states the caveat and its &sect;4.1 the positive-cost proposition). The two-round repair is a two-party result only: the <Math>{"N"}</Math>-party extension invoked above is stated, and claimed, at <Math>{"c = 0"}</Math>, and the repair does not itself extend to the <Math>{"N"}</Math>-party game. The organizational-form argument below inherits both: what is compared against the firm is the bonded alternative <em>as proved</em>, with dominance strength claimed only at zero performance cost.
                </PaperRun>
            ...
            <PaperSection title="3. The Coasean Threshold Shift">
                ...
                <div className="border-l-2 border-default pl-6 my-2 space-y-3">
                    ...
                    <p>
                        For an <Math>{"n"}</Math>-party process, the full bonded stake &mdash; both sides of the table, not the buyer&rsquo;s lockup alone &mdash; is the root buyer&rsquo;s <Math>{"2P"}</Math> per order together with the sellers&rsquo; aggregate <Math>{"\\sum_{i=1}^{n} 2G_i"}</Math>, where seller <Math>{"i"}</Math> posts <Math>{"2G_i"}</Math> against cumulative upstream value <Math>{"G_i = \\sum_{j \\leq i} P_j"}</Math>; for the illustrative linear chain of equal payments <Math>{"P"}</Math> this is <Math>{"2iP"}</Math>, so the seller aggregate is <Math>{"P\\,n(n+1)"}</Math> &mdash; growing with the square of chain depth, against the buyer&rsquo;s aggregate <Math>{"2nP"}</Math>, which grows linearly, so the total is dominated by the seller side and rises with depth. Where the opportunity cost of that full stake &mdash; approximately <Math>{"\\left[2nP + P\\,n(n+1)\\right] \\cdot rT = P\\,n(n+3) \\cdot rT"}</Math> over the process&rsquo;s bond duration <Math>{"T"}</Math> at rate <Math>{"r"}</Math> &mdash; is less than the firm&rsquo;s coordination overhead for the same transaction, the rational organizational choice is market exchange via the primitive rather than hierarchical integration.
                    </p>
                </div>
                <p>
                    Proposition 3.1 is a comparative-statics statement, not a sweeping institutional prediction. It identifies a class of transactions for which the make-or-buy boundary tilts toward <em>buy</em>. The class is non-empty (most retail-scale transactions in mature financial environments satisfy it) and bounded (very-high-frequency, very-low-margin, or very-long-duration transactions do not). The proposition does not claim that all transactions migrate; it claims that the migration threshold is now defined by the full stake&rsquo;s opportunity cost rather than by the cost of standing institutional infrastructure. The stake is not a hidden cost eroding that margin but its explicit price: temporarily locked and returned at settlement, not extracted as recurring rent, and posted in the open for a duration that is the process&rsquo;s own lifetime &mdash; a maritime leg may hold its bonds four to six weeks, and that span <em>is</em> the coordination being secured &mdash; at a depth bounded only by the chain&rsquo;s gas limit, not by any economic ceiling.
                </p>
            ...
            <PaperSection title="5. Conclusion">
                <p>
