    ...
    return (
        ...
        >
            <PaperSection title="1. Introduction">
                ...
                <p>
                    This paper asks what happens to that boundary when one of the underlying costs &mdash; contract enforcement between mutually distrusting parties &mdash; collapses to a fixed, jurisdiction-displaced capital lockup. The collapse is not a thought experiment: the mechanism admits formal proofs of cooperation as the unique surviving Nash equilibrium, and the Solidity implementation has been verified through TLA⁺ model checking, property-based fuzzing, symbolic execution, and specification checking.
                </p>
                <p>
                ...
                <PaperRun title="Paper organization.">
                    Section 2 summarizes the settlement primitive, taking the formal development and implementation as given. Section 3 develops the Coasean threshold shift. Section 4 treats the firm as one organizational pattern among many, introducing subordination as the variable whose cost advantage the primitive erases, and developing the three concrete implications for property-rights theory and for the unit of coordination. Section 5 examines token denomination as a coordination signal that supplants standing platform credentialing. Section 6 concludes.
                </PaperRun>
            </PaperSection>

            <PaperSection title="2. The Settlement Primitive">
                <p>
                    The settlement primitive composes two distinct mechanisms that together do the institutional work this paper draws on. <em>Asymmetric bonding</em> produces the bilateral Nash equilibrium and scales it to <Math>{"N"}</Math>-party processes through progressive collateralization: each party locks capital against the transaction&rsquo;s cumulative value, and cooperation is the unique profile surviving iterated elimination of weakly dominated strategies at every chain position. <em>Buyer dominance with atomic resolution</em> operates on the already-scaled mesh: only the buyer can trigger resolution, and resolution settles all orders in a process simultaneously or not at all. The two mechanisms are inseparable: bonding alone yields independently secured bilateral edges that cannot multi-party coordinate without external choreography; buyer dominance alone is without force absent the bonding equilibrium. Together they make the mesh resolvable from a single signature with cooperation pressure propagating through it. We use &ldquo;Mechanism 1&rdquo; and &ldquo;Mechanism 2&rdquo; below where the distinction is load-bearing.
                </p>
                <p>
                    We summarize here only the properties of the primitive that the present argument uses. The full definitions, theorems, implementation, and verification belong to their own registers and are taken as given here.
                </p>
                ...
                <PaperRun title="Equilibrium properties.">
                    In the resulting one-shot bonding game, cooperation weakly dominates defection for all parties, and the cooperative profile is the unique outcome surviving iterated elimination of weakly dominated strategies. The result extends to <Math>{"N"}</Math>-party processes through <em>progressive collateralization</em>, with the seller&rsquo;s risk-to-reward ratio growing with chain depth.
                </PaperRun>
                ...
                <PaperRun title="No escape hatches.">
                    The primitive has no timeout, no admin override, no governance mechanism, no protocol fee, and no upgrade path. The mechanism-design analysis proves that any such escape hatch weakens the equilibrium.
                </PaperRun>
            ...
            <PaperSection title="3. The Coasean Threshold Shift">
                ...
                <PaperRun title="Negotiation costs.">
                    Off-chain commitment building (EIP-712 typed structured data) lets parties iterate on terms without gas costs. The negotiation is bilateral and direct; counterparties can discover and iterate through shared graph visibility rather than through a standing intermediary&rsquo;s matching function. The substantive change here is asymmetric: the <em>cost</em> of negotiating is roughly unchanged from web2 baselines, but the <em>counterparty space</em> from which one can negotiate is no longer mediated by a platform&rsquo;s terms of service.
                </PaperRun>
                <PaperRun title="Enforcement costs.">
                    This is the decisive variable. In the pre-primitive world, enforcement requires courts (slow, expensive, jurisdiction-dependent), arbitrators (trusted third parties), or standing intermediaries that internalize coordination and charge for it. The primitive prices enforcement at exactly <Math>{"2\\times"}</Math> the transaction value &mdash; a fixed, predictable, jurisdiction-displaced cost requiring no third party.
                </PaperRun>
                <PaperRemark title="On the enforcement-cost accounting.">
                    The <Math>{"2P"}</Math> figure is the capital-lockup component, which is the substantive change relative to the prior regime. A full enforcement-cost accounting adds (i) gas for the <code>commit</code> and <code>resolveProcess</code> calls (bounded but non-zero), (ii) opportunity cost on locked capital over the process duration <Math>{"T"}</Math> at the prevailing risk-free rate <Math>{"r"}</Math> (roughly <Math>{"2P \\cdot rT"}</Math>), (iii) slippage if bond-denomination currency must be acquired at transaction time, and (iv) operational-security costs associated with key custody (signing infrastructure, backup, rotation). The Coasean-threshold comparison below abstracts (i)&ndash;(iv) because they are small relative to the standing-firm overheads against which the comparison is made for the retail-scale transactions this paper treats; the abstraction is not universally valid and should be revisited for applications in which any of (i)&ndash;(iv) is comparable in magnitude to <Math>{"2P"}</Math>.
                </PaperRemark>
                ...
                <PaperRun title="Holmstrom and Roberts&rsquo; caution.">
                    Holmstrom &amp; Roberts (1998) argue that asset-specificity-based hold-up is not by itself sufficient to explain firm boundaries: firms also do work that markets do not, particularly in promoting cooperation and coordinating decisions that no single contract can fully specify. The caution applies to our argument. Mechanism&nbsp;1 prices the enforcement cost; that addresses one component of the firm&rsquo;s bundle, not all of it. The cooperation-promotion work that Holmstrom and Roberts attribute to firms is precisely what Mechanism&nbsp;2 supplies through the weakest-link subgame: each seller&rsquo;s payout depends on universal cooperation, so the cooperation-promotion function is discharged by the bonding mesh&rsquo;s structure rather than by a manager. The two-mechanism composition addresses both components of the firm&rsquo;s bundle that the Holmstrom&ndash;Roberts critique identifies. The non-coordination components &mdash; capital aggregation, risk pooling, organizational knowledge &mdash; remain durable requirements, and we return to them in Section 4.
                </PaperRun>
            </PaperSection>
            ...
            <PaperSection title="4. The Firm as One Pattern Among Many">
                ...
                <PaperSubsection title="4.3 Three Concrete Implications">
                    <PaperRun title="(a) The wallet is the unit of coordination.">
                        Each productive asset is represented directly by a wallet address. At the protocol tier the wallet can hold content-addressed credentials &mdash; attested capabilities, operator profiles, and (at the runtime tier) NFT-anchored deeds &mdash; that certify the asset&rsquo;s identity and performance history. In the firm model, the cook&rsquo;s skill, the kitchen&rsquo;s equipment, the courier&rsquo;s vehicle are subsumed under the firm&rsquo;s legal identity and cease to be individually addressable. In the wallet model, each asset is its own coordination endpoint. The productive asset <em>is</em> the identity; it remains an identity whether the firm persists or not.
                    </PaperRun>
                    <PaperRemark title="On the kernel / protocol / runtime distinction.">
                        We use the three-tier distinction throughout: the <em>kernel</em> is the irreducible settlement primitive, guaranteeing only the <code>commit</code>/<code>resolveProcess</code> bond structure; the <em>protocol</em> adds the extension doctrine, where the credential and attestation apparatus (attestation coordinators, schema registries) is realized; the <em>runtime</em> adds the semantic and builder surfaces above it.
                    </PaperRemark>
                ...
                <PaperSubsection title="4.4 Engagement with the Property-Rights Tradition">
                    ...
                    <PaperRun title="The firm&rsquo;s self-knowledge function dissolves alongside.">
                        A function we have not separately named is that the firm historically has been the unit at which <em>books</em> are kept &mdash; the unit whose position is tracked over time through double-entry bookkeeping in the tradition of Pacioli (1494). The bookkeeping function is not incidental: at scale, an entity that cannot maintain accurate records of its position cannot grow, attract investment, pay taxes, or contract with third parties who require disclosure. A Figaro process is a self-closing ledger period &mdash; and the apparatus that grew up around the books (bookkeeping, accounting, audit) becomes optional for activity that settles through the protocol. The implication for the present argument is that the firm&rsquo;s demotion from privileged organizational container is supported by an additional structural shift: the records function that motivated the firm boundary historically is also no longer firm-bound. A wallet cohort participating in many processes has its full position visible without consolidation.
                    </PaperRun>
                </PaperSubsection>
            </PaperSection>
            ...
            <PaperSection title="6. Conclusion">
                ...
                <p>
                    The argument leaves three questions open. First, the empirical question: which industries and which transaction classes lie above the threshold today, and at what rate does the threshold shift as on-chain settlement matures? Second, the institutional question: which organizational patterns &mdash; firms, treasuries, peer networks, or forms not yet named &mdash; dominate in a given industry once the primitive is available, and what determines the dominance? The non-coordination functions of the firm (capital aggregation, organizational knowledge, risk pooling, legal personhood) must still be bundled somewhere; the bundling question is open. Third, a question of a different character: why this shift is qualitatively harder to recognize than to specify, and what that difficulty itself reveals about the architecture of contemporary coordination infrastructure. That third question belongs to the political-economy register.
                </p>
                <PaperRun title="A note on scope.">
                    This paper bounds its claim by two scope conditions: the Coasean comparison (Proposition 3.1: <Math>{"2P"}</Math> vs. coordination overhead) and asset specificity (Section 4). Both must hold for a transaction to lie within the paper&rsquo;s argumentative domain; readers constructing arguments about transactions that satisfy one condition but not the other should treat the argument with corresponding partial force. Adjacent questions &mdash; hegemonic legibility, labor-law specificity, evidentiary access &mdash; bound their claims by different conditions; the present paper does not adjudicate among those bounds.
                </PaperRun>
            </PaperSection>
        </PaperLayout>
    );
