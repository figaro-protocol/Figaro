        ...
        >
            ...
            <PaperSection title="2. The Settlement Primitive">
                ...
                <PaperRun title="Asymmetric bonding (Mechanism 1).">
                    For a transaction with payment <Math>{"P > 0"}</Math> and cumulative upstream value <Math>{"G \\geq P"}</Math>, the buyer locks <Math>{"2P"}</Math> and the seller locks <Math>{"2G"}</Math>. Cooperation is the unique profile surviving iterated elimination of weakly dominated strategies (derived in <Link href="/papers/asymmetric-bonding" className="text-ink-heading hover:underline">Asymmetric Bonding and Buyer Dominance</Link>, &sect;4.1, and extended to <Math>{"N"}</Math> parties in &sect;5.1). The mechanism scales to <Math>{"N"}</Math>-party process chains (each seller bonds against cumulative upstream value); the organizational topology over which orders compose is an upper-layer concern. The equilibrium holds under perfect monitoring with costless performance; what the bond posture prices is settlement discipline, not the adjudication of disputed performance, which remains with the forums that compose above the kernel. That second assumption is worth stating in the agent register rather than inheriting silently: an agent&rsquo;s performance consumes inference compute, tool calls, and paid API access, and at a positive performance cost <Math>{"c"}</Math> the performing party facing a withholding buyer compares <Math>{"-2G - c"}</Math> with <Math>{"-2G"}</Math>, so the seller-side weak dominance does not survive unchanged &mdash; the derivation cited above marks that as an exactness caveat in its &sect;3.1. What it does establish at positive cost, in its &sect;4.1, is survival rather than dominance in the two-party game: for a cost below the payment plus the seller&rsquo;s bond (<Math>{"c < P + 2G"}</Math>) the cooperative profile remains the unique survivor of iterated elimination, reached in two rounds instead of by a single seller-side comparison, and a seller assigning probability <Math>{"p"}</Math> to the buyer resolving finds cooperation a best response iff <Math>{"p \\geq c/(P + 2G)"}</Math>. That two-round repair does not extend to the <Math>{"N"}</Math>-party process chains invoked above: the <Math>{"N"}</Math>-party equilibrium this paper extends to agents is claimed at <Math>{"c = 0"}</Math>. The actor-neutrality result below transfers the result, the bound, and the caveat to agents on exactly the same terms.
                </PaperRun>
