    ...
    return (
        ...
        >
            ...
            <PaperSection title="4. The Agent Loop">
                ...
                <p>
                    The reference participation agent supplies two policy primitives: a human-in-the-loop primitive that defers every action to owner approval, and an autonomous primitive whose per-action decision is delegated to a caller-supplied predicate over the action and its context. Role-shaped reference policies &mdash; a human-in-the-loop owner policy, a buyer-with-budget policy, a seller-of-record policy, and an auditor policy &mdash; each compose these primitives with role-specific parameters. Operational deployments either fork one of these or compose the primitives directly; we treat typical roles in Section 5.
                </p>
                ...
                <PaperRun title="The full loop.">
                    An operator &mdash; the reference participation agent &mdash; wires the four phases together with a polling interval, loading the wallet from configuration, selecting the policy from configuration (HITL by default), and running the loop until shutdown. The operator is not a production system; it is a fork-and-modify reference that any operational deployment specializes into its own runtime. Other runtimes are expected; the protocol is language-agnostic, and the only requirements are a wallet, EIP-712 signing, and event-derived state.
                </PaperRun>
            </PaperSection>

            <PaperSection title="5. Policies as Controllers">
                <p>
                    Read in control-theory terms, the policy is the wallet&rsquo;s control law. The space of admissible policies is wide; the architecture constrains the policy interface but not the policy logic. We develop reference policies for typical operational roles. The roles are not agent-specific; the same policies apply when the wallet is held by a human owner approving each action through a UI, by an autonomous agent running its own decision logic, or by a hybrid system in which an autonomous proposer routes high-stakes actions to a human reviewer.
                </p>
                <PaperRun title="Human owner with HITL approval.">
                    The simplest policy under the architecture is the one a human owner running the agent loop from a web UI follows. Every proposed action is presented to the owner through an action queue; the owner approves or rejects each one, and the loop executes only the approved entries. The human-in-the-loop primitive realizes exactly this policy: every entry is queued for the owner&rsquo;s review; nothing is auto-approved. A wallet driven only by this primitive is operationally indistinguishable from a wallet driven by a person clicking &ldquo;confirm&rdquo; on every transaction. The kernel sees the same EIP-712 signatures and bond posting either way. This is the trivial case actor-neutrality permits and the case many production deployments will run for high-value processes.
                </PaperRun>
                ...
                <PaperRun title="Seller-of-record.">
                    A fan-out actor (an airline, an ocean carrier, a marketplace) acts as seller to the buyer and as buyer at every sub-procurement. By volume this is typically the busiest operator in any non-trivial assembly. A split policy is the right shape: autonomous for routine attestations and small sub-procurements, HITL for resolutions and exceptional sub-procurements.
                </PaperRun>
                <p>
                    A seller-of-record policy approves attestations (as seller or as buyer) autonomously, and approves sub-order commits whose cumulative value falls under a configured routine-procurement limit; everything else is rejected with a reason and escalates to the owner&rsquo;s HITL surface. An operational deployment configures the routine-limit threshold to match its operational reserves.
                </p>
            ...
            <PaperSection title="6. LLM-Augmented Policies">
                ...
                <PaperRun title="Prompt injection as a defection vector.">
                    LLM controllers consume context that may include adversarial input: attestation content surfaced through the proposer, off-chain agreement text, third-party-supplied operational notes (Greshake et al., 2023). A poisoned context can steer the LLM into approving actions the owner did not authorize. The defense is structural: the deterministic policy layer applies hard limits (per-commit cap, total-budget cap, allow-list of counterparties, clause-typed action validation) that the LLM&rsquo;s verdict cannot override. The bond posture amplifies the defense: even if the LLM is steered into approving a bonded commitment the owner would not have approved, the loss is capped at the bond that commitment entails, not at the owner&rsquo;s full treasury. The combination of hard policy limits plus per-action bond capping makes the LLM&rsquo;s worst case operationally analyzable rather than catastrophic.
                </PaperRun>
                ...
                <p>
                    Two distinct bounds therefore matter for LLM-augmented operation: the per-action bound (kernel-level, automatic, equal to the bond posted at that action) and the aggregate bound (policy-level, owner-imposed, the cap on cumulative bonded exposure across active processes). Both are required for autonomous deployment at scale, and their separation is a structural advantage relative to LLM agents operating outside the bonded primitive: an LLM that can autonomously execute arbitrary transactions on its owner&rsquo;s behalf has loss exposure bounded only by the owner&rsquo;s wallet balance and the owner&rsquo;s willingness to refill it, whereas an LLM operating through an operator has the kernel-level per-action bound for free plus a cleanly defined site (the policy&rsquo;s aggregate-cap field) at which the owner imposes treasury-level bounds. The bonded architecture makes LLM-policy risk decomposable into analyzable components rather than collapsed into a single unbounded surface; it does not eliminate the need for owner-imposed limits.
                </p>
            </PaperSection>
