    ...
    return (
        ...
        >
            ...
            <PaperSection title="4. The Agent Loop">
                ...
                <p>
                    The factotum reference implementation supplies the policy primitives at <code>agents/factotum/src/policy.ts</code> &mdash; <code>makeHitlPolicy()</code>, which returns a human-in-the-loop policy that defers every action to operator approval, and <code>{"makeAutonomousPolicy(shouldExecute)"}</code>, which returns an autonomous policy whose per-action decision is delegated to a caller-supplied <code>{"(action, ctx) => {execute, reason?}"}</code> callback. Role-shaped reference policies ship as separate factories at <code>agents/factotum/src/policies/</code>: <code>auditorPolicy</code>, <code>basicMerchantPolicy</code>, <code>buyerWithBudgetPolicy</code>, <code>courierBidderPolicy</code>, and <code>sellerOfRecordPolicy</code>, each composing the primitives above with role-specific parameters. Operational deployments either fork one of these or compose the primitives directly; we treat typical roles in Section 5.
                </p>
            ...
            <PaperSection title="5. Policies as Controllers">
                ...
                <PaperRun title="Human operator with HITL approval.">
                    The simplest policy under the architecture is the one a human operator running the SDK from a web UI follows. Every proposed action is presented to the operator through an <code>ActionQueue</code>; the operator approves or rejects each one, and the SDK executes only the approved entries. The shipping <code>makeHitlPolicy()</code> factory at <code>agents/factotum/src/policy.ts</code> returns exactly this policy: every entry is queued for operator review; nothing is auto-approved. A wallet driven only by <code>makeHitlPolicy()</code> is operationally indistinguishable from a wallet driven by a person clicking &ldquo;confirm&rdquo; on every transaction. The kernel sees the same EIP-712 signatures and bond posting either way. This is the trivial case actor-neutrality permits and the case many production deployments will run for high-value processes.
                </PaperRun>
            ...
            <PaperSection title="6. LLM-Augmented Policies">
                ...
                <PaperRun title="Prompt injection as a defection vector.">
                    LLM controllers consume context that may include adversarial input: attestation content surfaced through the proposer, off-chain agreement text, third-party-supplied operational notes (Greshake et al., 2023). A poisoned context can steer the LLM into approving actions the operator did not authorize. The defense is structural: the deterministic policy layer applies hard limits (per-commit cap, total-budget cap, allow-list of counterparties, schema-typed action validation) that the LLM&rsquo;s verdict cannot override. The bond posture amplifies the defense: even if the LLM is steered into approving a bonded commitment the operator would not have approved, the loss is capped at the bond that commitment entails, not at the operator&rsquo;s full treasury. The combination of hard policy limits plus per-action bond capping makes the LLM&rsquo;s worst case operationally analyzable rather than catastrophic.
                </PaperRun>
                ...
                <p>
                    This is a structural advantage relative to LLM agents operating outside the bonded primitive. An LLM that can autonomously execute arbitrary transactions on its operator&rsquo;s behalf has loss exposure bounded only by the operator&rsquo;s wallet balance and the operator&rsquo;s willingness to refill it. An LLM operating through a factotum on the bonded primitive has the kernel-level per-action bound for free, plus a cleanly defined site (the policy&rsquo;s aggregate-cap field) at which the operator imposes treasury-level bounds. The bonded architecture makes LLM-policy risk decomposable into analyzable components rather than collapsed into a single unbounded surface; it does not eliminate the need for operator-imposed limits.
                </p>
            </PaperSection>
            ...
            <PaperSection title="7. Discoverability">
                ...
                <p>
                    The Identity Registry is the layer that does not collapse into the bond. Cross-organization identifiers, capability descriptors, and service endpoints (MCP, A2A) live above the bond and serve a different function: helping a wallet <em>find</em> a counterparty that has not yet been bonded with. The architecture treats this as an off-chain concern. The <code>OperatorRegistry</code> contract carries a <code>metadataURI</code> per registered operator, pointing at an off-chain JSON document; the convention developed here aligns the document with ERC-8004 identity claims and the W3C <code>did:web</code> method (W3C, 2022):
                </p>
                <CodeBlock>{CODE_METADATA}</CodeBlock>
                <p>
                    The SDK provides <code>resolveDidWeb()</code>, <code>didDocumentMatchesAddress()</code>, and <code>buildOperatorDidDocument()</code> helpers in <code>@figaro/core/extensions</code>. The metadata document is the extension point at which ERC-8004 identity descriptors can be adopted by any deployment that wants them; no kernel-layer contract change is required.
                </p>
            ...
            <PaperSection title="8. Scope and Security Posture">
                ...
                <PaperRun title="Defense-in-depth in the policy.">
                    The proposer is correct by construction; the policy should still defend against unexpected proposer behavior as a defense-in-depth layer. Re-derive bond amounts from process state in the policy. Re-check counterparty addresses against expected sets. Bound gas in the execute layer. The policy is the operator&rsquo;s last line of defense against any architectural assumption that fails in operational context.
                </PaperRun>
