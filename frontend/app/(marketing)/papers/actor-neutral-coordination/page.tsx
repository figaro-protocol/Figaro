        ...
        >
            ...
            <PaperSection title="4. The Agent Loop">
                ...
                <PaperRun title="Propose.">
                    The propose phase is a pure function over process state: given a process and a participant address, it returns the actions admissible to that address under the current process state. The action types are commit-sub-order (extending the process chain), resolve-process (the buyer-dominance settlement), attest-as-seller, and attest-as-buyer. Each proposed action carries a human/LLM-readable description, pre-validated parameters, and bond math (approval amounts, payouts).
                </PaperRun>
                <p>
                ...
                <p>
                    The reference participation agent supplies two policy primitives: a human-in-the-loop primitive that defers every action to seller approval, and an autonomous primitive whose per-action decision is delegated to a caller-supplied predicate over the action and its context. Role-shaped reference policies &mdash; an auditor policy, a basic-seller policy, a buyer-with-budget policy, an auction-bidder policy, and a seller-of-record policy &mdash; each compose these primitives with role-specific parameters. Operational deployments either fork one of these or compose the primitives directly; we treat typical roles in Section 5.
                </p>
                <PaperRun title="Execute.">
                    The execute phase commits an approved action to the chain via the agent&rsquo;s wallet. Two execution surfaces are natural. For self-contained actions whose parameters are fully captured on the proposed action &mdash; resolution being the canonical case &mdash; the agent builds and submits the transaction directly. For actions whose execution requires inputs beyond what the proposer carried (signed commitments, attestation evidence, merkle proofs), the agent assembles those additional inputs at submission time and then submits. A batched-execution path is available for deployments that prefer to consolidate multiple actions into a single sequencer submission rather than direct on-chain calls. Execution on either surface is sequential per agent: actions are committed one at a time, with the agent&rsquo;s nonce advancing per transaction. Parallel execution would race the kernel state and is explicitly avoided.
                </PaperRun>
                <PaperRun title="The full loop.">
                    A factotum &mdash; the reference participation agent &mdash; wires the four phases together with a polling interval, loading the wallet from configuration, selecting the policy from configuration (HITL by default), and running the loop until shutdown. The factotum is not a production system; it is a fork-and-modify reference that any operational deployment specializes into its own runtime. Other runtimes are expected; the protocol is language-agnostic, and the only requirements are a wallet, EIP-712 signing, and event-derived state.
                </PaperRun>
            </PaperSection>
            ...
            <PaperSection title="5. Policies as Controllers">
                ...
                <p>
                    A seller-of-record policy approves attestation actions &mdash; attest-as-seller and attest-as-buyer &mdash; autonomously, and approves commit-sub-order actions whose cumulative value falls under a configured routine-procurement limit; everything else is rejected with a reason and escalates to the seller&rsquo;s HITL surface. An operational deployment configures the routine-limit threshold to match its operational reserves.
                </p>
                <PaperRun title="Dutch-auction bidder.">
                    A bidder on a Dutch auction &mdash; an on-network auction mechanism the kernel composes with &mdash; monitors a descending price curve and claims the position when the price meets a profitability threshold. A subtlety deserves naming: the proposer emits commit-sub-order proposals only for the address that already holds the buyer-side role in a process chain, not for prospective sellers competing on a Dutch auction. A factotum acting as a prospective bidder therefore does not receive a commit-sub-order proposal directly; the bidder&rsquo;s work-discovery happens through the auction mechanism&rsquo;s own auction-state events, surfaced off-chain through auction helpers that compute the current price and evaluate whether a claim clears the bidder&rsquo;s threshold. The policy acts at the moment the bidder is preparing a claim transaction rather than at the moment a kernel-level action is proposed. The bidder&rsquo;s logic is expressed as a pre-claim gate rather than as a policy in the strict loop sense: it computes the margin between the auction&rsquo;s current price and the bidder&rsquo;s own estimated cost, and claims only when that margin clears a configured minimum.
                </PaperRun>
                <p>
                    The control-theory reading: the policy implements a threshold-rule controller on the price-vs-cost gap, with the threshold parameterized by minimum acceptable margin. The agent does not optimize across competing auctions; it claims the first auction that crosses its threshold, leaving cross-auction optimization to a higher-layer policy if the deployment wants one. The bidder gate runs adjacent to the factotum loop rather than inside it, reflecting the architectural split: kernel-level actions live in the proposer; auction monitoring and evaluation live in the extension surface; the bidder composes both at the deployment layer.
                </p>
