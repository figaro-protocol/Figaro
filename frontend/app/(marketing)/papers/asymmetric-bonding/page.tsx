            ...
            <PaperSection title="3. The Two Mechanisms: Formal Definition">
                <PaperSubsection title="3.1 Notation and Setup">
                    ...
                    <FormalBlock label="Standing Assumptions.">
                        <p>Three things are maintained throughout, and they are the only inputs to the payoffs below. First, <em>transfers only</em>: every payoff in this paper is a sum of the deposits the mechanism pulls at commit and the payouts it makes at resolution &mdash; the bonds <Math>{"2P"}</Math> and <Math>{"2G"}</Math>, and the payment <Math>{"P"}</Math> &mdash; and of nothing else. No production cost, valuation, or outside option appears in any payoff; what a party gains or loses inside the mechanism is exhausted by the transfers defined in &sect;3.2. Second, the <em>standoff structure</em>: resolution is terminal and withholding is not. A resolved order cannot be re-opened, whereas an unresolved one stays live indefinitely, so the seller&rsquo;s option to perform survives any amount of withholding &mdash; and only the buyer may resolve. Third, <em>resolution as acceptance</em>: the buyer&rsquo;s signature on the commitment was given for this delivery, so between two continuations that cost the buyer the same in transfers, the buyer prefers the one in which the delivery occurs. This is an ordinal tie-break carrying no magnitude, and it is used at exactly one kind of decision node &mdash; the buyer&rsquo;s node after non-delivery &mdash; in &sect;4.1, and again by reference at the corresponding <Math>{"N"}</Math>-party node in &sect;5.1.</p>
                        <p>A fourth assumption is substantive rather than structural. <em>Perfect monitoring</em>: the buyer observes whether delivery occurred. This is a genuine restriction, not a bookkeeping convenience &mdash; where performance is honestly disputed, the mechanism decides nothing, and the disagreement is adjudicated by forums composing above it on the evidence record (&sect;6).</p>
                        <p>Nothing above says why the two parties are at the table, and that is a question about the market rather than about the mechanism. It has the ordinary answer: a buyer commits because what it expects to receive is worth at least what it pays, and a seller quotes a payment that covers what it takes to operate. Each participant is a productive asset &mdash; the wallet belongs to the asset, and whether a person or a program operates it is immaterial &mdash; and the asset stays productive for as long as it holds balances it can bond, payments received being its revenue and payments made its costs. The mechanism observes neither: it sees whether a party has the tokens to bond, and what that party does once bonded. These conditions hold in every market and explain that a market exists; they are not inputs to any theorem below, and no result in this paper is conditioned on them.</p>
                    </FormalBlock>
            ...
            <PaperSection title="4. Game-Theoretic Analysis">
                <PaperSubsection title="4.1 Two-Party Nash Equilibrium">
                    ...
                    <PaperRemark title="On the credibility of non-resolution.">
                        The payoffs treat any seller defection as preventing resolution &mdash; the buyer withholds. This is a claim about the buyer&rsquo;s rationality, not a mechanism rule: nothing stops a buyer from resolving an undelivered order, which returns <Math>{"P"}</Math> to the buyer and pays the non-performing seller <Math>{"+P"}</Math>. That the buyer does not is a best response, and the argument runs on the transfers of &sect;3.2 and the standoff structure of &sect;3.1, nothing else. Take the decision nodes in turn. <em>At the seller&rsquo;s node</em>, the comparison is <Math>{"+P"}</Math> against <Math>{"-2G"}</Math>: performing and being resolved returns the bond and earns the payment, while holding out leaves <Math>{"2G"}</Math> locked. <em>At the buyer&rsquo;s node after delivery</em>, resolving is <Math>{"-P"}</Math> and withholding is <Math>{"-2P"}</Math>, so the buyer resolves. <em>At the buyer&rsquo;s node after non-delivery</em>, resolving now is terminal at <Math>{"-P"}</Math> and leaves the buyer without the delivery; withholding is not terminal, and in the standoff that follows the seller&rsquo;s own node ranks performing above holding out, after which the buyer resolves at the same <Math>{"-P"}</Math> and has the delivery. The two continuations are equal in transfers, and the tie is ordered by the assent already given: the buyer signed this commitment for this delivery, and between two paths costing it the same it takes the one on which the delivery arrives. So the buyer withholds against non-delivery, and the <Math>{"-2P"}</Math> off-path payoff is what the buyer optimally realizes while the standoff runs, not an assumed constant. The standoff terminates, and it terminates on the asymmetry the bonds are built from: while it runs the buyer has <Math>{"2P"}</Math> locked and the seller <Math>{"2G"}</Math>, with <Math>{"G \\geq P"}</Math> by construction &mdash; so the seller bleeds at least as much as the buyer, strictly more at every position past the root, and the seller alone holds the cheap way out, which is to perform. Checking each node against its one available deviation therefore yields <em>(perform; resolve iff delivered)</em>, the same profile the theorem above obtains by iterated elimination. The threat is credible on the mechanism&rsquo;s own transfers and needs no appeal to spite or fairness, consistent with the risk-preference robustness below.
                    </PaperRemark>
                </PaperSubsection>
