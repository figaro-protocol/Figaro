    ...
    return (
        ...
        >
            <PaperSection title="1. Introduction">
                ...
                <PaperSubsection title="1.2 Our Contribution">
                    ...
                    <ol className="space-y-2 list-decimal pl-6 text-sm">
                        ...
                        <li><strong>A reduction over joint-liability microfinance.</strong> We prove the <em>cooperation-pressure component</em> of the peer-enforcement equilibrium common to Ghatak (1999) and Stiglitz (1990) is obtained by the bonded commitment mechanism under strictly weaker assumptions: no repeated interaction, no local information, no exogenous punishment technology, and no joint-liability contracting. Peer selection and peer monitoring are not reproduced.</li>
                    </ol>
                </PaperSubsection>
            </PaperSection>
            ...
            <PaperSection title="2. Related Work">
                <p>
                    Two consequences of locating the whole of the present argument at the mechanism level bear on how the comparisons below should be read. First, the mechanism admits no commitments about <em>what</em> is being coordinated or <em>under what normative regime</em>: currency, jurisdiction, identity type, arbitration forum, role structure, price-discovery mechanism, contribution metric &mdash; none of these are in the mechanism; all are expressed in the <em>graph</em> that parties compose on top of it, and the mechanism enables each and prefers none. Second, the mechanism&rsquo;s useful compositions are predominantly <em>external</em> rather than internal: it does not include a dispute forum, a carbon-offset market, a prediction market, an insurance pool, a lending facility, a tax-reporting service, an identity provider, a storage layer, or a messaging fabric. Each exists as a working external protocol or institution, and a graph composed on top can name any of them; the mechanism itself need not &mdash; and, by the escape-hatch-weakness theorem, must not &mdash; absorb the bookkeeping that such composition implies. The related work below is accordingly mechanism-level: prior bonding and escrow patterns judged on whether they reproduce the same narrow, composable primitive.
                </p>
                ...
                <PaperRun title="Joint-liability microfinance.">
                    Group lending models &mdash; most prominently the Grameen Bank (Yunus, 1999) and its formalizations by Ghatak (1999) and Stiglitz (1990) &mdash; show that joint liability combined with a social substrate can produce peer selection and peer monitoring. Figaro reproduces the <em>cooperation-pressure component</em> of that peer-enforcement equilibrium under strictly weaker assumptions, with no appeal to repeated interaction or local information; peer selection and peer monitoring it does not reproduce.
                </PaperRun>
            </PaperSection>
            ...
            <PaperSection title="3. The Two Mechanisms: Formal Definition">
                <PaperSubsection title="3.1 Notation and Setup">
                    ...
                    <FormalBlock label="Standing Assumptions.">
                        <p>Two assumptions are maintained throughout and enter every payoff below. First, <em>costless performance</em>: a seller who cooperates incurs no production or delivery cost, so the payoff structure records only the bonded transfers and the payment &mdash; there is no cost term <Math>{"c"}</Math> on either side of the seller&rsquo;s <Math>{"+P"}</Math>-versus-<Math>{"-2G"}</Math> comparison. The assumption is not innocuous; the remark below states exactly what it carries and where the results stop. Second, <em>value covers price</em>: the buyer&rsquo;s valuation <Math>{"V"}</Math> of the seller&rsquo;s performance satisfies <Math>{"V \\geq P"}</Math>, since a buyer who valued the good below its price would not have committed. This inequality is the premise of the non-resolution credibility argument in &sect;4.1, where it is used to show that &ldquo;resolve iff delivered&rdquo; is the buyer&rsquo;s best response.</p>
                    </FormalBlock>
                    <PaperRemark title="Known limitation: the dominance results are exact under costless performance.">
                        Everything proved in &sect;4 and &sect;5 is proved at <Math>{"c = 0"}</Math> and is claimed only there. Let <Math>{"c > 0"}</Math> be the seller&rsquo;s cost of performing, incurred on delivery and sunk whether or not the buyer subsequently resolves. Cooperation then pays <Math>{"P - c"}</Math> against a resolving buyer and <Math>{"-2G - c"}</Math> against a withholding one, while defection pays <Math>{"-2G"}</Math> in both rows, since a seller who never performs never incurs the cost. In the buyer-defection row the seller therefore compares <Math>{"-2G - c"}</Math> with <Math>{"-2G"}</Math> and is strictly worse off cooperating: <Math>{"\\mathrm{C}"}</Math> no longer weakly dominates <Math>{"\\mathrm{D}"}</Math> for the seller at any <Math>{"c > 0"}</Math>, however small. What is lost is not a margin but a direction &mdash; taking <Math>{"c"}</Math> small shrinks the adverse gap without restoring the inequality &mdash; and the seller&rsquo;s choice ceases to be settled by a single comparison, turning instead on its belief about the buyer&rsquo;s resolve decision. The buyer&rsquo;s side is untouched, since resolving costs the buyer nothing beyond the payment already committed; what a positive performance cost removes is specifically the seller-side weak dominance and the one-comparison property that rests on it. What does survive a positive cost is established at the end of &sect;4.1, and only that: for <Math>{"0 < c < P + 2G"}</Math> the cooperative profile is still the unique survivor of iterated elimination of weakly dominated strategies in the two-party game &mdash; by an elimination that now takes two rounds, the buyer&rsquo;s defection falling first and the seller&rsquo;s only in the reduced game &mdash; and a seller assigning probability <Math>{"p"}</Math> to the buyer resolving finds cooperation a best response iff <Math>{"p \\geq c/(P + 2G)"}</Math>. Dominance strength is not recovered by that proposition and is not recovered by any bound on <Math>{"c"}</Math>: at every positive cost the seller&rsquo;s choice turns on the buyer&rsquo;s rationality, or on the seller&rsquo;s belief about the buyer&rsquo;s conduct &mdash; and, in the <Math>{"N"}</Math>-party case, on its belief about its co-sellers&rsquo; conduct as well &mdash; where at <Math>{"c = 0"}</Math> it turned on neither. The <Math>{"N"}</Math>-party results of &sect;5 are stated at <Math>{"c = 0"}</Math>, as their proofs are, and the two-round argument does not extend to them: after the buyer&rsquo;s <Math>{"\\mathrm{D}"}</Math> falls, seller <Math>{"i"}</Math> still faces its co-sellers &mdash; cooperating yields <Math>{"P_i - c_i"}</Math> if all others cooperate and <Math>{"-2G_i - c_i"}</Math> otherwise, against <Math>{"-2G_i"}</Math> for defection &mdash; so neither seller action is weakly dominated in the reduced game, the elimination stalls, and the seller&rsquo;s choice turns on beliefs about co-sellers as well as about the buyer. A reader applying the mechanism to a domain in which performance consumes real resources should therefore read every <em>weak-dominance</em> claim below as exact at <Math>{"c = 0"}</Math>, and read the cooperative profile&rsquo;s survival at positive cost as resting on the two-round argument and the belief threshold instead.
                    </PaperRemark>
                ...
                <PaperSubsection title="3.2 Protocol Operations">
                    ...
                    <PaperRemark title="Conservation Law.">
                        For each order, the total distributed equals total locked: <Math>{"\\pi_{s,i} + \\pi_{b,i} = (2G_i + P_i) + P_i = 2G_i + 2P_i = C_{s,i} + C_{b,i}"}</Math>. The kernel holds zero balance after resolution.
                    </PaperRemark>
            ...
            <PaperSection title="4. Game-Theoretic Analysis">
                <PaperSubsection title="4.1 Two-Party Nash Equilibrium">
                        ...
                        <table className="text-sm border-collapse">
                            <thead>
                                <tr>
                                    ...
                                    <th className="border border-default px-3 py-1.5 font-semibold text-ink-heading"><Math>{"a_S = \\mathrm{C}"}</Math></th>
                                    <th className="border border-default px-3 py-1.5 font-semibold text-ink-heading"><Math>{"a_S = \\mathrm{D}"}</Math></th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <td className="border border-default px-3 py-1.5 font-semibold text-ink-heading"><Math>{"a_B = \\mathrm{C}"}</Math></td>
                                ...
                                <tr>
                                    <td className="border border-default px-3 py-1.5 font-semibold text-ink-heading"><Math>{"a_B = \\mathrm{D}"}</Math></td>
                                    <td className="border border-default px-3 py-1.5"><Math>{"(-2P,\\; -2G)"}</Math></td>
                                    <td className="border border-default px-3 py-1.5"><Math>{"(-2P,\\; -2G)"}</Math></td>
                                </tr>
                            </tbody>
                        </table>
                        <p className="text-xs text-ink-muted mt-2">Payoff matrix for the bonding game <Math>{"\\Gamma"}</Math>, rows the buyer&rsquo;s action and columns the seller&rsquo;s, each cell the pair <Math>{"(u_b, u_s)"}</Math> as defined in &sect;3.3 and above. Parameters: <Math>{"P > 0"}</Math>, <Math>{"G \\geq P > 0"}</Math>. The buyer&rsquo;s <Math>{"\\mathrm{C}"}</Math> is the conditional strategy <em>resolve iff the seller delivered</em>, so the <Math>{"(\\mathrm{C}, \\mathrm{D})"}</Math> cell records the buyer withholding against non-delivery.</p>
                    </div>
                    <p>
                    ...
                    <FormalBlock label="Proposition (Positive Performance Cost: Survival and the Belief Threshold).">
                        <p>Let <Math>{"c > 0"}</Math> be a cost the seller incurs on delivery, sunk whether or not the buyer subsequently resolves, and let <Math>{"\\Gamma_c"}</Math> be the bonding game of the matrix above with the seller&rsquo;s cooperation payoffs reduced by <Math>{"c"}</Math> in both rows &mdash; <Math>{"P - c"}</Math> against a resolving buyer, <Math>{"-2G - c"}</Math> against a withholding one &mdash; the seller&rsquo;s defection payoffs and both of the buyer&rsquo;s rows unchanged. Then:</p>
                        <p>(a) For every <Math>{"c"}</Math> with <Math>{"0 < c < P + 2G"}</Math>, <Math>{"(\\mathrm{C}, \\mathrm{C})"}</Math> is the unique profile surviving iterated elimination of weakly dominated strategies in <Math>{"\\Gamma_c"}</Math>, but the elimination now takes two rounds: the buyer&rsquo;s <Math>{"\\mathrm{D}"}</Math> falls first, and the seller&rsquo;s <Math>{"\\mathrm{D}"}</Math> only afterwards, in the game reduced by that first round. The bound is tight.</p>
                        <p>(b) Cooperation is weakly dominant for the seller at no <Math>{"c > 0"}</Math>, and is not restored anywhere in the range of (a); within that range neither seller action dominates the other.</p>
                        <p>(c) A seller assigning probability <Math>{"p \\in [0,1]"}</Math> to the buyer resolving finds cooperation a best response if and only if <Math>{"p \\geq c/(P + 2G)"}</Math>, and strictly better than defection when the inequality is strict.</p>
                    </FormalBlock>
                    <Proof>
                        <em>Round one (the buyer).</em> The cost <Math>{"c"}</Math> enters no buyer payoff: it is incurred by the seller in performing, and resolving costs the buyer nothing beyond the payment already committed. The buyer&rsquo;s rows are therefore exactly those of the matrix above &mdash; <Math>{"-P"}</Math> against a delivering seller, <Math>{"-2P"}</Math> otherwise &mdash; and the credibility argument of &sect;4.1 survives at this positive cost: post-non-delivery, delivering yields <Math>{"P - c"}</Math> against <Math>{"-2G"}</Math> for withholding-forever, preferred exactly while <Math>{"c < P + 2G"}</Math>, the same bound as (a) &mdash; so <Math>{"u_b(\\mathrm{C}, a_S) \\geq u_b(\\mathrm{D}, a_S)"}</Math> for both <Math>{"a_S"}</Math>, strictly at <Math>{"a_S = \\mathrm{C}"}</Math> since <Math>{"P > 0"}</Math>. Buyer-side weak dominance is thus untouched by the cost, and <Math>{"\\mathrm{D}"}</Math> is eliminated for the buyer in the first round. <em>Round two (the seller).</em> In the reduced game the buyer plays <Math>{"\\mathrm{C}"}</Math>, and the seller compares <Math>{"u_s(\\mathrm{C}) = P - c"}</Math> with <Math>{"u_s(\\mathrm{D}) = -2G"}</Math> &mdash; a seller who never performs never incurs the cost, forfeiting the bond alone. Now <Math>{"P - c > -2G \\Leftrightarrow c < P + 2G"}</Math>, so throughout the stated range <Math>{"\\mathrm{D}"}</Math> is strictly dominated in the reduced game and falls in the second round, leaving <Math>{"(\\mathrm{C}, \\mathrm{C})"}</Math> the unique survivor, which is (a). The bound is tight in both directions: at <Math>{"c = P + 2G"}</Math> the seller is indifferent in the reduced game and both actions survive, and for <Math>{"c > P + 2G"}</Math> the ranking reverses &mdash; <Math>{"\\mathrm{D}"}</Math> then strictly dominates <Math>{"\\mathrm{C}"}</Math> for the seller in <Math>{"\\Gamma_c"}</Math>, being strictly better in the withholding row and now also in the resolving one. <em>Part (b).</em> In the buyer-defection row the seller compares <Math>{"-2G - c"}</Math> with <Math>{"-2G"}</Math> and is strictly worse off cooperating, so <Math>{"\\mathrm{C}"}</Math> fails to weakly dominate <Math>{"\\mathrm{D}"}</Math> at every <Math>{"c > 0"}</Math>, however small; and while <Math>{"c < P + 2G"}</Math>, <Math>{"\\mathrm{D}"}</Math> fails to weakly dominate <Math>{"\\mathrm{C}"}</Math> because <Math>{"\\mathrm{C}"}</Math> is strictly better in the resolving row. Neither seller action dominates the other in <Math>{"\\Gamma_c"}</Math>; the order of elimination is load-bearing, and the seller&rsquo;s conclusion is available only once the buyer&rsquo;s action has been pinned by the buyer&rsquo;s own dominance. Because neither seller action is weakly dominated in <Math>{"\\Gamma_c"}</Math> (part (b)), the buyer&rsquo;s <Math>{"\\mathrm{D}"}</Math> is the only strategy eliminable in round one; the elimination path is therefore unique and the survivor set is independent of order. What (a) delivers is therefore survival of the cooperative profile under the same refinement, not the one-comparison property, which is lost at any positive cost and is not recovered by any bound on it. <em>Part (c).</em> Drop the restriction to a rational buyer and let the seller assign probability <Math>{"p"}</Math> to resolution. Then <Math>{"\\mathbb{E}[u_s(\\mathrm{C})] = p(P - c) + (1-p)(-2G - c) = p(P + 2G) - 2G - c"}</Math>, while <Math>{"\\mathbb{E}[u_s(\\mathrm{D})] = p(-2G) + (1-p)(-2G) = -2G"}</Math>, the cost never being incurred. Cooperation is a best response iff <Math>{"p(P + 2G) - 2G - c \\geq -2G"}</Math>, i.e. iff <Math>{"p(P + 2G) \\geq c"}</Math>, i.e. iff <Math>{"p \\geq c/(P + 2G)"}</Math>, with strict preference under strict inequality. <Math>{"\\square"}</Math>
                    </Proof>
                    <PaperRemark title="What the bond term does to the threshold.">
                        The denominator <Math>{"P + 2G"}</Math> of the threshold is the seller&rsquo;s own cooperation&ndash;defection gap: the payment it stands to earn plus the bond it has already posted. That the bond enters at all is what the mechanism earns here, and it is what keeps the threshold small. At a root order (<Math>{"G = P"}</Math>) the threshold is <Math>{"c/3P"}</Math>, so a seller whose performance costs a tenth of the payment cooperates on a belief of about one in thirty that the buyer will resolve; deeper in a chain the bond grows with cumulative upstream value and the threshold falls with it &mdash; for equal payments at position <Math>{"i"}</Math>, where <Math>{"G_i = iP"}</Math>, it is <Math>{"c/\\big((2i+1)P\\big)"}</Math>. For costs small relative to payment-plus-bonds, a sliver of confidence in the buyer suffices. Under a seller participation constraint symmetric to the buyer&rsquo;s <Math>{"V \\geq P"}</Math> &mdash; a seller signs only if its ex-ante surplus <Math>{"P - c"}</Math> is non-negative &mdash; any signed order has <Math>{"c \\leq P < P + 2G"}</Math>, so the range in part (a) binds vacuously for participating sellers. This is an additional assumption, not a consequence of the model; once bonded, a seller with <Math>{"c > P"}</Math> still prefers to perform, since <Math>{"P - c > -2G"}</Math> throughout the range. None of this restores dominance. The seller at a positive cost is choosing on a belief where at <Math>{"c = 0"}</Math> it was choosing on an inequality, and the mechanism&rsquo;s answer to a positive cost is that the belief required is cheap, not that the belief is unnecessary.
                    </PaperRemark>
                </PaperSubsection>
                <PaperSubsection title="4.2 Sufficiency of the 2× Multiplier">
                    ...
                    <Proof>
                        Part (a) is the Lemma. Part (b): for <Math>{"k > 1"}</Math>, cooperation yields <Math>{"u_b(\\mathrm{C}, \\mathrm{C}) = (k-1)P - kP = -P"}</Math> and defection <Math>{"u_b(\\mathrm{D}, \\mathrm{C}) = -kP"}</Math>; the gap <Math>{"(k-1)P"}</Math> is strictly positive for every <Math>{"k > 1"}</Math> and collapses to zero at <Math>{"k = 1"}</Math>. Seller-side weak dominance holds for any <Math>{"k \\geq 1"}</Math> (the seller&rsquo;s cooperation payoff is <Math>{"+P"}</Math> regardless of <Math>{"k"}</Math>, while defection forfeits <Math>{"kG"}</Math>). Part (c): the smallest <em>integer</em> admitting weak dominance is <Math>{"k = 2"}</Math>; a larger stake at <Math>{"k > 2"}</Math> deters nothing more, since weak dominance is already secured at <Math>{"k = 2"}</Math> and holds identically above it. The integer restriction reflects the discrete denomination of bonded transfers in deployment, not a property of the equilibrium analysis. <Math>{"\\square"}</Math>
                    </Proof>
                </PaperSubsection>
                <PaperSubsection title="4.3 Escape-Hatch Weakness">
                    <FormalBlock label="Theorem (Escape-Hatch Weakness).">
                        <p>Let <Math>{"\\Gamma_E"}</Math> be the bonding game augmented with a unilateral exit action <Math>{"E"}</Math> available to player <Math>{"i"}</Math>, returning fraction <Math>{"\\lambda \\in (0, 1]"}</Math> of player <Math>{"i"}</Math>&rsquo;s bond. Then either (a) <Math>{"\\mathrm{C}"}</Math> no longer weakly dominates <Math>{"\\mathrm{D}"}</Math> for at least one player, or (b) <Math>{"E"}</Math> requires a decision by a third party <Math>{"J \\notin \\{B, S\\}"}</Math> whose incentives are not constrained by the bond structure.</p>
                    </FormalBlock>
                    <Proof>
                        We analyze three exhaustive sub-cases. <em>Case 1 (timeout):</em> the buyer&rsquo;s payoff under <em>Wait</em> against a cooperating seller is <Math>{"u_b(\\text{Wait}, \\mathrm{C}) = -C_b + \\lambda C_b = -2P(1 - \\lambda)"}</Math>. The buyer prefers <Math>{"\\mathrm{C}"}</Math> iff <Math>{"-P > -2P(1-\\lambda) \\Leftrightarrow 2(1-\\lambda) > 1 \\Leftrightarrow \\lambda < \\tfrac{1}{2}"}</Math>. For <Math>{"\\lambda \\geq \\tfrac{1}{2}"}</Math>, Wait yields <Math>{"u_b \\geq -P"}</Math>, so <Math>{"\\mathrm{C}"}</Math> no longer dominates; even for <Math>{"\\lambda < \\tfrac{1}{2}"}</Math> the deterrent weakens from <Math>{"2P"}</Math> to <Math>{"2P(1-\\lambda)"}</Math>. <em>Case 2 (arbitrator override):</em> a third party <Math>{"J"}</Math> can invoke <Math>{"E"}</Math>; <Math>{"J"}</Math> is unbonded (deposits nothing, risks nothing, has no mechanism-constrained incentive), so the resolution path depends on an agent outside the bond structure &mdash; condition (b). <em>Case 3 (governance vote):</em> a special case of Case 2 with <Math>{"J"}</Math> replaced by a set of unbonded voters, inheriting all of Case 2&rsquo;s problems plus coordination failure, plutocratic capture, and rational ignorance. In all cases either <Math>{"\\mathrm{C}"}</Math> ceases to dominate (Case 1) or resolution depends on an unbonded external agent (Cases 2&ndash;3). <Math>{"\\square"}</Math>
                    </Proof>
                    ...
                    <FormalBlock label="Corollary.">
                        <p>No timeout duration <Math>{"T"}</Math> and no recovery fraction <Math>{"\\lambda > 0"}</Math> can be added without weakening the buyer&rsquo;s incentive to cooperate. Full recovery (<Math>{"\\lambda = 1"}</Math>) reduces the game to a zero-cost option, making defection strictly dominant for the buyer.</p>
                    </FormalBlock>
            ...
            <PaperSection title="5. N-Party Process Chains">
                ...
                <PaperSubsection title="5.3 Atomic Resolution and Seller Coordination">
                    ...
                    <FormalBlock label="Proposition (Collusion is Unprofitable Under a Benefit Bound).">
                        <p>Let <Math>{"K \\subseteq \\{S_1, \\ldots, S_n\\}"}</Math> be any non-empty seller coalition and <Math>{"X"}</Math> an external party paying a side transfer for joint defection by <Math>{"K"}</Math>. Under the bounding assumption on <Math>{"B_{\\max}"}</Math> stated in the proof, joint defection is not a Pareto improvement over universal cooperation for <Math>{"K \\cup \\{X\\}"}</Math>: the minimum side transfer at which <Math>{"K"}</Math>&rsquo;s members defect, <Math>{"T_{\\min}(K) = \\sum_{i \\in K}(P_i + 2G_i) \\geq 3\\sum_{i \\in K} P_i"}</Math>, weakly exceeds <Math>{"X"}</Math>&rsquo;s maximum benefit, which by assumption satisfies <Math>{"B_{\\max}(K) \\leq 3\\sum_{i \\in K} P_i"}</Math>, strictly whenever <Math>{"K"}</Math> contains any non-root seller.</p>
                    </FormalBlock>
                    <Proof>
                        For each <Math>{"S_i \\in K"}</Math>, the cooperation&ndash;defection gap is <Math>{"\\Delta_i = P_i + 2G_i"}</Math>, so <Math>{"X"}</Math> must transfer at least <Math>{"\\Delta_i"}</Math>, giving <Math>{"T_{\\min}(K) = \\sum_{i \\in K}(P_i + 2G_i) \\geq 3\\sum_{i \\in K} P_i"}</Math> (equality only when every <Math>{"S_i"}</Math> is a root order). On the briber&rsquo;s side we take as given that <Math>{"X"}</Math>&rsquo;s gross benefit from non-resolution is bounded by the total loss <Math>{"X"}</Math> can impose on <Math>{"B"}</Math> within the process &mdash; the locked buyer bonds <Math>{"\\sum_{i \\in K} 2P_i"}</Math> plus the goods&rsquo; value to <Math>{"B"}</Math> (bounded by willingness to pay <Math>{"\\sum_{i \\in K} P_i"}</Math>) &mdash; that is, <Math>{"B_{\\max}(K) \\leq 3\\sum_{i \\in K} P_i"}</Math>. This is a modelling assumption rather than a derived quantity: it presumes the briber values the disruption at no more than the harm the disruption inflicts inside the process, which fails for a briber whose gain lies elsewhere &mdash; in a competing position, say, whose value is unrelated to <Math>{"B"}</Math>&rsquo;s locked capital. Deriving the bound rather than assuming it would require a two-stage game in which <Math>{"X"}</Math>&rsquo;s outside option is a primitive: <Math>{"X"}</Math> offers transfers in the first stage, <Math>{"K"}</Math>&rsquo;s members play the bonding game in the second, and <Math>{"B_{\\max}"}</Math> emerges from <Math>{"X"}</Math>&rsquo;s own payoff function; we do not construct it, and the proposition is exactly as strong as the assumption. Combining with the coalition&rsquo;s side, <Math>{"T_{\\min}(K) \\geq B_{\\max}(K)"}</Math>, strict whenever <Math>{"K"}</Math> contains a seller with <Math>{"G_i > P_i"}</Math>. <Math>{"\\square"}</Math>
                    </Proof>
                    <PaperRemark title="Margin grows with chain depth.">
                        Under the same bound on <Math>{"B_{\\max}"}</Math>, the dominance margin <Math>{"T_{\\min}(K) - B_{\\max}(K) \\geq 2\\sum_{i \\in K}(G_i - P_i) \\geq 0"}</Math> is monotone in chain depth: deeper colluding sellers face a larger spread between their bonds <Math>{"2G_i"}</Math> and the harm imposable on the buyer. Coalitions of deep-chain sellers are the most strictly unprofitable.
                    </PaperRemark>
                </PaperSubsection>
