            ...
            <PaperSection title="3. The Two Mechanisms: Formal Definition">
                <PaperSubsection title="3.1 Notation and Setup">
                    ...
                    <PaperRemark title="Known limitation: the dominance results are exact under costless performance.">
                        Everything proved in &sect;4 and &sect;5 is proved at <Math>{"c = 0"}</Math> and is claimed only there. Let <Math>{"c > 0"}</Math> be the seller&rsquo;s cost of performing, incurred on delivery and sunk whether or not the buyer subsequently resolves. Cooperation then pays <Math>{"P - c"}</Math> against a resolving buyer and <Math>{"-2G - c"}</Math> against a withholding one, while defection pays <Math>{"-2G"}</Math> in both columns, since a seller who never performs never incurs the cost. In the buyer-defection column the seller therefore compares <Math>{"-2G - c"}</Math> with <Math>{"-2G"}</Math> and is strictly worse off cooperating: <Math>{"\\mathrm{C}"}</Math> no longer weakly dominates <Math>{"\\mathrm{D}"}</Math> for the seller at any <Math>{"c > 0"}</Math>, however small. What is lost is not a margin but a direction &mdash; taking <Math>{"c"}</Math> small shrinks the adverse gap without restoring the inequality &mdash; and the seller&rsquo;s choice ceases to be settled by a single comparison, turning instead on its belief about the buyer&rsquo;s resolve decision. The buyer&rsquo;s side is untouched, since resolving costs the buyer nothing beyond the payment already committed; what a positive performance cost removes is specifically the seller-side weak dominance and the one-comparison property that rests on it. We claim no positive-cost version of the result: whether a bound on <Math>{"c"}</Math> relative to <Math>{"P"}</Math> and <Math>{"G"}</Math> restores a dominance-strength selection of cooperation, and how the mechanism behaves on either side of such a bound, is open and is not established here. A reader applying the mechanism to a domain in which performance consumes real resources should read every dominance claim below as exact at <Math>{"c = 0"}</Math> and as an open question at <Math>{"c > 0"}</Math>.
                    </PaperRemark>
