            ...
            <PaperSection title="5. N-Party Process Chains">
                ...
                <PaperSubsection title="5.4 Seller Non-Performance and Externality Handling">
                    ...
                    <PaperRun title="Seller substitution.">
                        If <Math>{"S_i"}</Math> fails to perform, before resolving the buyer may commit a replacement order with a new seller <Math>{"S_i'"}</Math> bonding against the current cumulative value. When the buyer resolves, <Math>{"S_i'"}</Math> receives full settlement and <Math>{"S_i"}</Math>, having failed, forfeits their bond. The externality of <Math>{"S_i"}</Math>&rsquo;s non-performance is absorbed by <Math>{"S_i"}</Math>&rsquo;s own capital loss, not distributed to co-sellers.
                    </PaperRun>
