        ...
        <PaperLayout
            ...
            references={
                <>
                    ...
                    <li>Solidity Team. Safe Remote Purchase. In <em>Solidity Documentation &mdash; Solidity by Example</em>, 2016.</li>
        ...
        >
            <PaperSection title="1. Introduction">
                ...
                <PaperSubsection title="1.3 Architectural Posture">
                    <PaperRun title="The mechanism is ideologically agnostic; the graph is the politics.">
                        A consequence of locating the whole of the present argument at the mechanism level is that the mechanism admits no commitments about <em>what</em> is being coordinated or <em>under what normative regime</em>. Currency, jurisdiction, identity type, arbitration forum, role structure, price-discovery mechanism, contribution metric &mdash; none of these are in the mechanism; all are expressed in the <em>graph</em> that parties compose on top of it. A Dutch-auction-and-private-operator graph expresses a market-liberal coordination pattern; a cooperative-bonding-and-contribution-weighted-resolution graph expresses a cooperative one; an interest-free risk-sharing graph expresses an Islamic-finance one. The mechanism enables each and prefers none. This is the architectural consequence of placing the enforcement function at TCP/IP altitude: a layer below which nothing remains to factor out, and a layer above which normative choices become legible as choices rather than as properties of the infrastructure.
                    </PaperRun>
                    <PaperRun title="Composability is external-first.">
                        A second consequence of the mechanism&rsquo;s narrow construction is that its useful extensions are predominantly <em>external</em> rather than internal. The mechanism does not include a dispute forum, a carbon-offset market, a prediction market, an insurance pool, a lending facility, a tax-reporting service, an identity provider, a storage layer, or a messaging fabric. Each exists as a working external protocol or institution, and an assembly composed on top can name any of them in its graph. The mechanism itself need not &mdash; and, by the escape-hatch-weakness theorem, must not &mdash; absorb the bookkeeping that such composition implies. The mechanism makes external surfaces composable; the graph names which externals it composes with.
                    </PaperRun>
                </PaperSubsection>
            </PaperSection>
            ...
            <PaperSection title="2. Related Work">
                <PaperRun title="Mutual-destruction escrows.">
                    The direct ancestor of Figaro is the Safe Remote Purchase contract (Solidity Team, 2016), included in the Solidity documentation as a reference pattern: buyer and seller each lock <Math>{"2\\times"}</Math> the item value, and only mutual agreement releases the funds. This two-party, single-order pattern is the seed from which Figaro grew. BitHalo (Zimbeck, 2014) independently explored mutual-destruction deposits for peer-to-peer Bitcoin trade. The central problem was generalizing it beyond two parties: symmetric <Math>{"2\\times"}</Math> deposits with mutual agreement cannot compose &mdash; in a twelve-seller workflow there is no natural &ldquo;mutual agreement&rdquo; among all participants. Asymmetric bonding solves this by scaling each seller&rsquo;s bond with cumulative upstream value rather than fixing it per trade, creating a mesh of independently secured edges, each carrying its own Nash equilibrium.
                </PaperRun>
            ...
            <PaperSection title="3. The Two Mechanisms: Formal Definition">
                ...
                <PaperSubsection title="3.2 Protocol Operations">
                    ...
                    <PaperRun title="Operation 1: commit.">
                        Given a commitment <Math>{"c"}</Math> signed by both <Math>{"B"}</Math> and <Math>{"S"}</Math>: (1) verify both signatures; (2) if <Math>{"c.\\text{processId} = 0"}</Math>, derive a new process identifier from the commitment digest and initialize with <Math>{"G \\gets P"}</Math>, <Math>{"n \\gets 1"}</Math>; (3) otherwise verify <Math>{"c.\\text{processId}"}</Math> identifies an existing process, that <Math>{"c.B = \\Pi.B"}</Math> and the buyer signature recovers to <Math>{"c.B"}</Math> (buyer dominance), that <Math>{"c.\\tau = \\Pi.\\tau"}</Math> (single-currency binding), and that <Math>{"c.\\text{expectedCumulativeValue} = \\Pi.G + P"}</Math> (monotonicity); (4) pull <Math>{"C_b = 2P"}</Math> from <Math>{"B"}</Math> and <Math>{"C_s = 2 c.\\text{expectedCumulativeValue}"}</Math> from <Math>{"S"}</Math>; (5) create or extend the process, update the accumulator, and emit an evidence event.
                    </PaperRun>
                    <PaperRun title="Operation 2: resolveProcess.">
            ...
            <PaperSection title="4. Game-Theoretic Analysis">
                <PaperSubsection title="4.1 Two-Party Nash Equilibrium">
                    <FormalBlock label="Definition (The Bonding Game).">
                        <p>The <em>bonding game</em> <Math>{"\\Gamma = (\\{B, S\\}, \\{A_B, A_S\\}, \\{u_b, u_s\\})"}</Math> is a one-shot, simultaneous-move, complete-information game. Players: buyer <Math>{"B"}</Math>, seller <Math>{"S"}</Math>. Action sets <Math>{"A_B = A_S = \\{\\mathrm{C}, \\mathrm{D}\\}"}</Math> (cooperate / defect): <Math>{"\\mathrm{C}"}</Math> for <Math>{"S"}</Math> means delivering the agreed goods; <Math>{"\\mathrm{C}"}</Math> for <Math>{"B"}</Math> means calling <code>resolveProcess</code>. For a root order with <Math>{"P > 0"}</Math> and <Math>{"G = P"}</Math>, the net payoffs are <Math>{"u_b = -P"}</Math> if both cooperate and <Math>{"-2P"}</Math> otherwise; <Math>{"u_s = +P"}</Math> if both cooperate and <Math>{"-2G"}</Math> otherwise. The &ldquo;otherwise&rdquo; payoff is identical across all non-cooperative outcomes because any defection prevents resolution: bonds remain locked indefinitely, yielding a net loss equal to the full deposit.</p>
                    </FormalBlock>
            ...
            <PaperSection title="5. N-Party Process Chains">
                ...
                <PaperSubsection title="5.2 Self-Enforcing Cumulative-Value Reporting">
                    ...
                    <FormalBlock label="Theorem (Cumulative-Value Reporting Honesty).">
                        <p>Under the monotonic-accumulator assumption, under-reports (<Math>{"G'_i < G_i"}</Math>) are rejected at <code>commit</code>. Among feasible reports <Math>{"G'_i \\geq G_i"}</Math>, truth-telling (<Math>{"G'_i = G_i"}</Math>) weakly dominates every over-report, strictly whenever the probability of non-resolution is positive.</p>
                    </FormalBlock>
                    <Proof>
                ...
                <PaperSubsection title="5.4 Seller Non-Performance and Externality Handling">
                    ...
                    <ol className="space-y-2 list-decimal pl-6 text-sm">
                        ...
                        <li><em>Seller non-performance</em> is absorbed either by the buyer committing a substitute order before resolution, or by an external risk product (performance guarantee, surety bond, insurance) layered over the bonded position.</li>
                    ...
                    <PaperRun title="Seller substitution.">
                        If <Math>{"S_i"}</Math> fails to perform, before calling <code>resolveProcess</code> the buyer may commit a replacement order with a new seller <Math>{"S_i'"}</Math> bonding against the current cumulative value. When the buyer resolves, <Math>{"S_i'"}</Math> receives full settlement and <Math>{"S_i"}</Math>, having failed, forfeits their bond. The externality of <Math>{"S_i"}</Math>&rsquo;s non-performance is absorbed by <Math>{"S_i"}</Math>&rsquo;s own capital loss, not distributed to co-sellers.
                    </PaperRun>
                    <PaperRun title="External risk products.">
                        Where substitution is impractical, standard instruments (performance guarantees, surety bonds, insurance) layer over the bonded process: they hold independently bonded positions on the seller&rsquo;s behalf, do not call <code>commit</code> or <code>resolveProcess</code>, hold no mechanism state, and leave the bonding equilibrium intact.
                    </PaperRun>
                </PaperSubsection>
                <PaperSubsection title="5.5 Comparison with Joint-Liability Microfinance">
                    ...
                    <p>
                        The reduction is scoped to cooperation pressure. Two further microfinance results &mdash; peer selection on private types (Ghatak, 1999) and peer monitoring under principal information asymmetry (Stiglitz, 1990) &mdash; are not directly reproduced; both require additional structure above the bonded primitive (attested credentials, signal-contingent resolution rules) and lie outside the present scope. The reduction reframes rather than corrects the microfinance theorems: their assumptions characterize one settlement environment (reputation within a social network), not necessary conditions for endogenous peer monitoring. Once the settlement layer admits bilaterally signed, individually collateralized commitments with atomic resolution, the same outcome obtains among strangers in a single shot with no social substrate.
                    </p>
                </PaperSubsection>
            </PaperSection>
            ...
            <PaperSection title="6. Dispute Resolution">
                <p>
                    Dispute resolution is marginal in this mechanism by design. Defection is weakly dominated for every party at every position; the cooperation pressure that propagates through atomic resolution makes each seller&rsquo;s payout contingent on universal cooperation; and the reduction over joint-liability microfinance places the bonded mechanism in the same equilibrium-selection regime as group lending under strictly weaker assumptions. The bonded process itself absorbs almost all of the work that traditional arrangements externalise to courts, arbitrators, and platform adjudication.
                </p>
                <p>
                    The microfinance literature provides a qualitative empirical reference: joint-liability group lending has produced low documented default rates under conditions the bonded mechanism replaces with one-shot collateral and atomic resolution (Yunus, 1999; Ghatak, 1999). The fraction of bonded processes requiring external dispute resolution is therefore expected to be small, but the mechanism does not predict the rate quantitatively &mdash; the microfinance default rate emerges from peer selection, peer monitoring, and cooperation pressure, only the last of which the bonded mechanism reproduces. Calibration is an empirical matter for deployment data, not a theorem.
                </p>
                <p>
                    For the marginal cases where internal pressure does not resolve a dispute, the bonded commitment is compatible with external legal and arbitral forums. Such forums adjudicate <em>outside</em> the bonding game on the evidence record the mechanism produces, rather than serving as a constitutive resolution path within it. Parties may name a forum-clause in their assembly graph selecting Kleros, an arbitral body, a court, or any other forum (Lesaege, George, &amp; Ast, 2019); the mechanism does not bind a forum, and the choice does not affect the bonding equilibrium. The composition is asymmetric: Figaro composes with external forums as evidentiary consumers of the bonded record, but external forums cannot compose with Figaro as a constitutive resolution path without breaking the equilibrium (escape-hatch weakness, Case 2). The asymmetry is the formal expression of the design stance: dispute resolution is a marginal external concern, not an internal mechanism component.
                </p>
            </PaperSection>

            <PaperSection title="7. Conclusion">
                <p>
                    We have presented Figaro, a settlement primitive that achieves self-enforcing multi-party coordination through two composing mechanisms: <em>asymmetric bonding</em>, which produces the bilateral Nash equilibrium and scales it naturally to <Math>{"N"}</Math>-party process chains by tying each seller&rsquo;s bond to cumulative upstream value; and <em>buyer dominance with atomic resolution</em>, which enforces inter-seller coordination on the already-scaled mesh. The mechanism is minimal &mdash; no timeout, no governance, no escape hatches &mdash; and the minimality is itself a security requirement: any unilateral exit path weakens the Nash equilibrium, so the absence of admin overrides, timeouts, or arbitrators is not a simplification but a property the analysis depends on. Under perfect monitoring and complete information, cooperation is weakly dominant at every position in an <Math>{"N"}</Math>-party process chain, and the cooperative profile is the unique outcome surviving iterated elimination of weakly dominated strategies. The same per-edge structure makes seller coalitions unprofitable even with external side-payment. Finally, the mechanism reproduces the cooperation-pressure component of the peer-enforcement equilibrium in joint-liability microfinance under strictly weaker assumptions, with no appeal to repeated interaction, local information, or social sanction.
                </p>
            </PaperSection>
