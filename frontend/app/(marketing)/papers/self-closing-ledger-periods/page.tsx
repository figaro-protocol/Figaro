...
export default function SelfClosingLedgerPeriodsPaper() {
    return (
        <PaperLayout slug="self-closing-ledger-periods"
            ...
            subtitle="Self-Closing Ledger Periods in the Kernel"
            author="Figaro"
            date="April 2026"
        ...
        >
            ...
            <PaperSection title="3. Triple-Entry and Its Limits">
                ...
                <p>
                    A more deeply formal antecedent is Mattessich (1964), who develops accounting as an axiomatic system: the double-entry identity is an algebraic invariant over a set of basic propositions about value flows, analogous to the conservation laws of physics. The process identity of Section 4.1 is, in Mattessich&rsquo;s sense, the same algebraic invariant restated for a transaction-scoped entity: what the mechanism holds for a process equals the claims of that process&rsquo;s parties against it at every moment of the process&rsquo;s life, with the equality discharged at resolution rather than carried forward.
                </p>
                <p>
                    Grigg (2005) revived the idea in a blockchain-adjacent register. Grigg&rsquo;s triple entry proposes that two parties to a transaction each keep a record in the conventional way, and a third record &mdash; a cryptographically signed receipt both parties reference &mdash; serves as the attesting third. In practice that third record played the role now played by chain transaction records: a tamper-evident, timestamped, verifiable artefact neither party can modify after the fact. Grigg&rsquo;s contribution was to formalize what the Ricardian-contract practice had been doing: a cryptographic receipt can carry the weight of a signed document while also functioning as an accounting record.
                </p>
                <p>
            ...
            <PaperSection title="4. A Bonded Process as a Self-Closing Ledger Period">
                <p>
                    We now develop the central claim. It depends on properties of the kernel, and those properties are stated here rather than referred elsewhere, since an accounting argument that cannot say what it is accounting for is not an argument.
                </p>
                <PaperRun title="The mechanism, in the terms this paper needs.">
                    The mechanism has exactly two state-changing calls. <em>Commit</em> forms one bilateral order between the buyer and one seller. Both parties sign a commitment naming the payment <Math>{"P_i > 0"}</Math> for that order, the single unit of account the whole process uses, the cumulative value <Math>{"G_i"}</Math> the process has accumulated through that link with its own payment included, and a content hash of the agreement the parties struck. On that dual signature the mechanism pulls two bonds: <Math>{"2P_i"}</Math> from the buyer and <Math>{"2G_i"}</Math> from the seller. The accumulator is monotonic and admits exactly one declared value per commitment &mdash; the payment itself for the commitment that opens a process, and the standing accumulator plus the new payment for every commitment that extends it &mdash; so <Math>{"G_i = \\sum_{j \\leq i} P_j"}</Math> is determined by arithmetic rather than reported. A process has one buyer throughout, and the mechanism sees the orders as a linear sequence of commits extending that accumulator. <em>Resolve</em> closes the process: only the buyer may call it, and it resolves every active order at once or none at all. Resolution pays the seller of order <Math>{"i"}</Math> the amount <Math>{"2G_i + P_i"}</Math> and returns <Math>{"P_i"}</Math> to the buyer, and the two exhaust what was posted, since <Math>{"(2G_i + P_i) + P_i = 2G_i + 2P_i"}</Math>. On this direct path there is no third call: no timeout, no cancellation, no administrative override, and no path by which a bonded position leaves the mechanism except through the buyer&rsquo;s resolution. The protocol also resolves processes by a second route, a proof-carried batch in which many commits and resolutions are proved together and landed at once, reconciling net token positions and re-emitting each order&rsquo;s events under a proven state; a process resolved that way never acquires the direct path&rsquo;s status, and every reader of the record folds both routes. The ledger derived below is stated for the direct path. The batch route yields the same per-order byproduct &mdash; the same signed undertakings, the same closing transfers &mdash; from its proven events, and the propositions carry over order by order because they are stated in the orders&rsquo; own quantities.
                </PaperRun>
                <PaperRun title="The boundary.">
                    The goods, the work, every passage of value from one party to the next, and the parties&rsquo; own knowledge of all of it are outside the mechanism. They happen between the parties, off the record, and the mechanism can neither observe them nor undo them. What it holds is the bonds, the accumulator, and the record of what was committed to. The consequence for everything below is exact and must be carried through the whole argument: the record shows commitments, positions, and resolution or its absence &mdash; never performance. A defecting party keeps whatever is in its hands, and the record cannot see that either. The bonding schedule exists precisely because of this: crediting a defector with everything it retains, the doubled bond still leaves it out of pocket on either side &mdash; a seller that holds out by at least the value accumulated at its link, a buyer that keeps a delivery and never closes by exactly the payment &mdash; so the value moving outside the record moves honestly. That is a mechanism-design result, and its accounting consequence is the one this paper turns on &mdash; the accounts below are complete as the accounts of the process, and are not, and cannot be, a record of performance.
                </PaperRun>
                <PaperRun title="Why the period closes, and why it closes itself.">
                    Both calls carry load in the argument, and they carry different halves of it. The bonding schedule at commit is what makes every claim the process records a funded one: the amounts posted at commit are exactly the amounts payable at resolution, party by party, so the process never carries an obligation for which capital is not already held. It is also where the equilibrium sits, and the equilibrium is what makes the closing an event to be expected rather than hoped for: after performance, resolving is unconditionally strictly better for the buyer than never resolving &mdash; a comparison that needs no assumption whatever about the seller, since the buyer holds what it received either way and resolving returns its bond less the payment while withholding leaves the bond locked &mdash; and once that is decided, performance is each seller&rsquo;s strict best response, since performing into a buyer who will resolve earns the payment and returns the bond, while holding out leaves the seller with what it retains against twice that value posted. Buyer dominance with atomic resolution then supplies the period-close itself: the closing entry has exactly one author, it is written at exactly one moment, and it resolves every order in the process together. A resolution rule that closed orders one at a time would produce partial closes, which are not period-closes in the accounting sense, and the claim below would not survive. Resolution is also terminal: a resolved process cannot be reopened, and the mechanism holds nothing further for anyone to recover from it.
                </PaperRun>
                <p>
                ...
                <PaperSubsection title="4.1 The process is the accounting entity">
                    ...
                    <PaperRemark title="The identity in trust-account form.">
                        Proposition 4.2 is the shape that lawyers&rsquo; trust accounts, IOLTA arrangements, and client-money custody rules impose by regulation: funds held for others are segregated, the aggregate held equals the aggregate owed, and the holder&rsquo;s own position in the account is nil. Here the same shape is not imposed by a rule that a holder might breach, since there is no holder with an account of its own to breach it into, and no operation that could take the two sides out of step. Mattessich (1964) would recognize the identity as the double-entry invariant restated over a smaller and shorter-lived domain; what is new is not the invariant but the domain &mdash; an entity whose life is one transaction chain and whose books have a natural last page.
                    </PaperRemark>
                </PaperSubsection>
                ...
                <PaperSubsection title="4.2 Commitments as journal entries; resolution as closing entries">
                    ...
                    <p>
                        Debits equal credits by construction, since what is debited is exactly what the two parties bonded. The entry is dated by the kernel, carries a signature from each party, and binds the content hash of the agreement the two of them struck. Nothing in it is an estimate, an accrual, or a provision: the whole of each claim is funded by capital already posted, which is the departure from Ijiri noted in Section 3.
                    </p>
                    <p>
                ...
                <PaperSubsection title="4.3 The three books, and what is held where">
                    ...
                    <ul className="space-y-2 list-disc pl-6 text-sm">
                        ...
                        <li>The <em>giornale</em> &mdash; the formal chronological record, each transaction a balanced pair &mdash; is the sequence of commitment and resolution events in the order the kernel records them. Each is the balanced entry of &sect;4.2, dated, authenticated by the signatures the mechanism verified before acting on them, and immutable in the ordinary sense that no later act rewrites an earlier one.</li>
                    ...
                    <p>
                        The trial balance is then Proposition 4.2 evaluated at a moment, and the closing entries are the resolution. What Pacioli separated into three physical books separates here by <em>where the thing is held</em> rather than by which volume it is written in: the agreement off the record and bound by its hash, the entries on the record, the accounts derived from the entries. The evidentiary division of labor survives the change of medium intact.
                    </p>
                ...
                <PaperSubsection title="4.4 Reconstruction and the period-end property">
                    <p>
                        The complete state of a process is recoverable by deterministic replay of its events. Any reader of the kernel can post the commitments and the resolution to the accounts of Definition 4.1 and obtain the ledger of any process at any moment of its life, without the cooperation of any party and without asking anyone for anything. This is a universal trial-balance generator over the class of processes, and it is what removes the reconciliation step from the record&rsquo;s own audit: there is no second copy of the accounts against which the first could disagree.
                    </p>
                    <p>
                        The limit of the replay is the boundary of &sect;4. What replays is the ledger. The agreements the entries bind are recovered by hash from wherever they are pinned, and are not in the record; performance is in neither. A reader who replays a resolved process learns what was undertaken, what was posted, what resolved, and when &mdash; and learns nothing at all about whether the meal was cooked or the parcel handed over.
                    </p>
                    <PaperRun title="The period-end property.">
                        When a process resolves, its ledger period closes, in the full sense the word carries in accounting practice. Every balance returns to zero; every claim is discharged; every payment has reached its recipient; no further entry can be made against the accounts, because the mechanism holds nothing further and admits no operation that would touch a resolved order. The records persist and remain readable &mdash; the file does not disappear when the period closes &mdash; but the period does not reopen. Conventional practice reaches this state by procedure and convention: adjusting entries are posted, accounts are reconciled, a cut-off date is chosen, a subsequent-events window is observed, and prior periods are occasionally restated when the choice turns out to have been wrong. None of those instruments exists here, and none is needed: the closing date is not chosen by an accountant but produced by the buyer&rsquo;s act, and it is the moment at which that party declares itself satisfied.
                    </PaperRun>
                </PaperSubsection>

                <PaperSubsection title="4.5 Remedies inside the open period">
                    <p>
                        Because the closing act is the buyer&rsquo;s acceptance, everything that would conventionally be handled after a period closes is handled here before it does, and this is a structural feature rather than a matter of practice. The mechanism holds no test of performance and admits no report of one; resolution <em>is</em> the buyer&rsquo;s judgment that what it contracted for arrived. Where performance is late, partial, or wrong, the parties are not choosing between resolution and non-resolution &mdash; they are negotiating a remedy while the process stands open, and the buyer resolves once it is satisfied.
                    </p>
                    <p>
            ...
            <PaperSection title="6. What Continues">
                ...
                <PaperRun title="Audit of the relation between records and reality.">
                    The most substantive remaining audit function is verifying that records correspond to what happened. Did the cook prepare the food? Did the courier deliver it? Did the attested disclosure reflect what was measured? These questions ask whether the record faithfully represents events outside it, and the answer is fixed by &sect;4&rsquo;s boundary: the mechanism cannot answer them, has no operation that would, and never claimed to. It records what was undertaken and what resolved. The professional standards separate the assertions an auditor tests &mdash; <em>existence</em>, <em>rights and obligations</em>, <em>completeness</em>, <em>accuracy</em>, <em>cut-off</em>, <em>classification</em>, and <em>valuation and allocation</em> (AICPA, AU-C&nbsp;315; AU-C&nbsp;500) &mdash; and the bonded setting redistributes them rather than retiring them.
                </PaperRun>
                <p>
                    Of the assertions, one is discharged by construction and the rest are not. <em>Accuracy</em> &mdash; that what was recorded was recorded faithfully &mdash; holds for the undertaking and the resolution, since the mechanism records nothing that was not signed and nothing that it did not itself do, and no later act rewrites either. The remaining assertions stand &mdash; the transaction-level ones (occurrence, completeness, cut-off, classification) and the balance-level ones (existence, rights and obligations, valuation and allocation) &mdash; with the burden distributed differently than under conventional bookkeeping:
                </p>
                <ul className="space-y-2 list-disc pl-6 text-sm">
                    <li><em>Occurrence</em> is the records-to-reality question above: did the economic event the record describes actually occur? The record establishes the undertaking, never the performance, so this assertion carries more of the audit&rsquo;s weight here rather than less.</li>
                    <li><em>Existence</em>, the balance-level counterpart &mdash; does a recorded position exist at the reporting date &mdash; is read from the chain for every position still held under the mechanism: the bonds of an open process are readable at any block. For what has left the mechanism it relocates, with rights and obligations, to the wallet.</li>
                    <li><em>Valuation and allocation</em> remains interpretive: fair value, impairment, amortization, and similar measurement decisions are not what a process&rsquo;s data contains.</li>
                    ...
                    <li><em>Completeness</em> relocates to the boundary between activity that resolved through processes and activity that did not: are there transactions within the reporting scope that never appear because they never resolved this way?</li>
                ...
                <p>
                    The audit function that results is the one the records could never perform for themselves. Accuracy of recording is discharged by the mechanism; occurrence, completeness, and rights migrate toward the world and the keys, and existence is read from the chain for what the mechanism still holds; cut-off vanishes inside a process and reappears at the entity&rsquo;s own period boundary; valuation and classification stay where they always were, in judgment.
                </p>
                <PaperRun title="Regulatory compliance ahead of regulatory catch-up.">
                    Tax law, securities law, and sector regulation assume conventional books. For an extended period, participants resolving through bonded processes will need to produce conventional statements from the record to satisfy those requirements, and that translation is professional work. Regulation may eventually reference the on-chain data directly; in the interim, the accountant&rsquo;s role includes producing traditional outputs from non-traditional inputs.
                </PaperRun>
                <PaperRun title="A worked projection: the European e-invoice norm.">
                    The translation can be made concrete. EN&nbsp;16931-1:2017 (CEN, 2017) defines the <em>semantic data model</em> of the core electronic invoice &mdash; not a file format, but the meaning and cardinality of an invoice&rsquo;s information elements, serialised into one of two permitted syntaxes. It is the model Directive&nbsp;2014/55/EU obliged European public bodies to accept (European Parliament and Council, 2014), and the &ldquo;VAT in the Digital Age&rdquo; package adopted by the Council on 11 March 2025 (Council of the European Union, 2025) makes a conformant structured invoice the definition of an EU e-invoice for intra-Community transactions from 2030 &mdash; regulation moving, as the preceding paragraph anticipated, toward machine-readable structured records rather than away from them. A resolved process <em>projects</em> onto that model. The projection is a serialisation performed above the kernel, not a property of the mechanism, and it sorts the invoice&rsquo;s elements into two classes that recover precisely the byproduct-versus-interpretive split drawn throughout this paper.
                </PaperRun>
                <p>The first of the two element classes &mdash; the intrinsic commercial elements &mdash; falls out of the process&rsquo;s data as byproduct:</p>
                <div className="my-4 overflow-x-auto">
                    <table className="text-sm border-collapse">
                        <thead>
                            <tr>
            ...
            <PaperSection title="9. Scope">
                <p>
                    The argument has one boundary, and it has been drawn in the same place at every point above: it concerns the accounting record <em>of a bonded process</em>. Within that boundary the claim is strong &mdash; the process&rsquo;s accounts are complete, funded, derivable by any reader, and closed by an act that leaves no residue. Outside it the paper claims nothing. The parties&rsquo; own entity-level accounts, the reporting periods those accounts are prepared for, the frameworks they are prepared under, and every economic act that does not resolve as a bonded process are all outside, and Sections 5, 6, and 7 each hand work back across that line rather than absorbing it.
                </p>
                <p>
                    The boundary has a second face, which is the more important of the two. What the record contains is undertakings, positions, and resolution. It does not contain performance, and no reading of it will establish that a meal was cooked or a parcel handed over, because those events happen where the mechanism cannot look. A complete accounting record of a process is therefore not a complete account of the exchange the process resolved: the thing bought never appears in any account of Section 4. This is not a gap to be closed by a better record. It is the division of labor the whole design rests on, and it is why the audit function of Section 6 is not merely surviving but carrying more of the weight than before.
                </p>
                <PaperRun title="The record is only as reliable as the layer beneath it.">
                    The record inherits the reliability of the chain that carries it and the cryptographic primitives underneath. A reorganisation that unwinds resolved state, a signature-scheme break, or censorship at the layer&rsquo;s own level would each compromise the record. These risks are not new to accounting &mdash; a merchant&rsquo;s ledger could burn &mdash; but they are concentrated differently: at one layer rather than distributed across party-held books. Whether that concentration is a net improvement depends on the relative reliability of the two arrangements, which is an empirical question this paper does not decide.
                </PaperRun>
                <PaperRun title="Key custody is a first-order audit surface.">
                    If the wallet is the record of position, then control of its keys is control of that position. The Pacioli equivalent is the strongbox holding the journal: physical controls, segregation of duties, authorization. The analogous surface here is control over signing keys &mdash; threshold arrangements, hardware custody, rotation procedures, signer-set governance, recovery. This is not a new audit assertion; it sits within the <em>rights and obligations</em> assertion and within existing IT-control frameworks. It is a relocated emphasis: where the conventional regime puts its control weight on reconciling records against records, this one puts it on establishing who holds the keys.
                </PaperRun>
            </PaperSection>
