        ...
        >
            ...
            <PaperSection title="4. A Bonded Process as a Self-Closing Ledger Period">
                ...
                <PaperRun title="The mechanism, in the terms this paper needs.">
                    The mechanism has exactly two state-changing calls. <em>Commit</em> forms one bilateral order between the buyer and one seller. Both parties sign a commitment naming the payment <Math>{"P_i > 0"}</Math> for that order, the single unit of account the whole process uses, the cumulative value <Math>{"G_i"}</Math> the process has accumulated through that link with its own payment included, and a content hash of the agreement the parties struck. On that dual signature the mechanism pulls two bonds: <Math>{"2P_i"}</Math> from the buyer and <Math>{"2G_i"}</Math> from the seller. The accumulator is monotonic and admits exactly one declared value per commitment &mdash; the payment itself for the commitment that opens a process, and the standing accumulator plus the new payment for every commitment that extends it &mdash; so <Math>{"G_i = \\sum_{j \\leq i} P_j"}</Math> is determined by arithmetic rather than reported. A process has one buyer throughout, and the mechanism sees the orders as a linear sequence of commits extending that accumulator. <em>Resolve</em> settles the process: only the buyer may call it, and it settles every active order at once or none at all. Resolution pays the seller of order <Math>{"i"}</Math> the amount <Math>{"2G_i + P_i"}</Math> and returns <Math>{"P_i"}</Math> to the buyer, and the two exhaust what was posted, since <Math>{"(2G_i + P_i) + P_i = 2G_i + 2P_i"}</Math>. On this direct path there is no third call: no timeout, no cancellation, no administrative override, and no path by which a bonded position leaves the mechanism except through the buyer&rsquo;s resolution. The protocol also settles processes by a second route, a proof-carried batch in which many commits and resolutions are proved together and landed at once, reconciling net token positions and re-emitting each order&rsquo;s events under a proven state; a process settled that way never acquires the direct path&rsquo;s status, and every reader of the record folds both routes. The ledger derived below is stated for the direct path. The batch route yields the same per-order byproduct &mdash; the same signed undertakings, the same closing transfers &mdash; from its proven events, and the propositions carry over order by order because they are stated in the orders&rsquo; own quantities.
                </PaperRun>
