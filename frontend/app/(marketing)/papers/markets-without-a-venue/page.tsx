...
export default function MarketsWithoutAVenuePaper() {
    return (
        <PaperLayout slug="markets-without-a-venue"
            ...
            subtitle="Dispatch Races and Requests for Quotes as Market-Design Mechanisms over a Market-Blind Kernel"
            author="Figaro"
            date="August 2026"
        ...
        >
            <PaperSection title="1. Introduction">
                ...
                <PaperSubsection title="1.2 Contribution">
                    ...
                    <ol className="space-y-2 list-decimal pl-6 text-sm">
                        ...
                        <li><strong>The participation constraint is a balance.</strong> Each draft states the value the chain has accumulated through the answering candidate&rsquo;s link, and the kernel will require twice that figure from it. The quantity that decides participation is therefore observable, stated in advance, position-dependent, and increasing in the candidate&rsquo;s own quote &mdash; a coupling between what a bidder asks and what a bidder must hold that ordinary procurement markets do not have (&sect;5.4). A corollary runs along the chain: a cheaper answer accepted at any position lowers the balance required at every later one, so competition at one link weakly widens the market at all the links after it (&sect;5.5).</li>
                        <li><strong>Composition without awareness.</strong> Formation&rsquo;s entire output is an object the kernel already accepts, and formation reads nothing from the kernel but the arithmetic it must reproduce. We state the resulting interface-minimality property and its two consequences: no equilibrium result of the kernel needs re-derivation for market-formed offers, and the record that layer keeps cannot distinguish an offer that was raced for from one arranged by hand (&sect;6).</li>
                    </ol>
                    <p>
                        Section 2 fixes the setting and states, in the register this paper uses, the slice of the kernel the argument depends on. Section 3 develops signature order as a market primitive and disposes of the four venue roles. Sections 4 and 5 analyze the two mechanisms. Section 6 states the composition property. Section 7 concludes.
                    </p>
                </PaperSubsection>
            </PaperSection>

            <PaperSection title="2. The Setting">
                <PaperSubsection title="2.1 What the kernel holds">
                    <p>
                        The kernel this paper composes above has two operations and no others. <em>Commitment</em> forms one bilateral edge: a struct naming a buyer, a seller, a unit of account, a payment <Math>{"P \\geq 1"}</Math>, the value <Math>{"G"}</Math> the process has accumulated through this link, a digest of the parties&rsquo; agreement, a salt, and a deadline, admitted only when two signatures over that one struct recover to the two named parties. On admission the layer locks <Math>{"2P"}</Math> from the buyer and <Math>{"2G"}</Math> from the seller. <em>Resolution</em> closes the whole process: only the buyer may call it, and every open order resolves together or none does, each seller receiving <Math>{"2G + P"}</Math> and the buyer <Math>{"P"}</Math> back per order &mdash; so cooperation leaves each seller ahead by its payment and the buyer down by exactly what it agreed to pay.
                    </p>
                    <p>
                    ...
                    <PaperRun title="Two signatures, in no particular order.">
                        The layer verifies both signatures against a single digest and imposes no ordering between them. Which side signs first is therefore not a rule of the kernel at all; it is a convention of whatever produces the struct. Section 3 shows that this apparently empty degree of freedom is where the market lives.
                    </PaperRun>
                    ...
                    <PaperRun title="One unit of account per process, and a positive floor.">
                        A process is denominated throughout in one unit, because the doubled bond schedule is a comparison of magnitudes and comparability is what makes it enforceable from the record alone; and a payment of zero is refused. Both constraints are inherited by any market that forms offers here: quotes within one solicitation are commensurable by construction, and a zero-priced answer is not an expressible object.
                    </PaperRun>
                    <PaperRemark title="What is out of scope, and stays out.">
                        The equilibrium that makes performance the sellers&rsquo; best response, and resolution the buyer&rsquo;s, is a property of the kernel and is taken here as given rather than re-derived: the comparison at each node is over the transfers the layer executes, and it contains no term for what a participant spends to perform. This paper adds a layer above and takes care to add nothing to that comparison. Where an argument below needs a notion of a participant&rsquo;s willingness to trade, it uses the one quantity the mechanism actually contains &mdash; the balance a wallet can commit &mdash; and says so.
                    </PaperRemark>
                </PaperSubsection>
                ...
                <PaperSubsection title="2.2 What a market has to add">
                    ...
                    <p>
                        The third is where a kernel of the present kind changes the problem, because safety in a real market is two things that are usually supplied by one institution. There is <em>formation safety</em> &mdash; answering a solicitation must not expose the answerer to more than the deal it answered for &mdash; and there is <em>performance safety</em> &mdash; the counterparty, once matched, must actually deliver. Conventional venues supply both together: an exchange that holds margin is simultaneously making bidding safe and making resolution safe, and the two are not separable in its design. Here they are separable, and separated. Performance safety is the kernel&rsquo;s, complete before any market exists. Formation safety is all that the market layer must supply on its own, and the mechanisms below supply it structurally rather than by holding anything.
                    </p>
                    <p>
            ...
            <PaperSection title="3. Signature Order as a Market Primitive">
                <PaperSubsection title="3.1 Three states of one artifact">
                    ...
                    <p>
                        A <em>draft</em> carries none. It is a fully specified commitment struct &mdash; parties, unit, payment, accumulated value, agreement digest, salt, deadline &mdash; that no one has signed. It is structurally inert: the kernel will not admit it, and no recipient can make it admissible alone. Sending a draft is therefore a communication and nothing more, and the mechanisms below rely on the recipient being able to establish that: an answering party checks that the draft names it, that the agreement attached hashes to the digest the struct commits to, and that the terms in the agreement do not contradict the struct it would be bonded against, and refuses to answer otherwise.
                    </p>
                    <p>
                    ...
                    <p>
                        A <em>commitment</em> carries both signatures and is the only state the kernel has a name for. Producing it is the buyer&rsquo;s single act, and it is simultaneously the selection of a counterparty and the formation of the obligation.
                    </p>
                </PaperSubsection>
            ...
            <PaperSection title="5. The Request for Quotes">
                ...
                <PaperSubsection title="5.3 No second price without a second signature">
                    ...
                    <FormalBlock label="Proposition 5.2 (First price is forced).">
                        <p>
                            In a mechanism whose only resolution artifact is a struct that both parties sign, and in which the amount transferred to the winner is a field of that struct, no allocation rule that pays the winner a figure computed from other candidates&rsquo; reports is implementable without a party able to bind the winner to a struct it has not signed.
                        </p>
                    </FormalBlock>
                    <Proof>
                        Let the winner have answered at <Math>{"q_1"}</Math> and let the rule prescribe payment <Math>{"q_2 \\neq q_1"}</Math> computed from rival reports. The kernel transfers the payment field of the admitted struct, so admitting a struct that pays <Math>{"q_2"}</Math> requires a struct whose payment field is <Math>{"q_2"}</Math>; the winner&rsquo;s signature is over a struct whose payment field is <Math>{"q_1"}</Math>, and a signature over one struct is not a signature over another. Admission requires the winner&rsquo;s signature over the struct admitted. Hence either the winner is solicited a second time at <Math>{"q_2"}</Math> &mdash; which it may decline, so the rule is not an allocation rule but an offer &mdash; or some party supplies the winner&rsquo;s assent without the winner, which is the venue. <span className="italic">&#9633;</span>
                    </Proof>
                    <p>
                ...
                <PaperSubsection title="5.4 The participation constraint is a balance">
                    ...
                    <p>
                        A draft states the value <Math>{"G"}</Math> the process has accumulated through the answering candidate&rsquo;s link, and the kernel will pull <Math>{"2G"}</Math> from that candidate when the commitment is admitted. On the quote leg, if the position sits behind an accumulated <Math>{"V_0"}</Math> and the candidate quotes <Math>{"q"}</Math>, then <Math>{"G = V_0 + q"}</Math> and the requirement is <Math>{"2(V_0 + q)"}</Math>. A candidate whose free balance in the process&rsquo;s unit is <Math>{"B"}</Math> can therefore be committed only where
                    </p>
                ...
                <PaperSubsection title="5.5 Competition upstream widens the market downstream">
                    ...
                    <p>
                        The same running sum has now done two separate jobs, and it is worth naming the coincidence because nothing in the design arranged it. It sets the size of the deterrent at each link, which is what Proposition 5.3 is about; and it is the quantity whose movement excludes rival drafts at a link once one of them commits, which is what Proposition 3.1<em>(a&prime;)</em> is about. One accumulator therefore prices the deterrent and closes the position, and a market layer that reproduces the figure faithfully &mdash; the obligation of &sect;6.2 &mdash; gets both properties without asking for either.
                    </p>
                </PaperSubsection>
            </PaperSection>

            <PaperSection title="6. Composing with a Market-Blind Kernel">
                <PaperSubsection title="6.1 Interface minimality">
                    <p>
                        The composition property both mechanisms have is worth naming, because it is what allows a market to be added to a kernel whose guarantees were established without one.
                    </p>
                    <FormalBlock label="Definition 6.1 (Interface-minimal market layer).">
                        <p>
                            A market layer over a kernel <Math>{"S"}</Math> is <em>interface-minimal</em> if its entire output is an object <Math>{"S"}</Math> already accepts, formed only from inputs the parties themselves supply, and if <Math>{"S"}</Math>&rsquo;s acceptance of that object is conditioned on nothing the market layer did.
                        </p>
                    </FormalBlock>
                    <p>
                        Both mechanisms are interface-minimal. Their output is a dual-signed commitment, which is the object the kernel accepted before any market existed; their inputs are the parties&rsquo; signatures and published terms; and acceptance turns on the two signatures and the struct, never on provenance. The consequence is the one that matters for a paper that declines to re-derive the kernel&rsquo;s equilibrium:
                    </p>
                    ...
                    <p>
                        The proposition is nearly a tautology and is included because its contrapositive is the useful part. A market layer that holds a deposit adds a transfer, so its offers do not inherit; a market layer that can release a party&rsquo;s locked position changes the conditions of admission or of resolution, so its offers do not inherit; a market layer that requires the kernel to know a solicitation happened changes the acceptance condition, so its offers do not inherit. Each of those is a design that would have to earn its own equilibrium argument from scratch, and the argument would be about a different mechanism. This is the precise sense in which &ldquo;zero contracts&rdquo; is an analytical property rather than a boast about parsimony.
                    </p>
                </PaperSubsection>
