...
export default function DisclosureAsymmetryPaper() {
    return (
        <PaperLayout
            ...
            keywords="mechanism design, engineering of consent, manufacture of consent, disclosure, common knowledge, legitimacy, platform capitalism, trust"
            abstract={
                <>
                    <p>
                        Mechanism design and the engineering of consent deploy the same instruments &mdash; incentives, common knowledge, equilibrium selection. The mid-century craft of consent-engineering (Lippmann, 1922; Bernays, 1928, 1947) and its critical analysis (Herman &amp; Chomsky, 1988) describe a technology for manufacturing a disposition in a population. The Nobel-recognised theory of mechanism design (Hurwicz, Maskin, Myerson) describes a technology for selecting an equilibrium among self-interested agents. They are usually treated as unrelated &mdash; one a dark art of persuasion, the other a formal economics. This paper argues they are the same instrument pointed in opposite directions, and isolates the operation that reverses the vector.
                    </p>
                    <p>
                    ...
                    <p>
                        We argue this against a named opponent. Viljoen, Goldenfein &amp; McGuigan (2021) occupy the same conceptual seam and reach the contrary conclusion: mechanism design <em>is</em> the predatory instrument by which platforms &ldquo;opaquely generate and exploit information asymmetries&rdquo; for &ldquo;information domination.&rdquo; We concede their case for the platform setting in full, and show that the cryptoeconomic setting flips the sign &mdash; that the thing which flips it is exactly the property their own diagnosis turns on, opacity, negated. The worked instance is the Figaro settlement primitive; the closing move locates the deepest form of their critique and answers it at the layer where mechanism design becomes ownerless.
                    </p>
                </>
            }
            references={
        ...
        >
            ...
            <PaperSection title="3. The Inversion: Disclosure">
                <p>
                    The variable is the participant&rsquo;s epistemic accuracy about the mechanism. Let <Math>{"m"}</Math> be the mechanism actually in force, <Math>{"\\hat{m}"}</Math> the participant&rsquo;s model of it, and let accuracy <Math>{"a"}</Math> measure the closeness of <Math>{"\\hat{m}"}</Math> to <Math>{"m"}</Math> (maximal when <Math>{"\\hat{m} = m"}</Math>). Write <Math>{"E"}</Math> for the degree to which the designer&rsquo;s intended outcome obtains. The two regimes are distinguished not by their instruments &mdash; the instruments are identical &mdash; but by the sign of the dependence of <Math>{"E"}</Math> on <Math>{"a"}</Math>:
                </p>
            ...
            <PaperSection title="5. Answering Viljoen, Goldenfein & McGuigan">
                ...
                <PaperRun title="Determinism, against the commitment objection.">
                    A sophisticated reply grants disclosure and presses the commitment condition: a platform can publish its rules and still rule by retaining discretion to change or override them ex post, so that disclosure buys nothing. This is the strongest version of the platform critique that disclosure alone does not answer. The cryptoeconomic setting answers it not with a promise but with an inability: the mechanism is enforced by deterministic execution that its author cannot deviate from, and the kernel exposes no administrator, no pause, no upgrade, no override. The designer is bound by the same rules as everyone else because the rules are not the designer&rsquo;s to suspend. Commitment is not asserted; it is structural.
                </PaperRun>
                ...
                <p>
                    The three flips are cumulative. Disclosure raises accuracy; determinism makes the disclosed rules binding on their author; ownerlessness empties the rules of the designer&rsquo;s ends and relocates objective-setting to the participant. What remains is a mechanism whose efficacy depends entirely on the participant&rsquo;s accurate model of it and not at all on a designer&rsquo;s concealed advantage &mdash; the exact negation of the condition Viljoen et al. diagnose.
                </p>
            </PaperSection>

            <PaperSection title="6. Conclusion">
                <p>
                    Mechanism design and the engineering of consent are one instrument. The instrument is neither predatory nor trust-constructing in itself; it becomes one or the other according to the sign of its dependence on the participant&rsquo;s knowledge of it. Consent-engineering, and platform mechanism design as its formal heir, take the negative sign: they manufacture an outcome by concealing the rules and exploiting the ruled&rsquo;s ignorance of them, and they are corroded by disclosure. Cryptoeconomic mechanism design takes the positive sign: it constructs warranted confidence by making the rules common knowledge and binding, and it is completed by disclosure. The operation that sets the sign is disclosure; the conditions that hold it are determinism and ownerlessness.
                </p>
                <p>
