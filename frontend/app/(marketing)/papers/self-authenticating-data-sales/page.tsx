    ...
    return (
        ...
        >
            ...
            <PaperSection title="2. A Data Sale Is an Ordinary Bonded Order">
                <p>
                    The mechanism is not a purpose-built data-market contract; it is the same two-mechanism settlement kernel used for any bonded trade, composed with two clauses that name the subject matter as data. Recalling the kernel briefly, because the argument below depends on it: a buyer and seller each lock a bond at commit &mdash; the buyer twice the payment, the seller twice the cumulative value of the process up to and including its own order &mdash; and only the buyer can subsequently trigger resolution, which settles every order in the process atomically or not at all. Under perfect monitoring &mdash; that is, each party observes whether the other performed, without error &mdash; cooperation is the weakly dominant strategy for both parties and the unique profile surviving iterated elimination of weakly dominated strategies (Myerson, 1979; the derivation is <Link href="/papers/asymmetric-bonding" className="text-ink-heading hover:underline">Asymmetric Bonding and Buyer Dominance</Link>): a defecting seller forfeits a bond worth strictly more than the payment it forgoes, and a defecting buyer forfeits its own locked capital with nothing to show for it. There is no timeout, no administrator, and no unilateral exit from the bonded state; the only path back to either party&rsquo;s capital is the buyer&rsquo;s own resolution call.
                </p>
                <p>
