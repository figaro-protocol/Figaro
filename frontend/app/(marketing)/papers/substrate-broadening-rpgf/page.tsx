    ...
    return (
        ...
        >
            <PaperSection title="1. Introduction">
                <p>
                    The Figaro settlement kernel is deliberately minimal: it verifies two signatures, escrows two bonds, and resolves a process atomically. Everything that gives a bonded commitment meaning beyond &ldquo;party A paid party B&rdquo; &mdash; what was promised, what was attested, what credential was presented, what handoff occurred &mdash; lives in <em>clauses</em>: content-typed agreement and attestation definitions, each registered permissionlessly on the clause registry. Clauses are the protocol&rsquo;s substrate. The reachable surface of coordinations the protocol can express is exactly the set of clauses that exist.
                </p>
                <p>
                    Authoring a clause is public-goods work in the strict sense. A clause is non-rival (one author&rsquo;s registration does not consume another&rsquo;s ability to use it), non-excludable (anyone may attest under a registered clause), and uncompensated by the protocol&rsquo;s own operation (the kernel takes no fee, and there is no per-attestation or per-settlement revenue path). The author who writes a high-quality geographic-attestation clause, or a handoff clause that a hundred unrelated assemblies later adopt, captures none of the value their work creates for the network. This is the canonical under-provision problem of public goods, reproduced at the protocol-substrate layer.
                </p>
                <p>
            ...
            <PaperSection title="2. The Allocation Problem">
                ...
                <PaperRun title="To whom.">
                    A clause&rsquo;s allocation goes to its registering author &mdash; the first-write-wins wallet that registered the clause on the registry. One clause, one recipient. There is no per-attestation, per-seller, or per-settlement reward path anywhere in the system: a seller who attests heavily under someone else&rsquo;s clause earns nothing from RPGF, and a buyer who settles a thousand processes earns nothing from RPGF. The recipient set is exactly the set of clause authors, determined by registry state, with no curation step.
                </PaperRun>
            ...
            <PaperSection title="5. Tier-1 Graph Weighting">
                ...
                <PaperRun title="Category weight.">
                    <Math>{"w_{\\text{cat}}"}</Math> is <Math>{"3.0"}</Math> for clauses whose <em>article group</em> is tier-1 and <Math>{"1.0"}</Math> otherwise. Tier-1 article groups are the analytically load-bearing categories &mdash; geographic and logistics attestation &mdash; whose data feeds the protocol&rsquo;s public coordination graphs. The set of tier-1 article groups is hard-coded at deployment and is reversible only by deploying a new token system. Crucially, the <em>groups</em> are frozen but the <em>clauses inside each group</em> are not: any third-party clause whose article falls in a tier-1 group inherits the tier-1 category weight at the next tranche, with no change to the kernel and no permission required. The weighting privileges a category of work, not a privileged set of authors.
                </PaperRun>
                <PaperRun title="Topology weight.">
                    <Math>{"w_{\\text{topo}}"}</Math> is the clause&rsquo;s mean position in the process chains it appeared in over the window, clamped to <Math>{"[1.0, 3.0]"}</Math>. Clauses used deeper in process chains &mdash; later in the process&rsquo;s commit sequence, where accumulated upstream value is larger and more coordination depends on correct attestation &mdash; weigh more. Depth is a proxy for structural load: a clause that reliably carries meaning at chain position three is doing more coordination work than one used only at the root.
                </PaperRun>
            ...
            <PaperSection title="6. The Concentration Cap">
                ...
                <p>
                    Capped shares scale to FIG amounts against the tranche budget, and the resulting <Math>{"(\\text{author}, \\text{amount})"}</Math> pairs form the leaves of the tranche&rsquo;s Merkle tree.
                </p>
            </PaperSection>
