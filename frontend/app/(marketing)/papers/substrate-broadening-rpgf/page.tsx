...
export const metadata: Metadata = {
    ...
    description:
        "A frozen, zero-knowledge-verified allocation mechanism that distributes 600M FIG to schema authors across three tranches. Schemas are scored by counterparty diversity over raw volume, value is deliberately excluded, and a 15% per-author cap is enforced by iterative water-filling.",
};
...
export default function SubstrateBroadeningRpgfPaper() {
    return (
        <PaperLayout
            ...
            subtitle="A Verifiable Allocation Mechanism for Protocol Schema Authors"
            author="Alessandro Daliana"
            date="May 2026"
            watermark="Figaro Protocol · Preprint"
            keywords="retroactive public goods funding, mechanism design, resource allocation, quadratic funding, zero-knowledge verification, schema authoring, coordination protocols"
            abstract={
                <>
                    <p>
                        A permissionless coordination protocol grows by accretion of the artifacts that extend its substrate &mdash; in the Figaro protocol, the schemas that define new attestation and agreement types. The authors of those schemas perform public-goods work: a schema, once registered, is usable by every participant at no charge to them and no recurring revenue to its author. This paper develops the mechanism by which the Figaro token (FIG) funds that work retroactively: a fixed 600-million-unit reserve, released across three tranches at years 2, 5, and 9, allocated to schema authors by a frozen scoring formula.
                    </p>
                    <p>
                        The mechanism&rsquo;s distinctive choice is what it measures. It does not reward volume, and it does not reward value moved: it rewards <em>substrate-broadening</em> &mdash; adoption of a schema across many distinct counterparty relationships, in processes that actually settled. A schema written into a hundred agreements that never resolve earns nothing; a schema used once between each of fifty distinct buyer&ndash;seller pairs broadens the protocol&rsquo;s reachable surface more than the same schema used fifty times between one pair, and the formula weights counterparty diversity above raw process count accordingly. Payment and bond size never enter the score. A tier-1 graph weighting rewards schemas that anchor analytically valuable families and that sit deeper in process chains; a 15% per-author cap, enforced by iterative water-filling, bounds concentration.
                    </p>
                    <p>
        ...
        >
            <PaperSection title="1. Introduction">
                <p>
                    The Figaro settlement kernel is deliberately minimal: it verifies two signatures, escrows two bonds, and resolves a process atomically. Everything that gives a bonded commitment meaning beyond &ldquo;party A paid party B&rdquo; &mdash; what was promised, what was attested, what credential was presented, what handoff occurred &mdash; lives in <em>schemas</em>: content-typed agreement and attestation definitions, each registered permissionlessly on the schema registry and bound to a validator contract. Schemas are the protocol&rsquo;s substrate. The reachable surface of coordinations the protocol can express is exactly the set of schemas that exist.
                </p>
                <p>
                    Authoring a schema is public-goods work in the strict sense. A schema is non-rival (one author&rsquo;s registration does not consume another&rsquo;s ability to use it), non-excludable (anyone may attest under a registered schema), and uncompensated by the protocol&rsquo;s own operation (the kernel takes no fee, and there is no per-attestation or per-settlement revenue path). The author who writes a high-quality geographic-attestation schema, or a fulfilment-handoff schema that a hundred unrelated assemblies later adopt, captures none of the value their work creates for the network. This is the canonical under-provision problem of public goods, reproduced at the protocol-substrate layer.
                </p>
                <p>
                    Retroactive public-goods funding (RPGF), developed in practice by the Optimism Collective (2021), addresses under-provision by rewarding work after its value is observable rather than speculating on it in advance. The intuition is that judging impact is easier in retrospect than prospect. This paper specifies a fully mechanized instance of that intuition for schema authoring: a fixed FIG reserve, released in tranches years after the work, allocated by an objective scoring function computed over on-chain settlement history &mdash; with no committee, no vote, and no discretionary curation.
                </p>
                <PaperRun title="Contribution.">
                    The contribution is a concrete, frozen, zero-knowledge-verified allocation mechanism with three properties we develop in turn: (i) it scores schemas by <em>substrate-broadening</em> &mdash; counterparty diversity in settled processes &mdash; rather than by volume or value; (ii) it bounds concentration with a per-author cap enforced by iterative water-filling; and (iii) it makes each tranche&rsquo;s allocation verifiable behind a proof that the canonical formula was applied to the supplied inputs, with an honest account of the residual trust the proof does not discharge.
                </PaperRun>
            ...
            <PaperSection title="2. The Allocation Problem">
                <PaperRun title="What is allocated.">
                    The FIG token reserves 600 million units &mdash; 60% of the one-billion fixed supply &mdash; for schema-author funding. The reserve is released in three tranches: 300M at year 2, 200M at year 5, and 100M at year 9 (50% / 33% / 17% of the reserve; 30% / 20% / 10% of total supply). The schedule is front-loaded because early-network substrate growth has higher marginal coordination value, and the nine-year tail reserves a share for sustained contribution. The tranche budgets and unlock times are immutable constants set at deployment.
                </PaperRun>
                <PaperRun title="To whom.">
                    A schema&rsquo;s allocation goes to its <code>schemaAuthor</code> &mdash; the first-write-wins wallet that registered the schema on the registry. One schema, one recipient. There is no per-attestation, per-operator, or per-settlement reward path anywhere in the system: an operator who attests heavily under someone else&rsquo;s schema earns nothing from RPGF, and a buyer who settles a thousand processes earns nothing from RPGF. The recipient set is exactly the set of schema authors, determined by registry state, with no curation step.
                </PaperRun>
                ...
                <PaperRemark title="Why not prospective or per-use funding.">
                    Two alternatives are deliberately excluded. <em>Prospective</em> funding (grants decided before the work) requires a committee to predict impact and reintroduces discretion. <em>Per-use</em> funding (a micro-payment on each attestation under a schema) couples token issuance to protocol activity, which invites wash-attestation to farm the reward and muddies the token&rsquo;s meaning &mdash; the same coupling the token&rsquo;s focal-point design forbids. Retroactive, lump-sum, formula-based allocation avoids both failure modes: impact is observed, not predicted, and the reward attaches to the durable artifact (the schema) rather than to the gameable event (an attestation).
                </PaperRemark>
            </PaperSection>
            ...
            <PaperSection title="4. The Scoring Formula">
                ...
                <ul className="space-y-1 list-disc pl-6 text-sm">
                    <li><Math>{"c(s)"}</Math> &mdash; the number of distinct resolved <em>processes</em> in which schema <Math>{"s"}</Math> appeared.</li>
                ...
                <p>
                    A schema with zero processes or zero pairs scores zero. The exponent split is the load-bearing choice. With <Math>{"\\alpha = 0.33"}</Math>, the diversity term <Math>{"d(s)"}</Math> carries the larger exponent (<Math>{"1-\\alpha = 0.67"}</Math>), so counterparty diversity dominates raw process count. Holding the product <Math>{"c \\cdot d"}</Math> fixed, a schema scores higher the more its activity is spread across distinct pairs rather than concentrated in repeated processes between the same parties. The split is a deploy-time constant, chosen to express &mdash; not to derive &mdash; the design judgment that breadth outweighs volume; the simulator that accompanies the implementation explored alternatives, and <Math>{"33/100"}</Math> is the value the system commits to.
                </p>
                <PaperRun title="Value is deliberately excluded.">
                    Neither payment nor bond size enters the formula. The protocol&rsquo;s cost to move one unit of value equals its cost to move a trillion; a schema that broadens coordination across many small settlements has done the same substrate work as one used in a few large ones. Weighting by value would import a &ldquo;total value locked matters&rdquo; metric from financial protocols that the coordination layer rejects &mdash; and it would skew the reward toward whoever authors the schemas that high-value transactions happen to use, which is uncorrelated with how broadly a schema extends the substrate. The omission is principled, not an oversight.
                </PaperRun>
            </PaperSection>
            ...
            <PaperSection title="5. Tier-1 Graph Weighting">
                ...
                <PaperRun title="Category weight.">
                    <Math>{"w_{\\text{cat}}"}</Math> is <Math>{"3.0"}</Math> for schemas belonging to a tier-1 <em>family</em> and <Math>{"1.0"}</Math> otherwise. Tier-1 families are the analytically load-bearing categories &mdash; geographic and fulfilment attestation &mdash; whose data feeds the protocol&rsquo;s public coordination graphs. The set of tier-1 families is hard-coded at deployment and is reversible only by deploying a new token system. Crucially, the <em>families</em> are frozen but the <em>schemas inside each family</em> are not: any third-party schema registered under a tier-1 family key inherits the tier-1 category weight at the next tranche, with no change to the kernel and no permission required. The weighting privileges a category of work, not a privileged set of authors.
                </PaperRun>
                <PaperRun title="Topology weight.">
                    <Math>{"w_{\\text{topo}}"}</Math> is the schema&rsquo;s mean position in the process chains it appeared in over the window, clamped to <Math>{"[1.0, 3.0]"}</Math>. Schemas used deeper in process chains &mdash; further from the root buyer, where progressive collateralization has accumulated more upstream value and more coordination depends on correct attestation &mdash; weigh more. Depth is a proxy for structural load: a schema that reliably carries meaning at chain position three is doing more coordination work than one used only at the root.
                </PaperRun>
                <PaperRemark title="Why additive, not multiplicative.">
                    Composing the two dimensions additively (each as an excess-over-one increment) rather than multiplicatively bounds the combined weight at <Math>{"5.0"}</Math> and keeps each dimension&rsquo;s contribution legible and independent. A multiplicative composition would let the two dimensions compound to <Math>{"9.0"}</Math> and would make a schema&rsquo;s weight disproportionately sensitive to scoring well on both at once &mdash; a sharper incentive edge than the mechanism intends.
                </PaperRemark>
            </PaperSection>

            <PaperSection title="6. The Concentration Cap">
                <p>
                    Raw scores become pro-rata shares of the tranche budget: a schema&rsquo;s share is its score divided by the sum of all scores in the tranche. A per-author cap of <Math>{"15\\%"}</Math> is then enforced, so that no single author can take more than 15% of a tranche regardless of how dominant their schemas&rsquo; scores are. The cap bounds capture: it prevents a single prolific or well-positioned author from absorbing a tranche, preserving the funding&rsquo;s public-goods character.
                </p>
                ...
                <p>
                    Capped shares scale to FIG amounts against the tranche budget, and the resulting <Math>{"(\\text{schemaAuthor}, \\text{amount})"}</Math> pairs form the leaves of the tranche&rsquo;s Merkle tree.
                </p>
            </PaperSection>
            ...
            <PaperSection title="9. Scope and Limitations">
                <PaperRun title="Cohort selection is the formula, by construction.">
                    The mechanism deliberately has no cohort-selection policy. The recipient set is the set of schema authors with non-zero resolved adoption in the window, and the proportions are the formula&rsquo;s output. There is no list of approved recipients to curate and no committee to convene. This is a feature &mdash; it is what makes the mechanism discretion-free &mdash; but it means the mechanism funds exactly one kind of contribution (schema authoring) and is silent on every other kind of protocol work (client development, documentation, review, operations). Those are funded, if at all, through the separately-held treasury allocation, not through this mechanism.
                </PaperRun>
