    ...
    return (
        ...
        >
            <PaperSection title="1. Introduction">
                ...
                <p>
                    The full-rationality equilibrium argument for the bonded primitive (derived in <Link href="/papers/asymmetric-bonding" className="text-ink-heading hover:underline">Asymmetric Bonding and Buyer Dominance</Link>, &sect;4.1, with the <Math>{"N"}</Math>-party extension in &sect;5.1) establishes that cooperation is weakly dominant for both parties in the bonded game, and is the unique profile surviving iterated elimination of weakly dominated strategies. The proof assumes complete information, full rationality, no non-pecuniary preferences, no coordination failure under pessimistic beliefs, perfect monitoring, and costless performance; what it prices is settlement discipline, not the adjudication of disputed performance, which remains with the forums that compose above the mechanism. The result holds within those assumptions. The present paper asks: how does the result carry over when the assumptions are weakened in the directions the experimental literature has identified as empirically relevant?
                </p>
                <p>
                    One of those assumptions is weakened by the setting rather than by the participant, and the present paper does not repair it: real performance costs effort, and at a positive performance cost <Math>{"c"}</Math> a seller facing a withholding buyer compares <Math>{"-2G - c"}</Math> with <Math>{"-2G"}</Math>, so the seller-side weak dominance the analysis below takes as its baseline does not survive unchanged. The derivation cited above states this as an exactness caveat in its &sect;3.1 and, in its &sect;4.1, establishes what does survive a positive cost in the two-party game: for a cost below the payment plus the seller&rsquo;s bond (<Math>{"c < P + 2G"}</Math>) cooperation is still the unique profile surviving iterated elimination, now in two rounds, and a seller assigning probability <Math>{"p"}</Math> to the buyer resolving finds cooperation a best response iff <Math>{"p \\geq c/(P + 2G)"}</Math>. That two-round repair does not extend to the <Math>{"N"}</Math>-party extension cited above: the <Math>{"N"}</Math>-party equilibrium is claimed at <Math>{"c = 0"}</Math>. What is not restored is dominance strength &mdash; and dominance strength is what Claim 2&rsquo;s dominance-solvability argument rests on, and what Claim 4&rsquo;s legibility claim rests on in turn, since a belief threshold is a harder cognitive object than a single comparison, however low the threshold sits. The behavioral claims that follow &mdash; loss aversion at the kink, the non-transfer of the coordination-failure finding, peer-pressure activation, incentive legibility &mdash; are claims about how participants read the <Math>{"c = 0"}</Math> payoff structure, and they inherit its limitation rather than removing it.
                </p>
            ...
            <PaperSection title="6. Incentive Legibility and Interface Cognition">
                ...
                <PaperRun title="Scope: the level-1 claim is exact at c = 0.">
                    The comparison above is the <Math>{"c = 0"}</Math> comparison, and the qualification &sect;1 states for the whole paper binds here specifically. At a positive performance cost <Math>{"c"}</Math>, the buyer-defection row is no longer a tie: the seller compares <Math>{"-2G - c"}</Math> with <Math>{"-2G"}</Math> and is strictly worse off cooperating, so cooperation stops weakly dominating defection and the certain, belief-free comparison above is no longer available to the seller. What survives at positive cost is the belief-threshold comparison of the <Link href="/papers/asymmetric-bonding" className="text-ink-heading hover:underline">Asymmetric Bonding and Buyer Dominance</Link> positive-cost proposition (&sect;4.1): a seller assigning probability <Math>{"p"}</Math> to the buyer resolving finds cooperation a best response iff <Math>{"p \\geq c/(P + 2G)"}</Math>. That is still level-1 reasoning about a single quantity &mdash; one belief, about how likely the buyer is to resolve, checked against one threshold &mdash; and not the iterated reasoning about counterparty rationality that standard mechanism design demands. But, as &sect;1 states, a belief threshold is a harder cognitive object than a single comparison, however low the threshold sits: the <Math>{"c = 0"}</Math> case asks the participant to compare two certain outcomes and needs no belief about the counterparty at all, while the threshold check asks the participant to form an estimate of <Math>{"p"}</Math> and clear a bar. The honest claim at positive cost is accordingly narrower than the claim above: incentive legibility survives as a single-threshold belief check &mdash; weaker than the dominance argument, and harder than the belief-free comparison it replaces &mdash; but it remains one comparison rather than a multi-step reasoning chain about counterparty rationality.
                </PaperRun>
            ...
            <PaperSection title="7. Conclusion">
                <p>
                    The bonded settlement primitive&rsquo;s full-rationality equilibrium analysis carries over to the behavioral setting: loss aversion sharpens the weak-dominance comparison at the kink rather than blunting it, the Van Huyck coordination failure does not transfer because the per-player game is dominance-solvable, the atomic-resolution externality structure activates the peer-pressure dynamics the joint-liability literature documents through a cleaner channel that does not require the social substrate, and the whole argument is verifiable by the participant with a single comparison at the margin &mdash; exact at zero performance cost; at positive cost the single comparison becomes the single belief-threshold check of &sect;6, one quantity against one bar, weaker than dominance but never an iterated chain.
                </p>
                <p>
