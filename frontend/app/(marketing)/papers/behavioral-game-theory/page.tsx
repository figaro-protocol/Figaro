    ...
    return (
        ...
        >
            <PaperSection title="1. Introduction">
                ...
                <p>
                    The full-rationality equilibrium argument for the bonded primitive (derived in <Link href="/papers/asymmetric-bonding" className="text-ink-heading hover:underline">Asymmetric Bonding and Buyer Dominance</Link>, together with its <Math>{"N"}</Math>-party extension) establishes that cooperation is weakly dominant for both parties in the bonded game, and is the unique profile surviving iterated elimination of weakly dominated strategies. The proof assumes complete information, full rationality, no non-pecuniary preferences, no coordination failure under pessimistic beliefs, and perfect monitoring; what it prices is settlement discipline, not the adjudication of disputed performance, which remains with the forums that compose above the mechanism. The result holds within those assumptions. The present paper asks: how does the result carry over when the assumptions are weakened in the directions the experimental literature has identified as empirically relevant?
                </p>
                <p>
                    One feature of the payoff structure should be named before the behavioral analysis reads it, because a behavioral reader reaches for a production-cost term by reflex and will look for one here. There is none to find. The payoffs are the transfers the mechanism executes &mdash; the bonds it pulls at commitment and the payment it releases at resolution &mdash; so what a seller weighs at its decision node is <Math>{"+P"}</Math> against <Math>{"-2G"}</Math>, and what a buyer weighs at its own is <Math>{"-P"}</Math> against <Math>{"-2P"}</Math>. Effort, inputs, and time are of course real; they are outlays the performing party makes on its own account, revenue in and outlay out of the productive asset behind the wallet, which the mechanism neither meters nor settles, and whose only bearing on participation is whether the wallet still holds enough to bond the commitment it is signing. The behavioral claims that follow &mdash; loss aversion at the kink, the non-transfer of the coordination-failure finding, peer-pressure activation, incentive legibility &mdash; are claims about how participants read that transfer structure. The fourth of them turns on its shape directly: a comparison of two certain amounts is a cheaper cognitive object than any probability weighed against a bar, and the mechanism presents the participant with the former.
                </p>
                ...
                <PaperRun title="What the paper is not.">
                    Every claim below is a prediction, not a finding. The mechanism analyzed here is built and machine-checked &mdash; by its own authors, with no external audit of those checks yet performed &mdash; but pre-launch: no public deployment exists, so no play has been observed on it, and no laboratory replication of the bonded game has been run. The behavioral arguments are therefore transfers from the experimental record on adjacent games &mdash; minimum-effort coordination, public-goods punishment, prospect-theoretic valuation &mdash; to a payoff structure that differs from all of them in the respect the arguments turn on. A reader should treat them as hypotheses with stated structural grounds and an obvious experimental programme, not as results about how participants in this mechanism behave.
                </PaperRun>
            ...
            <PaperSection title="3. Weak Dominance and Loss Aversion at the Kink">
                ...
                <p>
                    How much loss aversion adds to the magnitude depends on the reference point, which prospect theory leaves under-determined, so the argument takes the conservative framing throughout: both outcomes measured from pre-commitment wealth, so the payer&rsquo;s two outcomes are both losses, loss aversion multiplies both and cancels, and only the loss-domain convexity remains. This is the worst case for the argument &mdash; a more natural framing, in which the posted bond is already at risk and defection is the loss that cooperation avoids, would only strengthen the conclusion &mdash; so we develop the conservative case. The disutility of a loss <Math>{"x \\geq 0"}</Math> is approximately <Math>{"\\lambda \\cdot |x|^{\\alpha}"}</Math>, where <Math>{"\\lambda \\in [1.5, 2.5]"}</Math> is the loss-aversion ratio (the asymmetry between gain and loss domains at the reference point) and <Math>{"\\alpha \\approx 0.88"}</Math> is the loss-domain curvature parameter (Tversky &amp; Kahneman, 1992). Cooperation costs <Math>{"\\lambda \\cdot P^{\\alpha}"}</Math> and defection <Math>{"\\lambda \\cdot (2P)^{\\alpha} = 2^{\\alpha} \\cdot \\lambda \\cdot P^{\\alpha}"}</Math>. Even with <Math>{"\\lambda"}</Math> cancelling, <Math>{"2^{\\alpha} \\approx 2^{0.88} \\approx 1.84"}</Math> places the full-bond forfeiture roughly 84% above the net flow, and <Math>{"2^{\\alpha} > 1"}</Math> for any <Math>{"\\alpha > 0"}</Math>, so forfeiture is strictly more aversive for any feasible curvature parameter the empirical literature has documented. The magnitude does the work; loss aversion only adds to it.
                </p>
            ...
            <PaperSection title="5. Atomic Resolution as Peer Pressure">
                ...
                <PaperRun title="The social-substrate analogue.">
                    Joint-liability microfinance (Ghatak, 1999; Stiglitz, 1990) and the empirical literature on the Grameen Bank report substantially lower default under joint liability than under comparable unsecured lending. The figure usually quoted for Grameen &mdash; a default rate of roughly two per cent, taken from the lender&rsquo;s own reporting (Yunus, 1999) &mdash; is contested exactly where it is most often used. Morduch (1999) argues on accounting grounds that loan rescheduling and latitude in the definition of default make the self-reported figure an optimistic reading of the portfolio, and no independently reconciled replacement figure has settled the question since. We therefore do not carry the number into the argument at any point. What we take from this literature is the finding that survives its accounting dispute: joint liability lowers default relative to comparable unsecured lending, and the contemporary literature attributes whatever part of the reduction is real to some combination of peer-monitoring activation, social-sanction technology, and reputation effects. It is the mechanism, and not the magnitude, that does work below.
                </PaperRun>
                <p>
                    The comparison that carries weight is structural rather than numerical. The bonded mechanism reproduces the cooperation-pressure component of the joint-liability peer-enforcement equilibrium under strictly weaker assumptions: no repeated interaction, no local information, no exogenous punishment technology, no joint-liability contracting. The behavioral reading of the same result is that the social-pressure mechanism the microfinance literature documents is, in the bonded setting, constituted directly by the bonding rule rather than mediated through the social substrate. The bonded mechanism is therefore not merely <em>equivalent</em> to joint-liability microfinance in equilibrium; it produces the social-pressure dynamics through a structurally cleaner channel that does not require the social substrate to be in place.
                </p>
                <PaperRun title="What the comparison licenses, and what it does not.">
                    The Grameen comparison licenses a direction and not a magnitude. Even were the headline figure uncontested it would not transfer as a number, since the settings differ in the respects the pressure runs on; contested as it is, it could not serve as a benchmark in any case. What does transfer is an ordering, and three structural margins set it: the externality magnitude scales with chain depth (deeper sellers face larger co-seller-defection costs); information availability is higher (every other seller&rsquo;s bond exposure is on chain rather than inferred from a village&rsquo;s local knowledge); and the punishment technology is mechanical (the bonding rule rather than social sanction). Together they say that the peer-pressure channel should be expected to be at least as strong here as in the settings the microfinance literature studies. They do not say what default rate to expect, and we predict none.
                </PaperRun>
            </PaperSection>
            ...
            <PaperSection title="6. Incentive Legibility and Interface Cognition">
                ...
                <PaperRun title="What the comparison does not require.">
                    The strength of the legibility claim is the shortness of the list of things the comparison contains, so it is worth stating what is absent from it. The seller&rsquo;s node holds two certain amounts, <Math>{"+P"}</Math> against <Math>{"-2G"}</Math>, both of them transfers the mechanism itself executes and both readable off the commitment before it is signed; the buyer&rsquo;s node after delivery holds <Math>{"-P"}</Math> against <Math>{"-2P"}</Math> on the same footing. No probability enters: neither amount is weighted by how likely the counterparty is to play one way or the other, so the participant has no estimate to form and no bar to clear. No private accounting enters either: what performing consumes in effort, inputs, or time is an outlay on the participant&rsquo;s own account and is not a term at the node, so the participant is not asked to price their own performance before they can tell which branch is better. And no model of the counterparty&rsquo;s reasoning enters &mdash; only the mechanism&rsquo;s arithmetic, which is public. The one premise in the underlying derivation that is not arithmetic &mdash; a buyer equal in transfers either way takes the path on which the delivery arrives &mdash; does its work off-path, at the buyer&rsquo;s node after non-delivery, and never enters the comparison a performing participant faces. A comparison of two certain quantities is the cheapest cognitive object a mechanism can put in front of a participant; a belief weighed against a threshold, which is what a mechanism carrying a private-cost term would have to present, is strictly dearer. The bonded mechanism presents the cheap object, and presents the same one at every position of the chain, with only the magnitudes changing.
                </PaperRun>
            ...
            <PaperSection title="7. Conclusion">
                <p>
                    The bonded settlement primitive&rsquo;s full-rationality equilibrium analysis carries over to the behavioral setting: loss aversion sharpens the weak-dominance comparison at the kink rather than blunting it, the Van Huyck coordination failure does not transfer because the per-player game is dominance-solvable, the atomic-resolution externality structure activates the peer-pressure dynamics the joint-liability literature documents through a cleaner channel that does not require the social substrate, and the whole argument is verifiable by the participant with a single comparison at the margin between two certain amounts &mdash; a comparison that asks for no belief about the counterparty, no accounting of what performance costs the participant, and no step of reasoning about how the counterparty reasons.
                </p>
                <p>
