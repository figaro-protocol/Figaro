    ...
    return (
        <PaperLayout
            ...
            abstract={
                <>
                    ...
                    <p>
                        The paper develops four claims. First, the primitive&rsquo;s reliance on weak dominance &mdash; not strict dominance &mdash; is more behaviorally robust than it might appear, because the operationally-relevant choice is a single comparison at the margin &mdash; recover the bond or forfeit it &mdash; and asymmetric bonding makes the stake the entire bond, twice the transaction value. The magnitude alone makes defection strongly aversive; loss aversion only reinforces it, and by how much depends on a reference point prospect theory leaves under-determined, so the argument does not lean on it. The behavioral prediction matches the rational-choice prediction where it matters. Second, the Van Huyck&ndash;Battalio&ndash;Beil coordination-failure result, which appears to threaten the multi-party reading of the architecture, does not transfer because the per-player game in the bonded setting is dominance-solvable in a way the experimental weakest-link games are not. Third, the externality structure of atomic resolution produces peer pressure that is both economically real (the mechanism-design analytical result) and behaviorally activated (the experimental literature on social-pressure responses). Fourth, the architecture&rsquo;s <em>incentive legibility</em> &mdash; the property that participants can verify the equilibrium argument with a single comparison rather than through iterated reasoning about counterparty rationality &mdash; makes it unusually well-suited to the cognitive constraints of real participants.
                    </p>
                    <p>
                        The paper closes with the empirical questions the architecture invites: experimental measurement of cooperation rates relative to controls without bonded backing, behavioral measurement of the weakest-link pressure activation across process depths, and field measurement of the cognitive load the architecture imposes relative to the alternatives it is intended to displace.
                    </p>
                </>
            }
            references={
        ...
        >
            <PaperSection title="1. Introduction">
                ...
                <p>
                    First, the bond&rsquo;s reliance on weak rather than strict dominance is more robust than it appears, because the strict-case gap is the entire bond &mdash; and asymmetric bonding (Mechanism 1) sets that stake at twice the transaction value. The magnitude alone makes defection strongly aversive; loss aversion reinforces it under the natural framing in which a locked bond is at risk. This is the behavioral counterpart of the magnitude axis; the breadth axis &mdash; coordination across the multi-party cohort, treated in Sections 4 and 5 &mdash; is the work of buyer dominance with atomic resolution (Mechanism 2).
                </p>
                <p>
            ...
            <PaperSection title="3. Weak Dominance and Loss Aversion at the Kink">
                ...
                <PaperRun title="The gap is the entire bond.">
                    The strict-inequality case occurs when the counterparty cooperates: cooperation recovers the bond and settles the net flow (<Math>{"-P"}</Math> for the buyer, <Math>{"+P"}</Math> for the seller), while defection forfeits the bond entirely (<Math>{"-2P"}</Math> for the buyer, <Math>{"-2G"}</Math> for the seller). The gap between the two outcomes is the whole bond: <Math>{"P"}</Math> for the buyer, <Math>{"P + 2G"}</Math> for the seller, the latter scaling with chain depth via the cumulative-value accumulator. That magnitude is the work of asymmetric bonding (Mechanism 1): it sets the stake at twice the transaction value, so the strict-case gap is large in absolute terms and large relative to the natural reference point, the bond posted at commitment.
                </PaperRun>
                <p>
            ...
            <PaperSection title="4. The Van Huyck Coordination Failure Does Not Transfer">
                ...
                <PaperRun title="What this leaves open.">
            ...
            <PaperSection title="5. Atomic Resolution as Peer Pressure">
                ...
                <PaperRun title="The social-substrate analogue.">
                    Joint-liability microfinance (Ghatak, 1999; Stiglitz, 1990) and the empirical literature on the Grameen Bank document a substantial reduction in reported default rates under joint-liability lending. Yunus (1999) reports a default rate of approximately 2% under the joint-liability regime, compared to industry-conventional 20% benchmarks for unsecured rural lending; Morduch (1999) contests the headline figures on accounting grounds, arguing that loan rescheduling and definition-of-default ambiguity make the self-reported 2% figure optimistic. We treat the <Math>{"20\\% \\to 2\\%"}</Math> comparison as a stylized benchmark of the joint-liability literature&rsquo;s headline finding rather than as a peer-reviewed empirical estimate; the contemporary literature attributes whatever reduction is real to a combination of social peer-monitoring activation, social-sanction technology, and reputation effects.
                </PaperRun>
                <p>
                    The reduction-from-microfinance result shows that the bonded mechanism reproduces the equilibrium peer-pressure outcome of joint-liability microfinance under strictly weaker assumptions: no repeated interaction, no local information, no exogenous punishment technology, no joint-liability contracting. The behavioral reading of the same result is that the social-pressure mechanism the microfinance literature documents is, in the bonded setting, constituted directly by the bonding rule rather than mediated through the social substrate. The bonded mechanism is therefore not merely <em>equivalent</em> to joint-liability microfinance in equilibrium; it produces the social-pressure dynamics through a structurally cleaner channel that does not require the social substrate to be in place.
                </p>
                <PaperRun title="Empirical extrapolation.">
                    Read as a stylized upper-bound benchmark for what joint-liability-style peer pressure has been claimed to achieve, the Grameen <Math>{"20\\% \\to 2\\%"}</Math> comparison gives an indication of the order of magnitude the bonded mechanism&rsquo;s peer-pressure activation might support. Three structural margins favor the bonded mechanism over the Grameen setting: the externality magnitude scales with chain depth (deeper sellers face larger co-seller-defection costs); information availability is higher (every other seller&rsquo;s bond exposure is on chain); and the punishment technology is mechanical (the bonding rule rather than social sanction). We do not claim a specific quantitative prediction. We note that the architectural features that produced whatever effect Grameen achieves are present, in the bonded mechanism, in strengthened form, and that the Morduch-style accounting concerns about Grameen&rsquo;s headline figures do not apply in the same way to a mechanism whose default and resolution events are on-chain and unambiguous.
                </PaperRun>
            </PaperSection>

            <PaperSection title="6. Incentive Legibility">
                <p>
                ...
                <p>
                    The argument&rsquo;s robustness follows: a participant who can perform level-1 reasoning &mdash; which is the floor of strategic reasoning in any documented adult population &mdash; has all the cognitive machinery the bonded mechanism requires. The mechanism does not ask the participant to compute the iterated elimination or to verify common knowledge of payoffs; it asks them to compare two outcomes at the margin. The dominance argument the mechanism-design treatment formalizes corresponds, behaviorally, to the comparison the participant naturally performs.
                </p>
            ...
            <PaperSection title="7. Interface Cognition">
                <p>
                    The architecture&rsquo;s behavioral properties also depend on the <em>interface</em> through which participants interact with the mechanism. A clean equilibrium argument can be defeated by an interface that obscures the relevant choice or surfaces irrelevant information. We treat interface cognition briefly because it is where the architecture meets real participants.
                </p>
                <PaperRun title="What the architecture asks the participant to think about.">
                    The participant in a bonded commitment is asked to attend to four things at the moment of commitment: (i) the goods or services being exchanged, (ii) the price, (iii) the bond posture (which is mechanically determined by the price and the cumulative-value context), and (iv) the resolution path (which is the buyer&rsquo;s signature plus any clauses the parties have agreed). The mechanism&rsquo;s invariants are encoded in the structure rather than left to the participant&rsquo;s verification: the bonding rule produces <Math>{"2P"}</Math> and <Math>{"2G"}</Math> automatically, the buyer-dominance rule constrains who can resolve, and the atomic-resolution rule constrains how resolution proceeds.
                </PaperRun>
                <p>
                    The participant does not need to verify the mechanism; the mechanism enforces itself. The participant&rsquo;s cognitive task is limited to the substantive commercial decision (is this exchange worth this price?) plus the bond-feasibility decision (is the bond within my treasury capacity?). Both are routine commercial decisions that participants are well-equipped to perform.
                </p>
                <PaperRun title="Comparison with the platform-mediated alternative.">
                    A participant in a contemporary platform-mediated commerce arrangement is typically asked to attend to: the goods or services, the price, the platform&rsquo;s terms-of-service, the platform&rsquo;s complaint-resolution process, the platform&rsquo;s recommendation-signal mechanics, the platform&rsquo;s reputation system, the platform&rsquo;s payment-flow timing, and the platform&rsquo;s jurisdictional-coverage rules. Most of these are not substantive commercial decisions; they are infrastructural-property verifications the platform asks the participant to perform on its behalf. The cognitive load is correspondingly higher.
                </PaperRun>
                <p>
                    The bonded architecture&rsquo;s interface cognition is structurally lower because the architectural properties are enforced rather than verified. A participant does not need to read the terms-of-service of a permissionless protocol; the protocol&rsquo;s behavior is its terms-of-service, and the protocol cannot violate its terms because the terms are encoded in the bytecode. This shifts the participant&rsquo;s cognitive load from infrastructural verification to substantive commercial decision-making, which is the right direction for a coordination architecture.
                </p>
                <PaperRun title="The risk of over-loading the interface.">
                    A counter-argument is that the bonded architecture&rsquo;s interface may over-load participants who are unfamiliar with cryptographic key management, on-chain transactions, gas estimation, and similar substrate-tier concerns. This is a real concern at the runtime tier and is the work of the wallet, signing, and onboarding infrastructure rather than of the kernel itself. The kernel is incentive-legible; the runtime tier may or may not be, depending on how it presents the kernel to participants. Interface design at the runtime tier is therefore where the architecture&rsquo;s behavioral promises are kept or broken in practice. The kernel makes the promises tractable; the runtime delivers them.
                </PaperRun>
            </PaperSection>

            <PaperSection title="8. Open Empirical Questions">
                <p>
                    The architecture&rsquo;s behavioral analysis above is theoretical. The experimental questions it invites are several.
                </p>
                <PaperRun title="Bilateral cooperation rates.">
                    The first-order empirical question is whether participants in a bonded bilateral commitment in fact converge on cooperation at rates significantly higher than the conventional contractual baseline. The experimental design is a controlled bilateral exchange where one treatment uses bonded commitment and the control uses conventional contract; the dependent variable is the cooperation rate. We predict significantly higher cooperation under the bonded treatment, with the loss-aversion-amplified kink producing the bulk of the effect.
                </PaperRun>
                <PaperRun title="Multi-party coordination success.">
                    The second-order question is whether <Math>{"N"}</Math>-party processes resolve successfully at rates that match the dominance-solvability prediction. The Van Huyck literature establishes that conventional multi-party coordination decays with <Math>{"N"}</Math>; the present paper&rsquo;s argument is that the bonded mechanism does not exhibit this decay because the per-player game is dominance-solvable. An experiment varying <Math>{"N"}</Math> across treatments would test the prediction directly.
                </PaperRun>
                <PaperRun title="Peer-pressure activation.">
                    The third-order question is whether the externality structure of atomic resolution is behaviorally activated as peer pressure. The experimental design measures whether participants in an <Math>{"N"}</Math>-party bonded process modify their behavior in response to other participants&rsquo; visible bond exposure &mdash; whether peer monitoring activates at the rates the externality magnitude would predict. The Grameen literature provides the upper-bound benchmark; we expect the bonded mechanism&rsquo;s activation to fall between the Grameen rate and the conventional-contract rate, with the position depending on how much the social-substrate channel contributed to Grameen relative to the joint-liability channel.
                </PaperRun>
                <PaperRun title="Cognitive load measurement.">
                    The fourth-order question is the architecture&rsquo;s interface cognition relative to the alternatives. The experimental design measures cognitive load (response time, error rate, self-reported difficulty) across treatments where participants interact with bonded commerce, platform-mediated commerce, and direct off-platform commerce. We predict the bonded treatment&rsquo;s load is intermediate between platform-mediated (highest, due to infrastructural verification) and direct off-platform (lowest, due to absence of architectural superstructure). Whether the architecture&rsquo;s load is significantly lower than platform-mediated in operational deployment is the question that determines the adoption story.
                </PaperRun>
                <PaperRun title="Field measurements.">
                    Beyond the laboratory, field measurements of the architecture&rsquo;s operational performance in deployments &mdash; an air-service assembly, a container-shipping assembly, the local-commerce reference assembly &mdash; will produce the binding evidence on whether the behavioral predictions hold in operational settings. Laboratory results are suggestive; field results are dispositive.
                </PaperRun>
            </PaperSection>
