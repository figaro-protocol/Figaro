    ...
    return (
        ...
        >
            <PaperSection title="1. Introduction">
                ...
                <p>
                    One of those assumptions is weakened by the setting rather than by the participant, and the present paper does not repair it: real performance costs effort, and at a positive performance cost <Math>{"c"}</Math> a seller facing a withholding buyer compares <Math>{"-2G - c"}</Math> with <Math>{"-2G"}</Math>, so the seller-side weak dominance the analysis below takes as its baseline does not survive unchanged. The derivation cited above states this as an exactness caveat in its &sect;3.1 and establishes no positive-cost version of the result. The behavioral claims that follow &mdash; loss aversion at the kink, the non-transfer of the coordination-failure finding, peer-pressure activation, incentive legibility &mdash; are claims about how participants read the <Math>{"c = 0"}</Math> payoff structure, and they inherit its limitation rather than removing it.
                </p>
