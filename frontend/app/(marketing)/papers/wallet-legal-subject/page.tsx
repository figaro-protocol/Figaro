        ...
        <PaperLayout slug="wallet-legal-subject"
            ...
            abstract={
                <>
                    ...
                    <p>
                        This paper argues that the bonded settlement primitive changes the doctrinal terrain by making subordination architecturally optional in a region of transactions. Within that region, the parties are bonded counterparties; there is no employment relation to classify. The wallet collapses a distinction the Roman legal tradition kept separate: <em>res</em> (property) and <em>persona</em> (legal party). Every productive asset is simultaneously property and party &mdash; self-owned rather than owned or rented.
                    </p>
                    <p>
                        We develop the argument in three moves. First, we locate subordination as the doctrinal variable in the long-running labor-classification debate, drawing on Anderson&rsquo;s analysis of the firm as private government (2017), Ellerman&rsquo;s argument that the employment contract renews the master&ndash;servant relation in softer form (1992), and Taleb&rsquo;s compressed version of the same point in <em>Skin in the Game</em> (2018). Second, we show that the gig-economy case law is a classification dispute whose ground narrows once subordination ceases to be architecturally required. Third, we name the trade-off: Figaro is a leveller on the subordination axis but simultaneously a privatizer of employment-attached protections (unemployment insurance, healthcare, pensions, collective bargaining). The levelling claim carries a limit we state here rather than at the end, because it bounds everything above it: bonding requires a stake, so the primitive levels only those who have one. A worker with no capital to bond is not freed by the substrate; they remain subordinated by unequal access to the stake rather than by directional control, and how that access is distributed is, in our view, the most important policy question the primitive raises.
                    </p>
                </>
            }
            references={
        ...
        >
            ...
            <PaperSection title="2. Subordination as the Labor-Law Variable">
                ...
                <PaperSubsection title="2.2 The Philosophical Position">
                    <p>
                        A scholarly tradition with roots in classical liberalism and in Marx, refreshed more recently by legal and political philosophers, reads the employment relationship as lying on the same continuum as older forms of property-in-persons. We summarize three representative positions.
                    </p>
            ...
            <PaperSection title="3. The Settlement Primitive">
                <p>
                    We treat the settlement primitive as a black box and summarize only the features relevant to the legal argument.
                </p>
                <p>
                    The primitive composes two mechanisms whose institutional work the present argument relies on, derived in <Link href="/papers/asymmetric-bonding" className="text-ink-heading hover:underline">Asymmetric Bonding and Buyer Dominance</Link>, which establishes the bilateral equilibrium and its extension along a process chain. <em>Asymmetric bonding</em> &mdash; each party bonds against its own performance &mdash; makes defection more expensive than cooperation at every bonded edge, and scales the bilateral case into a many-party mesh of independently secured commitments. <em>Buyer dominance with atomic resolution</em> &mdash; only the buyer resolves, and all orders settle together or not at all &mdash; makes that mesh resolvable without a firm-shaped nucleus. The labor-law argument below depends on both: without the first the bilateral counterparties have no self-enforcing escrow; without the second the multi-party fulfillment workflow would still require an organizing nucleus to clear all parties simultaneously, and that nucleus would inherit the subordination apparatus the primitive is supposed to retire. The self-enforcement the bonds supply is of settlement discipline under perfect monitoring; an honest dispute over whether performance met the agreement is adjudicated not by the bonds but by the forums composed above the kernel.
                </p>
                <PaperRun title="Bonded counterparties.">
                    A transaction between two parties is consummated by commitments cryptographically signed by both. Each party signs from their own wallet. Each party locks a bond against its own performance &mdash; the buyer against the payment it has promised, the seller against the cumulative value already bonded upstream in the process. Only the buyer can trigger resolution, and resolution settles every active order in the process atomically. Defection is structurally more expensive than cooperation. There is no platform between the parties, no arbitrator authorized to resolve disputes on-chain, and no admin key permitting external override.
                </PaperRun>
            ...
            <PaperSection title="4. The Wallet as Legal Subject">
                ...
                <ol className="list-[lower-alpha] pl-6 space-y-3">
                    ...
                    <li><strong>Attestation is self-attached.</strong> Attestations and profile records attach to the wallet itself, and richer deeds and attestations can be layered on top. A worker&rsquo;s skills, certifications, and performance history travel with the wallet, not with the employer. The resume becomes the wallet; the reference becomes an attestation.</li>
                </ol>
                <PaperRun title="A fourth consequence: the wallet is self-accounting.">
                    The Roman private-law inheritance keeps <em>res</em> and <em>persona</em> separate not only from each other but from the <em>liber rationum</em> &mdash; the book of account in which the entity&rsquo;s dealings are recorded. The wallet collapses this third category too: its settlement record is constituted by the protocol itself rather than maintained by any party, so the worker-as-wallet carries its own continuously updated book of performance, attested capability, and settled compensation. This removes one traditional asymmetry between employer and employee &mdash; the employer keeps the books, the employee does not &mdash; by making the books symmetrically readable.
                </PaperRun>
            </PaperSection>

            <PaperSection title="5. The Gig-Economy Cases, Re-read">
                <p>
                    The gig-economy case law can be read as a series of attempts to place the employee&ndash;contractor threshold in the face of a technology (the platform) that had engineered a new position on the subordination continuum. Re-read through the primitive&rsquo;s lens, four landmark measures share a fate within the region where a bonded relation is genuinely, not merely nominally, structured as one: each one&rsquo;s operative test loses its subject. The qualification is not decorative &mdash; a bonded counterparty who in substance still works a single schedule for a single buyer under buyer-dictated flow control remains reachable by the corresponding doctrine, for the reasons given after the list.
                </p>
                <ul className="list-disc pl-6 space-y-2">
                    <li><strong>California AB5 (2019)</strong>, codifying the ABC test from <em>Dynamex Operations West v Superior Court</em> (2018): none of the three prongs is well-posed for a bonded relation that is substantively, not just formally, bonded. There is no hiring entity to direct and control (prong A has no subject), no usual course of business for the bonded counterparty relation (prong B presupposes a firm), and the bonded party is by construction independently established (prong C is satisfied by default).</li>
                    <li><strong>Proposition 22 (2020)</strong>: the statutory carve-out for app-based drivers is unnecessary because the category it carves out of does not apply. The question it tries to answer &mdash; minimum earnings and healthcare outside the employment relation &mdash; is the leveller/privatizer trade-off of Section 6.</li>
                    <li><strong><em>Uber BV v Aslam</em> (UKSC 2021)</strong>: the substance-over-label examination that classified drivers as <em>workers</em> has no entity whose substance can be examined when the substance itself has changed &mdash; when settlement runs commitment-by-commitment to a wallet the worker controls rather than through a rating-and-algorithmic-direction apparatus. Where that apparatus is reproduced in a surrounding layer built on top of the primitive (see below), the <em>Aslam</em> analysis regains its subject and reaches it in the ordinary way.</li>
                    <li><strong>EU Platform Work Directive (2024)</strong>, the most sophisticated of the four, with a presumption of employment rather than a bright-line test: the presumption criteria (algorithmic management, supervision of work, restrictions on working for others) have no referent at the settlement layer itself &mdash; there is no platform between the bonded parties whose algorithmic management would trigger the presumption, though an intermediary reintroducing that management above the settlement layer remains squarely within the Directive&rsquo;s presumption.</li>
                </ul>
                <PaperRun title="Scope of the re-reading.">
                    Two qualifications bound all four readings, and we state them together rather than as sequential retreats. First, the doctrines above were built to reach behind contractual labels to operational reality, and they still do: a bonded counterparty who performs forty hours a week for a single buyer on a buyer-dictated schedule, with rating-driven flow control producing economic dependence, is not rescued from substance-over-form analysis merely because the commitments are denominated as bonded orders rather than as an employment contract. What the primitive changes is the choice set available to structure a productive relationship, not the doctrine&rsquo;s reach within any relationship that remains, in substance, employment-shaped. Second, the settlement layer&rsquo;s freedom from direction-and-control does not extend automatically to the layers built on top of it &mdash; discoverability, dispatch, reputation aggregation, credit provision, identity vouching, key custody. An intermediary supplying those layers at scale can recreate a platform&rsquo;s economic position relative to the bonded counterparties beneath it, and the substance-over-form doctrines will have exactly the purchase against that intermediary that they have against existing platforms today. The architectural claim is therefore narrower than &ldquo;subordination is impossible&rdquo;: subordination is no longer architecturally required at settlement, and where it reappears it appears as a choice made in a surrounding layer the existing labor-law apparatus is well-equipped to evaluate.
                </PaperRun>
            </PaperSection>
            ...
            <PaperSection title="6. Leveller and Privatizer">
                ...
                <PaperRun title="Four options for the political response.">
                    We name them so the trade-off is legible:
                </PaperRun>
                ...
                <PaperRun title="Voluntary participation as a scope condition.">
                    The mechanism&rsquo;s equilibrium results hold on the assumption of agency and a stake alone &mdash; that each party can choose its own actions and controls a stake sufficient to post the bond. The results are jurisdiction-invariant: they invoke no legal freedom, status, or standing, only the ability to sign and to bond. For a labor-law analysis the stake half of that assumption is load-bearing in a way it is not for the mechanism analysis itself. A worker whose only realistic income path runs through bonded assemblies, and who lacks access to a stake, faces a choice set that is not substantively voluntary in the sense the equilibrium argument presupposes. Such a worker is not freed by the substrate change; they remain in a subordinated position the substrate equally well supports &mdash; one of unequal access to the stake rather than of directional control. The primitive is symmetric in form and asymmetric in the access conditions that make use of the form possible.
                </PaperRun>
                <p>
                    The leveller register therefore applies fully only to the participants for whom the bonding option is substantively available &mdash; and how that availability is itself distributed is, in our view, the most important policy question the primitive raises. Of the four options above, (ii) addresses the access asymmetry directly by uncoupling productive participation from social-protection access; (i) and (iii) leave the asymmetry where the historical employment relation left it and propose that the bonded counterparty purchase protections from earnings; (iv) treats the asymmetric-access region as the residual-employment region and accepts the partition. The choice among them is the question on which the leveller-versus-privatizer reading ultimately turns.
                </p>
            </PaperSection>

            <PaperSection title="7. Conclusion">
                <p>
                    Labor law has treated subordination as an axis on which to place threshold after threshold, in each jurisdiction against each technology. The bonded settlement primitive does not move the threshold; it makes subordination architecturally optional in a region of transactions. Within that region, the employee&ndash;contractor line, the platform-worker line, the <em>worker</em> category, the <em>subordonné</em> test &mdash; all of which presuppose a directional control relation between two parties &mdash; find no relation to classify. The classification problem has lost its subject.
                </p>
                <PaperRun title="A note on vocabulary.">
                    The vocabulary of &ldquo;subordination&rdquo; and &ldquo;slavery,&rdquo; borrowed from Anderson and Ellerman, is structural rather than moral: modern labor law has developed substantial protections that distinguish the employee from the serf and the serf from the slave, and the primitive removes the arrangement, not the moral progress the arrangement&rsquo;s softening represented. Whether the removal is desirable is the policy question named in Section 6; whether it is happening is an engineering one; the two should not be conflated. Legal scholarship will be better served by engaging the primitive on its own terms than by treating it as one more gig-economy platform to be reclassified under the existing test.
                </PaperRun>
            </PaperSection>
        </PaperLayout>
    );
