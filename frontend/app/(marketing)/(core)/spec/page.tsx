        ...
        rows: [
            {
                ...
                cause: "msg.value is not EXACTLY registrationDeposit(). Over-paying reverts too: there is no admin and no sweep, so excess ETH would be trapped forever.",
    ...
    },
    {
        ...
        rows: [
            ...
            },
            {
                ...
                from: "UsageCounter",
                cause: "That key is in the counter's excluded set. Of the three mandatory clauses, commerce and topology EARN — they ride on every order and are scored for their designer of record. The reference deployments exclude exactly one key, figaro-assembly-provenance: it is attribution plumbing, and scoring it would double-reward every assembly trade (its designer accrues through recordAssemblyUsage instead). ROUTINE where it appears, not a fault.",
        ...
        <>
            ...
            <MarketingSection title="Inheritance" sectionId="inheritance">
                <p className="text-base text-ink-body leading-relaxed mb-3">
                    This page catalogues the <strong>on-chain composition</strong> layer (the kernel plus the permissionless primitives built around it). Each contract below inherits the kernel&apos;s decentralized, permissionless, tamper-evident, atomic-resolution properties &mdash; the invariants stated on <Link href="/kernel" className="underline">Kernel</Link>. The kernel in turn inherits execution security from whichever EVM chain it is deployed on &mdash; network → kernel → on-chain composition → off-chain composition → trade. Remove any floor and what&apos;s above collapses.
                </p>
...
function attestViaResolver(
    ...
    bytes32 contentRef
            ...
            <MarketingSection title="The sequencer: the batch path&rsquo;s entry point, and the only off-chain piece." sectionId="sequencer">
                <p className="text-base text-ink-body leading-relaxed mb-4">
                    Everything else on this page is a contract. This one is not: a <strong>sequencer</strong> is an off-chain HTTP relay that pools signed operations, assembles a batch, proves it with SP1, and calls <code>settleBatch</code>. It is the ordinary way onto the batch path &mdash; not because the path is gated, but because producing a batch proof is the work it does for you. Who runs one: anyone; the operator with reason to is whoever resolves enough trade for one amortized proof to undercut per-order gas &mdash; a market&apos;s designer, a high-volume seller, a community. Nothing in the protocol pays a sequencer, and <code>settleBatch</code> is permissionless, so a stalled relay is routed around, never waited on.
                </p>
                <p className="text-base text-ink-body leading-relaxed mb-4">
                    <strong>A relay, not an authority.</strong> <code>FigaroBatchVerifier.settleBatch</code> is <code>external</code> with no caller gate, no admin, no charge and no upgrade path &mdash; so a sequencer is one relay among any number, and running your own needs nobody&apos;s permission. It holds no keys of yours and confers no privilege: its own signer pays gas and has no protocol role. Its admission checks call the <em>same</em> kernel functions the proof runs (EIP-712 recovery; the attestation witness gates), so it can reject earlier than the proof and can never accept more. Its honest powers are exactly <strong>censor and delay</strong> &mdash; never forge, never alter a signed struct, never land what you did not sign, never touch a bond. The fallback is always direct <code>FigaroCore</code> submission with the same artifacts.
                </p>
                <p className="text-base text-ink-body leading-relaxed mb-4">
