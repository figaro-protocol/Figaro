        ...
        <>
            ...
            <MarketingSection title="The verification stack" sectionId="verification">
                <p className="text-base text-ink-body leading-relaxed mb-5">
                    What is in place today is a verification stack &mdash; seven independent benches: six on the protocol&apos;s contracts and one on the equilibrium algebra those contracts enforce &mdash; each from a different angle. Seven, and not one, because each catches a class of defect the others structurally cannot. What each class of check covers and what it structurally cannot reach, for the kernel and its two composition surfaces, and the bounds the kernel&rsquo;s model runs under, are the subject of <Link href="/papers/verified-resolution-kernel" className="text-ink-heading font-medium hover:underline">A Verified Resolution Kernel</Link>; the counts on this page are derived from the tree, and the paper names no tool and states no count:
                </p>
                <ul className="space-y-2 text-base text-ink-body mb-5 ml-6">
                    <li>&mdash; <strong className="text-ink-heading font-medium">Foundry</strong>: the unit + integration suite &mdash; 319 test functions. Class: <em>behavioral and regression</em> &mdash; what a contract does on concrete inputs, and what it stops doing after an edit. It proves nothing about the inputs nobody wrote a test for.</li>
                    ...
                    <li>&mdash; <strong className="text-ink-heading font-medium">TLA+</strong>: model-checking of four protocol state machines &mdash; the kernel, the florin token, one machine holding the two resolution paths, and the swap/commit coordinator. Class: <em>temporal and liveness</em> &mdash; ordering, interleaving and races across a whole protocol, and states that can be entered but never left. No per-function bench can see these.</li>
