            ...
            <div className="container mx-auto px-6 pt-8 pb-16 max-w-5xl">
                <div className="mb-6">
                    <p className="text-sm text-ink-body leading-relaxed max-w-2xl">
                        <strong>Forked design.</strong> Pre-populated from <strong>{reference.identity.name}</strong> ({reference.roles.length} roles · {reference.mechanisms.filter((m) => m.enabled).length} active mechanisms). The seed produced the canonical tree (root commitment + sub-orders implied by the assembly&apos;s declarations). Modify it: drag the <span className="inline-block align-middle w-3 h-3 rounded-full border border-neutral-400 bg-white text-[8px] leading-[10px] text-center text-neutral-600">+</span> handle on any node to add a sub-order; click a node to open its agreement drawer (fulfilment, logistics, and every other clause are edited there). Save as a draft (local storage); publishing to the on-chain registry is a follow-up.
                    </p>
                </div>
                <ProcessGraphCanvas
                    orders={orders}
                ...
                <p className="mt-6 text-xs text-ink-muted">
                    Same affordances as <Link href="/builders/designer/new" className="underline">/new</Link>. <strong>Drag</strong> a node&apos;s <span className="inline-block align-middle w-3 h-3 rounded-full border border-neutral-400 bg-white text-[8px] leading-[10px] text-center text-neutral-600">+</span> handle to empty space to add a sub-order, or onto another node to merge it as an additional parent. <strong>Click</strong> any node to open the agreement drawer or to delete it; fulfilment and every other clause are edited in the drawer. The <span className="inline-block align-middle w-3 h-3 rounded-full border border-red-300 bg-white text-red-600 text-[8px] leading-[10px] text-center">×</span> in a node&apos;s top-right deletes that node and any descendants. Payment + currency are committed at runtime, not designed here.
                </p>
