...
const institutionAssemblyPath = process.env.FIGARO_INSTITUTION_ASSEMBLY_PATH
    ? path.resolve(process.env.FIGARO_INSTITUTION_ASSEMBLY_PATH)
    : path.resolve(__dirname, "../lib/shared/institutionAssembly.ts");

function printUsage() {
    console.log(`Usage:
  npm run create:assembly -- --name "Institution Name" --slug institution-slug [options]

Options:
  --name           Required. Institution display name.
  --slug           Required. Kebab-case institution slug.
  ...
  --out            Optional. Output path. Default: lib/shared/assemblies/<slug>.reference.json
    --register       Optional. Register the new assembly in lib/shared/institutionAssembly.ts.
    --unregister     Optional. Remove an existing assembly from lib/shared/institutionAssembly.ts and optionally delete its JSON document.
...
function registerAssembly(slug, dryRun) {
    const source = fs.readFileSync(institutionAssemblyPath, "utf8");
    ...
    if (dryRun) {
        console.log("Would update: lib/shared/institutionAssembly.ts");
        console.log(`Register import: ${importLine}`);
        console.log(`Register constant: ${constantName}`);
        return;
    }

    fs.writeFileSync(institutionAssemblyPath, updated, "utf8");
}
...
function main() {
    ...
    if (args.unregister) {
        ...
        if (!args.dryRun) {
            console.log(`Unregistered ${slug} from lib/shared/institutionAssembly.ts`);
            if (args.deleteFile) {
    ...
    const description = args.description ?? `${name} reference institution assembly.`;
    ...
    if (args.register) {
        console.log("Registered the assembly in lib/shared/institutionAssembly.ts");
    } else {
