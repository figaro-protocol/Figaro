...
describe('runtime identity registry', () => {
    ...
    ])('resolves a runtime subject preview for $assemblySlug', ({ address, displayName, assemblySlug }) => {
    ...
    it('lists assembly-bound subject summaries for local-commerce', () => {
        ...
        expect(summaries[0]?.operatorProfile?.location?.geohash).toBe('dr5reg');
    });
    ...
    it('lists assembly-bound subject summaries for figaro-procurement', () => {
        ...
        expect(summaries[0]?.operatorProfile?.slug).toBe('acme-components-supply');
    });
    ...
    it('lists assembly-bound subject summaries for figaro-disclosure-review', () => {
        ...
        expect(summaries[0]?.operatorProfile?.slug).toBe('greenledger-review-desk');
    });
});