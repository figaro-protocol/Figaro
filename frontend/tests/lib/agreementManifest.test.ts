...
const LINE_ITEMS: AgreementLineItem[] = [
    { itemId: "pizza1", name: "Margherita Pizza", quantity: 2, unitPrice: "0.01" },
    { itemId: "drink1", name: "Soft Drink", quantity: 1, unitPrice: "0.002" },
];
...
const GEO_SECTION: AgreementSection = {
    ...
    data: { originGeohash: "dr5reg", destinationGeohash: "dr5reh", massGrams: 500, volumeMl: 2000 },
};
...
const GHG_SECTION: AgreementSection = {
    schema: "figaro-ghg-disclosure-v1",
    data: { standard: "iso-14064-1", scope: 1 },
};
...
describe("buildAgreement", () => {
    ...
    it("composes a full terms-of-sale agreement", () => {
        ...
        expect(hasSection(a, "figaro-ghg-disclosure-v1")).toBe(true);
...
describe("section accessors", () => {
    it("getSection returns the matching section", () => {
        ...
        const ghg = getSection(a, "figaro-ghg-disclosure-v1");
        expect(ghg).toBeDefined();
        expect(ghg!.data.standard).toBe("iso-14064-1");
    ...
    it("getSection returns undefined for missing schema", () => {
        ...
        expect(getSection(a, "figaro-ghg-disclosure-v1")).toBeUndefined();
    });

    it("hasSection returns correct boolean", () => {
        const a = makeAgreement({ sections: [COMMERCE_SECTION, GEO_SECTION] });
        expect(hasSection(a, "figaro-commerce-v1")).toBe(true);
        expect(hasSection(a, "figaro-ghg-disclosure-v1")).toBe(false);
    });
});
...
describe("sellerCatalogueMetadata schema utilities", () => {
    it("sellerSupportsSchema returns true for declared schemas", async () => {
        ...
        expect(sellerSupportsSchema(SELLER_CATALOGUE_METADATA_EXAMPLE, "figaro-ghg-disclosure-v1")).toBe(true);
    ...
    it("sellerSchemaConfig returns config for declared schema", async () => {
        ...
        const config = sellerSchemaConfig(SELLER_CATALOGUE_METADATA_EXAMPLE, "figaro-ghg-disclosure-v1");
