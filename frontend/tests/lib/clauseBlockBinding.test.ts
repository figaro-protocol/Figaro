...
describe("parseBlockBinding — clause block-binding (frontend presentation slice)", () => {
    ...
    it("rejects an invalid tier", () => {
        const { block } = parse({ tier: "made-up", mechanismKinds: [], moduleIds: [] });
        expect(block).toBeNull();
    });
    ...
    it("rejects an empty-string nestsUnder", () => {
        const { block } = parse({ tier: "cross-checked", mechanismKinds: [], moduleIds: [], nestsUnder: "" });
        expect(block).toBeNull();
    });
    ...
    it("rejects a malformed (non-array) mechanismKinds", () => {
        const { block } = parse({ tier: "runtime", mechanismKinds: "nope" });
        expect(block).toBeNull();
    });
    ...
    it("preserves structural", () => {
        const { block } = parse({ tier: "cross-checked", mechanismKinds: [], moduleIds: [], structural: true });
        expect(block?.structural).toBe(true);
    });

    it("rejects a non-boolean structural", () => {
        const { block } = parse({ tier: "cross-checked", mechanismKinds: [], moduleIds: [], structural: "yes" });
        expect(block).toBeNull();
    });
    ...
    it("preserves defaultOn", () => {
        const { block } = parse({ tier: "cross-checked", mechanismKinds: [], moduleIds: [], defaultOn: true });
        expect(block?.defaultOn).toBe(true);
    });

    it("rejects a non-boolean defaultOn", () => {
        const { block } = parse({ tier: "cross-checked", mechanismKinds: [], moduleIds: [], defaultOn: "yes" });
        expect(block).toBeNull();
    });
    ...
    it("preserves attestation", () => {
        const { block } = parse({ tier: "runtime", mechanismKinds: [], moduleIds: [], attestation: "bilateral" });
        expect(block?.attestation).toBe("bilateral");
    });

    it("rejects an invalid attestation", () => {
        const { block } = parse({ tier: "runtime", mechanismKinds: [], moduleIds: [], attestation: "buyer" });
        expect(block).toBeNull();
    });
    ...
    it("preserves handoffStages", () => {
        const { block } = parse({ tier: "runtime", mechanismKinds: [], moduleIds: [], handoffStages: ["handed-off"] });
