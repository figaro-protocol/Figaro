...
describe('runtime resolution', () => {
    ...
    it('resolves runtime-bound shell metadata from the matched subject', () => {
        ...
        const shellPresentation = resolveInstitutionShellPresentation(
    ...
    it('resolves a runtime skin bundle from the matched subject shell presentation', () => {
        ...
        const skinBundle = resolveInstitutionSkinBundle(shellPresentation);
    ...
    it('hydrates a remote skin bundle and prefers it over seller metadata fallback', async () => {
        ...
        const skinBundle = await resolveInstitutionSkinBundleFromTransport(
            shellPresentation,
            evidenceTransport,
            { fetcher },
        );
    ...
    it('falls back to assembly shell metadata when no bound subject is selected', () => {
        const context = resolveAssemblyRuntimeContext('figaro-procurement', 'local-anvil');
        const shellPresentation = resolveInstitutionShellPresentation(context!.artifact.assembly);
        ...
        expect(shellPresentation.subtitle).toBe(context!.artifact.assembly.narrative?.institutionSummary ?? '');
    ...
    it('resolves a skin bundle from binding asset documents even without seller metadata', () => {
        ...
        expect(resolveInstitutionSkinBundle(shellPresentation)).toEqual(
    ...
    it('throws when a required runtime context is missing', () => {
        expect(() => requireAssemblyRuntimeContext('missing-assembly', 'local-anvil')).toThrow(/Unknown institution assembly/);
    });
});