...
import {
    BUILT_IN_MODULE_IDS,
    BUILT_IN_ASSEMBLY_COMPOSITION_MODULE_IDS,
...
import { SUPPORTED_RENDER_MODULE_SLOTS } from '@/lib/shared/institutionAssemblyValidation';
import {
...
describe('analytical modules in assembly schema', () => {
    ...
    it('the built-in disclosure, attestation, FIG, coordinator, and registry mechanism packages declare their default modules and capability bindings', () => {
        const disclosurePackage = getMechanismPackage('disclosure');
        const attestationPackage = getMechanismPackage('attestation');
        const figPackage = getMechanismPackage('fig');
        ...
        expect(figPackage?.capabilityBindings).toEqual([
            'claim-airdrop',
            'claim-vesting',
        ]);
        expect(figPackage?.modules.map((module) => module.moduleId)).toContain('fig-token');
        expect(figPackage?.hooks).toMatchObject({
            useFigTokenMetrics: expect.any(Function),
            useFigBalance: expect.any(Function),
            useStagedAirdropClaim: expect.any(Function),
            useStagedAirdropClaimed: expect.any(Function),
            useStagedAirdropStage: expect.any(Function),
        });
        ...
        expect(registryPackage?.hooks).toMatchObject({
            ...
            useUpdateOperatorProfile: expect.any(Function),
    ...
    it('catalogue-editor remains an assembly-specific discovery surface rather than a registry package default', () => {
        const eatsArtifact = getInstitutionArtifactBySlug('figaro-eats');
