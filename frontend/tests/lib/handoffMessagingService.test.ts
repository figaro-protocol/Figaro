...
const getHandoffChannelMock = vi.fn();

vi.mock("@/lib/handoff/channel", () => ({
    getHandoffChannel: (...args: unknown[]) => getHandoffChannelMock(...args),
}));

describe("handoffMessagingService", () => {
    beforeEach(() => {
        ...
        getHandoffChannelMock.mockReset();
        getHandoffChannelMock.mockResolvedValue({
    ...
    it("bridges wallet signMessage into coordination channel resolution", async () => {
        ...
        await DEFAULT_HANDOFF_MESSAGING_SERVICE.getChannel({
            address: "0x1234",
            walletClient,
        });

        expect(getHandoffChannelMock).toHaveBeenCalledTimes(1);
        const [, signMessage] = getHandoffChannelMock.mock.calls[0];
    ...
    it("delegates handoff-key sends through the resolved channel", async () => {
        await DEFAULT_HANDOFF_MESSAGING_SERVICE.sendHandoffKey({
            address: "0x1234",
    ...
    it("delegates wrapped-key subscriptions and returns the unsubscribe handle", async () => {
        ...
        const result = await DEFAULT_HANDOFF_MESSAGING_SERVICE.subscribeWrappedKey({
            address: "0x1234",
            orderId: "order-2",
            callback,
        });
    ...
    it("delegates commitment payload sends through the resolved channel", async () => {
        await DEFAULT_HANDOFF_MESSAGING_SERVICE.sendCommitmentPayload({
            address: "0x1234",
            recipientAddress: "0xabcd",
            orderId: "order-3",
            payload: "{\"commitment\":{}}",
    ...
    it("delegates any-commitment subscriptions and returns the unsubscribe handle", async () => {
        ...
        const result = await DEFAULT_HANDOFF_MESSAGING_SERVICE.subscribeAnyCommitmentPayload({
            address: "0x1234",
            callback,
        });
