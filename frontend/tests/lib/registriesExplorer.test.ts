...
const ROWS: ExplorerRow[] = [
    row({ key: "c1", id: "figaro-commerce", name: "Commerce", article: "mandatory", registrar: "0xDAO", blockNumber: 10n, text: "figaro-commerce Commerce mandatory 0xDAO" }),
    row({ key: "c2", id: "figaro-schedule", name: "Schedule", article: "coordination", registrar: "0xFounder", blockNumber: 30n, text: "figaro-schedule Schedule coordination 0xFounder" }),
    row({ key: "c3", id: "figaro-old", name: "Old", article: "coordination", registrar: "0xFounder", blockNumber: 5n, stakeWithdrawn: true, text: "figaro-old Old coordination" }),
    row({ family: "assemblies", key: "a1", id: "asm-1", name: "Equipment hire", registrar: "0xFounder", blockNumber: 40n, clauses: ["figaro-commerce", "figaro-utility-token"], text: "asm-1 Equipment hire figaro-commerce figaro-utility-token" }),
    row({ family: "assemblies", key: "a2", id: "asm-2", name: "Point of sale", registrar: "0xDAO", blockNumber: 20n, clauses: ["figaro-commerce", "figaro-geolocation"], text: "asm-2 Point of sale figaro-commerce figaro-geolocation" }),
    row({ family: "members", key: "m1", id: "0xfounder", name: "Figaro founder", registrar: "0xFounder", blockNumber: 50n, text: "0xfounder Figaro founder" }),
];
...
describe("parseExplorerQuery / serializeExplorerQuery", () => {
    it("defaults to live clauses sorted by article, and tolerates junk", () => {
        ...
        expect(s).toEqual({ family: "clauses", q: "", sort: "article", article: "", registrar: "", stake: "live", clause: "" });
    });
...
describe("selectRows", () => {
    ...
    it("facets: article, registrar (case-insensitive), composing clause, and free text", () => {
        expect(selectRows(ROWS, parseExplorerQuery({ family: "clauses", article: "mandatory" })).map((r) => r.id)).toEqual(["figaro-commerce"]);
        expect(selectRows(ROWS, parseExplorerQuery({ family: "assemblies", registrar: "0xfounder" })).map((r) => r.id)).toEqual(["asm-1"]);
    ...
    it("sorts: block most-recent first, name, registrar", () => {
        ...
        expect(selectRows(ROWS, parseExplorerQuery({ family: "clauses", sort: "registrar", stake: "all" })).map((r) => r.registrar)).toEqual(["0xDAO", "0xFounder", "0xFounder"]);
    });
...
describe("facetValues + explorerBreadcrumb", () => {
    it("derives facet values from the rows, never a fixed list", () => {
        ...
        expect(facetValues(ROWS, "assemblies", "registrar")).toEqual(["0xdao", "0xfounder"]);
    });
    it("builds the deep-link trail from the active facet", () => {
        expect(explorerBreadcrumb(parseExplorerQuery({ family: "clauses", article: "mandatory" })).map((b) => b.label))
            .toEqual(["Builders", "Registries", "Clauses", "mandatory"]);
        expect(explorerBreadcrumb(parseExplorerQuery({ family: "members" })).map((b) => b.label))
            .toEqual(["Builders", "Registries", "Members"]);
    });
});
