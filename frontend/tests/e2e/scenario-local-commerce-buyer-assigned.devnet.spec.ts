...
test.describe('Author + publish the local-commerce-buyer-assigned assembly (devnet)', () => {
    ...
    test('designer canvas authors + publishes local-commerce-buyer-assigned; it persists, anchored + pinned', async ({ page }) => {
        const config = readLocalDeploymentConfig();
        const assemblyRegistry = (process.env.NEXT_PUBLIC_ASSEMBLY_REGISTRY
            ?? config.assemblyRegistry) as Hex;
        const publicClient = createPublicClient({ chain: LOCAL_ANVIL, transport: http(RPC_URL) });

        const slug = SCENARIO_SLUG['local-commerce-buyer-assigned'];
        ...
        if (!(await assemblyAnchored(slug))) {
            ...
            await page.getByTestId('designer-name-input').fill('Local Commerce — Buyer-Assigned');
        ...
        expect(assemblyTemplate.name).toBe('Local Commerce — Buyer-Assigned');
