 ...
 *
 * Authors the `local-commerce-buyer-assigned` reference assembly through the
 * real designer canvas and publishes it on-chain — the buyer-assigned
 * coordination variant of `local-commerce`.
 *
 * The two-node shape is identical to `scenario-local-commerce`; the only
 * difference is the courier coordination — `buyer-assigned` (the buyer
...
test.describe('Author + publish the local-commerce-buyer-assigned assembly (devnet)', () => {
    ...
    test('designer canvas authors a two-node buyer-assigned delivery assembly and publishes it', async ({ page }) => {
        const config = readLocalDeploymentConfig();
        const assemblyRegistry = (process.env.NEXT_PUBLIC_ASSEMBLY_REGISTRY
            ?? config.assemblyRegistry) as Hex;

        const draftName = `Local Commerce Buyer-Assigned ${Date.now()}`;
        ...
        // ── Root order: delivery modality + buyer-assigned coordination ──
        ...
        // Coordination is required once `delivery` is offered. buyer-assigned
        // = the buyer chooses the courier at checkout.
        await page.getByTestId('drawer-fulfilment-coordination-buyer-assigned').click();
