 ...
 * splits its own receipts onward through the composed public multisender
 * (Disperse, `0xD152…2150` — the same canonical, ownerless contract on Sepolia
 * and mainnet; the devnet composes `MockDisperse`, its interface mirror). This
 * is a composition with a LIVE external contract, so — the SP1-gateway lesson —
 * it is proved by BEHAVIOUR on the public chain, not by the mirror alone.
 *
...
test.describe('PAYOUT ROUTING — a settled seller routes receipts through the composed public multisender', () => {
    ...
    test('two earmarked legs, one atomic disperse — each leg lands exactly, the seller pays exactly the total', async ({}, testInfo) => {
        ...
        // ── The legs: earmarked recipients (real wallets, so the testnet USDC stays ours) ──
        const recipients: [Hex, Hex] = E2E_CHAIN === 'sepolia'
            ? [buyer.address, (process.env.FOUNDER_WALLET as Hex | undefined) ?? buyer.address]
            : [ANVIL_ACCOUNTS[7] as Hex, ANVIL_ACCOUNTS[8] as Hex];
        ...
        testInfo.annotations.push({ type: 'legs', description: `process=${processId} multisender=${multisender} legs=${formatUnits(legs[0], decimals)}+${formatUnits(legs[1], decimals)} ${symbol} → ${recipients.join(', ')}` });
        ...
        try {
            ...
            if (recipients[0].toLowerCase() === recipients[1].toLowerCase()) {
                expect(r0After - r0Before, 'the single recipient received both legs exactly').toBe(total);
            } else {
                expect(r0After - r0Before, 'leg 0 arrived exactly').toBe(legs[0]);
                expect(r1After - r1Before, 'leg 1 arrived exactly').toBe(legs[1]);
            }
