 ...
 * (`local-commerce-dispute-runtime`) then CONSUMES this anchored + pinned assembly via
...
test.describe('Author + publish the local-commerce-dispute assembly (devnet)', () => {
    ...
    test('designer canvas authors + publishes local-commerce-dispute; it persists, anchored on-chain + pinned in IPFS', async ({ page }) => {
        ...
        const slug = SCENARIO_SLUG['local-commerce-dispute'];
        ...
        if (!(await assemblyAnchored(slug))) {
            ...
            await page.getByTestId('designer-name-input').fill('Local Commerce — Dispute');
        ...
        expect(assemblyTemplate.name).toBe('Local Commerce — Dispute');
