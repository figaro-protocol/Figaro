 ...
 * row's identity (slug, author, content hash) is on-chain; the assemblyTemplate
 ...
 * purpose. The assemblyTemplate fetch will fail and the row will render in
...
test.describe('Assemblies marketing inventory (devnet)', () => {
    ...
    test('renders a newly-published assembly from on-chain events', async ({ page }) => {
        ...
        // Slug appears in the row twice: as the fallback name (assemblyTemplate
