 ...
 * Validates institution assembly rendering is identical across chain modes.
 */
import { test, expect } from './figaro-test';

test.describe('Builder prototype — shared rendering', () => {
    test('lists multiple registered prototype institutions', async ({ page, figaroMode }) => {
        const qs = figaroMode === 'mock' ? '?e2e=mock' : '?e2e=devnet';
        await page.goto(`/builders/prototype${qs}`, { waitUntil: 'load' });

        await expect(page.getByRole('heading', { name: 'Multi-Institution Builder Prototype' })).toBeVisible({ timeout: 15000 });
    ...
    test('renders figaro-eats institution shell with the buyer-scoped derived surface', async ({ page, figaroMode }) => {
        const qs = figaroMode === 'mock' ? '?e2e=mock' : '?e2e=devnet';
