 ...
 *
 * direct-sale is the one-node scenario — a single root order, buyer <->
 * merchant, consume-onsite, no courier, no sub-orders.
 *
 ...
 * (consume-onsite), geo, jurisdiction, topology.
 *
...
test.describe('Author + publish the direct-sale assembly (devnet)', () => {
    ...
    test('designer canvas authors a one-node consume-onsite assembly and publishes it', async ({ page }) => {
        ...
        // direct-sale shape: consume-onsite fulfilment — single node, no
        // courier sub-order. (Selecting `delivery` would spawn one.)
        ...
        // direct-sale's clause set — a fresh root order carries commerce +
        // fulfilment + geo + jurisdiction + topology. Geo is default-on by
        // RPGF design (schemas feeding the analytics graphs are
        ...
        // One node, consume-onsite — no courier sub-order.
