...
test.describe('Evidence Display page (/evidence-display)', () => {
    test('loads inside a same-origin iframe', async ({ page }) => {
        ...
        await page.goto('/workbench?e2e=mock', { waitUntil: 'load' });
        ...
        await page.setContent(`
            <iframe
                id="evidence-frame"
                src="/evidence-display?processId=${fakeProcessId}&chainID=31337&arbitrableJsonRpcUrl=http://127.0.0.1:8545"
    ...
    test('shows error when processId is missing', async ({ page }) => {
        await page.goto('/evidence-display?arbitrableJsonRpcUrl=http://127.0.0.1:8545', { waitUntil: 'load' });
        await expect(page.getByText('Missing processId query parameter')).toBeVisible({
            timeout: 15000,
        });
    });

    test('shows error when arbitrableJsonRpcUrl is missing', async ({ page }) => {
        const fakeProcessId = '0x' + 'a1b2c3d4'.repeat(8);
        await page.goto(
            `/evidence-display?processId=${fakeProcessId}&chainID=31337`,
            { waitUntil: 'load' },
        );
        await expect(page.getByText('Missing arbitrableJsonRpcUrl query parameter')).toBeVisible({
            timeout: 15000,
        });
    });

    test('renders page header when all params are provided', async ({ page }) => {
        const fakeProcessId = '0x' + 'a1b2c3d4'.repeat(8);
        await page.goto(
            `/evidence-display?processId=${fakeProcessId}&chainID=31337&arbitrableJsonRpcUrl=http://127.0.0.1:8545`,
            { waitUntil: 'load' },
        );

        const title = page.getByRole('heading', { name: 'Figaro Process Timeline' });
        const errorBox = page.getByText('Error');
        // Either the timeline renders or the RPC call fails — both valid in mock mode
        await expect(title.or(errorBox)).toBeVisible({ timeout: 15000 });
    });

    test('displays dispute ID in subtitle', async ({ page }) => {
        const fakeProcessId = '0x' + 'a1b2c3d4'.repeat(8);
        await page.goto(
        ...
        const subtitle = page.getByText('#42');
        const errorBox = page.getByText('Error');
        await expect(subtitle.or(errorBox)).toBeVisible({ timeout: 15000 });
    });

    test('footer shows on-chain attribution', async ({ page }) => {
        const fakeProcessId = '0x' + 'a1b2c3d4'.repeat(8);
        await page.goto(
            `/evidence-display?processId=${fakeProcessId}&chainID=31337&arbitrableJsonRpcUrl=http://127.0.0.1:8545`,
            { waitUntil: 'load' },
        ...
        const footer = page.getByText('All data sourced from on-chain events');
        const errorBox = page.getByText('Error');
        await expect(footer.or(errorBox)).toBeVisible({ timeout: 15000 });
    });
});
