/**
 * seller-update-profile.devnet.spec.ts
 *
 * SellerRegistry.updateProfile is the canonical edit path for
 * registered sellers — re-points metadataURI without touching the
 * deposit or lock period (see `feedback_seller_registry_no_role.md`
 * + reference_seller_registry_no_role memory). Four UI sub-routes
 * call it: /sellers/edit/identity, /catalogue, /agents, /assemblies.
 *
 * The UI surfaces require an IPFS-pinned existing-profile JSON to
 * mount the form (SellerEditProfile bails with "Couldn't load the
 ...
 * relies on:
 *
 *  - A registered seller can fire updateProfile with a new
 *    metadataURI; the event stream gets an SellerProfileUpdated
 *    entry while SellerRegistered count stays at 1 (no
...
test.describe('SellerRegistry.updateProfile (devnet)', () => {
    let testSnapshot: string;
    test.beforeEach(async () => { testSnapshot = await evmSnapshot(); });
    test.afterEach(async () => { if (testSnapshot) await evmRevert(testSnapshot); });

    test('registered seller can update metadataURI — emits Updated, register count unchanged', async () => {
        await registerSeller('ipfs://test-G1-initial');
    ...
    test('multiple sequential updateProfile calls each emit a distinct Updated event', async () => {
        await registerSeller('ipfs://test-G1-multi-initial');
        ...
        for (const uri of uris) {
            const { request } = await publicClient.simulateContract({
                account: seller.address, address: registry,
                abi: SELLER_REGISTRY_ABI, functionName: 'updateProfile', args: [uri],
            });
            await publicClient.waitForTransactionReceipt({ hash: await sellerClient.writeContract(request) });
        }
