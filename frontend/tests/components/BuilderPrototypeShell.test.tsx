...
describe('BuilderPrototypeShell', () => {
    it('shows bundled source state by default', () => {
        ...
        expect(screen.getByLabelText('Runtime manifest URL')).toBeInTheDocument();
    ...
    it('loads a remote runtime manifest override when provided', async () => {
        ...
        createRuntimeIdentityDataSourceFromUrlMock.mockResolvedValueOnce(
            createRuntimeIdentityDataSourceFromManifestDocument(remoteManifest, 'https://example.com/runtime.manifest.json')
        );

        render(
            <BuilderPrototypeShell
                slug="figaro-eats"
                runtimeManifestUrl="https://example.com/runtime.manifest.json"
            />
        );
        ...
        expect(screen.getAllByText('https://example.com/runtime.manifest.json').length).toBeGreaterThan(0);
    });
    ...
    it('lets the user apply a remote manifest from the shell control', async () => {
        ...
        createRuntimeIdentityDataSourceFromUrlMock.mockResolvedValueOnce(
            createRuntimeIdentityDataSourceFromManifestDocument(remoteManifest, 'https://example.com/applied.manifest.json')
        );

        render(<BuilderPrototypeShell slug="figaro-eats" />);

        await user.type(screen.getByLabelText('Runtime manifest URL'), 'https://example.com/applied.manifest.json');
        await user.click(screen.getByRole('button', { name: 'Apply Manifest' }));
        ...
        expect(screen.getAllByText('https://example.com/applied.manifest.json').length).toBeGreaterThan(0);
    });
    ...
    it('lets the user reset back to the bundled manifest', async () => {
        ...
        createRuntimeIdentityDataSourceFromUrlMock.mockResolvedValueOnce(
            createRuntimeIdentityDataSourceFromManifestDocument(remoteManifest, 'https://example.com/resettable.manifest.json')
        );

        render(<BuilderPrototypeShell slug="figaro-eats" />);

        await user.type(screen.getByLabelText('Runtime manifest URL'), 'https://example.com/resettable.manifest.json');
        await user.click(screen.getByRole('button', { name: 'Apply Manifest' }));
    ...
    it('falls back to bundled runtime data when the remote manifest fails', async () => {
        ...
        render(
            <BuilderPrototypeShell
                slug="figaro-eats"
                runtimeManifestUrl="https://example.com/missing.manifest.json"
            />
        );
        ...
        expect(screen.getByText(/Remote manifest failed to load; using bundled fixture fallback./i)).toBeInTheDocument();
    ...
    it('renders runtime-bound subject context for figaro-procurement', () => {
        ...
        expect(screen.getByText(/missing-signature-refs: local-runtime.manifest.json subject 0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC has provenance issue missing-signature-refs./i)).toBeInTheDocument();
    ...
    it('renders runtime-bound subject context for figaro-disclosure-review', () => {
        ...
        expect(screen.getByText('Runtime Context')).toBeInTheDocument();
        expect(screen.getByText('Runtime Validation Warnings')).toBeInTheDocument();
        expect(screen.getByText(/missing-signature-refs: local-runtime.manifest.json subject 0x90F79bf6EB2c4f870365E785982E1f101E93b906 has provenance issue missing-signature-refs./i)).toBeInTheDocument();
    ...
    it('renders equipment-rental assembly without runtime fixture subjects', () => {
        ...
        expect(screen.queryByText('Unknown institution slug')).not.toBeInTheDocument();
    });
    ...
    it('renders the not-found state for an unknown slug', () => {
        ...
        expect(screen.getByText('Unknown institution slug')).toBeInTheDocument();
        expect(screen.getByText(/No registered institution artifact matched/)).toBeInTheDocument();
    });
});