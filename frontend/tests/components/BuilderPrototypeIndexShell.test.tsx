...
describe('BuilderPrototypeIndexShell', () => {
    ...
    it('applies a remote manifest to previews and propagated prototype links', async () => {
        ...
        createRuntimeIdentityDataSourceFromUrlMock.mockResolvedValueOnce(
            createRuntimeIdentityDataSourceFromManifestDocument(remoteManifest, 'https://example.com/index.manifest.json')
        );

        render(<BuilderPrototypeIndexShell />);

        await user.type(screen.getByLabelText('Runtime manifest URL'), 'https://example.com/index.manifest.json');
        ...
        expect(screen.getAllByRole('link', { name: 'Open Prototype' })[0]).toHaveAttribute(
            'href',
            '/builders/prototype/figaro-eats?manifest=https%3A%2F%2Fexample.com%2Findex.manifest.json'
        );
    });
    ...
    it('resets back to bundled previews and plain prototype links', async () => {
        ...
        createRuntimeIdentityDataSourceFromUrlMock.mockResolvedValueOnce(
            createRuntimeIdentityDataSourceFromManifestDocument(remoteManifest, 'https://example.com/index-reset.manifest.json')
        );

        render(<BuilderPrototypeIndexShell />);

        await user.type(screen.getByLabelText('Runtime manifest URL'), 'https://example.com/index-reset.manifest.json');
        await user.click(screen.getByRole('button', { name: 'Apply Manifest' }));
