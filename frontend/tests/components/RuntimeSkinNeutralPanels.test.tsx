...
vi.mock('@/components/core/AssemblyProcessWorkspace', () => ({
    AssemblyProcessWorkspace: (props: unknown) => {
        assemblyProcessWorkspaceMock(props);
...
function createAuctionProps(overrides?: Record<string, unknown>) {
    ...
    const mockAssembly = {
        identity: { id: 'mock-id', name: 'Mock Assembly', slug: 'mock', description: 'desc', networkTargets: ['mock-network'], version: '1.0.0' },
        contracts: [],
        mechanisms: [],
        roles: [],
        policies: [],
            ...
            services: {
                identity: {
                    getFallbackSource: () => ({
                        ...
                        listOperatorProfileMetadata: () => [], listSellerCatalogueMetadata: () => [],
                    }),
...
describe('runtime skin-aware neutral panels', () => {
    beforeEach(() => {
        ...
        assemblyProcessWorkspaceMock.mockReset();
    ...
    it('wraps the delegated process workspace in shell chrome while forwarding runtime props', () => {
        ...
        expect(assemblyProcessWorkspaceMock).toHaveBeenCalledWith(expect.objectContaining({
