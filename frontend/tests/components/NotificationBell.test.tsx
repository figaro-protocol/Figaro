...
describe("NotificationBell", () => {
    ...
    it("marks a notification as read, selects the process, and routes to the workbench", async () => {
        const user = userEvent.setup();
        render(<NotificationBell />);
        await user.click(screen.getByRole("button", { name: /notifications: 1 unread/i }));
        ...
        expect(mocks.pushMock).toHaveBeenCalledWith("/workbench");
    });

    it("does not push when already on the workbench", async () => {
        mocks.pathnameMock.mockReturnValue("/workbench");
        const user = userEvent.setup();
        render(<NotificationBell />);
        await user.click(screen.getByRole("button", { name: /notifications: 1 unread/i }));
        await user.click(screen.getByLabelText(/unread notification: new order committed/i));
    ...
    it("cancels navigation when builder authoring has an unsaved draft", async () => {
        ...
        expect(confirmSpy).toHaveBeenCalledWith(
            "You have unsaved builder changes. Leave this page and open the protocol workbench?"
        );
