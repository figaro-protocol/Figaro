...
import "../src/WitnessSwapAndCommitCoordinator.sol";
...
/// @notice Test-local (mirrors MockPermit2's pattern, but verifies the witness
///         signature — that is the whole point of this sibling). It reconstructs
///         the exact EIP-712 digest real Permit2 builds for a witness transfer
///         (name-only domain, `PermitWitnessTransferFrom` stub + caller type
///         string, `TokenPermissions` sub-hash, `spender = msg.sender`, witness),
///         recovers the signer, and reverts `InvalidSigner` on mismatch. This is
///         what makes a substituted `swapData` provably impossible: change the
///         route, the witness changes, the recovered signer is not the owner.
