...
| 2 | **TS encoder** | `sdk/src/clauses/encode.ts` (function `encode<Name>Content`) | every runtime-attestable clause (NOT manifest-only ones) |
| 3 | **TS validator export** | `sdk/src/clauses/index.ts` re-exports parser + encoder | every clause |
| 4 | **On-chain validator** | `src/clauseValidators/Figaro<Name>V1Validator.sol` | every clause EXCEPT manifest-only (`topology-v1`) |
...
1. **Layer A** — Read `frontend/lib/shared/clauses/<id>.json`. Note `version`, declared `format`, and whether the spec carries a `manifestOnly` flag or is referenced as such in `sdk/src/clauses/index.ts`.
2. **TS encoder** — Grep `sdk/src/clauses/encode.ts` and `sdk/src/clauses/index.ts` for `<Name>` (camelCase from `<id>` minus the `figaro-` prefix and `-v1` suffix). Mark present/absent. Manifest-only clauses should be absent — that is correct.
3. **On-chain validator** — Glob `src/clauseValidators/Figaro<Name>V1Validator.sol`. Manifest-only clauses should be absent — that is correct.
...
- **Missing on-chain validator for non-manifest-only clause** — every runtime-attestable clause requires its own `IClauseValidator` contract (validator-contract pattern: 1:1 with `clauseId`, ABI-encoded content, first-write-wins registration).
...
- Manifest-only clauses (currently `topology-v1`) are a real category. Do not flag absent encoders or validators for them.
