...
- **Protocol** = extension contracts (`SchemaRegistry.sol`, `AttestationCoordinator.sol`, `OperatorRegistry.sol`, `DutchAuction.sol`, `schemaValidators/*`, `FigaroBatchVerifier.sol`). Extension doctrine in `docs/v5/PROTOCOL_EXTENSION_DOCTRINE.md` applies.
...
| Finalized flag on resolved process | Multi-round composition (progressive collateralization) |
...
2. The game-theoretic theorems in `paper/figaro-mechanism.tex` (Two-Party Nash, Minimal Viable Bond Multiplier, Escape-Hatch Weakness, N-Party Nash, Self-Enforcing DAG Honesty).
...
Theorem citation:      <name from paper/figaro-mechanism.tex>
