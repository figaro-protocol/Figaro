 ...
 *
 * Prerequisites:
 *   anvil running + forge script DeployV3.s.sol --broadcast + mint-tokens.sh for
 *   both MockTokenV3 and MockPermitToken.
 *
 * Addresses below are generated by the most recent DeployV3.s.sol broadcast.
 * Re-run `setup-local.sh` and update these constants if the node is reset.
 */
...
/**
 * MockPermitToken address — read from NEXT_PUBLIC_PERMIT_TOKEN_ADDRESS (populated
 * from .env.local by the .env.local loader at the top of playwright.config.ts).
 * Falls back to the most-recently-deployed address so the test can still run
 * manually without a full `setup-local.sh` run.
 * IMPORTANT: re-run `forge script DeployV3.s.sol --broadcast` and update
 * .env.local (or let setup-local.sh do it) whenever the Anvil node is reset.
 */
