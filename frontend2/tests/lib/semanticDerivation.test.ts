...
import {
    ...
    getRegisteredAssemblyIndexValidation,
    listAssemblySelectorCards,
...
describe('assembly artifact registry', () => {
    it('resolves more than one assembly artifact from the registry', () => {
        const artifacts = listRegisteredAssemblies();
        const selectorCards = listAssemblySelectorCards();
    ...
    it('resolves a non-Eats procurement artifact through the same semantic contract', () => {
        const artifact = getRegisteredAssemblyBySlug('figaro-procurement');
    ...
    it('resolves a disclosure-oriented artifact with non-coordinator role binding through the same registry', () => {
        const artifact = getRegisteredAssemblyBySlug('figaro-disclosure-review');
    ...
    it('resolves an equipment-rental artifact with rental coordinator and two roles', () => {
        const artifact = getRegisteredAssemblyBySlug('figaro-equipment-rental');
    ...
    it('resolves a core-only freelance artifact with minimal composition', () => {
        const artifact = getRegisteredAssemblyBySlug('figaro-freelance');
    ...
    it('derives assembly-scoped operator registration capabilities from visible mechanisms', () => {
        ...
        const restaurantCapabilities = deriveAssemblyCapabilities(
            artifact!.assembly.identity.id,
            restaurantRole!.roleKind,
            restaurantMechanisms,
        );
        const buyerCapabilities = deriveAssemblyCapabilities(
            artifact!.assembly.identity.id,
            buyerRole!.roleKind,
            buyerMechanisms,
        );
