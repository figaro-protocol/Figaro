import { Assembly, RoleAssembly } from "@/lib/shared/assembly";
...
function deriveMechanismIds(role: RoleAssembly, assembly: Assembly): string[] {
...
function deriveCapabilities(role: RoleAssembly, assembly: Assembly): CapabilityModel[] {
    return (role.sampleCapabilities ?? []).map((capabilityKind, index) => ({
        ...
        source: {
            ...
            sourceLabel: "assembly sample capability",
            referenceId: `${assembly.identity.id}:${role.roleKind}:${capabilityKind}`,
        },
    }));
}

export function deriveRoleContextsFromAssembly(assembly: Assembly): RoleContext[] {
    return assembly.roles
        ...
        .map((role, index) => ({
            ...
            authoritySource: {
                truthClass: "assembly-declared",
                sourceLabel: "assembly prototype role",
