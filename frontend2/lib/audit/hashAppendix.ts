...
export function buildHashAppendix(
    ...
    // 3. Each agreement section's leaf.
    for (const section of agreement.sections) {
        anchors.push({
            kind: "agreement-section-leaf",
            label: `Section leaf — ${section.schema}`,
            hash: computeSectionLeaf(section),
            sourceLocation: `Merkle leaf under agreementHash root. Reader recomputes via computeSectionLeaf({schema: "${section.schema}", data}) and verifies inclusion in the agreementHash tree.`,
        });
    }
