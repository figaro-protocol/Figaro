...
function validateIdentity(assembly: Assembly): AssemblyValidationIssue[] {
    const issues: AssemblyValidationIssue[] = [];
    const { identity } = assembly;

    if (!identity.id.trim()) {
        issues.push({ severity: "error", path: "identity.id", message: "Assembly id is required." });
    }
    if (!identity.name.trim()) {
        issues.push({ severity: "error", path: "identity.name", message: "Assembly name is required." });
    }
    if (!identity.slug.trim()) {
        issues.push({ severity: "error", path: "identity.slug", message: "Assembly slug is required." });
    }
...
function validateCapabilityBindings(assembly: Assembly): AssemblyValidationIssue[] {
    const issues: AssemblyValidationIssue[] = [];
...
function validateServiceBindings(assembly: Assembly): AssemblyValidationIssue[] {
    const issues: AssemblyValidationIssue[] = [];
...
export function validateAssembly(assembly: Assembly): AssemblyValidationResult {
    const issues: AssemblyValidationIssue[] = [];
