...
% Computer Science and Cryptography register; second paper alongside C in
% the discipline. C covers the verified kernel implementation; the present
% paper covers what stands above the kernel as a research object — schema
% design as a CS discipline, the coordinator pattern formally, and runtime
% composition.
% Audience: software-architecture researchers, mechanism-design /
% protocol-extension theorists, the COALA-adjacent protocol-design
% community, computer-science readers approaching the protocol from a
% systems-architecture vantage.
% Companion to Paper C (verified Solidity implementation).
% Preprint, May 2026
...
% ════════════════════════════════════════════════════════════════════
\begin{abstract}
The companion mechanism paper presents the \figaro{} kernel as a
settlement primitive: two operations (\texttt{commit} and
\texttt{resolveProcess}), three storage mappings, no upgrade path, no
escape hatches. The companion implementation paper develops the
verification surface: TLA\textsuperscript{+}, symbolic execution,
property-based fuzzing, and Certora specifications discharging the
kernel's invariants. Both papers stay strictly at the kernel tier.
...
The companion mechanism paper formalizes the \figaro{} kernel as a
settlement primitive whose two operations, three storage mappings,
and frozen invariants produce a Nash equilibrium under specified
bonding rules. The companion implementation paper shows that the
kernel as deployed satisfies those invariants under multiple
formal-verification methods. Both papers stop at the kernel
boundary because that is where the formal results live.
...
two are inseparable in any working deployment, and yet the second
half — what stands above the kernel — has been treated in the corpus
to date only inline in worked examples (the Spirit Air assembly of
Paper~J, the TradeLens replacement of Paper~K, the Agent SDK reading
of Paper~L). The general design discipline by which protocol
extensions are written and runtime compositions are organized has
not had a paper of its own.

This paper supplies that paper. The argument is in three parts.
...
The implementation paper introduces the \emph{coordinator pattern}
as Proposition~1: an external mechanism contract composes with the
kernel without breaking the bonding equilibrium when it satisfies
specified conditions on its read/write profile against kernel state.
The implementation paper states the conditions informally; we
restate them as a formal proposition with composition semantics
and prove that the conditions are sufficient. The
\texttt{AttestationCoordinator} contract is the worked verification
example, with the parametric Core-immutability rules in
...
\Cref{sec:open} states open questions and future work.
...
mechanism paper and verified formally in the companion implementation
paper. We summarize the features the present paper relies on.
...
(ii) progressive collateralization across $N$-party process trees;
...
humans and agents interact with. The companion mechanism paper
introduces the three-tier distinction; the present paper develops it.
...
the equilibrium of the mechanism paper; widening it would weaken the
equilibrium. Extensions to the kernel therefore must add capability
...
$\langle \schemaid, \mathrm{version}, \mathrm{contentHash},
\mathrm{active} \rangle$ where $\schemaid$ is the anchor identity,
$\mathrm{version}$ is the version within the family,
$\mathrm{contentHash}$ is an immutable cryptographic hash of the
off-chain content, and $\mathrm{active}$ is the binding's admission
status.
\end{definition}
...
integrity collapses. The lockstep contract is enforced at the
deploy-script level (the schema's registration script registers the
\texttt{schemaId} and binds the validator atomically) and at the
...
\begin{enumerate}
  ...
  \item No silent mutation of the content reference behind an
    existing identity. The \texttt{contentHash} field of the anchor
    is immutable; off-chain documents may be mirrored for
    availability but the canonical reference is the hash, not any
    URI.
...
composing the calls in a single deployment transaction. For third-party
schemas registered post-deploy, the
\texttt{SchemaRegistrationHelper} contract supplies the same
atomic-bind pattern as a stateless, no-admin, no-fee public helper:
...
The companion evidence-and-law paper introduces a taxonomy that
schema design intersects with directly. \emph{Class~A} records
are kernel byproducts (commitment digest, bond deposits,
cumulative-value accumulator state, order status transitions,
resolution events): emitted by the kernel, schema-typed at the
kernel level, discretion-free, and cryptographically
authenticated. \emph{Class~B} records are discretionary
schema-validated attestations (delivery confirmations, GHG
disclosures, handoff attestations, custody-of-seal events,
lifecycle stage transitions): produced by participants through
the \texttt{AttestationCoordinator}, schema-typed at the
\emph{protocol} level, validator-gated, and substantively
contestable on truth-of-content grounds. The schema-design
discipline of this paper produces Class~B records: Layer~A
specifies the content shape, Layer~C enforces the shape at the
on-chain validator, the runtime tier renders and submits, and
the resulting attestation enters the off-chain forum's
evidentiary record as Class~B input under the framework
...
breaking the bonding equilibrium? The implementation paper introduces
the \emph{coordinator pattern} as Proposition~1, asserted at the
implementation level. We restate the proposition with formal
composition semantics and prove that the conditions are sufficient.
...
(i) preserves the kernel state predicates the implementation paper's
verification establishes (status monotonicity, transition
correctness, cumulative integrity, etc.); condition (ii) preserves
buyer dominance; condition (iii) preserves the no-escape-hatches
discipline; condition (iv) preserves the schema-validation surface.
A mechanism that satisfies all four can be composed onto the kernel
with the kernel's equilibrium guarantees still in force.
...
$\delta_{\mathcal{K}}$ (verified in the implementation paper), and
...
mechanism paper's escape-hatch-weakness theorem applies and the
...
breaks the invariants the implementation paper proves; a
mechanism with an alternative settlement path (relax condition ii)
implements the timeout / arbitrator / governance-override
escape-hatch the mechanism paper's Theorem~4.7 forbids; a
mechanism with a discretionary lock-bypass (relax condition iii)
implements an admin-controlled fund-withdrawal path the
no-escape-hatches discipline forbids; a mechanism that accepts
unvalidated schema-typed content (relax condition iv) creates a
content surface that downstream consumers cannot trust as
protocol-grade. We do not claim the conditions are necessary in
...
ever modifying kernel state. The contract's verification surface
discharges \Cref{prop:coordinator}'s conditions explicitly.

\paragraph{Conditions discharged.}
The Certora specification \texttt{certora/AttestationCoordinator.spec}
contains seven CVL rules. Two of them are parametric over every
...
\texttt{AgreementDrawer}). The canvas is a DAG editor where assembly
authors fork an existing reference assembly or start from blank,
modify the bonded-process tree on a node-and-edge graph, edit
per-node clauses through a side drawer, and save drafts to local
storage.
...
schemaId is flagged; a process-tree topology that fails the
N-party-Nash conditions is flagged. The canvas surfaces the
...
proposal for a different protocol. The companion mechanism paper's
escape-hatch-weakness theorem makes this not a stylistic preference
but a mathematical necessity: any unilateral exit path from the
bonded state weakens the Nash equilibrium. The protocol's
...
Any proposed extension that lets a process tree denominate
...
refused. The mechanism paper's analysis is that reputation is a
graph-tier signal, not a kernel-tier construct; introducing
soulbound reputation at the protocol layer reifies what is
appropriately a graph-level pattern (settlement history is
already on chain and readable as reputation by any party who
wants to read it that way) into a privileged-credential
...
2$\times$ bonding ratio is the minimum viable equilibrium per
the mechanism paper's Theorem~4.6; introducing a fee-discount
...
Several questions remain open under the discipline.

\paragraph{Layer B prover deferral.}
The TypeScript Layer~A validator and the Solidity Layer~C validator
are jointly the conformance specification for the eventual Rust
Layer~B port. The deferral is reasonable for the current deployment
phase (single-chain, no batched zero-knowledge attestation), but
the eventual port produces a verification question: is the Rust
implementation byte-for-byte equivalent to the TypeScript
specification? A property-based-equivalence test suite is the
natural answer. The discipline-level question is whether the suite
is part of the verification stack or part of the conformance
documentation.

\paragraph{Coordinator-pattern formal verification.}
\Cref{prop:coordinator}'s sufficient conditions can be discharged
mechanically per coordinator (\texttt{AttestationCoordinator} is the
worked example), but a parametric meta-verification that proves the
discipline is correct for any coordinator that satisfies the
conditions is not currently in the verification stack. The
implementation paper acknowledges this. The open question is
whether parametric verification at the coordinator-pattern level is
worth the work, or whether per-coordinator verification suffices.

\paragraph{Runtime-tier verification.}
The runtime tier currently has no formal-verification surface
analogous to the kernel's. The discipline is operational (modules
go through the action model, lenses constrain context, the canvas
flags compositional errors) but not theorem-discharged. The open
question is whether the runtime tier is amenable to formal
verification in any useful sense: typed-action correctness might
be checkable; lens-context separation might be checkable;
compositional integrity might be checkable. The work is open.

\paragraph{Cross-chain composition.}
The protocol is currently single-chain. Cross-chain extensions
(commitments anchored on chain $A$ with attestations anchored on
chain $B$) would require an extension of the schema-validation
surface and the coordinator pattern to handle cross-chain
references. The discipline-level question is whether such
extensions belong in the protocol or in graph-tier composition; we
conjecture the latter but the question is open.

\paragraph{Schema deprecation.}
The append-only identity rule makes schema introduction monotonic:
schemas are added but never removed. Operationally this is correct
(historical attestations remain interpretable), but it produces a
catalogue that grows monotonically over time. The discipline
question is whether deprecation (marking a schema as not-recommended
for new use while leaving its on-chain anchor intact for historical
reads) is a runtime-tier concern, a documentation concern, or a
protocol-tier concern. The current treatment is documentation-only;
whether more is needed is open.
...
research-grade. The mechanism-design and verification work the
discipline rests on is in the companion mechanism and implementation
papers; the operational worked examples are in the companion
industrial-engineering papers (J for scheduled service, K for
container shipping). The present paper supplies the connecting tissue:
...
incrementally alongside the kernel and the companion papers. The
extension doctrine document originated as an architectural note on
GHG schema anchoring and generalized; the runtime model emerged
from operational requirements of the local-commerce reference
archetype and generalized. The COALA collaborators have been a
consistent interlocutor on the legal-tier interactions of the
protocol-extension and schema-validation surfaces.
