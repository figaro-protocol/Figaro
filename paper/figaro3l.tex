...
We present the bonded settlement primitive of the companion mechanism
paper as the missing enforcement layer. The primitive's actor-neutrality
...
The companion mechanism paper of this corpus (Paper A) presents a
settlement primitive whose equilibrium properties are obtained
...
mechanism paper and the Solidity implementation is verified in
the companion implementation paper through TLA\textsuperscript{+},
Halmos symbolic execution, Echidna property-based fuzzing, and
Certora specification rules. The features the present paper
...
scales to $N$-party process trees through progressive
collateralization.
...
state-changing operation. There is no party-type input parameter;
...
or autonomous. The proof in the mechanism paper makes no appeal to
party-type; it constructs the dominance argument from the bonding
arithmetic alone. An autonomous agent that defects forfeits its
...
\texttt{commit-sub-order} (extending the process tree),
...
a synchronous-or-asynchronous function over actions:

\begin{lstlisting}[language=]
type Policy<TAction, TContext> = (
  action: TAction,
  ctx: TContext,
) => Promise<{ execute: boolean; reason?: string }>;
\end{lstlisting}

Two reference factories ship with the factotum reference
implementation at \texttt{agents/factotum/src/policy.ts} (not in
the \texttt{@figaro/core/agent} SDK itself, which exposes the
lower-level \texttt{ActionQueue} HITL gateway and the
\texttt{executeAction} autonomous helper as composition
primitives): \texttt{makeHitlPolicy<...>()} returns a
human-in-the-loop policy that always defers to operator approval,
and \texttt{makeAutonomousPolicy<...>(rule)} returns an
autonomous policy whose acceptance is governed by the supplied
rule function. Beyond the two reference shapes, an operational
deployment writes its own policy in the language of its
operational role; we treat typical roles in
\Cref{sec:policies}.

\paragraph{Execute.}
The execute phase commits an approved action to the chain via the
agent's wallet. The reference implementation
(\texttt{executeAction} in \texttt{@figaro/core/agent}) builds
the appropriate transaction (\texttt{commit},
\texttt{resolveProcess}, \texttt{attestAsBuyer}, or
\texttt{attestAsSeller}) and submits via a viem
\texttt{WalletClient}. Execution is sequential: actions are
committed one at a time, with the agent's nonce advancing per
transaction. Parallel execution would race the kernel state and
...
against rather than as a field on the proposed action. The
policy therefore receives the proposed payment from its
operating context (the agent's signing pipeline) rather than from
the action itself:

\begin{lstlisting}
const PER_COMMIT_LIMIT = 1_000_000_000n; // $1K
const TOTAL_BUDGET = 50_000_000_000n;   // $50K
const buyerPolicy = makeAutonomousPolicy<ProposedAction, Ctx>(
  (action, ctx) => {
    if (action.type === "commit-sub-order") {
      const proposedPayment = ctx.proposedPayment(action.processId);
      if (proposedPayment > PER_COMMIT_LIMIT)
        return { execute: false, reason: "above per-commit limit" };
      if (ctx.totalSpent + proposedPayment > TOTAL_BUDGET)
        return { execute: false, reason: "above total budget" };
      return { execute: true };
    }
    if (action.type === "resolve-process") return { execute: true };
...
the buyer-side role in the process tree, not for prospective
...
contract's own auction-state events (read off-chain through the
\texttt{@figaro/core/extensions} auction helpers), and the policy
acts at the moment the bidder is preparing a claim transaction
rather than at the moment a kernel-level action is proposed. The
bidder's logic, expressed as a pre-claim gate rather than as a
factotum policy in the strict SDK sense, looks like:
...
a higher-layer policy if the deployment wants one. A natural
extension to the SDK's proposer would be to surface
\texttt{claim-auction-position} as a first-class
\texttt{ProposedAction} type for addresses competing on a known
auction; the present SDK does not, and the bidder gate runs
adjacent to the factotum loop rather than inside it.
...
off-chain JSON document. The convention developed in the
companion AI-agent-coordination doctrine document
\citep{ai_agent_coordination_2026} aligns the document with
...
the agent's expected and observed state. The companion
evidence-and-law paper develops the off-chain dispute architecture
in detail (Class~A kernel-byproduct records vs Class~B discretionary
attestations; the three-layer mediation pipeline from MAD through
coordination pressure to off-chain forum); the agent's audit trail
is the operational complement on the agent's side. This is
...
The mechanism-design work is in the companion mechanism paper. The
SDK and the factotum reference implementation are operational
realizations of that mechanism's actor-neutrality property in working
TypeScript. The agents that participate are not protocol-aware in
any privileged sense; they sign, they bond, they perform. The
substrate makes their participation legible to their counterparties
and economically rational for them to undertake. The control-theory
analysis that has historically been blocked by the enforcement
question is now unblocked.

A natural next step is empirical: deployments of agent-coordination
patterns under the bonded architecture in operational settings (the
Spirit Air assembly of Paper~J, the TradeLens replacement of
Paper~K, the local-commerce reference assembly of the runtime
documentation). We do not undertake the empirical work here; we
identify it as the work the architecture invites.

\section*{Acknowledgments}
The Agent SDK and the factotum reference were developed alongside
the kernel and the runtime; they are operational realizations of
the actor-neutrality property analyzed in the mechanism paper.
...
\bibitem[AI Agent Coordination(2026)]{ai_agent_coordination_2026}
Daliana, A.
\newblock AI Agent Coordination via Public Graphs.
\newblock Internal protocol-doctrine document, ongoing.
