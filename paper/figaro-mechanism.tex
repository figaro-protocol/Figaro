...
substrate-level delegation primitives that bind delegated authority
to $B$'s identity at the settlement layer, not through
signature-only conveyance.
\end{definition}
...
Alice opens a process. Bob (seller 1) commits to provide the food
(\$10) and Charlie (seller 2) commits to deliver it (\$2). Both
sellers commit against Alice's cumulative-value accumulator under
the standard bonding rule:
\begin{center}
...
Given the monotonic-accumulator assumption
(\Cref{rem:monotonicity-assumption}), under-reports
($\cumval'_i < \cumval_i$) are rejected by the settlement layer at
\texttt{commit} time. Among the remaining feasible reports
$\cumval'_i \geq \cumval_i$, truth-telling
($\cumval'_i = \cumval_i$) weakly dominates every over-report. The
...
\begin{proposition}[Collusion is Unprofitable]
...
defect weakly exceeds the maximum monetary benefit $X$ can
obtain from $B$'s non-resolution. The dominance margin is at
least $3 \sum_{i \in K} \pay_i$ versus a benefit upper-bounded
by $3 \sum_{i \in K} \pay_i$, with equality only when every
$S_i \in K$ is a root order and the gap strictly positive
whenever $K$ contains any non-root seller.
\end{proposition}
...
cost; this is impossible. Joint defection is therefore
Pareto-dominated, weakly when $K$ consists entirely of root
orders and strictly otherwise.
\end{proof}
...
minimality is itself a security requirement: any unilateral exit
path weakens the Nash equilibrium
(\Cref{thm:escape-hatch-weakness}), so the absence of admin overrides,
timeouts, or arbitrators is not a simplification but a property the
analysis depends on. Under perfect monitoring
...
elimination of weakly dominated strategies. The same per-edge
asymmetric-bonding structure makes seller coalitions unprofitable
even with external side-payment
(\Cref{prop:collusion-unprofitable}): any coalition's defection cost
weakly exceeds the harm imposable on the buyer, with strict
inequality whenever the coalition contains any non-root seller.
Finally, the mechanism reproduces the cooperation-pressure
component of the peer-enforcement equilibrium in joint-liability
