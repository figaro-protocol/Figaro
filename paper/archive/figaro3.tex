...
by TLA\textsuperscript{+} model checking (7~invariants, 2M+ states),
property-based fuzzing, symbolic execution, and specification
checking against kernel invariants, and
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{Formal verification.} The implementation is verified
    ...
    2M+ states exhaustively explored), property-based fuzzing
    (7~Echidna invariants, 50{,}000 call sequences), symbolic execution
    via Halmos (7~kernel properties), and specification checking via
    Certora (CVL rules covering the kernel and attestation surfaces).
...
\subsection{Reduction from Microfinance Mechanisms}
\label{sec:microfinance-reduction}
...
\begin{proposition}[Assumption Reduction over Group Lending]
\label{prop:microfinance-reduction}
The peer-enforcement outcome common to \citet{ghatak1999} and
\citet{stiglitz1990}---cooperative equilibrium selection driven by
co-participant externalities---is obtained by the bonded commitment
mechanism under strictly weaker assumptions. Specifically, Figaro
...
enforced by the contract's monotonic accumulator, independent of any
seller's information about co-sellers' types, actions, or identities;
(ii) is not required.
...
Halmos (z3-backed) exhaustively explores symbolic traces for
seven kernel properties, complementing Echidna's bounded random
sampling with symbolic coverage of the commit/resolve state machine:
token conservation, bond amount correctness, resolution payout
correctness, status transition legality, buyer dominance,
cumulative monotonicity, and atomic-resolution enforcement.

\paragraph{Specification checking.}
Certora CVL rules verify status monotonicity, transition legality,
active-order-count consistency, buyer dominance, no-double-commit,
cumulative monotonicity, and immutability of the root buyer and
denomination currency across the kernel. Additional rules cover the
attestation surface: non-buyer reverts, unknown-process reverts, buyer
identity, process boundary, and the inability of the attestation
coordinator to modify kernel state.
...
The game-theoretic results of
Sections~\ref{sec:game-theory} and~\ref{sec:process-trees} establish
strong properties under explicit assumptions. Two scope conditions are
worth stating.

\paragraph{Liveness under non-rational failure.}
\Cref{thm:n-party-nash} establishes that cooperation weakly dominates
for all sellers in an $N$-party process tree under the assumption of
rational actors. Non-rational failure modes---loss of signing keys,
hardware failure, participant death, or unrecoverable client
error---cause non-resolution and, via atomic resolution, impose losses
on all co-sellers regardless of incentive alignment. The
coordination-pressure argument of
\Cref{prop:coordination-pressure} assumes each seller's payoff depends
on every other's \emph{choice}; catastrophic liveness failure is not a
choice. The theorems therefore bound rational-actor equilibria;
operational reliability under non-rational failure depends on
participant continuity practices (key management, multisig, delegated
signing) that sit above the kernel. In deployments with heterogeneous
participant reliability, this bounds empirical process-tree depth
independent of the mechanism's incentive properties.

\paragraph{Capital opportunity cost.}
The minimality proof of \Cref{thm:2x-sufficient} assumes locked capital
forgoes zero return. In practice, over process duration $t$ at
prevailing risk-free rate $r$, the buyer and seller forgo $rPt$ and
$2r\cumval t$ respectively. The dominance relation of
\Cref{thm:two-party-nash} is preserved at any $rt > 0$: cooperation
still strictly exceeds defection, since defection incurs the full
opportunity cost on locked capital in addition to the principal loss.
The operational margin nonetheless narrows with $rt$; the $2\times$
multiplier is sufficient in any rate regime as a matter of
game-theoretic derivation, but its practical applicability favors
...
The dissolution is narrower than \Cref{prop:coasean-shift} may suggest.
The firm historically bundles several distinct functions: coordination
of production, aggregation of capital across projects, accumulation of
...
liability. Asymmetric bonding addresses the first. The remaining
functions persist as requirements that some durable institution must
satisfy; they likely reconstitute around token-denominated treasury
communities whose members repeatedly participate in compatible process
trees---structures that resemble firms at the meta-layer. The precise
claim this analysis supports is therefore: the \emph{coordination
firm}, whose value is internal management of a bounded labor pool,
loses its economic rationale where asymmetric bonding is applicable;
the \emph{capital firm}, whose value is durable asset ownership and
...
provides competitive advantage in a specific market. In the firm model, key components are
...
model checking (7~invariants, 2M+ states), property-based fuzzing,
symbolic execution via Halmos, and specification checking via Certora.
The three-layer enforcement architecture---economic, social,
legal---provides defense-in-depth without on-chain governance or
dispute resolution.
...
% Removed self-citation: Daliana (2013) — ROKC
