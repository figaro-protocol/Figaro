...
formally in a companion paper~\citep{daliana2026mechanism}, with a
verified Solidity implementation in a second
companion~\citep{daliana2026impl}---that prices the cost of trust at
exactly $2\times$ the transaction value in temporarily locked
...
organizational container to one pattern among several. A companion
political-economy paper~\citep{daliana2026hegemony} develops what
this demotion looks like from the outside; the present paper is
internal institutional-economics.
\end{abstract}
...
lockup. The collapse is not a thought experiment: a companion
paper~\citep{daliana2026mechanism} presents the mechanism with
formal proofs of cooperation as the unique surviving Nash
equilibrium, and a second companion~\citep{daliana2026impl} presents
a verified Solidity implementation audited through
...
companion papers for the formal development and implementation.
...
in~\citep{daliana2026mechanism}; the implementation and verification
...
mechanism, no protocol fee, and no upgrade path. The companion
mechanism paper proves that any such escape hatch weakens the
equilibrium.
...
equilibrium outcome but is no longer.
...
runtime'' in the sense defined in~\citep[§1.4]{daliana2026mechanism}.
...
discipline is developed in the implementation
companion~\citep{daliana2026impl}.} the wallet can hold
...
\emph{where} residual control rights are allocated. In the firm
model, those rights are allocated to the ownership nucleus. In the
wallet model, they are allocated to whoever controls the wallet ---
which, under self-sovereign key management and NFT-anchored
credentials, is the productive asset itself (or the individual
operating it). Residual control rights do not disappear; they are
held by the asset rather than aggregated under an ownership layer.
...
The companion accounting paper~\citep{daliana2026accounting}
develops the argument that a \figaro{} process is a
self-closing ledger period, and that the apparatus that grew up
...
of the token's financial-mechanism features. The cryptoeconomics
companion~\citep{daliana2026fig} develops this category in
detail, with \textsc{Btc} and \textsc{Eth} as precedents at the
...
Asymmetric bonding prices the cost of trust at $2\times$ the
transaction value in temporarily locked (not spent) capital, with
zero recurring extraction.
...
itself reveals about the architecture of contemporary coordination
infrastructure. That third question is addressed in the companion
political-economy paper~\citep{daliana2026hegemony}.

\paragraph{A note on scope alignment across the companion papers.}
This paper bounds its claim by two scope conditions: the Coasean
comparison (\Cref{prop:coasean-shift}: $2\pay$ vs. coordination
overhead) and asset specificity (\Cref{sec:patterns}). The
political-economy companion~\citep{daliana2026hegemony} bounds its
claim differently, by hegemonic legibility --- its argument applies
wherever the primitive makes a previously-presupposed
coordination function visible as a design artifact. The labor-law
companion~\citep{daliana2026labor} bounds its claim by asset
specificity alone. The electronic-evidence
companion~\citep{daliana2026legal} bounds its claim least
restrictively --- wherever the primitive is used, its evidentiary
framework applies. Each bound is internally reasonable within
its paper's discipline. Read across the set, these bounds are
compatible but not identical. The theoretical-cluster domain is
the \emph{intersection} of the four scope conditions, not their
union: a given transaction lies within the cluster's shared
domain only if the primitive's Coasean threshold is met, asset
specificity is low or structurally handled, the coordination
function is legible as a design choice, and the evidentiary
framework is practically accessible for the parties' jurisdictional
situation. A transaction that satisfies three of the four
conditions but not the fourth is in-scope for three papers and
out-of-scope for one; readers constructing arguments about such
transactions should check which papers apply. We flag this here
because each companion's scope condition is developed within its
own register and the inter-paper alignment is not elsewhere made
explicit.
...
political-economy half (now in the companion
paper~\citep{daliana2026hegemony}). The two were entangled in
earlier drafts in a way that did neither audience justice.
...
\bibitem[Daliana(2026c)]{daliana2026hegemony}
Daliana, A.
...
\bibitem[Daliana(2026e)]{daliana2026labor}
Daliana, A.
...
\bibitem[Daliana(2026f)]{daliana2026fig}
Daliana, A.
...
\bibitem[Daliana(2026g)]{daliana2026accounting}
Daliana, A.
