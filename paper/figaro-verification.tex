...
\end{align}
Here $H_{\text{EIP-712}}$ denotes the EIP-712 typed-data digest:
$\mathrm{keccak256}(\texttt{0x1901} \;\|\; \text{domainSeparator}
\;\|\; \text{structHash})$, computed via OpenZeppelin's
\texttt{\_hashTypedDataV4} at \texttt{src/FigaroCore.sol:158}.
...
A separate four-property harness for the staged airdrop contract
existed at the time of the original verification round but was
retired in 2026-05 when the airdrop contract itself
(\texttt{StagedMerkleAirdrop.sol}) was deleted and replaced by
\texttt{RpgfMinter.sol}; the replacement does not yet carry a
Halmos harness, and the open work is tracked outside this paper.
The airdrop harness is out of scope for the kernel claim made in
...
\texttt{FigToken.spec} (6 rules) covers the FIG token registry;
out of scope for the kernel claim of this paper but ships in the
same repository. A 3-rule spec for the staged airdrop contract
(\texttt{StagedMerkleAirdrop.spec}) existed during the original
verification round but was retired in 2026-05 when the contract
was replaced by \texttt{RpgfMinter.sol}; the replacement does not
yet carry a Certora spec.
...
total.\footnote{Per-spec rule counts are tracked in
\texttt{docs/v5/VERIFICATION\_MAP.md}; the $23$ sub-rule total
corresponds to the cloud report identifier pinned in that document.
Re-runs of the cloud prover may reorganize sub-cases; the declared
count of $22$ is the stable artifact.}
All discharged on Certora cloud as of the snapshot date. The
...
\paragraph{Signature replay across forks and chain splits.}
The EIP-712 domain separator binds the digest to the chain id and
verifying contract, which prevents replay across distinct
deployments under standard chain identification. OpenZeppelin's
\texttt{\_hashTypedDataV4} recomputes the domain separator when
\texttt{block.chainid} changes, so a chain split that updates
chainId on one fork invalidates pre-fork signatures on that fork
and they cannot be replayed against the post-fork state. A chain
split that does \emph{not} update chainId leaves both forks
sharing the same domain separator, in which case a single
commitment is admittable on either fork; this is a property of
the chain identification scheme, not of the kernel, and is
addressed at the deployment layer (chainId update on fork).

\paragraph{Cross-contract reentrancy via the attestation surface.}
A malicious schema validator registered first-write-wins for some
schema id could attempt to call \texttt{commit} or
\texttt{resolveProcess} during validation. Two guards apply: the
\texttt{ISchemaValidator} interface specifies \texttt{validate} as
\texttt{pure}, so a validator that performs external calls violates
the interface contract; and the kernel's \texttt{nonReentrant}
modifier blocks the nested call regardless of validator behavior.
The attestation surface itself is therefore a denial-of-service
vector at worst (a malicious validator can revert), not a state-
corruption vector against the kernel.

\paragraph{Allowance front-running on bond pulls.}
The kernel pulls the buyer and seller bonds via
\texttt{safeTransferFrom} against pre-existing ERC-20 allowances.
A buyer who approves the kernel for an amount sufficient for two
distinct sub-orders can have either commitment landed first by an
adversarial transaction-orderer, and the second commit will revert
on insufficient allowance. This is the standard ERC-20 approval-
race pattern. The kernel's content-addressed orderHash gating
(\texttt{DuplicateCommitment} on the second admission of the same
hash) ensures the adversary cannot duplicate a commitment, only
reorder distinct ones; bond capital is never lost, only the order
of process extension is non-deterministic at the wallet's approval
boundary.
...
\bibitem[Lamport(2002)]{lamport2002}
Lamport, L.
...
\bibitem[Park et~al.(2024)]{halmos2024}
Park, D., Hildenbrandt, E., Rosu, G., and the Halmos contributors.
\newblock {Halmos}: A symbolic bounded model checker for {EVM}
  smart contracts.
\newblock a16z crypto / open-source project, 2024.
\newblock \url{https://github.com/a16z/halmos}

\bibitem[Grieco et~al.(2020)]{grieco2020}
Grieco, G., Song, W., Cygan, A., Feist, J., and Groce, A.
\newblock {Echidna}: Effective, usable, and fast fuzzing for smart
  contracts.
\newblock In \emph{Proc.\ ACM SIGSOFT International Symposium on
  Software Testing and Analysis (ISSTA)}, pages 557--560, 2020.

\bibitem[Certora(2024)]{certora2024}
Certora.
\newblock The {Certora} prover and the {CVL} specification language.
\newblock Certora Technology Documentation, 2024.
\newblock \url{https://docs.certora.com/}

\bibitem[Foundry(2024)]{foundry2024}
Paradigm.
\newblock The {Foundry Book}: A blazing-fast, portable and modular
  toolkit for {Ethereum} application development.
\newblock Foundry Documentation, 2024.
\newblock \url{https://book.getfoundry.sh/}

\bibitem[OpenZeppelin(2024)]{openzeppelin2024}
OpenZeppelin.
\newblock OpenZeppelin Contracts: a library for secure smart-contract
  development (\texttt{EIP712}, \texttt{ECDSA}, \texttt{ReentrancyGuard},
  \texttt{SafeERC20}).
\newblock OpenZeppelin Documentation, version 5.x, 2024.
