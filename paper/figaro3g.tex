...
% ════════════════════════════════════════════════════════════════════
\section{Treasury Composition Patterns}
\label{sec:treasury-composition}

The self-closing-ledger property the previous sections developed
is a property of every process. An institution that disburses
through Figaro processes therefore inherits the property without
needing any institution-specific accommodation in the kernel: the
kernel reads a wallet, an EIP-712 signature, and a bond, and the
institutional shape behind that wallet --- a multisig, a DAO
governance executor, a streaming contract, a single-key treasury
agent --- is invisible to the kernel and irrelevant to the
ledger-period guarantee. We catalogue the four composition
patterns by which on-chain treasuries operate as buyers in
