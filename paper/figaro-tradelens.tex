...
the architecture, and not specific to TradeLens.
...
service. The full RWA-as-wallet apparatus is developed in the
accounting paper of this corpus.
...
specialised value-adders), with the bonding equilibrium holding at
every edge regardless. The composition is not a TradeLens
...
needs, with the bonding equilibrium holding at every edge
regardless. This paper develops the fine-grained version because
...
admissible and may be operationally appropriate in many
deployments.
...
still be filled by parties providing those specific services, but
they cannot insert themselves as kernel intermediaries between
the buyer-of-record and the operational sellers; the kernel
forbids any party other than the rootBuyer from being the buyer
of within-process commits.
...
\begin{itemize}
  \item \emph{importer $\to$ shipper-of-record}:
    \texttt{commerce-v1} payment for the cargo's invoice value,
    \texttt{handoff-v1} on goods presented for shipment, optional
    \texttt{ghg-protocol-v1} disclosure for the manufacturing
    emissions.
  \item \emph{importer $\to$ freight forwarder}:
    \texttt{commerce-v1} payment, \texttt{handoff-v1} on cargo
    receipt at consolidation, \texttt{container-seal-v1} applied
    at the consolidation point.
  \item \emph{importer $\to$ NVOCC}: \texttt{commerce-v1} payment,
    house bill-of-lading reference (the BoL itself is treated as
    a protocol-layer composition pattern in
    \Cref{sec:bol}).
  \item \emph{importer $\to$ origin inland carrier} and
    \emph{importer $\to$ destination inland carrier}:
    \texttt{commerce-v1} payment, \texttt{handoff-v1} at pickup
    and dropoff, \texttt{geo-v1} pickup and dropoff geohashes.
  \item \emph{importer $\to$ inspection service}:
    \texttt{commerce-v1} payment, \texttt{container-seal-v1}
    inspection event, optional evidence-hash pointer to a
    pre-shipment inspection report.
  \item \emph{importer $\to$ port-of-loading authority} and
    \emph{importer $\to$ port-of-discharge authority}:
    \texttt{commerce-v1} payment for terminal handling and gate
    access, \texttt{handoff-v1} at port-side transfers,
    \texttt{container-seal-v1} inspected-intact attestations at
    the port handoffs.
  \item \emph{importer $\to$ ocean carrier}: \texttt{commerce-v1}
    ...
    cadence, \texttt{ghg-protocol-v1} disclosure for the voyage
    emissions, and (for reefer cargo)
    cold-chain-monitoring attestations.
  \item \emph{importer $\to$ customs agent}: \texttt{commerce-v1}
    payment for entry-filing services plus
    \texttt{jurisdiction-v1} attestation recording the customs
    authority's determination.
  \item \emph{importer $\to$ customs authority}:
    \texttt{commerce-v1} payment for the import duty. The customs
    authority is an RWA-as-wallet of the same kind as a
    commercial provider --- a public service whose continued
    operation depends on receipts covering its operating expenses
    --- in the role of a bonded seller of border-clearance service
    in this commit, with the sovereign decisional authority on
    the customs agent's parallel attestation handled as an
    off-chain regulatory function the kernel does not encode.
  \item \emph{importer $\to$ marine insurer}: \texttt{commerce-v1}
    payment for the policy premium; the insurer's bond is the
    underwriting capacity at risk against the insured cargo
    value. Insurance composes with the bonded primitive without
    modifying the kernel.
  \item \emph{importer $\to$ LC issuer (if applicable)}:
    \texttt{commerce-v1} payment for the LC issuance fee; the
    issuer's bond is the credit capacity backing the LC
...
importer-of-record as \texttt{rootBuyer}. The import duty (\$39)
is $5\%$ of the declared CIF value (\$775 = \$700 cargo + \$15
marine insurance + \$60 ocean freight). $G$ is the process-wide
...
The composition reuses the following existing schemas.
...
\texttt{figaro-fulfilment-v1} carries the final delivery to the
consignee.
...
at port handoffs where required. These schemas are already on
chain and bound to validators; no further protocol-extension
...
\paragraph{Axes of underspecification.}
The pattern is sketched here as a candidate, not asserted as a
deliverable mechanism. It is underspecified along three axes
that bound how seriously a deployment can rely on it.

\emph{Sub-seller consent under the wrapper.} The cancellable-seller
wrapper relies on programmatic acknowledgement of the
cancellation by the affected sub-seller under a pre-agreed fee
schedule. Whether this constitutes sub-seller consent in the
kernel-discipline sense, or constitutes a unilateral exit dressed
in counterparty clothing, is unsettled. A sub-seller bonded under
a CancellableSeller wrapper has consented to the wrapper's terms
at commit time, but consent at commit time to a future programmatic
acknowledgement is structurally different from consent at the
moment of cancellation; the design-discipline reading depends on
which framing one accepts.

\emph{Cargo-handoff operationalization.} A fresh transport process
...
described attestation surface in the present sketch.

\emph{Cancellation-fee arithmetic.} Setting the fee too low
produces an early-exit option the original sub-sellers will
refuse; setting it too high forecloses the transfer use case.
The present paper does not defend any specific fee schedule;
the arithmetic is left for the deployment-specific design.
