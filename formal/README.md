...
| `SettlementUniverses.tla` + `.cfg` | CROSS-CONTRACT: FigaroCore + FigaroBatchVerifier + UsageCounter + guest kernel under arbitrary interleavings — no cross-universe double payout, per-pool deposits, score composition, kernel blindness. Carries two named assumption constants (`AssumeDomainSeparation`, `AssumeAccrualGatesAligned`); flipping either to FALSE is the experiment and is EXPECTED to fail | 21 |
