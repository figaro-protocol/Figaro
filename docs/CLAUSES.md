...
| `figaro-denomination` | THE PRIVILEGED TOKEN — the one ERC-20 the whole assembly's processes bond and settle in. SPECIFIC-T&C (`block.terms: "specific"`, `settlement` article): the designer pins the token address into the template (identity-bearing — the pin is part of the compositionHash), tailoring the generic assembly; every bond (2×) and payment then moves in it, so demand scales with the trade flowing through the assembly (use-priced value capture). Elective, composed on the ROOT order (process-scoped; the kernel enforces one currency per process); absent = the seller's default denominates | Layer A (off-chain) |
