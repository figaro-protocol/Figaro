...
## Anchored Artifact Pattern

The minimal-on-chain-anchor pattern that the extension doctrine
above produces in practice. Originally drafted as a standalone
design note for the GHG family; the same pattern applies to any
artifact family the protocol decides to anchor.
