...
| Consignee | `order.buyer` | Same address as the contractual shipper in local commerce. In supply-chain DAGs the buyer may designate the consignee via a destination address inside `figaro-handoff-v1` (encrypted) and an address inside `figaro-geo-v1.destinationGeohash`. |
| Origin | `figaro-geo-v1.originGeohash` | Geohash, 1–12 chars precision. |
| Destination | `figaro-geo-v1.destinationGeohash` | As above. |
