...
2. merchants and sellers can join an archetype with less bespoke app setup
...
2. merchants and sellers onboard into app silos rather than institution templates
...
1. merchant wallet or seller address is associated with an institution template or archetype
...
3. a place to validate real seller flows before they are generalized into the runtime
...
1. what is the canonical merchant or seller binding model from wallet address to institution template
...
That is enough to render a buyer storefront, seller cockpit, fulfiller workspace, reviewer surface, or agent-facing control plane without redefining the protocol each time.
...
3. descriptor-backed institution, process, and order actions in the live semantic workspace for seller profile writes, seller disclosure writes, auction claims, delivery coordinator signals, resolve, and downstream composition entry points, with the legacy workbench GHG workflow panel now routing disclosure writes through that same capability executor instead of a direct disclosure hook
...
5. restaurant seller in this institution assembly
