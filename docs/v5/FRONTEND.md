...
**`(marketing)/` (no wallet provider):** `/` (root), `/assemblies`, `/builders` (hub), `/builders/agents`, `/builders/composability`, `/cryptoeconomics`, `/integrate`, `/local-commerce` (worked example), `/protocol`, `/rpgf`, `/schemas`, `/spec`, `/why`. The `/schemas` and `/assemblies` inventories read on-chain state event-driven through the standalone `publicClient` — marketing-tier reads do not require the wallet provider.
...
- **Marketing** — pure publication / explanation. Lives in `app/(marketing)/`; does not load the wallet provider. Current routes: `/`, `/assemblies`, `/builders`, `/builders/agents`, `/builders/composability`, `/cryptoeconomics`, `/integrate`, `/local-commerce`, `/protocol`, `/rpgf`, `/schemas`, `/spec`, `/why`.
