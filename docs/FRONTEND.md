...
**Marketing — `(marketing)/`, no wallet provider:** `/` (root), `/agents`, `/assemblies`, `/builders` (hub), `/builders/composability`, `/working-groups`, `/integrate`, `/local-commerce` (worked example), `/papers/<slug>` (the paper corpus), `/physics`, `/protocol`, `/artifact-rewards`, `/clauses`, `/composes`, `/security`, `/spec`, `/users`, `/why`. The `/clauses` and `/assemblies` inventories read on-chain state event-driven through the standalone `publicClient` — marketing-tier reads do not require the wallet provider.
