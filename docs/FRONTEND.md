...
**The three sections.** One host, one export, three sections — use, build, core — and the apex as their router: `frontend/lib/shared/sections.json` says which section owns which route (a shared surface lists more than one; the first owns it, and every listed section shows it in its nav), each section has its landing page (`landings`), its own name beside the logo (`SectionName`), its own FAQ in the footer (`sectionFaqRoute`: `/faq`, `/build/faq`, `/core/faq`), and its own nav. The header has two levels: the three sections as plain links in the main row on every page (`SectionLinks`, the reader's own marked current), and under it the reader's section's own tree (`NavTreeRow`, the groups the section shows per `navGroupShown`); the apex has the first level only. The drawer carries the same two levels: the three section rows, then the section's groups. `scripts/lint-section-map.sh` fails a page route the map does not hold.
...
| Group | Routes | Notes |
|---|---|---|
| `(use)` | `/use`, `/use/assemblies`, `/members`, `/faq`, `/local-commerce` | The Use section — for buyers and sellers: join, trade, by hand or through an agent; the tagline is its title; six benefits as bullets, then as cards, the apex's shape. `/members` is Join. `/use/assemblies` is the users' showcase: every published assembly, who offers it, and its designer's own questions, read from the registries as the page opens (`?slug=` is one assembly); nothing listed in the tree. `/faq` is the users' FAQ: the protocol's questions as static sections, then every published assembly's own `faq` entries read from the registry at runtime (`components/assemblies/AssemblyFaqs`). `/local-commerce` is ONE example among unbounded kinds, never THE model, told from where each of its three participants stood. Discover and Orders, in `(app)`, are admitted under it in the nav. |
| `(build)` | `/build`, `/build/faq`, `/clauses`, `/assemblies`, `/registries`, `/composition`, `/pitfalls`, `/rpgf`, `/tokenomics`, `/dao` | The Build section. The two authoring tools in `(tools)` are admitted under it. Rewards, Tokenomics, and The DAO are three concepts, three pages, in reading order. The docs-site is its second step. |
| `(core)` | `/core`, `/core/faq`, `/kernel`, `/invariants`, `/spec`, `/security` | The Core section. `/security` is testing + audit results only. |
...
| `(agents)` | `/agents`, `/agents/how`, `/agents/worked-example` (the two-agent process, every bond per party) | The Agents door: agents are participants, not a feature; the door leads to the agents' own surface (`ecosystem-agents/`, the signer, the machine-readable index). |
