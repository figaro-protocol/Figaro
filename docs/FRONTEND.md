...
- **`composition/`** — third-party on-network contract composition (the fifth noun): the generic dispatch (`compositionTarget.ts`, `useCompositionActions.ts`) + per-contract hooks/readers, incl. the swap-funded bond leg (`swapFunding.ts` — devnet venue quote/route + the witness-signed leg builder; `useSwapAndCommitActions.ts` — the `swapAndCommit` broadcast a funded payload routes through)
