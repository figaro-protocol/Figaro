...
**Marketing — `(marketing)/`, no wallet provider:** `/` (root), `/agents`, `/assemblies`, `/builders` (hub), `/builders/pitfalls` (the canonical footguns page), `/working-groups`, `/glossary`, `/local-commerce` (worked example), `/papers/<slug>` (the paper corpus), `/physics`, `/kernel`, `/rpgf`, `/clauses`, `/composes` (the single composition page — catalogue + coordinator conditions; absorbed `/builders/composability` 2026-08-05), `/security`, `/spec`, `/members` (the membership pitch — the wizard lives beneath it in `(app)`), `/why`. The `/clauses` and `/assemblies` inventories read on-chain state event-driven through the standalone `publicClient` — marketing-tier reads do not require the wallet provider.
