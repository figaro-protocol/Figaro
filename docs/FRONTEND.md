...
| Group | Routes | Notes |
|---|---|---|
| `(use)` | `/use`, `/members`, `/assemblies-app` (served at `/assemblies` on the app host), `/faq`, `/local-commerce`, `/worked-example` | The Use door. `/assemblies-app` is the users' showcase: every published assembly, who offers it, and its designer's own questions, read from the registries as the page opens (`?slug=` is one assembly); nothing listed in the tree. `/faq` is the users' FAQ: the protocol's questions as static sections, then every published assembly's own `faq` entries read from the registry at runtime (`components/assemblies/AssemblyFaqs`). `/local-commerce` is ONE example among unbounded kinds, never THE model. Discover and Orders, in `(app)`, are admitted under it in the nav. |
| `(build)` | `/build`, `/clauses`, `/assemblies`, `/registries`, `/composition`, `/pitfalls`, `/rpgf`, `/tokenomics`, `/dao`, `/faq-build` (served at `/faq` on the build host) | The Build door. The two authoring tools in `(tools)` are admitted under it. Rewards, Tokenomics, and The DAO are three concepts, three pages, in reading order. The docs-site is its second step. |
| `(core)` | `/core`, `/kernel`, `/invariants`, `/spec`, `/security`, `/faq-core` (served at `/faq` on the core host) | The Core door. `/security` is testing + audit results only. |
