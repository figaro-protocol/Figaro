...
**The six doors.** One host, one export, six sections — join, trade, communities, terms, evidence, code — and the home page as their router: `frontend/lib/shared/sections.json` says which section owns which route (a shared surface lists more than one; the first owns it, and every listed section shows it in its nav), each section has its door page (`landings`: `/members`, `/local-commerce`, `/communities`, `/terms`, `/data`, `/core`), its own FAQ in the footer (`sectionFaqRoute`: the builders' `/terms/faq` for terms, `/core/faq` for the code, the users' `/faq` for every other door), and its own nav. The header has two levels: the six doors as plain links in the main row on every page (`SectionLinks`, the reader's own marked current), and under it the reader's section's own tree (`NavTreeRow`, the groups the section shows per `navGroupShown`); the home page has the first level only. The drawer carries the same two levels: the six door rows, then the section's groups. `scripts/lint-section-map.sh` fails a page route the map does not hold; `scripts/lint-nav-structure.sh` fails a nav that does not equal the route tree (one route group per door, its door page first, labels equal to page titles).
...
| `(trade)` | `/local-commerce` | The One trade door: one trade, start to finish, as a strip of six pictures (`components/figures/TradeStripFigure`) a layman follows without reading — three strangers, the terms signed before the work, a bond locked by each, the work, everyone paid at once when the buyer closes, the evidence each keeps. ONE example among unbounded kinds, never THE model. |
...
| `(evidence)` | `/data`, `/data/yours`, `/attestations` | The Your evidence door: the data a trade leaves — signed evidence any forum reads, the parties' books, and in aggregate the public data. Audit and the data explorer, in `(app)`, are admitted under it; the explorer is shown by the code section too. |
