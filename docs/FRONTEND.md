...
**Reference / read-only — in `(app)/` or `(tools)/`, wallet provider mounted for inline write affordances via `WalletGate`:** `/assemblies/designer` (landing), `/assemblies/designer/new`, `/assemblies/designer/edit?slug=<slug>`, `/assemblies/designer/view?slug=<slug>` (in `(tools)/`; drafts in localStorage), `/clauses/register` (in `(tools)/`; clause authoring — paste a spec, validate it off-chain, register on `ClauseRegistry`, reclaim a registered clause's stake; reads walletlessly, register + reclaim writes gated by `WalletGate`), `/discover` (seller catalogue), `/audit` + `/audit/view?process=<id>` (audit / forensics — the spectator surface: no account hook anywhere in the tree; a walletless visitor reads all of it), `/data/explore` (the data explorer — the graph-query surface `DATA_LAYER.md` promises, in `(app)/` but SPECTATOR-CAPABLE like `/audit`: no account hook anywhere in its tree. Its IA is that doc's § "Truth boundaries" — one layer at a time, each carrying its own truth boundary: market shape per assembly, one overlay per attestable clause family the corpus contains, value flow per denomination, any wallet's public history, and a hand-off to `/audit/view?process=`. Views + the wallet subject ride query params via `lib/data/explorer.ts`; the graphs are projected in the browser by `lib/data/graphCorpus.ts` over the existing event caches; the prompt box renders ONLY when an analyst endpoint resolves), `/evidence-display` (forum-iframe evidence reader — needs neither the provider nor an account: it builds its own read client from query params), `/s/view?seller=<addr>` (seller detail + cart — read-mode catalogue with WalletGate-protected place-order CTA), `/rewards` (a designer's reward-tranche claim, read from `UsageCounter` + `RpgfMinter`; the claim writes are `WalletGate`-protected).

**Transactional — `(app)/`, requires a connected wallet:** `/sign`, `/s/checkout?seller=<addr>` (order review + commit), `/members/manage` (the registered dashboard + deposit claim) + the wizard sub-routes `/members/{agents,assemblies,buyer,catalogue,endpoints,identity,review}` (the buyer step: subscribe the assemblies the wallet buys through + declare the data offered for sale) and `/members/edit/{agents,assemblies,buyer,catalogue,endpoints,identity}` (endpoints = the member's own infrastructure — device configuration via `lib/shared/userEndpoints.ts`, never pinned or published; the wizard's optional `/members/endpoints` step collects the same values), `/orders` (the wallet's actor-neutral order list — buyer + seller; the "Your turn" section is where counter-sign/accept fires) + `/orders/view?process=<id>` (per-order live timeline; resolveProcess fires here). The former `/settings` page is DELETED (maintainer four-SoC ruling 2026-08-07); the build-baked `NEXT_PUBLIC_*` values remain defaults only, so a hosted deploy never seizes users onto the operator's provider key or pinning node.
...
- `/assemblies/designer/view?slug=<slug>` — read-only view of a published assembly.
