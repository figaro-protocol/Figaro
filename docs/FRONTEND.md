...
**The three sections.** One host, one export, three sections — use, build, core — and the home page as their router: `frontend/lib/shared/sections.json` says which section owns which route (a shared surface lists more than one; the first owns it, and every listed section shows it in its nav), each section has its landing page (`landings`), its own FAQ in the footer (`sectionFaqRoute`: `/faq`, `/build/faq`, `/core/faq`), and its own nav. The header has two levels: the three sections as plain links in the main row on every page (`SectionLinks`, the reader's own marked current), and under it the reader's section's own tree (`NavTreeRow`, the groups the section shows per `navGroupShown`); the home page has the first level only. The drawer carries the same two levels: the three section rows, then the section's groups. `scripts/lint-section-map.sh` fails a page route the map does not hold.
