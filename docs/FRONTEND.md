...
**The three sections.** One host, one export, three sections — use, build, core — and the apex as their router: `frontend/lib/shared/sections.json` says which section owns which route (a shared surface lists more than one; the first owns it, and every listed section shows it in its nav), each section has its landing page (`landings`), its own name beside the logo (`SectionName`), its own FAQ in the footer (`sectionFaqRoute`: `/faq`, `/build/faq`, `/core/faq`), and its own nav — the header's tree and the drawer show only the groups the section shows (`navGroupShown`); on the apex they show the three sections and nothing inside them. `scripts/lint-section-map.sh` fails a page route the map does not hold.
