...
| `(trade)` | `/local-commerce` | The Trade door: one trade, start to finish, as a strip of six pictures (`components/figures/TradeStripFigure`) a layman follows without reading — three strangers, the terms signed before the work, a bond locked by each, the work, everyone paid at once when the buyer closes, the evidence each keeps. ONE example among unbounded kinds, never THE model. |
