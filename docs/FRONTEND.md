...
**The six doors.** One host, one export, six sections — join, trade, communities, terms, evidence, code — and the home page as their router: `frontend/lib/shared/sections.json` says which section owns which route (a shared surface lists more than one; the first owns it), each section has its door page (`landings`: `/members`, `/local-commerce`, `/communities`, `/terms`, `/data`, `/core`) and its own FAQ in the footer (`sectionFaqRoute`: the builders' `/terms/faq` for terms, `/core/faq` for the code, the users' `/faq` for every other door). The header is one row: the six doors as dropdowns on every page, the home page included (`NavTreeRow`, fed by `MARKETING_MAP`; each door a disclosure button whose panel lists the door's pages, the door page first; the door holding the route marked current). The drawer is the same six doors as an accordion, the door holding the route open on arrival. `scripts/lint-section-map.sh` fails a page route the map does not hold; `scripts/lint-nav-structure.sh` fails a nav that does not equal the route tree (one route group per door, its door page first, labels equal to page titles).
...
| Group | Routes | Notes |
|---|---|---|
| `(join)` | `/members`, `/faq`, `/agents`, `/agents/how`, `/agents/worked-example` | The Join door: a member is a wallet with a published profile, a person's or an agent's. `/members` is Join. `/faq` is the users' FAQ: the protocol's questions as static sections, then every published assembly's own `faq` entries read from the registry at runtime (`components/assemblies/AssemblyFaqs`). `/agents` leads to the agents' own surface (`ecosystem-agents/`, the signer, the machine-readable index); `/agents/worked-example` is the two-agent process, every bond per party. Discover and Orders, in `(app)`, are admitted under it in the nav. |
| `(trade)` | `/local-commerce`, `/shipment` | The Trade door: one trade, start to finish, as a strip of six pictures (`components/figures/TradeStripFigure`) a layman follows without reading — three strangers, the terms signed before the work, a bond locked by each, the work, everyone paid at once when the buyer closes, the evidence each keeps. `/shipment` is the same six pictures with another cast, a warehouse and a haulier (`TradeStripFigure`'s `cast`), for the reader who never orders a meal one at a time. ONE example among unbounded kinds, never THE model. |
| `(communities)` | `/communities`, `/tokenomics`, `/dao` | The Communities door: `/communities` is the showcase of every published assembly, who offers it, and its designer's own questions, read from the registries as the page opens (`?slug=` is one assembly); nothing listed in the tree. Tokenomics and The DAO are the communities' tokens. |
| `(terms)` | `/terms`, `/terms/faq`, `/clauses`, `/assemblies`, `/registries`, `/composition`, `/pitfalls`, `/rpgf` | The Terms door, for designers: clauses, assemblies, registries, composition, sharp edges, designer rewards, the builders' FAQ. The two authoring tools in `(tools)` are admitted under it. |
| `(evidence)` | `/data`, `/data/yours`, `/attestations` | The Evidence door: the data a trade leaves — signed evidence any forum reads, the parties' books, and in aggregate the public data. Audit and the data explorer, in `(app)`, are admitted under it; the explorer is shown by the code section too. |
| `(code)` | `/core`, `/core/faq`, `/kernel`, `/invariants`, `/spec`, `/security`, `/research`, `/working-groups`, `/working-groups/for/[tag]`, `/working-groups/on/[tag]` | The Code door: the four contracts, mechanism design, security (`/security` is testing + audit results only), and research. `/papers/<slug>` is reached through Working Groups — the corpus has ONE surface, and no papers index; the two `[tag]` routes are the reader's index into it, derived from each paper's `industries` (`for`) and `keywords` (`on`) in `frontend/app/(marketing)/_lib/paperGroups.ts`. |
...
- **CSP + security headers → ARTIFACT-ENFORCED at the hosting layer.** Middleware is incompatible with `output: export`, so there is no per-request CSP nonce. The policy ships INSIDE the export as `public/_headers` (checked in; Next copies it into the export dir), in the Cloudflare Pages / Netlify `_headers` convention: the full set (CSP + HSTS / X-Frame-Options / X-Content-Type-Options / Referrer-Policy / Permissions-Policy / COOP / CORP), `'unsafe-inline'` in place of the nonce (script **hashes** are the future hardening), scheme-wide `connect-src` because the member-endpoints surface points the app at the USER'S own RPC/IPFS endpoints, and the `/evidence-display` `frame-ancestors` override (deployment config — a recognized forum edits that line) for arbitration-forum iframing. An IPFS-gateway mirror serves no custom headers — the mirror, which nobody administers, trades the header layer for re-pinnability.
...
- **`shared/`** — shell/utility, and `Link` — the one Link the tree imports (never `next/link` directly; `scripts/lint-section-map.sh` enforces it), the one seam through which every in-tree link is emitted; **`ui/`** — design primitives; **`icons/`** — SVGs
