...
| Group | Routes | Notes |
|---|---|---|
| `(use)` | `/use`, `/members`, `/faq`, `/local-commerce`, `/worked-example` | The Use door. `/faq` is the users' FAQ: the protocol's questions as static sections, then every published assembly's own `faq` entries read from the registry at runtime (`components/assemblies/AssemblyFaqs`). `/local-commerce` is ONE example among unbounded kinds, never THE model. Discover and Orders, in `(app)`, are admitted under it in the nav. |
