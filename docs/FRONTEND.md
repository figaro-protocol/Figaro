...
**`(builders)/` (wallet provider mounted — authoring publishes on-chain):** `/builders/designer` (landing), `/builders/designer/new`, `/builders/designer/edit/[slug]`, `/builders/designer/view/[slug]`, `/builders/clauses` (clause authoring — paste a spec, validate via Layer-A, register on `ClauseRegistry`; reclaim a registered clause's stake). (`/builders` and `/builders/composability` are `(marketing)/` pages.)
