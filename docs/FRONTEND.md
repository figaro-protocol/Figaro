...
**Reference / read-only — in `(app)/` or `(tools)/`, wallet provider mounted for inline write affordances via `WalletGate`:** `/assemblies/designer` (landing), `/assemblies/designer/new`, `/assemblies/designer/edit?slug=<slug>`, `/assemblies/designer/view?slug=<slug>` (in `(tools)/`; drafts in localStorage), `/clauses/register` (in `(tools)/`; clause authoring — paste a spec, validate via Layer-A, register on `ClauseRegistry`, reclaim a registered clause's stake; reads walletlessly, register + reclaim writes gated by `WalletGate`), `/discover` (seller catalogue), `/audit` + `/audit/view?process=<id>` (audit / forensics — the spectator surface: no account hook anywhere in the tree; a walletless visitor reads the full record), `/data/explore` (the data explorer — the graph-query surface `PUBLIC_GRAPH_MODEL.md` promises, in `(app)/` but SPECTATOR-CAPABLE like `/audit`: no account hook anywhere in its tree. Its IA is that doc's "Graph Separation in the UI" table — one layer at a time, each carrying its own truth boundary: market shape per assembly, one overlay per attestable clause family the corpus contains, value flow per denomination, any wallet's public record, and a deal-story hand-off to `/audit/view?process=`. Views + the wallet subject ride query params via `lib/data/explorer.ts`; the graphs are projected in the browser by `lib/data/graphCorpus.ts` over the existing event caches; the prompt box renders ONLY when an analyst endpoint resolves), `/evidence-display` (forum-iframe evidence reader — needs neither the provider nor an account: it builds its own read client from query params), `/s/view?seller=<addr>` (seller detail + cart — read-mode catalogue with WalletGate-protected place-order CTA), `/rewards` (an author's RPGF tranche claim, read from `UsageCounter` + `RpgfMinter`; the claim writes are `WalletGate`-protected).
