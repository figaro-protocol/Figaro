...
| E-8 | A `private`-disposition section's plaintext never reaches a PUBLIC surface — the standalone public IPFS pin or the shareable audit bundle. The chain sees only the section FINGERPRINT (the merkle leaf), so `agreementHash` is unchanged whether the section carries plaintext or is withheld | SDK `publicForm(agreement, specs)` (`projection.ts`) content-withholds every section whose spec is LOADED and declares a private field (identical merkle leaf → same root) — POSITIVE-knowledge only: a section with an unknown spec stays plaintext, since a private value can only be entered through a disposition-aware input that loaded the spec, so a genuinely-private section always has a warm spec (withholding on unknown instead would blank the structural PUBLIC clauses on a cold cache); `publishAgreement` (`frontend/lib/kernel/agreementFetch.ts`) applies it via the app-boundary-warmed clause-spec singleton before the standalone pin; the audit bundle renders a withheld section's body as absent automatically; `parseClauseSpec` REJECTS a clause mixing public + private field dispositions (the leaf model withholds a whole section, never one field). The signed + counterparty-relayed forms keep plaintext (Arm 2 encrypts that relay leg) | SDK `projection.test.ts` (withholds private, keeps public plaintext, preserves `agreementHash`, keeps an unknown-spec section plaintext, `specHasPrivateField`); `spec.test.ts` (mixed-disposition rejected, all-private accepted) | `/clauses` → the disposition seam ("public = coordination commons, private = paid edge behind the fingerprint") |
