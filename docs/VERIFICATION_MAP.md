...
| K-2 | Only root buyer can call `resolveProcess` | `if (msg.sender != ps.rootBuyer) revert NotProcessBuyer()` in `resolveProcess()` | `FigaroCoreTest`: buyer-only paths; `FigaroCoreRevertBranchTest`: the revert-branch suite (count derived from the file, never stored here) | `ResolveProcess` constrains resolver to root buyer | `echidna_buyer_dominance` — non-buyer resolve always fails | `/rpgf` → /papers/asymmetric-bonding; `/spec` → Kernel (`FigaroCore.sol`) |
