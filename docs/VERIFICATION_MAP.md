...
| K-8 | Both parties sign off-chain via EIP-712 typed data | `ECDSA.recover(digest, buyerSig)` + `ECDSA.recover(digest, sellerSig)` checks in `commit()` | `FigaroCoreTest`: signature verification; SDK `commitments.test.ts`: EIP-712 domain/typed-data build. SDK↔Solidity parity is now UNCONDITIONAL: `sdk/tests/eip712Parity.test.ts` freezes SDK-computed vectors, `test/Eip712ParityTest.t.sol` asserts the kernel reproduces every hash (domain separator both ways, `hashStruct`, digest, order hash) — runs in sdk-ci + foundry-ci with no chain; the skipIf-gated `integration.test.ts` round-trip is now the redundant belt | Not modeled (TLA+ abstracts signature mechanics) | — | `/sign` → commitment signing UI |
