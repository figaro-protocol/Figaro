...
- `E-9` **Public-values completeness**: every persistent effect of `settleBatch` is bound by a public value the proof committed — nothing the verifier, the token, or the counter writes is outside the eight words and the four calldata hashes
- `E-10` **Deploy wiring**: every immutable and genesis parameter the deploy scripts set reads back from the deployed contracts as what the script was given
...
|---|---|---|---|---|---|---|
| K-1 | Buyer bond = $2 \times payment$; seller bond = $2 \times cumulativeValue$ | `_pullExact(token, buyer, payment * 2)` and `_pullExact(token, seller, cumVal * 2)` in `commit()` | `FigaroCoreTest`: bond-amount coverage; `KernelTransitionVectorsTest` harvests six scenarios from the live kernel into `test/fixtures/kernel-transition-vectors.json` and `prover/lib/tests/transition_vectors.rs` replays them through the batch path's mirror — every bond pulled, every payout, every accumulator and active count asserted; SDK `bonds.test.ts`: the 2× math the frontend/agents derive from | `DeterrentEscrowMagnitudes` — the 2×payment + 2×cumulativeValue held-deposit magnitudes, a checked invariant (see A-8 and §7); bond correctness also carried by Halmos + Certora | `echidna_solvency` — core holds ≥ sum of active bonds | `/kernel` → /papers/asymmetric-bonding (mechanism); `/spec` → Kernel (`FigaroCore.sol`) |
...
| K-4 | No timeout, no admin exit from Active state | No timeout action exists; only `resolveProcess` transitions Active→Resolved; no admin functions | `FigaroCoreRevertBranchTest`: no alternate exit paths | Model has no timeout action; only Committed→Resolved via buyer | `echidna_state_monotonicity` — status only moves forward (0→1→2) | `/kernel` → /papers/asymmetric-bonding (the Escape-Hatch Weakness theorem); `/faq` → "no admin, no pause function" |
...
| K-8 | Both parties sign off-chain via EIP-712 typed data | `ECDSA.recover(digest, buyerSig)` + `ECDSA.recover(digest, sellerSig)` checks in `commit()` | `FigaroCoreTest`: signature verification; `test_commit_signatureFromAnotherChain_reverts` — a commitment signed for this kernel on another chain id is accepted there and refused here (the domain moves with `block.chainid`); SDK `commitments.test.ts`: EIP-712 domain/typed-data build. SDK↔Solidity parity is unconditional: `sdk/tests/eip712Parity.test.ts` freezes SDK-computed vectors, `test/core/kernel/Eip712ParityTest.t.sol` asserts the kernel reproduces every hash (domain separator both ways, `hashStruct`, digest, order hash) — runs in sdk-ci + foundry-ci with no chain; the skipIf-gated `integration.test.ts` round-trip is the redundant belt | Not modeled (TLA+ abstracts signature mechanics) | — | `/sign` → commitment signing UI |
...
| A-10 | Every committed order is bound to its process: `orderProcessId[orderHash] == processId`, written once at commit, never changed | `commit`: `orderProcessId[orderHash] = processId` beside the status write | `FigaroCoreTest`: `test_orderProcessId_bindsEveryOrderToItsProcess` (root, sub, and an unknown order); `test_resolve_orderFromAnotherProcess_reverts` — another process's root and sub-order presented at resolve are `OrderNotCommitted` under this process id, no bond moves, the other process's bindings stand | Not modeled (the TLA+ state carries the binding as the order's identity, not a separate map) | — | `/kernel` |
...
|---|---|---|---|---|
| E-1 | Only verified role-holder can attest | `attestAsSeller`: verifies seller via commitment orderHash lookup; `attestAsBuyer`: verifies via ProcessState.rootBuyer; `attestViaResolver`: delegates to IRoleResolver | `AttestationCoordinatorTest`: all 3 paths + cross-order same-process; the agreement tree's three-way lock — `sdk/tests/merkleParity.test.ts` freezes SDK-built leaves and proofs, `MerkleParityTest` verifies them with the OpenZeppelin library both on-chain consumers call, `prover/lib/tests/merkle_parity.rs` with the guest's verifier | `/papers/on-chain-evidence` → evidentiary properties; `/spec` → Attestation & clause |
...
| E-6 | Florin supply cap: $\leq$ 1B on every mint | `mint()`: `if (totalSupply() + amount > MAX_SUPPLY) revert SupplyCapExceeded()` + reentrancy guard | `FlorinToken.t.sol`: cap enforcement, multi-minter, renounce | `/papers/florin-schelling-point-token` → § 5, the token |
| E-7 | Batch-resolved trade counts for the 600M, and counts ONCE. The two paths share no state (a batch-resolved process never acquires kernel status), so the accrual crosses as PROVED numbers: only `FigaroBatchVerifier` may write it, only with values an immutable vkey committed. Idempotence is guest-owned and holds ACROSS batches because the counted set rides the batch state root. The two paths merge as SCORES — never components, which would over-count breadth for a seller active on both; summing scores still over-counts a shared seller by a bounded factor (≤ 2^(2/3), the RPGF paper §4 and §7.3) — for a seller active on both | `applyBatchAccrual()` in `UsageCounter` — `msg.sender == batchVerifier`, `period == currentPeriod()`, provenance-key match, live `members.registered` per distinct seller (reverts — caught by the verifier), a live clause-or-assembly registration stake AND `excludedClauseOrAssembly` SKIPPED per clause or assembly (never revert — audit Fix 1a/1c), non-decreasing counts, empty accrual = no-op (so trade outlives the reward). `settleBatch()` in `FigaroBatchVerifier` — `_hashUsage` re-derives the 8th public value from calldata, BOTH array lengths length-prefixed so the split cannot be forged; the accrual call is wrapped in try/catch (`BatchAccrualSkipped`) so a reward-gate revert NEVER unwinds the token transfers, and the sequencer pre-filters poison usage claims (excluded/unregistered/unstaked) against chain state before proving so the catch fires only on the genuine stake-race. Guest (`prover/lib/src/kernel.rs::apply_usage_claims`): post-state `orderStatus == 2`, merkle inclusion against the signed `agreementHash`, `usage_counted` insert-or-reject | Rust `prover/lib/tests/usage.rs` (same-batch credit, unresolved reject, not-in-agreement reject, cross-batch replay reject, breadth vs depth, assembly-via-provenance, wrong-composition reject, hash covers period/provenance/sellers, length-prefix anti-collision, root advances, cross-language vector); Foundry `UsageCounterTest` (writer gate, period, member stake of the seller of record, exclusion, provenance, monotonicity, overwrite-not-accumulate, empty-after-close liveness, score-merge + superadditivity) and `FigaroBatchVerifierTest` (accrual reaches the counter — read back from counter storage, tampered accrual rejected, a counter-rejection is CAUGHT so the batch still lands its positions + advances state with the accrual dropped — `test_settleBatch_settlesEvenWhenTheCounterRejectsTheAccrual`, lands after accrual closes, Rust vectors match the contract's assembly — the usage packer's, and in `test_packerHashes_matchTheRustVectors` / `prover/lib/tests/packers.rs` the positions, attestations and spec-bindings packers); `UsageCounterTest` also covers the clause-or-assembly registration gate (direct-path `ClauseOrAssemblyNotRegistered`, batch-path skip of excluded + unregistered); SDK `rpgf.test.ts` (both-stream fold, replace-not-accumulate); `batch-e2e.test.ts` runs through a real sequencer + counter | `/rewards` shows both paths' components and the merged `scoreOf` — the figure the minter actually pays |
...
| E-9 | Every persistent effect of `settleBatch` is bound by a public value: positions → `tokenOpsHash`, attestations → `attEventsHash`, spec bindings → `specBindingsHash`, the accrual → `usageAccrualHash`, the state advance → `newRoot`; nothing else is written | `settleBatch` in `FigaroBatchVerifier`: the four hash checks, `_checkSpecBindings`, `_executePositions`, `_emitAttestations`, the counter call, the root write — and no other write | `FigaroBatchVerifierTest`: the tampered-hash tests (each binding bites) and `test_settleBatch_writesNothingThePublicValuesDoNotBind` — the state diff of a settling batch is recorded and every storage write is accounted for: the verifier's state root and batch count (a slot that returns to rest within the call is no effect), the token's balances, the counter's accrual; no other account written, no ether moved, nothing created; an effect added without a public value surfaces as an unaccounted write | — | — | `/spec` → Batch verifier |
| E-10 | The deploy scripts wire what they are given: the counter↔verifier pair binds mutually; the coordinator names the kernel, Permit2 and the router; the verifier names the gateway, vkey, registry, counter and the derived genesis root; the counter names its gates, provenance clause, exclusion, floor and schedule; the registries carry their deposits and cooldown; the florin's genesis mints, minter cap and renounced deployer hold | `script/Deploy.s.sol`, `script/DeployMainnet.s.sol` (in-script requires, the verifier address prediction) | `test/script/DeployWiringTest.t.sol`: runs each script as it would broadcast and reads every binding back from the deployed contracts; `scripts/deploy-mainnet.sh` repeats the read-back with `cast` on the live chain before writing the deployment record | — | — | `/deployments` |
